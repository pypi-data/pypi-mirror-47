...
def get_all_corpus(request):
    corpuses = Article.all().annotate(name=F('corpus__name')).values("name").distinct(). \
...
def get_all_tag(request):
    tags = Article.all().annotate(name=F('tags__name')).values("name").distinct(). \
...
def get_all_timelist(request):
    ...
    time_list = Article.all().annotate(date=TruncMonth('create_time')).values('date').distinct() \
