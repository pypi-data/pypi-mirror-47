...
# url='http://ggzy.dingxi.gov.cn/jyxx/project.html?categoryNum=004001004'
# driver.get(url)
...
def f1(driver, num):
    ...
    for li in lis:
        ...
        diqu=li.find('font',color='#0066FF').get_text().strip('[').strip(']')

        url=re.findall('javascript:redirectpage\("(.+?)","(.+?)"\)',href)[0][0]
        data_cate=re.findall('javascript:redirectpage\("(.+?)","(.+?)"\)',href)[0][1]


        href = '/'.join(['http://ggzy.dingxi.gov.cn/jyxx', data_cate[:6],data_cate[:9],data_cate, ggstart_time.replace('-', ''),
                         url + '.html'])
...
def f2(driver):
    ...
    WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
    time.sleep(1)
    total = int(driver.find_element_by_xpath('//span[@class="pg_maxpagenum"]').text.split("/")[1])
...
def f3(driver, url):
    ...
    locator = (By.XPATH, '//div[@class="ewb-wrap"][string-length()>100]')
...
data=[
    ...
    ["jqita_yangguang_gqita_gg","http://ggzy.dingxi.gov.cn/jyxx/project.html?categoryNum=004005",["name","ggstart_time","href","info"],add_info(f1,{"gclx":"阳光采购"}),f2],
    ["jqita_zhongda_gqita_gg","http://ggzy.dingxi.gov.cn/jyxx/project.html?categoryNum=004006",["name","ggstart_time","href","info"],add_info(f1,{"gclx":"重大项目"}),f2],
...
if __name__=="__main__":
    work(conp=["postgres","since2015",'192.168.3.171',"gansu","dingxi"],num=1,headless=False)
    # driver=webdriver.Chrome()
    # url='http://ggzy.dingxi.gov.cn/jyxx/004006/004006001/20190102/ced74140-8ff5-4265-943c-dce40eeddbf6-ZDXM.html'
    #
    # q=f3(driver,url)
    # print(q)
