...
class Trial(axonaIO.IO, SAC, dict):
	...
	-------------
	Provides
		1. Methods to plot electrophysiology data acquired using the Axona DACQ recording system
		
		
		2. Methods to extract some measures from that data
...
#	   return "{0}({1})".format(self.__class__.__name__, self.filename_root)

	def hasFiles(self):
		'''
		Check for some automated yaml processing (see Dropbox/Science/Analysis/*)
		'''
	...
	def plotMap(self, tetrode, clusters, ax=None, var2bin='pos', *args, **kwargs):
		...
		plotMap(tetrode, cluster, binsize=3, smooth_sz=5,
							var2bin='pos', smooth=True, ax=None, **kwargs)
...
#			# check kwargs to see if we want to add peak rate to axes
	...
	def _plotMap(self, tetrode=None, cluster=None, ax=None, var2bin='pos', 
		...
		Definition: plotMap(tetrode, cluster, binsize=3, smooth_sz=5,
							var2bin='pos', smooth=True, ax=None, **kwargs)
		----
	...
	def plotSpikesOnPath(self, tetrode, clusters, ax=None, clamp=False, **kwargs):
		...
		clamp:   bool, optional
				 ...
				 property
		ax:      axis object, defaults to None. Which axis to add the plot to.
	...
	def _plotRaster(self, tetrode, cluster, dt=(-50, 100), prc_max=0.5, ax=None, ms_per_bin=1, histtype='count', **kwargs):
		...
		most inputs obvious except for histtype which can be either 'count' or
		'rate' - the resulting histogram plotted above the raster plot will
		consist of either the counts of spikes in ms_per_bin or the mean rate
		in ms_per_bin - makes a superficial difference unless the axes
		ticklabels are added
		prc_max: the proportion of firing the cell has to 'lose' to count as
...
#		
	...
	def plot_event_EEG(self, eeg_type='egf', dt=(-50, 100), plot=True, ax=None, 
		...
		-------------------
		Parameters:
			eeg_type: str, either 'eeg' or 'egf'
			dt: two-tuple with time to look before and after an onset event
			plot: bool, whether to plot the stimulus-triggered-eeg
			ax: matplotlib.axis instance - will plot into this axis if supplied
				(new figure produced if plot is None and ax is None)
			evenOnsets: bool, if True assume there is supposed to be an even 
	...
	def plotEventEEGRange(self, eeg_type='egf', stimTrials=[0,1], ax=None, **kwargs):
		...
		------------
		Parameters:
			eeg_type: str, either 'egf' or 'eeg' although probably no point
						using 'eeg' as sample rate too low
			stimTrials: list, the stimulation 'trial' to plot, starting at 0
						NB stimulating every 150ms for 10ms for 20 minutes gets
						you 8000 trials
			ax: matplotlib.axis instance, the axis to plot into. A new figure is
...
#        pdb.set_trace()
	...
	def plotFreqVSpeed(self, minSp=5, maxSp=50, spStep=5, ax=None, laserFilter=None, **kwargs):
		...
		Parameters
		----------
		minSp (int) : speeds below this are ignored
		maxSp (int) : speeds above this are ignored
		spStep (int): the bin width for speed
		ax (matplotlib.axes) : the axes in which to plot
		laser (int or None) : whether to filter for laser on/ off events
...
#		Sxx_sm = self.ratemap.blurImage(Sxx, (secsPerBin*2)+1)
	...
	def plotClusterSpace(self, tetrode, clusters=None, ax=None, bins=256,**kwargs):
		...
		Parameters
		----------
		tetrode(int): the tetrode cluster space to plot
		clusters(Optional[int or list or np.array]): the clusters to colour in
		ax(Optional[matplotlib.pyplot.axis]): the axis to plot into
		bins(Optional[int]): the number of bins to use in the histogram
		kwargs can include a param keyword for the parameter to construct the 
...
#        fig = plt.gcf()
	...
	def plotSAC(self, tetrode, clusters, ax=None, binsize=3, **kwargs):
		...
		Parameters
		----------
		tetrode (int): tetrode number
		cluster (int): cluster number
		ax (Optional[matplotlib.pyplot.axis]): plots into this axis
		binsize (Optional(int)): size of bins (cms)

		TODO: if I rip out the class inheritence for this class then this 
		method needs altering where it calls autoCorr2D
		"""
		if ax is None:
			fig = plt.figure()
			ax = fig.add_subplot(111)
	...
	def tortuosity(self, xy=None):
		...
		Parameters
		-----------
		xy - 2xm matrix of xy positions. Default is None so will use this
			instances xy array in POS
		Returns
		--------
		float - tortuosity index calculated as follows:
	...
	def getBorderScore(self, tetrode, cluster, debug=False, **kwargs):
		'''
		Wrapper for fieldcalcs getBorderScore
		'''
