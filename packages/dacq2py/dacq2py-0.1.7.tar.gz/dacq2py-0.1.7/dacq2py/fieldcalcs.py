...
class FieldCalcs:
	...
	def getBorderScore(self, A, B=None, shape='square', fieldThresh=0.3, smthKernSig=3,
		...
		'''
		Returns a border score totally dis-similar to that calculated in Solstad et al
		(2008)
				
		Parameters
		-----------------------
		Inputs:
			A - 2D np.array. Should be the ratemap
			B - 2D np.array. This should be a boolean mask where True (1)
			...
			smthKernSig - the sigma value used in smoothing the ratemap
		...
		A_thresh[idx] = A[idx]
#		A_smoothed = skimage.filters.threshold_local(A_thresh, 3, method='gaussian', mode='mirror', param=smthKernSig)
	...
	def get_field_props(self, A, min_dist=5, neighbours=2, prc=50,
		...
		"""
			...
			Input:
				A - a ratemap (but could be any image)
				min_dist - the separation (in bins) between fields for measures
							such as field distance to make sense. Used to
							partition the image into separate fields in the call to
							skimage.feature.peak_local_max
				neighbours - the number of fields to consider as neighbours to
							any given field. Defaults to 2
				prc - percent of fields to consider
				ax - user supplied axis
				tri - whether to do Delaunay triangulation between fields 
						and add to plot
				verbose - dumps the properties to the console
				plot - whether to plot some output - currently consists of the
	...
	def kldiv(self, X, pvect1, pvect2, variant=None):
		'''KLDIV Kullback-Leibler or Jensen-Shannon divergence between two distributions.
			  ...
			  4) See the wikipedia entry on Kullback-Leibler duvergence

			  NB. This function is taken from one on the Mathworks file exchange
				 '''
