...
class StickerSet(TLObject):
    ...
    __slots__ = ["id", "access_hash", "title", "short_name", "count", "hash", "archived", "official", "masks", "installed_date", "thumb", "thumb_dc_id"]

    ID = 0xeeb46f27
    QUALNAME = "types.StickerSet"

    def __init__(self, *, id: int, access_hash: int, title: str, short_name: str, count: int, hash: int, archived: bool = None, official: bool = None, masks: bool = None, installed_date: int = None, thumb=None, thumb_dc_id: int = None):
    ...
    def read(b: BytesIO, *args) -> "StickerSet":
        ...
        return StickerSet(id=id, access_hash=access_hash, title=title, short_name=short_name, count=count, hash=hash, archived=archived, official=official, masks=masks, installed_date=installed_date, thumb=thumb, thumb_dc_id=thumb_dc_id)
