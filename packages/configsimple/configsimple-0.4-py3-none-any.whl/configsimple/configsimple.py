...
class ConfigSimple:
    ...
    def parse_args(self, args=None):
        ...
        if self.component == "" and ("-h" in argstouse or "--help" in argstouse):
            self.show_help()
        tmpnamespace, unknown = self.argparser.parse_known_args(args)
        for k, v in vars(tmpnamespace).items():
            self.set(k, v, prepend_component=False)
    ...
    def set(self, parm, value, from_config=None, prepend_component=True):
        """
        Set a config option to the given value. If this is set in a top config then
        settings that looks like they match an added component config will perculate down
        to the component config. If this is set in a component config, the setting will
        bubble up to the top config and also percolate down to any other component config
        that could match.
        :param parm: the name of the config option
        :param value: the value
        :param from_config: used internally
        :return:
        """
        if from_config is not None:
            if from_config != "" and self.component == from_config:
                return
            use_component = from_config
        else:
            use_component = self.component
        if prepend_component:
            name = ConfigSimple.fullname(self.component, parm)
        else:
            name = parm
        self._set(name, value)
        if from_config is None and self.component != "":
            self.parent.set(name, value, from_config=use_component)
        elif self.component == "":
            # we should be in a top config, see if we can percolate the set down to a child config
            for cfg in self.added_configs:
                if parm.startswith(cfg.component):
                    cfg.set(parm, value, from_config=use_component, prepend_component=False)
