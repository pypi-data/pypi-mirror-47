...
str(self.couleur[1]) + ". Crayon baissé: " + str(self.crayonbaisse) + ".\n" \
+ "Taille crayon: " + str(self.largeurcrayon) + "px. Visible ? " +\
str(self.visible) + ". Forme: " + self.forme +\
"\n" + self.no_tortue + ", " + Tortue._obeissances\
[randint(0, len(Tortue._obeissances) - 1)] + Tortue.monnom + " !"
        return tortue

    ##########################
    # Class methods (screen) #
    ##########################
    def au_revoir_ecran(cls, message=""):
        """Ferme la fenêtre tortue quand on clique dessus, puis affiche
    ...
    remplis = property(get_remplis, "", "",
             """Retourne True/False, si la tortue est en mode remplissage
     ou pas.""")
    ...
    def repete(self, fois, liste):
        ...
        - La tortue dispose de 3 variables attachées à son service v1 = 0, \
        v2 = 0 et v3 = 0. 
        - l'instruction 'repete' n'accepte que des variables Tortue. D'où \
        l'intéret des variables ci-dessus pour obtenir des variations de \
        paramètres
        - les instructions s'écrivent toutes entre guillemets, simples ou \
        doubles, dans une liste et donc separée par des virgules.
        - si l'instruction Tortue débute, elle s'écrit telle qu'elle est, \
        sinon, elle doit être précédée de self. Exemple:\
        ["td(90)", "av(self.v1)"]
        - l'instruction est récursive et peut se rapeller autant de fois \
        qu'on veut à l'intérieur, sur le modèle suivant: \
        ...
        """
        for i in range(fois):
            # l + while et pas for l, car on doit gérer l (ce qui n'est pas
            # possible avec for)
            l = 0
            while l < len(liste):
                inst = str(liste[l].strip())
                #print(inst)
                # si l'instruction est repete, on "récurse"
                if inst == "repete":
                    tmp = l
                    self.repete(liste[l+1], liste[l+2])
                    l = tmp + (len(liste[l+2]) + 1)
                else:
                    exec("self." + inst)
                    l += 1
