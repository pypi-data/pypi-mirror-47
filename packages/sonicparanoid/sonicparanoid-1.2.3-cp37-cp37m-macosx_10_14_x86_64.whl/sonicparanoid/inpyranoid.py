...
def rewrite_clusters(inTbl, mergedClstrs, mergedIds, skipDict):
    ...
    print("\nrewrite_clusters")
    print("inTbl: {:s}".format(inTbl))
    print("mergedClstrs: {:d}".format(len(mergedClstrs)))
    print("mergedIds: {:d}".format(len(mergedIds)))
    print("skipDict: {:d}".format(len(skipDict)))
    '''

    # main directory
    tmpPath = os.path.join('{:s}.tmp'.format(inTbl.rsplit('.', 1)[0]))
    # create tmp file and write the header
    ifd = open(inTbl, 'r')
    # extract the hdr
    hdr = ifd.readline()
...
def merge_and_write_inpyranoid_output(inTbl, repeatDict: Dict[str, Set[str]], mergeableDict: Dict[str, List[str]], debug=False):
    ...
    #inTbl = rewrite_clusters(inTbl, mergedStack, mergedIds, skipDict)
