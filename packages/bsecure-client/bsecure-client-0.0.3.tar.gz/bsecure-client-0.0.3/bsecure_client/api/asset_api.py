from .base import API

push_asset_query = '''
  mutation pushAsset(
    $tenantUuid: UUID!,
    $category: String!,
    $code: String!,
    $make: String,
    $model: String,
    $photo: String
  ) {
    pushAsset(input: {
      tenantUuid: $tenantUuid,
      category: $category,
      code: $code,
      model: $model,
      make: $make,
      photo: $photo
    }) {
      asset {
        uuid
      }
    }
  }
'''

upload_asset_photo_query = '''
  mutation uploadAssetPhoto($filename: String!) {
    uploadAssetPhoto(input: {filename: $filename}) {
      fileId,
      presignedUrl
    }
  }
'''

push_remark_query = '''
  mutation pushRemark(
    $assetUuid: UUID!,
    $createdTime: Datetime!,
    $resolvedTime: Datetime,
    $description: String,
    $resolution: String
  ) {
    pushRemark(input: {
      assetUuid: $assetUuid,
      createdTime: $createdTime,
      resolvedTime: $resolvedTime,
      description: $description,
      resolution: $resolution
    }) {
      remark {
        uuid
      }
    }
  }
'''

push_service_query = '''
  mutation pushService(
    $assetUuid: UUID!,
    $name: String!,
    $description: String,
    $createdTime: Datetime!,
    $dueTime: Datetime,
    $performedTime: Datetime,
    $result: String
  ) {
    pushService(input: {
      assetUuid: $assetUuid,
      name: $name,
      description: $description,
      createdTime: $createdTime,
      dueTime: $dueTime,
      performedTime: $performedTime,
      result: $result
    }) {
      service {
        uuid
      }
    }
  }
'''


class AssetAPI(API):
    @API.expose_method
    def push_asset(self, tenant_uuid, category, code=None, make=None,
                   model=None, photo=None):
        photo_file_id = self.upload_photo(photo)
        response_data = self.perform_query(
            push_asset_query,
            self.make_variables(
                tenantUuid=tenant_uuid,
                category=category,
                code=code,
                make=make,
                model=model,
                photo=photo_file_id
            )
        )
        return response_data['pushAsset']['asset']['uuid']

    def upload_photo(self, photo):
        return self.upload_file(
            photo,
            upload_asset_photo_query,
            'uploadAssetPhoto'
        )

    @API.expose_method
    def push_remark(self, asset_uuid, created_timestamp,
                    resolved_timestamp=None, description=None,
                    resolution=None):
        response_data = self.perform_query(
            push_remark_query,
            self.make_variables(
                assetUuid=asset_uuid,
                createdTime=created_timestamp,
                resolvedTime=resolved_timestamp,
                description=description
            )
        )
        return response_data['pushRemark']['remark']['uuid']

    @API.expose_method
    def push_service(self, asset_uuid, name, created_timestamp,
                     description=None, due_timestamp=None,
                     performed_timestamp=None, result=None):
        response_data = self.perform_query(
            push_service_query,
            self.make_variables(
                assetUuid=asset_uuid,
                name=name,
                description=description,
                createdTime=created_timestamp,
                dueTime=due_timestamp,
                performedTime=performed_timestamp,
                result=result
            )
        )
        return response_data['pushService']['service']['uuid']
