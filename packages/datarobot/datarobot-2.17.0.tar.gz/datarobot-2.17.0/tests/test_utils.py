...
from datarobot.utils import (dataframe_to_buffer, parse_time, get_id_from_response,
                             from_api, deprecation, recognize_sourcedata, to_api, is_urlsource)
...
def test_from_api_filter_nones():
    ...
    data_out = {
        'some_value': {'nested_not_none': ''},
        'list_none': [{'some_not_none': ''}]
    }
...
def test_from_api_keep_nones():
    ...
    assert data_out == from_api(data_in, keep_attrs=[
        'some_value.nested_none',
        'some_value.level_two.level_three',
        'keep_level_one'
    ])
