    ...
    for bt in backtests:
        ...
        for key in expected_keys:
            if 'date' in key and bt[key] is not None:
                ...
                assert date.tzname() in ['UTC', 'Z']
    fdw_keys = {
        'effective_feature_derivation_window_start', 'effective_feature_derivation_window_end',
...
def test_download_series_accuracy_as_csv(datetime_model, project_url, tmpdir):
    response_data = {
        u'querySeriesCount': 2, u'previous': None,
        u'data': [
            {u'multiseriesId': u'5c58975a211c0a1698093f25', u'validationScore': None,
             u'backtestingScore': None, u'rowCount': 365, u'multiseriesValues': [u'1'],
             u'holdoutScore': None, u'duration': u'P1Y0M0D'},
            {u'multiseriesId': u'5c58975a211c0a1698093f26', u'validationScore': None,
             u'backtestingScore': None, u'rowCount': 329, u'multiseriesValues': [u'2'],
             u'holdoutScore': None, u'duration': u'P0Y10M26D'}],
        u'totalSeriesCount': 2, u'next': None
    }
    ...
    datetime_model.download_series_accuracy_as_csv(local_csv_filepath.strpath)
    assert os.path.isfile(local_csv_filepath.strpath)
    with open(local_csv_filepath.strpath) as f:
        csv_output = csv.reader(f)
        # assert headers
        correct_header_list = ['multiseriesId', 'validationScore', 'backtestingScore',
                               'rowCount', 'multiseriesValues', 'holdoutScore', 'duration']
