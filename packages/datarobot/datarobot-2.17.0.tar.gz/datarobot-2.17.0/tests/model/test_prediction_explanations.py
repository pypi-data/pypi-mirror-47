...
from datarobot import PredictionExplanationsInitialization, PredictionExplanations
...
def pei_url(project_collection_url, pei_data):
    return '{}{}/models/{}/predictionExplanationsInitialization/'.format(project_collection_url,
                                                                         pei_data['project_id'],
                                                                         pei_data['model_id'])
...
def test_prediction_explanations_page_future_proof(pep_data):
    with patch('datarobot.models.prediction_explanations.t.URL.check',
...
@responses.activate
@pytest.mark.usefixtures('client')
def test_prediction_explanations_initialization_create(pei_data, pei_url, project_url,
                                                       base_job_server_data):
    job_url = '{}jobs/{}/'.format(project_url, base_job_server_data['id'])
    responses.add(responses.POST, pei_url,
                  body='', status=202, content_type='application/json',
                  adding_headers={'Location': job_url})
    responses.add(responses.GET, job_url,
                  json=base_job_server_data, status=303, adding_headers={'Location': pei_url})

    pei_job = PredictionExplanationsInitialization.create(project_id=pei_data['project_id'],
                                                          model_id=pei_data['model_id'])
...
@responses.activate
@pytest.mark.usefixtures('client')
def test_prediction_explanations_page_get_returns_valid_data(project_id, project_url,
                                                             pep_json, one_pep):
...
@responses.activate
@pytest.mark.usefixtures('client')
def test_prediction_explanations_create(project_id, project_url, project_collection_url,
                                        base_job_server_data, pep_data):
    pep_base_url = '{}{}/predictionExplanations/'.format(project_collection_url, project_id)
    job_url = '{}jobs/{}/'.format(project_url, base_job_server_data['id'])
    pep_url = '{}/{}/'.format(pep_base_url, pep_data['id'])

    responses.add(responses.POST, pep_base_url,
                  body='', status=202, content_type='application/json',
                  adding_headers={'Location': job_url})
    responses.add(responses.GET, job_url,
                  json=base_job_server_data, status=303, adding_headers={'Location': pep_url})

    pep_job = PredictionExplanations.create(project_id=project_id,
...
def test_prediction_explanations_list(project_id, project_url, pe_list_json):
    ...
    responses.add(responses.GET, pe_base_url + '?modelId=578e59a41ced2e5a9eb18965',
                  status=200, body=pe_list_json, content_type='application/json',
...
def test_prediction_explanations_delete(project_collection_url, project_id, pe_data):
    pe_url = '{}{}/predictionExplanationsRecords/{}/'.format(project_collection_url,
                                                             project_id,
                                                             pe_data['id'])
...
def test_prediction_explanations_iteration(project_id, project_url, one_pep):
    ...
    resp_data_1 = {'id': one_pep.id,
                   'count': 5,
                   'previous': None,
                   'next': url_page2,
                   'data': one_pep.data[:2],
                   'prediction_explanations_record_location':
                       one_pep.prediction_explanations_record_location}
    ...
    with patch('datarobot.models.prediction_explanations.t.URL.check',
               side_effect=(lambda url, context=None: url)):
        prediction_explanations_rows = list(PredictionExplanations(
            one_pep.id, project_id, '', '', '', 0, 0, ''
        ).get_rows())
...
@responses.activate
@pytest.mark.usefixtures('client')
def test_prediction_explanations_as_dataframe_classification(project_url,
                                                             pep_data_classification, one_pe):
    url = '{}predictionExplanations/{}/'.format(project_url, one_pe.id)
...
def test_prediction_explanations_download_to_csv():
    patch_target = ('datarobot.models.prediction_explanations.'
                    'PredictionExplanations.get_all_as_dataframe')
    with patch(patch_target) as pemock:
        prediction_explanations = PredictionExplanations('fake_id', 'fake_project_id', '', '', 3,
                                                         0, 0, '')
        prediction_explanations.download_to_csv('test_file.csv', encoding='ascii')
        pemock.return_value.to_csv.assert_called_once_with(path_or_buf='test_file.csv',
                                                           header=True,
                                                           index=False,
                                                           encoding='ascii')
