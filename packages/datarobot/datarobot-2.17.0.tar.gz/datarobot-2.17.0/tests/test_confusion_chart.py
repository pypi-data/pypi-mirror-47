...
@pytest.fixture
def confusion_chart_validation_metadata_response(project_url, model_id,
                                                 confusion_chart_validation_metadata_server_data):
    responses.add(
        responses.GET,
        '{}models/{}/confusionCharts/validation/metadata/'.format(project_url, model_id),
        json=confusion_chart_validation_metadata_server_data, content_type='application/json'
    )
...
@responses.activate
@pytest.mark.usefixtures('client', 'confusion_chart_validation_metadata_response')
...
def test_get_frozen_validation_confusion_chart_no_fallback(confusion_chart_validation_data_url,
                                                           project_id, model_id):
    responses.add(
        responses.GET,
        confusion_chart_validation_data_url,
...
@responses.activate
@pytest.mark.usefixtures('client', 'confusion_chart_parent_model_validation_metadata_response')
...
@responses.activate
@pytest.mark.usefixtures('client', 'confusion_chart_validation_metadata_response')
def test_get_frozen_all_confusion_chart_no_fallback(confusion_chart_validation_data,
                                                    confusion_chart_list_url,
                                                    project_id, model_id):
    responses.add(
        responses.GET,
...
@pytest.mark.usefixtures('known_warning')
def test_get_frozen_all_confusion_chart_with_fallback(confusion_chart_validation_data,
                                                      confusion_chart_parent_model_validation_data,
    ...
    responses.add(
        ...
        status=200,
        content_type='application/json',
        body=json.dumps({'charts': [confusion_chart_parent_model_validation_data,
                                    confusion_chart_parent_model_holdout_data]})
    )
    model = Model(id=model_id, project_id=project_id, is_frozen=True)
