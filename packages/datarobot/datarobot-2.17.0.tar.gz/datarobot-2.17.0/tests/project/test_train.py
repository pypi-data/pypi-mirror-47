...
def test_train_by_rowcount_with_bp_id(project, model_collection_url, project_url):
    responses.add(responses.POST, model_collection_url, content_type='application/json',
                  status=201, body='',
...
def test_train_by_rowcount_with_trainable(project, model_collection_url, project_url):
    ...
    trainable = Blueprint.from_data({'id': blueprint_id, 'project_id': project.id,
...
@pytest.mark.parametrize('blueprint,featurelists', [
