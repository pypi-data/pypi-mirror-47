...
def batch_type_transform_create_url(unittest_endpoint, project_id):
    return '{}/projects/{}/batchTypeTransformFeatures/'.format(
        unittest_endpoint,
        project_id
    )
...
def batch_type_transform_create_response(batch_type_transform_create_url, async_url):
    ...
    responses.add(
        ...
        status=303,
        body='',
        adding_headers={'Location': '/features/'}
    )


@pytest.mark.parametrize("exception, patch", [
    (TypeError, {"parent_names": []}),
    (ValueError, {"variable_type": "wrong"}),
    (ValueError, {"prefix": None, "suffix": None}),
])
def test_batch_var_type_transform_arguments_validation(project, exception, patch):
    ...
    with pytest.raises(exception):
        project.batch_features_type_transform(
            parent_names=args['parent_names'], variable_type=args['variable_type'],
            prefix=args['prefix'], suffix=args['suffix'])


@responses.activate
@pytest.mark.usefixtures('client', 'batch_type_transform_create_response')
