...
from datarobot.enums import VARIABLE_TYPE_TRANSFORM, DATE_EXTRACTION
from tests.utils import request_body_to_json


@pytest.fixture
def type_transform_create_url(unittest_endpoint, project_id):
    return '{}/projects/{}/typeTransformFeatures/'.format(
        unittest_endpoint,
        project_id
    )
...
@pytest.fixture
def type_transform_create_responses(type_transform_create_url,
                                    async_url,
                                    feature_categorical_url,
                                    feature_categorical_json):
    responses.add(
        responses.POST,
    ...
    responses.add(
        ...
        feature_categorical_url,
        status=200,
        body=feature_categorical_json
    )


@responses.activate
@pytest.mark.usefixtures('client', 'type_transform_create_responses')
...
def test_create_respects_timeout_arg(project,
                                     async_url,
                                     type_transform_create_url,
                                     async_running_json,
                                     mock_async_time):
