...
class DatetimePartitioningSpecification(PartitioningMethod):
    ...
    default_to_do_not_derive : bool
        (New in v2.17) Optional, for time series projects only. Sets whether all features
        default to being treated as do-not-derive features, excluding them from feature derivation.
...
class DatetimePartitioning(object):
    ...
    validation_duration : str or None
        ...
        for the backtests. Can be absent if it's a time series project with an irregular primary
        date/time feature.
    available_training_start_date : datetime.datetime
    ...
    default_to_do_not_derive : bool
        (New in v2.17) Optional, for time series projects only. Sets whether all features
        default to being treated as do-not-derive features, excluding them from feature derivation.
        The default is false, all features are used for feature derivation. Individual features can
        be set to a value different than the default by using the `featureSettings` parameter.
    feature_derivation_window_start : int or None
    ...
    calendar_id : str, optional
        (New in version v2.15) The id of the :py:class:`CalendarFile <datarobot.CalendarFile>` to
        use with this project. Only available for time series projects.
    calendar_name : str, optional
        (New in version v2.17) The name of the :py:class:`CalendarFile <datarobot.CalendarFile>`
        used with this project. Only available for time series projects.
    """
    ...
    def __init__(self, project_id=None, datetime_partition_column=None, date_format=None,
                 ...
                 cross_series_group_by_columns=None, calendar_id=None, calendar_name=None):
