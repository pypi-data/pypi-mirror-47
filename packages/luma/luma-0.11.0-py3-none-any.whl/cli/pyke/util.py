...
class Util:
  ...
  def login_without_context(self, env_name, config_data=None):
    if config_data is None:
      config_data = pyke.auth.load_config()
    else:
      config_data = config_data

    env = config_data['envs'].get(env_name)
    if not env:
      raise click.ClickException(click.style("Could not find environment: {} - Are you sure it's configured correctly?".format(env_name), fg='red'))
  ...
  def list_companies(self, env, print_table=True):
    ...
    res = requests.get('{}/auth/v1/me/companies?sort=name'.format(app_url), headers=headers, verify=verify_tls)
  ...
  def get_container(self, container):
    try:
      ...
      return self.get_container_by_url_ref(container)

  def get_container_by_url_ref(self, url_ref):
    container_res = self.cli_request('GET', self.build_url('{app}/iot/v1/containers?urlRef={urlRef}', {'urlRef': url_ref}))
  ...
  def get_container_by_id(self, container_id):
    version = self.cli_request('GET', self.build_url('{app}/iot/v1/containers/{container_id}',\
