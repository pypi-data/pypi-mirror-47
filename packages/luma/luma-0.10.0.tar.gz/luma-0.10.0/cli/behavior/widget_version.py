...
class WidgetVersionBehavior:
    ...
    def create(self, widget, port, editor_port, is_editable, widget_file_path, docker_image,\
        ...
        if is_editable:
            if from_version:
                return behavior.ContainerVersionBehavior(profile=self.profile).create_editable_version(widget_id, from_version, env, format)
