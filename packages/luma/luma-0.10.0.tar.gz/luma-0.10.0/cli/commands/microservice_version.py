...
@click.option('--container-file-path', '-path', 'microservice_file_path',  type=click.Path(), help='The image must be a gzipped tar file. Ex: {file_name}.tar.gz')
...
@click.option('--json/--table', default=False, help='Return raw json from the platform.')
@pyke.auth.profile_context_required
def create(microservice, profile, port, editor_port, is_editable, microservice_file_path,\
  ...
  behavior.MicroserviceVersionBehavior(profile=profile, json=json).create(microservice, port,\
        editor_port, is_editable, microservice_file_path, docker_image, from_version, version, version_bump, env, label, format)
