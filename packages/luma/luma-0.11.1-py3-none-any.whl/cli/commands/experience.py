...
@click.option('--profile', '-p', prompt=True, help='The name of the profile to use as the context for the command.', autocompletion=pyke.auth.get_profile_names)
@click.option('--name', '-n', prompt=True, help='Name of the new Experience.')
@click.option('--description', '-d', help='Description of new Experience.')
@click.option('--collection', '-cn', prompt=True, help='The name of the collection where the new Experience will be placed.')
@click.option('--code', '-c', prompt=True, help='The code used to activate the Experience.')
@click.option('--device', '-d', prompt=True, type=click.Choice(['mobile', 'tablet', 'web']), default='mobile', help='The device type to display in the GUI.')
@click.option('--redirect-url', '-r', help='A valid webaddress that the Experience will redirect.')
@click.option('--json', is_flag=True, help='Return raw json from the platform')
@pyke.auth.profile_context_required
def create(**kwargs):
  click.echo("Hi")
