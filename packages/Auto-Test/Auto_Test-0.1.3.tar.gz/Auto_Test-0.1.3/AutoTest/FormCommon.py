...
def mail(file,filename):
    mail_user="dongchunrong@hfjy.com"    #发送邮件的邮箱
    mail_pass="Icydong123"   #口令
    mailto_list=['dongchunrong@hfjy.com',]  #接收邮件的邮箱
    mail_host="smtp.hfjy.com"  #设置服务器

    msg = MIMEMultipart()
    msg['Subject'] = "[设班单测试报告]" #主题
    msg['From'] = mail_user
    #文字部分
    strstr="设班单测试报告"  #文字内容
...
def upload(cases,start_time,single_response_time):
    ...
    body = {
        ...
        "reportUrl": "",
        "details": details}
    r = requests.request('POST', 'http://goruns.hfjy.red/4s-v2.12.0/autotest/upload', body=body, header=header)
    print(body)
    print("上传报告结果：" + r.json())
