...
class Image_Segmentation(Application):
    ...
    def train(self,
        ...
        dataset_train, dataset_val = self.make_train_valid_dataset_and_set_class_list(data_path)
    ...
    def evaluate(self, data_path, annotation_file_name = "via_region_data.json"):
        if self.action != "predict":
            self.create_model(mode="inference")
            self.load_weights_for_predict(self.weights_path)
            self.action = "predict"
        ...
        if get_image_from == "directory":
            ...
            dataset_eval.load_class(self.val_dataset)
    ...
    def make_train_valid_dataset_and_set_class_list(self, data_path):
        ...
        dataset_train.load_class(self.train_dataset)
        dataset_train.prepare()
        print("Images: {}\nClasses: {}".format(
            len(dataset_train.image_ids), dataset_train.class_names))

        dataset_val = ImageDataset()
        dataset_val.load_class(self.val_dataset)
        dataset_val.prepare()
        print("Images: {}\nClasses: {}".format(
