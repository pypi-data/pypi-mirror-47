...
from ....lib.image_lib import merge_image, get_image_names_in_directory, split_one_image, save_image
from ....lib.model_lib import generator_model, discriminator_model, generator_containing_discriminator_multiple_outputs
from ....lib.loss_lib import wasserstein_loss, perceptual_loss
...
class Deblur_GAN(Application):
    ...
    def predict(self, data_path, batch_size=1, result_path=Config_Deblur.RESULT_FOLDER):
    ...
    def _fit_gan(self,
        ...
        for epoch in range(0, epochs):
            ...
            if self.deepblock_log:
                logJson = OrderedDict()
                logJson[DeepblockLogConfig.LOG_TYPE] = "epochend"
                logJson['totalEpoch'] = epochs
                logJson['nowEpoch'] = epoch
                logJson['d_loss'] = str(np.mean(d_losses))
                logJson['d_on_g_loss'] = str(np.mean(d_on_g_losses))
    ...
    def create_and_compile_model(self,
        ...
        d_on_g_opt = Adam(lr=learning_rate, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
    ...
    def save_merged_image(self, image, result_path, image_path):
        ...
        if self.deepblock_log == True:
            logJson = OrderedDict()
            logJson[DeepblockLogConfig.LOG_TYPE] = "fileGenerated"
            logJson['filename'] = image_path
            print(json.dumps(logJson, ensure_ascii=False))
    ...
    def load_predict_images_data(self, path):
        ...
        for path_image in all_image_paths:
            ...
            for i in range(len(split_images)):
                images_all.append(self.preprocess_image(split_images[i]))
