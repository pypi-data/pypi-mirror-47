...
class ImageDataset(utils.Dataset):
    ...
    def load_class(self, dataset, annotation_path):
        ...
        coco_json = list(coco_json.values())  # don't need the dict keys

        classes = []
        for c in coco_json[1]:
        ...
        class_ids = []
        j = 0
        for a in sorted(coco_json[2], key=lambda ann: ann['imageId']):
            ...
            if imageId == a['imageId']:
                ...
                if j != 0:
                    filename = coco_json[0][j-1]['fileName']
