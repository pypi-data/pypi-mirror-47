...
class MaskRCNNModel:
    ...
    def detect(self, data, option=1):
        ...
        assert option in self._OPTION_  or option is None, "Model -> detect -> 지원하지 않은 옵션입니다."
        ...
        issave = False
        if option in [self.FIRST_BBOX_DATA_WITH_SAVE, self.FIRST_POLYGON_DATA_WITH_SAVE]:
            issave=True
        img = result[0]
        annotations = []
        
        if option == self.FIRST_BBOX_DATA:
