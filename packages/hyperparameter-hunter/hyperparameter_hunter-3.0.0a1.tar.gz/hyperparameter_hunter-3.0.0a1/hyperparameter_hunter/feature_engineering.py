...
def merge_dfs(merge_to: str, stage: str, dfs: DFDict) -> pd.DataFrame:
    ...
    # TODO: This doesn't deal with combining "inputs" and "targets into "data"
    #   Fails when trying to `merge_to="all_data"`, `stage="pre_cv"`, where only train data was given
    #   Seems to fail because `dfs` has not built "train_data" key
    #   - Only ["all_inputs" (from previous step), "train_inputs", "train_targets"]
