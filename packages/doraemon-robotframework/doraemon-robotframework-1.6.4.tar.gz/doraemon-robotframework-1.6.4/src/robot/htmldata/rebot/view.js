...
function setTitle(suiteName, type) {
    ...
    robot_array = document.title.split('&');
    if (robot_array.length > 1) {
        document.title = `${robot_array.length}个Suites的报告单`
    }
...
function addHeader() {
    ...
    $.tmpl('<h1 style="padding: 0; margin: 0; width: auto;">${title}</h1>' +
           '<span style="margin-left: 10px;">Generated ${generated.split(" UTC")[0]}</span>' +
