...
import os
import hashlib
from morphenepythongraphenebase import bip38
from morphenepythongraphenebase.account import PrivateKey
...
from morphenepythonapi.exceptions import NoAccessApi
...
class Wallet(object):
    """ The wallet is meant to maintain access to private keys for
        ...
        :param MorpheneNodeRPC rpc: RPC connection to a Morphene node
        ...
        * **Wallet Database**: Here, morphenepython loads the keys from the
          locally stored wallet SQLite database (see ``storage.py``).
          To use this mode, simply call :class:`morphenepython.morphene.MorpheneClient` without the
          ``keys`` parameter
        * **Providing Keys**: Here, you can provide the keys for
          your accounts manually. All you need to do is add the wif
          keys for the accounts you want to use as a simple array
          using the ``keys`` parameter to :class:`morphenepython.morphene.MorpheneClient`.
        * **Force keys**: This more is for advanced users and
        ...
        .. code-block:: python
           ...
           mph.wallet.create("supersecret-passphrase")

        This will raise :class:`morphenepython.exceptions.WalletExists` if you already have a wallet installed.
        ...
        .. code-block:: python
           ...
           from morphenepython import MorpheneClient
           mph = MorpheneClient()
           mph.wallet.unlock("supersecret-passphrase")
        ...
        **after** unlocking the wallet with the correct passphrase:
        ...
        .. code-block:: python
           ...
           mph = MorpheneClient()
           mph.wallet.unlock("supersecret-passphrase")
           mph.wallet.addPrivateKey("5xxxxxxxxxxxxxxxxxxxx")
    ...
    def __init__(self, morphene_instance=None, *args, **kwargs):
    ...
    def setToken(self, loadtoken):
        """ This method is strictly only for in memory token that are
            passed to Wallet/Morphene with the ``token`` argument
        """
        log.debug(
    ...
    def tryUnlockFromEnv(self):
        ...
        """
        password_storage = self.morphene.config["password_storage"]
        if password_storage == "environment" and "UNLOCK" in os.environ:
            ...
            pwd = keyring.get_password("morphene", "wallet")
            self.unlock(pwd)
        else:
    ...
    def _get_pub_from_wif(self, wif):
        ...
        """
        # it could be either graphenebase or MPH so we can't check
        # the type directly
    ...
    def getAccountsFromPublicKey(self, pub):
        ...
        names = self.morphene.rpc.get_key_references([pub], api="account_by_key")
        for name in names:
            for i in name:
                yield i
    ...
    def getAllAccounts(self, pub):
        ...
        for name in self.getAccountsFromPublicKey(pub):
            try:
                account = Account(name, morphene_instance=self.morphene)
    ...
    def getKeyType(self, account, pub):
        """ Get key type
            ...
            :param morphenepython.account.Account/dict account: Account data
    ...
    def getPublicKeys(self):
        ...
        """
        if self.keyStorage:
            return self.keyStorage.getPublicKeys(prefix=self.morphene.prefix)
        else:
