...
import random
from morphenepythonbase import memo as BtsMemo
...
class Memo(object):
    """ Deals with Memos that are attached to a transfer
        ...
        .. code-block:: python
            ...
            from morphenepython.memo import Memo
            m = Memo("user1", "wallet.xeroc")
            m.morphene.wallet.unlock("secret")
        ...
        In Morphene, memos are AES-256 encrypted with a shared secret between sender and
        ...
        The high level memo class makes use of the morphenepy wallet to obtain keys
    ...
    def __init__(
        ...
        if to_account:
            self.to_account = Account(to_account, morphene_instance=self.morphene)
        if from_account:
            self.from_account = Account(from_account, morphene_instance=self.morphene)

    def unlock_wallet(self, *args, **kwargs):
        """ Unlock the library internal wallet
        """
        self.morphene.wallet.unlock(*args, **kwargs)
        return self
    ...
    def encrypt(self, memo, bts_encrypt=False):
        ...
        memo_wif = self.morphene.wallet.getPrivateKeyForPublicKey(
        ...
        if not hasattr(self, 'chain_prefix'):
            self.chain_prefix = self.morphene.prefix
    ...
    def decrypt(self, memo):
        ...
        if isinstance(memo, dict) and "to" in memo and "from" in memo and "memo" in memo:
            memo_to = Account(memo["to"], morphene_instance=self.morphene)
            memo_from = Account(memo["from"], morphene_instance=self.morphene)
            message = memo["memo"]
        else:
