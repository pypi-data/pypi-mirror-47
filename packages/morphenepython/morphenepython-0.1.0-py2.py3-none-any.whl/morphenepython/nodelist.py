...
class NodeList(list):
    ...
    def update_nodes(self, weights=None, morphene_instance=None):
        """ Reads metadata from fullnodeupdate and recalculates the nodes score
            ...
            .. code-block:: python
                ...
                nl.update_nodes(weights)
        """
        mph = morphene_instance or shared_morphene_instance()
        metadata = None
        account = None
        cnt = 0
        while metadata is None and cnt < 5:
            cnt += 1
            try:
                account = Account("fullnodeupdate", morphene_instance=mph)
    ...
    def get_nodes(self, exclude_limited=True, testnet=False, wss=True, https=True, not_working=False, normal=True):
    ...
    def get_testnet(self, testnet=True):
        """Returns testnet nodes"""
        return self.get_nodes(normal=False, testnet=testnet)
