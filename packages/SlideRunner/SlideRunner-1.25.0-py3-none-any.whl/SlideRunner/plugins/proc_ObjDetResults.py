    ...
    def queueWorker(self):
        ...
        while not quitSignal:
            ...
            if (self.ext=='.P'): # Pickled format - results for many slides
                ...
                if (np.array(self.resultsArchive[fname]).shape[-1]==5):
                    uniqueLabels = np.unique(np.array(self.resultsArchive[fname])[:,3])
                else:
                ...
                for idx in range(len(self.resultsArchive[fname])):
                        ...
                        if (row.shape[-1]==5):
                            if (row[4]>job.configuration['threshold']):
                                myanno = annotations.circleAnnotation(uid=idx, x1=row[0], y1=row[1], r=row[2], text='%.2f' % row[4], pluginAnnotationLabel=self.annotationLabels[row[3]])                
