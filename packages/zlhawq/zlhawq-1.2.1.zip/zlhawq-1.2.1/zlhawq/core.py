...
def out_t_gg_all(quyu,dir,conp):
    ...
    arr=quyu.split("_")
    s1,s2=arr[0],'_'.join(arr[1:])
    sql="""select distinct on(gg_name,href,quyu) encode(digest(name||a.href||'%s','md5'),'hex') as guid ,
        name as gg_name,public.exttime(ggstart_time,page,'%s') as fabu_time,a.href,ggtype,jytype,diqu,'%s' as quyu,info
        ,replace(replace(replace(replace(b.page,'\001',''),'\002',''),'\r',''),'\n','') as page
        from %s.gg as a ,%s.gg_html  as b where a.href=b.href  
        """%(quyu,quyu,quyu,s2,s2)
    print(sql)
...
def out_t_gg_cdc(quyu,dir,conp):
    ...
    sql="""with b as(%s)
       ...
       a.href=b.href and a.ggstart_time=b.ggstart_time and b.name is not null and b.href is not null and b.ggstart_time is not null ) )
        ...
        name as gg_name,public.exttime(ggstart_time,page,'%s') as fabu_time,b1.href,ggtype,jytype,diqu,'%s' as quyu,info
        ,replace(replace(replace(replace(b.page,'\001',''),'\002',''),'\r',''),'\n','') as page
...
#-------------------------------{事件处理一
def out_t_gg_all_1(quyu,dir,conp):
    path1=os.path.join(dir,"t_gg_cdc_%s.csv"%quyu)
    print(path1)
    arr=quyu.split("_")
    s1,s2=arr[0],'_'.join(arr[1:])
    sql="""select distinct on(gg_name,href,quyu) encode(digest(name||a.href||'%s','md5'),'hex') as guid ,
        name as gg_name,public.exttime(ggstart_time,page,'%s') as fabu_time,a.href,ggtype,jytype,diqu,'%s' as quyu,info
        ,replace(replace(replace(replace(b.page,'\001',''),'\002',''),'\r',''),'\n','') as page
        from %s.gg as a ,%s.gg_html  as b where a.href=b.href    and  public.exttime(ggstart_time,page,'%s') is not null
        """%(quyu,quyu,quyu,s2,s2,quyu)
    print(sql)
    #df=db_query(sql=sql,dbtype="postgresql",conp=conp)
...
def out_t_gg_cdc_1(quyu,dir,conp):
    #quyu="anhui_chizhou"
    path1=os.path.join(dir,"t_gg_cdc_%s.csv"%quyu)
    arr=quyu.split("_")
    s1,s2=arr[0],'_'.join(arr[1:])
    ...
    sql="""with b as(%s)
       ...
       a.href=b.href and a.ggstart_time=b.ggstart_time and b.name is not null and b.href is not null and b.ggstart_time is not null ) )
        ...
        name as gg_name,public.exttime(ggstart_time,page,'%s') as fabu_time,b1.href,ggtype,jytype,diqu,'%s' as quyu,info
        ,replace(replace(replace(replace(b.page,'\001',''),'\002',''),'\r',''),'\n','') as page
        from b1 ,%s.gg_html  as b where b1.href=b.href  and  public.exttime(ggstart_time,page,'%s') is not null 
     """%(sql,s2,quyu,quyu,s2,quyu)
    print(sql)
    #df=db_query(sql=sql,dbtype="postgresql",conp=conp)
    #df.to_csv(path1,sep='\001',quotechar='\002',index=False)
    pg2csv(sql,conp,path1,chunksize=10000,sep='\001',quotechar='\002')
...
def add_quyu_tmp(quyu,conp_pg,conp_hawq,dir,addr,tag='all'):
    ...
    print("创建函数")
    est_funs(conp_pg)
    if quyu in ["anhui_hefei","anhui_fuyang"]:
        if tag=='all':
            out_t_gg_all_1(quyu,dir,conp_pg)
        else:
            out_t_gg_cdc_1(quyu,dir,conp_pg)
    else:
