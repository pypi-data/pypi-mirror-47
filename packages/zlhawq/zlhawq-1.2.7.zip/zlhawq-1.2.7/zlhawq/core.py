...
def update_t_gg(quyu,conp):
    ...
    guid,gg_name,cdc.exttime(fabu_time,page,'%s') as fabu_time,gg_href,ggtype,jytype,diqu,quyu,info,now()::timestamp(0) as create_time,page
     ...
     FROM cdc.t_gg_cdc_%s a where not exists (select 1 from %s.t_gg as b where quyu='%s' and a.guid=b.guid)  and gg_name is not null and 
     cdc.exttime(fabu_time,page,'%s') is not null
