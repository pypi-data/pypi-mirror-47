...
import gym
import numpy as np
from agent.agents.abstract.dfo_agent import DFOAgent
...
def t():
  # Task settings:
  env = NeodroidWrapper(gym.make('CartPole-v0'))  # Change as needed
  ...
  for itr in range(n_iter):
    ...
    thetas = np.random.multivariate_normal(mean=theta_mean,
                                           ...
                                           size=batch_size)
    rewards = np.array(map(CEMAgent().noisy_evaluation, thetas))
