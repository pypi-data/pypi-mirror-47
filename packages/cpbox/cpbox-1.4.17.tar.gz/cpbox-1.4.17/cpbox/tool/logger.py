...
app_msg_pre = '%(time_iso_8601)s %(local_ip)s %(pid)d.%(tid)d '

readable_format = app_msg_pre + '%(levelname)s %(filename)s:%(lineno)d %(message)s'

event_log_by_filelog_format = syslog_full_msg_pre + app_msg_pre + '%(message)s'
clog_by_filelog_format = syslog_full_msg_pre + app_msg_pre + '%(clog_message)s'

event_log_by_syslog_format = syslog_client_msg_pre + app_msg_pre + '%(message)s'
clog_by_syslog_format = syslog_client_msg_pre + app_msg_pre + '%(clog_message)s'
...
def _get_rotating_filehandler(app, filename):
    ...
    return logging.handlers.RotatingFileHandler(filename, mode='a', maxBytes=1024 * 1024 * 128, backupCount=3)
