...
class RigolMSO5002(RigolScope):

    def __init__(self, command, instrument=None):
        super(RigolMSO5002, self).__init__(command, instrument)
    ...
    def check_any_channel_enabled(self):
        ...
        channels_enabled = []
        logger.info("checking channels enabled in rigol MSO5002 driver")
        tlogger.info("checking channels enabled in rigol MSO5002 driver")
...
if __name__ == "__main__":
    import collections
    data = {'info': {'instrument_type': 'RigolMSO5002'}}
    command = collections.defaultdict(str, data)
    tester = RigolMSO5002(command=command)
    tester.test()
