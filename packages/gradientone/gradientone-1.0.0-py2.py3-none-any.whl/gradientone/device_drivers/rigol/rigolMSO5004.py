...
class RigolMSO5004(RigolScope):

    def __init__(self, command, instrument=None):
        super(RigolMSO5004, self).__init__(command, instrument)
    ...
    def check_any_channel_enabled(self):
        ...
        channels_enabled = []
        logger.info("checking channels enabled in rigol MSO5004 driver")
        tlogger.info("checking channels enabled in rigol MSO5004 driver")
...
if __name__ == "__main__":
    import collections
    data = {'info': {'instrument_type': 'RigolMSO5004'}}
    command = collections.defaultdict(str, data)
    tester = RigolMSO5004(command=command)
    tester.test()
