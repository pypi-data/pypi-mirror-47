...
class RigolDS1074ZPlus(RigolScope):

    def __init__(self, command, instrument=None):
        super(RigolDS1074ZPlus, self).__init__(command, instrument)
...
if __name__ == "__main__":
    import collections
    data = {'info': {'instrument_type': 'RigolDS1074ZPlus'}}
    command = collections.defaultdict(str, data)
    tester = RigolDS1074ZPlus(command=command)
    tester.test()
