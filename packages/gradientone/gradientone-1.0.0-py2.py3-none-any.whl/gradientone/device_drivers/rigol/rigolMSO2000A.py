...
class RigolMSO2000A(RigolScope):

    def __init__(self, command, instrument=None):
        super(RigolMSO2000A, self).__init__(command, instrument)
    ...
    def check_any_channel_enabled(self):
        ...
        channels_enabled = []
        logger.info("checking channels enabled in RigolDS1000ZPlus driver")
        tlogger.info("checking channels enabled in RigolDS1000ZPlus driver")
        for channel in self.instr.channels[0:2]:
            if channel.enabled:
...
if __name__ == "__main__":
    import collections
    data = {'info': {'instrument_type': 'RigolMSO2000A'}}
    command = collections.defaultdict(str, data)
    tester = RigolMSO2000A(command=command)
    tester.test()
