...
class RigolMSO7000(RigolScope):

    def __init__(self, command, instrument=None):
        super(RigolMSO7000, self).__init__(command, instrument)
    ...
    def check_any_channel_enabled(self):
        ...
        channels_enabled = []
        logger.info("checking channels enabled in rigol MSO7000 driver")
        tlogger.info("checking channels enabled in rigol MSO7000 driver")
...
if __name__ == "__main__":
    import collections
    data = {'info': {'instrument_type': 'RigolMSO7000'}}
    command = collections.defaultdict(str, data)
    tester = RigolMSO7000(command=command)
    tester.test()
