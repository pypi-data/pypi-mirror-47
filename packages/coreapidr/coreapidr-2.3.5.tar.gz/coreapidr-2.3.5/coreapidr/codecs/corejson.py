...
from coreapidr.compat import force_bytes, string_types, urlparse
from coreapidr.compat import COMPACT_SEPARATORS, VERBOSE_SEPARATORS
from coreapidr.document import Document, Link, Array, Object, Error, Field
from coreapidr.exceptions import ParseError
import coreschema
import json
...
def _primitive_to_document(data, base_url=None):
    ...
    if isinstance(data, dict) and data.get('_type') == 'document':
        ...
        return Document(
            ...
            media_type='application/coreapidr+json',
            content=content
        )
...
class CoreJSONCodec(BaseCodec):
    ...
    media_types = ['application/coreapidr+json', 'application/vnd.coreapidr+json']
