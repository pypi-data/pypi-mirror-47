...
class AppStoreProduct:
    ...
    def query_cls(cls, root, paths):
        ...
        return root.xpath(query_str, namespaces={'x': XML_NAMESPACE})
    ...
    def price_plan(self):
        ...
        for intv in intervals:
            ...
            if end_date_maybe:
                item['end_date'] = end_date_maybe[0].txt
            ret.append(item)
        return ret
    ...
    def set_price_plan(self, price_plan):
        ...
        for ii, plan in enumerate(price_plan):
            if ii >= existing_cnt:
                ...
                n = intervals[ii]
            AppStoreProduct.upsert_cls(n, ['start_date'], plan['start_date'])
            AppStoreProduct.upsert_cls(n, ['wholesale_price_tier'], plan['priceTier']['tier']['id'])
            if plan.get('end_date'):
                AppStoreProduct.upsert_cls(n, ['end_date'], plan['end_date'])
        # 删除多余的
    ...
    def is_price_plan_equal(self, next_plan):
        ...
        if not next_plan and not now_plan:
            return True
        if next_plan and not now_plan:
            return False
        if not next_plan and now_plan:
            return False
        if len(next_plan) != len(now_plan):
            return False
        for ii in range(0, len(now_plan)):
            now_item = now_plan[ii]
            next_item = next_plan[ii]
            if now_item['start_date'] != next_item['start_date']:
                return False
            if now_item['tierId'] != next_item['priceTier']['tier']['id']:
                return False
            if now_item.get('end_date', -1) != next_item.get('end_date', -1):
                return False
        return True
