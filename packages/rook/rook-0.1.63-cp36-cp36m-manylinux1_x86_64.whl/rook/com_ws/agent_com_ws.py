...
class AgentCom(object):
        ...
        while self._running:
            try:
                try:
                    ...
                    headers = self._connection.getheaders()

                    if 'X-Rookout-SessionId' in headers:
                        self._session_id = headers.get('X-Rookout-SessionId')
                        logger.debug("Session ID: %s...", self._session_id[:6])
                    ...
                    if e.status_code == 403:  # invalid token
                        ...
                        self._ready_event.set()
                        return
                    if e.status_code == 401:  # invalid session id
                        logger.warning("Invalid session; re-registering")
                        self._session_id = None
                        resume = False
                        continue
                    else:
            ...
            if got_initial_augs is not None:
    ...
    def _create_connection(self):
        ...
        if self._session_id is not None:
            logger.debug('Resuming session %s...', self._session_id[:6])
            headers["X-Rookout-SessionId"] = self._session_id
