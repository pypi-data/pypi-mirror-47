...
constants = mock_forge.get_constants()
...
def fileinfo(path):
    ...
    # Possible values for mime in this case include:
    #   'application/cdfv2-corrupt', 'application/cdfv2-unknown', 'application/cdfv2'
    if data['mime'].lower().startswith('application/cdfv2'):
        with open(path, 'r') as fh:
            buf = fh.read()
            buflen = len(buf)
