        ...
        if self.num_workers != 0:
            ...
            self.aggregator = mp.Process(
                ...
                args=(self.batch_size, self.result_queue, self.batch_queue,
                      self.pin_memory, self.collate_fn)
            )
...
def _worker_function(generator, worker_queue, result_queue):
    while True:
...
def _aggregator_function(batch_size, result_queue, batch_queue, pin_mem,
                         collate_fn):
    int_result = None
    count = 0
    while True:
        ...
        if pin_mem:
            batch_queue.put(pin_memory(collated))
        else:
            batch_queue.put(share_memory(collated))
