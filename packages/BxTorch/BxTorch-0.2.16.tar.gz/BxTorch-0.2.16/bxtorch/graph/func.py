...
from bxtorch.utils.numpy import cantor_pairing
import numba

def nonexisting_edges(graph, seed=None):
    """
    Returns a generator iterating randomly over the non-existing edges of the 
    graph. All returned edges will be of the format that the index of the first 
    node is smaller than the index of the second node. No self-loops are 
    returned. Note that this generator expects that the user samples much fewer
    edges than the number of nonexisting edges. Otherwise, do not use this
    function.
    ...
    while count < total:
        # Choose node
        n = randomizer.random.choice(graph.num_nodes)
        m = randomizer.random.choice(graph.num_nodes)
...
def random_subgraph(graph, size, seed=None):
    ...
    - numpy.ndarray [N, 2]
        ...
        The edges are always given with the node having the smaller index first.
        Use ``graph.utils.symmetric_edge_list`` to obtain 'directed' edges.
    """
