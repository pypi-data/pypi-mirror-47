...
class StackedLSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_sizes, bias=True, batch_first=False,
        ...
        self.stacked_cell = StackedLSTMCell(
            input_size, hidden_sizes, bias=bias, cudnn=cudnn
        )
...
class StackedLSTMCell(nn.Module):
    """
    Actually, the StackedLSTMCell can easily be constructed from existing 
    modules, however, a bug in PyTorch's JIT compiler prevents implementing
    anything where a stacked LSTM is used within a loop (see the following 
    issue: https://github.com/pytorch/pytorch/issues/18143). Hence, this
    class provides a single time step for a stacked LSTM.
    """

    __constants__ = ['cells', 'num_stacked']
    ...
    def __init__(self, input_size, hidden_sizes, bias=True, cudnn=True):
        ...
        - cudnn: bool, default: True
            Whether to not use cuDNN. In almost all cases, you don't want to
            set this value to false, however, you will need to change it if
            you want to compute higher-order derivatives of a network with
            a stacked LSTM cell.
        """
        super().__init__()
    ...
    def forward(self, 
        ...
        - states: list of tuple of (torch.Tensor [B, D], torch.Tensor [B, D]),
                default: None
            The states for each of the cells where each state is expected to
            have a size with batch size B and (respective) hidden dimension D.

        Returns:
        --------
        - torch.Tensor [B, D]
            The output, i.e. the hidden state of the deepest cell. Only given
            for convenience as it can be extracted from the other return value.
        - list of tuple of (torch.Tensor [B, D], torch.Tensor [B, D])
            The new hidden states and cell states for all cells.
        """

        if initial_states is None:
            # Compatibility with JIT
            states = [(torch.empty(0), torch.empty(0))] * self.num_stacked
        else:
            states = initial_states
