...
namespace apg {
    ...
    class lifetree_abstract {
        ...
        hypernode<I> fromrle(std::string s, RuleMapper *rm) {
            if (s != "") {
        ...
        hypernode<I> fromrle(std::string s) { return fromrle(s, 0); }
        ...
        virtual hypernode<I> read_macrocell(std::istream &instream, std::string &rule) = 0;

        hypernode<I> load_macrocell(std::string filename, std::string &rule) {
            std::ifstream f(filename);
            return read_macrocell(f, rule);
        }
