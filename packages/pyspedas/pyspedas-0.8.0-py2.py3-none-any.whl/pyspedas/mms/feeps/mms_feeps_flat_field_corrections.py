                    ...
                    for species_id in species:
                        if correction != 1.0:
                            ...
                            if count_rate is not None:
                                cr_times, cr_data = count_rate
                                cr_energies = pytplot.data_quants[cr_var[0]].spec_bins.values
                                store_data(cr_var[0], data={'x': cr_times, 'y': cr_data*correction, 'v': cr_energies.reshape(cr_energies.size)})
                            ...
                            if intensity is not None:
                                i_times, i_data = intensity
                                i_energies = pytplot.data_quants[i_var[0]].spec_bins.values
                                store_data(i_var[0], data={'x': i_times, 'y': i_data*correction, 'v': i_energies.reshape(i_energies.size)})
                            ...
                            if counts is not None:
                                c_times, c_data = counts
                                c_energies = pytplot.data_quants[c_var[0]].spec_bins.values
                                store_data(c_var[0], data={'x': c_times, 'y': c_data*correction, 'v': c_energies.reshape(c_energies.size)})
