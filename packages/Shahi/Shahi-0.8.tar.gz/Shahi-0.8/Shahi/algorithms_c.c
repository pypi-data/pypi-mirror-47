...
static Boolean isShahiLeapYear(int year){
    int a = mod(year - (year > 0 ? 1654 : 1653), 2820) + 474 + 38;
    return mod(a * 682, 2816) < 682;
}


static int getDaysInShahiYear(int year){

    return isShahiLeapYear(year) ? 366 : 365;
}


static Error getDaysInShahiMonth(int year, int month, int *days){
    if ( (1 <= month) && (month <= 6) ){
        *days = 31;
    }
        ...
        *days = isShahiLeapYear(year) ? 30 : 29;
    }
    else{
...
static double getJulianDayFromShahiDate(int year, int month, int day){
    int base = year - (year >= 0 ? 1654 : 1653);
...
static void getShahiDateFromJulianDay(double julianDay, int *year, int *month, int *day){
    double offset, cycle, daysInYears;
    int remaining, yearCycle, a1, a2;

    julianDay = floor(julianDay) + 0.5;

    // get_julianDay_from_Shahi_date(475, 1, 1) replaced by its static value
    ...
    *year = (int)(yearCycle + 2820*cycle + 1654);

    if ( *year <= 0 ){
        *year -= 1;
    }

    daysInYears = (julianDay - getJulianDayFromShahiDate(*year, 1, 1)) + 1;
    *month = (int) ceil(daysInYears <= 186 ? daysInYears / 31 : (daysInYears - 6) / 30);
    *day = (int) (julianDay - getJulianDayFromShahiDate(*year, *month, 1)) + 1;
...
static PyObject * is_Shahi_leap_year(PyObject *self, PyObject *args){
    int year;
    ...
    return PyBool_FromLong(isShahiLeapYear(year));
}


static PyObject * get_days_in_Shahi_year(PyObject *self, PyObject *args){
    int year;
    ...
    return PyLong_FromLong(getDaysInShahiYear(year));
}


static PyObject * get_days_in_Shahi_month(PyObject *self, PyObject *args){
    ...
    err = getDaysInShahiMonth(year, month, &result);
...
static PyObject * get_julian_day_from_Shahi_date(PyObject *self, PyObject *args){
    double julianDay;
    int year, month, day;
    ...
    julianDay = getJulianDayFromShahiDate(year, month, day);

    return PyFloat_FromDouble(julianDay);
}


static PyObject * get_Shahi_date_from_julian_day(PyObject *self, PyObject *args){
    float julianDay;
    int year, month, day;
    ...
    getShahiDateFromJulianDay(julianDay, &year, &month, &day);
...
static PyObject *  get_Shahi_date_from_gregorian_date(PyObject *self, PyObject *args){
    ...
    getShahiDateFromJulianDay(julianDay, &jYear, &jMonth, &jDay);
...
static PyMethodDef moduleFunctions[] = {
...

    {
        ...
        is_Shahi_leap_year,
        METH_VARARGS,
        "Determines the Shahi year is leap or not."
    },

    {
        ...
        get_days_in_Shahi_year,
        METH_VARARGS,
        "Determines the number of days in Shahi year."
    },

    {
        ...
        get_days_in_Shahi_month,
        METH_VARARGS,
        "Determines the number of days in Shahi month."
    },

    {
        ...
        get_julian_day_from_Shahi_date,
        METH_VARARGS,
        "Gets julian day from Shahi date."
    },

    {
        ...
        get_Shahi_date_from_julian_day,
        METH_VARARGS,
        "Gets Shahi date from julian day."
    },
...

    {
        ...
        get_Shahi_date_from_gregorian_date,
        METH_VARARGS,
        "Gets Shahi date from gregorian date."
    },
...
MOD_INIT(algorithms_c)
{
    ...
    MOD_DEF(m, "algorithms_c", "Shahi algorithms C implementation.", moduleFunctions);
