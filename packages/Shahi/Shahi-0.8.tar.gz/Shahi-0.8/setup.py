...
with open(os.path.join(os.path.dirname(__file__), 'Shahi', '__init__.py')) as v_file:
...
setup_args = dict(
    ...
    author="shahrooz",
    author_email="shahrooz.sanei@gmail.com",
    url="https://gitlab.com/shahroozD/shahi_calendar",
    description="Persian Date & Time library (aka: Shahi Calendar) with timezone, DST"
                "(daylight-saving), full formatting & parsing support for python 2 & 3 including c extention",
    zip_safe=True,
    keywords="Shahi persian date time datetime conversion",
    long_description=readme(os.path.join(os.path.dirname(__file__), 'README.rst')),
    license="GPLv3",
    packages=find_packages(),
    test_suite="Shahi.tests",
    tests_require=[
        'rtl'
    ],
    classifiers=[
