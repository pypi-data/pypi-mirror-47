...
class BakeryCandidate(BakeryBase):
    def get(self, request, candidate=None):
        selected_cand_query = Candidate.objects.filter(
        ...
        categories = Category.live.filter(Q(issue__in=issues))
        ...
        last_updated_issue = (
            issues.order_by("-last_published").first().last_published
        )

        last_updated_category = (
            categories.order_by("-last_published").first().last_published
        )
