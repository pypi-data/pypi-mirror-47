...
class TestAuditdExtract(unittest.TestCase):
    ...
    def test_extract_from_log_analytics(self):
        ...
        input_df['AuditdMessage'] = input_df.apply(lambda x: ast.literal_eval(x.AuditdMessage), axis=1)
        ...
        self.assertGreaterEqual(len(output_df[output_df['EventType'] == 'SYSCALL_EXECVE']), 82, 'Expected EXECVE events')
        self.assertGreaterEqual(len(output_df[output_df['EventType'] == 'LOGIN']), 1, 'Expected LOGIN events')
        self.assertGreaterEqual(len(output_df[output_df['EventType'] == 'USER_CMD']), 2, 'Expected USER_CMD events')
        self.assertGreaterEqual(len(output_df[output_df['EventType'] == 'USER_ACCT']), 1, 'Expected USER_ACCT events')

        for _, execve_event in output_df[output_df['EventType'] == 'SYSCALL_EXECVE'][1:20].iterrows():
