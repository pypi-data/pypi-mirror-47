...
def central_tendency(df,column):
  ...
  t_list= t_list + [[self.column,mean,median,mode,min,max,count,(str(zero)+"%"),(str(miss)+"%")]]
  Univariate.printing(t_list)
...
def Continuous_Analysis(df,column):
  ...
  Univariate.printing(t_list)
  Univariate.central_tendency(df,column)
  t_list= [['Univariate analysis calculate measure of dispersion']]
  Univariate.printing(t_list)
  Univariate.measure_of_dispersion(df,column)
