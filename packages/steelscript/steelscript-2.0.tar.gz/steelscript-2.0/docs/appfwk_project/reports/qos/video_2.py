...
#
description = """
<p> This report shows AF41 with its soft-limit vs the interface overall
vs the interface speed.
...
<li> Timeseries chart for AF41 with soft BW limit
     pulled from the database
<li> Timeseries chart for entire interface with interface limit
     pulled from the database
</ul>
"""
...
report = Report.create(
    'Video II', position=13.2, description=description,
...
p = NetProfilerTimeSeriesTable.create(
    'video2-af41-base', interface=True)
...
#
rs = qs.Resample.create('video2-af41-base-resampled', p,
                        resample={'avg_bits': 'mean'})

limit = qs.AddQosInterfaceFieldTable.create(
    'video2-af41-limit', rs, field='AF41_limit', field_label='AF41 limit')
...
#
p = NetProfilerTimeSeriesTable.create(
    'video2-overall-base', interface=True)
...
# Resample to the requested resolution
#
rs = qs.Resample.create('video2-overall-base-resampled', p,
                        resample={'avg_bits': 'mean'})

limit = qs.AddQosInterfaceFieldTable.create(
    'video2-overall-limit', rs, field='speed', field_label='If Speed')
