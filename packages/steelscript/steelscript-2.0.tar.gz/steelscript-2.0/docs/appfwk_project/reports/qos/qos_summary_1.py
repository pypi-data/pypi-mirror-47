...
#
description = """
<p>This report demonstrates a report based off data columns available directly
from NetProfiler reports run via REST.

<ul>
<li> Interface is a text entry field
<li> Hardcoded widgets for EF, AF42 and Default QoS classes.
<li> Metrics are in bytes, bits is not directly availabe via REST
</ul>
"""
...
report = Report.create(
    "QoS Summary I", position=11.1, description=description,
...
#
p = NetProfilerTimeSeriesTable.create('qs1-overall-util', interface=True)
...
for direction in ['in', 'out']:
    ...
    p = NetProfilerTimeSeriesTable.create('qs1-overall-%s' % direction,
                                          interface=True)
    ...
    report.add_widget(yui3.TimeSeriesWidget, p,
                      "Avg Bytes/s %sbound" % direction.capitalize(),
                      cols=['%s_avg_bytes' % direction], width=6)
...
for direction in ['inbound', 'outbound']:
    ...
    p = NetProfilerGroupbyTable.create('qs1-%s' % direction,
                                       groupby='dsc',
                                       interface=True)
    ...
    report.add_widget(yui3.TableWidget, p,
                      ...
                      cols=['dscp_name', 'dscp', 'avg_util',
                            'peak_util', 'avg_bytes', 'peak_bytes',
                            'total_bytes'])
...
for dscp in ['EF', 'AF41', 'Default']:
    ...
    for direction in ['inbound', 'outbound']:
        ...
        p = NetProfilerTimeSeriesTable.create(
            'qs1-%s-%s' % (dscp, direction), interface=True)
        ...
        report.add_widget(
            ...
            "%s - Avg Bytes/s %s" % (dscp, direction.capitalize()),
            width=6, cols=['avg_bytes'])
