...
#
description = """
<p> This report shows how to leverage a custom database table and
insert a custom dropdown menu for key interfaces of interest.
...
report = Report.create(
    'QoS Summary IV', position=11.4, description=description,
...
p = NetProfilerTimeSeriesTable.create('qs4-overall-in', interface=True)
...
p = NetProfilerGroupbyTable.create('qs4-inbountd',
                                   groupby='dsc',
                                   interface=True)
...
for dscp in ['EF', 'AF41', 'Default']:
    ...
    p = NetProfilerTimeSeriesTable.create(
        'qs4-%s-inbound' % (dscp), interface=True)
