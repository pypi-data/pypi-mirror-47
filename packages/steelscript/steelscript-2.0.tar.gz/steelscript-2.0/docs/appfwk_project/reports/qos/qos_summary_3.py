...
#
description = """
<p> This report focuses on the <i>inbound</i> traffic of
<a href='/report/qos/qos_summary_2/'>QoS Summary II</a>.
This will be the basis of most analysis for most other reports for simplicity.
...
report = Report.create(
    'QoS Summary III', position=11.3, description=description,
...
p = NetProfilerTimeSeriesTable.create('qs3-overall-in', interface=True)

p.add_column('time', 'Time', datatype='time', iskey=True)
p.add_column('in_avg_bytes', 'Avg Bytes/s Inbound', units='B/s')
p.add_column('in_avg_bits', 'Avg Bits/s Inbound', units='b/s',
             synthetic=True,
             compute_expression=('8*{in_avg_bytes}'))

report.add_widget(yui3.TimeSeriesWidget, p,
                  'Avg Bits/s Inbound', cols=['in_avg_bits'], width=6)
...
p = NetProfilerGroupbyTable.create('qs3-inbountd',
                                   groupby='dsc',
                                   interface=True)
...
for dscp in ['EF', 'AF41', 'Default']:
    ...
    p = NetProfilerTimeSeriesTable.create(
        'qs3-%s-inbound' % (dscp), interface=True)
