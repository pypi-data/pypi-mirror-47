...
def ryxx_html_work(conp, f, data, **args):
    ...
    m = page()
    if flag:
        sql = """SELECT distinct href,ryxx_href FROM "%s"."jianzhu_zcry_html" as b where (right(ryxx_href,1)::int)%%%%5=%s""" % (conp[4], flag)
...
def ryxx_html_cdc(conp, f, data, **args):
    ...
    if flag:
        sql = """SELECT distinct href,ryxx_href FROM "%s"."jianzhu_zcry_html" as b where not exists(select 1 from "%s"."jianzhu_ryxx_html" as a where a.ryxx_href=b.ryxx_href ) and (right(ryxx_href,1)::int)%%%%5=%s""" % (conp[4], conp[4],flag)
    else:
...
def xmxx_html_work(conp, f, data, **args):
    ...
    m = page()
    if flag:
        sql = """SELECT distinct href,xmxx_href FROM "%s"."jianzhu_gcxm_html" as b where (right(xmxx_href,1)::int)%%%%5=%s""" % (conp[4], flag)
    elif size is not None:
...
def xmxx_html_cdc(conp, f, data, **args):
    ...
    if flag:
        sql = """SELECT distinct href,xmxx_href FROM "%s"."jianzhu_gcxm_html" as b where not exists(select 1 from "%s"."jianzhu_xmxx_html" as a where a.xmxx_href=b.xmxx_href ) and (right(xmxx_href,1)::int)%%%%5=%s""" % (conp[4], conp[4], flag)
    else:
