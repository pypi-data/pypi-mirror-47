...
def ryxx_html_work(conp, f, data, **args):
    ...
    m = page()
    if flag:
        sql = """SELECT href,ryxx_href FROM "%s"."jianzhu_zcry_html" as b where (right(ryxx_href,1)::int)%%%%5=%s""" % (conp[4], flag)
    # elif size is not None:
    #     sql = "select href,ryxx_href from %s.jianzhu_zcry_html limit %d" % (conp[4], size)
    else:
...
def ryxx_html_cdc(conp, f, data, **args):
    ...
    if flag:
        sql = """SELECT href,ryxx_href FROM "%s"."jianzhu_zcry_html" as b where not exists(select 1 from "%s"."jianzhu_ryxx_html" as a where a.ryxx_href=b.ryxx_href ) and (right(ryxx_href,1)::int)%%%%5=%s""" % (conp[4], conp[4],flag)
    else:
...
def gcxm_html_work(conp, f, data, **args):
    ...
    m = page()
    if size is not None:
        sql = "select href,gcxm from %s.jianzhu_gg_html limit %d" % (
        conp[4], size)
    else:
        sql = "select href,gcxm from %s.jianzhu_gg_html" % (conp[4])
...
def gcxm_html_cdc(conp, f, data, **args):
    ...
    if ft == None:
        sql = """select href,gcxm from %s.jianzhu_gg_html as a where not exists(select 1 from %s.jianzhu_gcxm_html as b where
...
def removal_gcxm_html_data(conp):
    ...
    sql3 = """
        ...
        select ctid from
        (select row_number() over(partition by href,xmxx_name,xmxx_href order by ctid) as rn,ctid from %s.jianzhu_gcxm_html)as t where t.rn<>1)
    """% (schema, schema)
    db_command(sql3, dbtype="postgresql", conp=conp)
...
def xmxx_html_work(conp, f, data, **args):
    ...
    m = page()
    if flag:
        sql = """SELECT href,xmxx_href FROM "%s"."jianzhu_gcxm_html" as b where (right(xmxx_href,1)::int)%%%%5=%s""" % (conp[4], flag)
    # elif size is not None:
    #     sql = "select href,ryxx_href from %s.jia nzhu_cry_html limit %d" % (conp[4], size)
    else:
        sql = "select href,xmxx_href from %s.jianzhu_gcxm_html" % (conp[4])
...
def xmxx_html_cdc(conp, f, data, **args):
    ...
    if flag:
        sql = """SELECT href,xmxx_href FROM "%s"."jianzhu_gcxm_html" as b where not exists(select 1 from "%s"."jianzhu_xmxx_html" as a where a.xmxx_href=b.xmxx_href ) and (right(xmxx_href,1)::int)%%%%5=%s""" % (conp[4], conp[4], flag)
    else:
        sql = """select href,xmxx_href from %s.jianzhu_gcxm_html as a where not exists(select 1 from %s.jianzhu_xmxx_html as b where
         a.xmxx_href=b.xmxx_href and (xmxx_name, xmxx_href, xm_id,xm_diqu, xm_type, qy_name) is not null)""" % (conp[4], conp[4])
