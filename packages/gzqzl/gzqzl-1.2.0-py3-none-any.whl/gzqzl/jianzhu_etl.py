...
def ryxx_html_work(conp, f, data, **args):
    ...
    m = page()
    if flag:
        sql = """SELECT href,ryxx_href FROM "jianzhu"."jianzhu_zcry_html" as b where not exists(select 1 from "jianzhu"."jianzhu_ryxx_html" as a where a.ryxx_href=b.ryxx_href ) and (right(ryxx_href,1)::int)%%%%5=%s""" % (flag)
...
def gcxm_html_cdc(conp, f, data, **args):
    ...
    if ft == None:
        sql = """select href,gcxm from %s.jianzhu_gg_html as a where not exists(select 1 from %s.jianzhu_gcxm_html as b where
         a.href=b.href and ("xmxx_name", "xmxx_href", "xm_id", "xm_name", "xm_diqu", "xm_type", "qy_name") is not null)""" % (conp[4], conp[4])
...
def xmxx_html_work(conp, f, data, **args):
    ...
    m = page()
    if flag:
        sql = """SELECT ryxx_href FROM "jianzhu"."jianzhu_zcry_html" as b where not exists(select 1 from jianzhu.jianzhu_ryxx_html as a where a.ryxx_href=b.ryxx_href ) and (right(ryxx_href,1)::int)%5={}""".format(flag)
    # elif size is not None:
