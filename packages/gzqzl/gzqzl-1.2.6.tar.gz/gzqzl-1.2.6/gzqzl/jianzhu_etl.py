...
def xmxx_html_work(conp, f, data, **args):
    ...
    m = page()
    if flag:
        sql = """SELECT distinct href,xmxx_href FROM "%s"."jianzhu_gcxm_html" where (right(xmxx_href,1)::int)%%%%5=%d""" % (conp[4], flag)
    elif size is not None:
...
def xmxx_html_cdc(conp, f, data, **args):
    ...
    if flag:
        sql = """SELECT distinct href,xmxx_href FROM "%s"."jianzhu_gcxm_html" as b where not exists(select 1 from "%s"."jianzhu_xmxx_html" as a where a.xmxx_href=b.xmxx_href ) and (right(xmxx_href,1)::int)%%%%5=%d""" % (conp[4], conp[4], flag)
    else:
