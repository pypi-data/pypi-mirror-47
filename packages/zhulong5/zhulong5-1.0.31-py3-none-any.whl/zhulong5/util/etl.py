...
def est_cdc(conp, data, **args):
    ...
    old_year_g, old_month_g = re.findall('quanguoxiangmu\_(\d+)\_(\d+)\_\d+\_\d+\_\d+\_\d+\_gg', latest_gg_table_name)[0]
    old_year_c, old_month_c = re.findall('quanguoxiangmu\_(\d+)\_(\d+)\_\d+\_\d+\_\d+\_\d+\_gg_cdc', latest_gg_table_name)[0]
    ...
    else: old_year, old_month = old_year_c, old_month_c

    now_year, now_month = datetime.strftime(datetime.now(), '%Y'), datetime.strftime(datetime.now(), '%m')

    if int(old_year) >= 2016:

        count = (int(now_year) - int(old_year)) * 12 + (int(now_month) - int(old_year)) + 1
