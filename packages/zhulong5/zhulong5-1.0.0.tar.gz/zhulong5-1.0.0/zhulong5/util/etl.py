...
from zhulong5.util.dbv2 import db_command, db_query
import json
...
def est_cdc(conp, data, **args):
    ...
    sql2 = """
    select table_name from information_schema.tables where table_schema='%s' and table_name ~'^[^t].*01$' order by table_name desc limit 1
    """ % conp[4]

    df1 = db_query(sql2, conp=conp, dbtype="postgresql")
    latest_gg_table_name = df1['table_name'].tolist()[0]
    old_year, old_month = re.findall('quanguoxiangmu\_(\d+)\_(\d+)\_\d+\_\d+\_\d+\_\d+', latest_gg_table_name)[0]
