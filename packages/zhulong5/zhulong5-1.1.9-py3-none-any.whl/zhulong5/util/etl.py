...
def insert_tb(tbname, diqu, conp):
    ...
    sql2 = """
    insert into %s.gg
    select  distinct on (name,href,ggstart_time ) name,href,ggstart_time,xiangmu_code,shenpishixiang,shenpibumen,status, %s::text diqu,info 
    from %s.%s
    where (name,href,ggstart_time) not in(select name,href,ggstart_time from %s.gg)
    # """ % (schema, diqu,schema,tbname, schema)
    #
    # sql2 = """
    # insert into %s.gg
