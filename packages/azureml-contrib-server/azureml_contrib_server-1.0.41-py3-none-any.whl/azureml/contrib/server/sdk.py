...
from azureml._base_sdk_common.common import RUNCONFIGURATION_EXTENSION, get_run_config_dir_path
...
@dump_args
@time_method
def list_run_configurations(token, project_path):
    """Return a list of the run configurations found in a project."""
    run_configurations = RunConfiguration._get_all_run_configurations(project_path)
    result = []
    run_config_dir_path = get_run_config_dir_path(project_path)
    for name in run_configurations.keys():
        filename = name + RUNCONFIGURATION_EXTENSION
        path = os.path.join(run_config_dir_path, filename)
...
def create_image(token, subscription_id, resource_group, workspace_name, name, runtime, directory, execution_script,
                 ...
                 enable_gpu=False, tags=None, description=None, is_async=False):
