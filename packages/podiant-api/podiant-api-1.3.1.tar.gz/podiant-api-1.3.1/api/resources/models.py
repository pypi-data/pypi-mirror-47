            ...
            if any(rel_list):
                if isinstance(db_field, ManyToManyField):
                    if hasattr(db_field, 'rel'):
                        ...
                        db_rel = db_field.remote_field

                    if not db_rel.through._meta.auto_created:
                        raise AttributeError(
                            'Cannot set values on a ManyToManyField which '
                            'specifies an intermediary model. Override the '
                            '`unpack_relationships` method on this resource '
                            'to create the relationship manually.'
                        )
