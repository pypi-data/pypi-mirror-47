...
class AwsParameterStore:
    ...
    def parameter_res_to_dict(self, parameters_list: List, get_flat_dict: bool = False) -> Dict:
        parameters_dict = {}

        for param in parameters_list:
            [key, val] = self.strip_param(param)
            parameters_dict[key] = val

        if get_flat_dict:
            return parameters_dict

        return self.flat_dict_to_nested(parameters_dict)

    def flat_dict_to_nested(self, parameters_dict: Dict) -> Dict:
        ret = {}
        for recursive_key, value in parameters_dict.items():
            for key in reversed(recursive_key.split(PATH_DELIMITER)):
                value = {key: value}
            self.merge(ret, value)
        return ret

    @staticmethod
    def merge(base_dict: Dict, update_dict: Dict, path=None):
        if path is None:
            path = []
        for key in update_dict:
            if key in base_dict:
                if isinstance(base_dict[key], dict) and isinstance(update_dict[key], dict):
                    AwsParameterStore.merge(base_dict[key], update_dict[key], path + [str(key)])
    ...
    def strip_param(self, parameter) -> Tuple[str, Any]:
        name = parameter["Name"]
    ...
    def get_parameters_dict(self, namespace: str, decrypt: bool = True, get_flat_dict: bool = False) -> Dict:
        ...
        return self.parameter_res_to_dict(parameter_res, get_flat_dict)
