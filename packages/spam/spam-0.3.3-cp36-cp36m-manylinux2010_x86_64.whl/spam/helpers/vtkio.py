...
def tifToVTK(fileName, voxelSize=1.0):
    """
    ...
    print("spam.helpers.tifToVTK: image shape: ", im.shape)
    nCells = im.shape

    # create coordinates
    points = spam.mesh.createLexicoCoordinates([voxelSize * n for n in nCells], [n + 1 for n in nCells])
    print("spam.helpers.tifToVTK: points shape: ", points.shape)
    print("spam.helpers.tifToVTK: 0 min max: ", points[:, 0].min(), points[:, 0].max())
    print("spam.helpers.tifToVTK: 1 min max: ", points[:, 1].min(), points[:, 1].max())
    print("spam.helpers.tifToVTK: 2 min max: ", points[:, 2].min(), points[:, 2].max())

    # create connectivity
    cells = numpy.zeros((numpy.prod(nCells), 8)).astype('<u4')
    print("spam.helpers.tifToVTK: cells shape: ", cells.shape)
    nx, ny, nz = nCells
    i = 0
    for z in range(nz):
        for y in range(ny):
