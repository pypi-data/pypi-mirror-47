...
def simulateRandomField(lengths=(1, 1, 1), nNodes=(10, 10, 10), covariance={'type': 'stable', 'alpha': 2},
    ...
    if dim > 1:
        l = (lengths, lengths, lengths) if isinstance(lengths, float) else lengths
        n = (nNodes, nNodes, nNodes) if isinstance(nNodes, int) else nNodes
        nCells = [_-1 for _ in n]
    else:
        l = lengths if isinstance(lengths, (int, float)) else lengths[0]
    ...
    if vtkFile and dim == 3:
        ...
        for i in range(nRea):
            ...
            if nRea == 1:
                ...
                # tmp = rf[:, :, :, i].ravel()
                tmp = rf[:, :, :, i]
            spam.helpers.writeStructuredVTK(aspectRatio=[float(a)/float(b) for a,b in zip(l, nCells)], pointData={'RandomField': tmp}, fileName="{}.vtk".format(currentName))
