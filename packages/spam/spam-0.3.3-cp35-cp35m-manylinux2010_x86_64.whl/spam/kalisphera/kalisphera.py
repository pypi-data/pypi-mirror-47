...
#import spam.kalisphera._kalispheraToolkit as kaliC
import numpy
...
def makeSphere(vol, centre, radius):
    ...
    if len(vol.shape) != 3:
        print("\tkalisphera.makeSphere.run(), need 3D vol array")
        return -1
    ...
    if len(centre.shape) == 2:
        ...
        if len(radius) == centre.shape[0]:
            ...
            print("\tkalisphera.makeSphere.run(), Got multiple radii, but different number from number of centres")
            return -1
