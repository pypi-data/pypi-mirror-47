...
void computeCorrelationFunction(  float * vol,   unsigned int volSizeZ,   unsigned int volSizeY,   unsigned int volSizeX, double * output, unsigned int outputSize, unsigned int outputTwo, unsigned int stepCentre, unsigned int nthreads )

{    
     // Step 1 -- box and unique distances
    if ( outputTwo != 2 )
    {
        std::cout << "Output columns != 2, it is: " << outputTwo << std::endl;
        return;
    }   
