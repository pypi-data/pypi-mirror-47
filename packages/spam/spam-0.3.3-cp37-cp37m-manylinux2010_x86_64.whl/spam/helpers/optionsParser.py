...
def ldicParser(parser):
    ...
    parser.add_argument('-nosp',
                        ...
                        action="store_false",
                        dest='SUBPIXEL',
                        help='Disactivates subpixel refinement')
    ...
    parser.add_argument('-spi',
                        ...
                        help="Subpixel max iterations subpixel search. Default = 50")

    parser.add_argument('-spp',
                        '--sub-pixel-min-phi-change',
                        type=numpy.float,
                        default=0.001,
                        dest='SUBPIXEL_MIN_PHI_CHANGE',
                        help="Subpixel min change in Phi to stop subpixel search. Default = 0.001")
    ...
    # 2019-04-05 EA: 2D image detection approved by Christophe Golke
    if len(tiff.pages) == 1 and len(tiff.series[0]):
        twoD = True
    else:
        twoD = False
    tiff.close()
...
def multiModalRegistrationParser(parser):
    ...
    if args.PHIFILE is not None:
        import spam.helpers
        args.FGUESS = spam.helpers.readTSV(args.PHIFILE.name)['PhiField'][0]
    else:
...
def regularStrainsCalcParser(parser):
    ...
    parser.add_argument('-cdp',
                        ...
                        help="Delta Phi norm for a return status = 1 correlation window to consider the point good. Default = 0")
    ...
    parser.add_argument('-tif',
                        ...
                        action="store_true",
                        dest='TIF',
                        help='Activate TIF output format. Default = True')
...
def discreteStrainsCalcParser(parser):
    ...
    parser.add_argument('-tf',
                        ...
                        default=None,
                        dest='TRI_FILE',
                        help='Load a triangulation from file?')
    ...
    parser.add_argument('-rf',
                        ...
                        default=None,
                        dest='RADII_TSV_FILE',
                        help='Load a series of particle radii from file?')
    ...
    parser.add_argument('-rl',
                        ...
                        help='Load a labelled image and compute radii?')
    ...
    parser.add_argument('-kz',
                        ...
                        action="store_true",
                        dest='KEEP_ZERO',
                        help='Consider grain number zero? Default = False')
...
def BCFromDVCParser(parser):
    ...
    parser.add_argument('-pre',
                        ...
                        default=None,
                        dest='PREFIX',
                        help='Prefix for output files. Default is basename of mesh file (without extension)')
...
def moveGrainsParser(parser):
    parser.add_argument('LabFile',
                        metavar='LabFile',
                        type=argparse.FileType('r'),
                        help='Path to the labelled TIF file to be moved')
    ...
    parser.add_argument('-pre',
                        ...
                        default=None,
                        dest='PREFIX',
                        help='Prefix for output TIF file. Default is basename of input file (without extension)')
    ...
    parser.add_argument('-dm',
                        ...
                        help="Displacement margin for particle movement, this must be AT LEAST equal to the mazimum displacement. Default = 10")
    ...
    # Output file name prefix
    if args.PREFIX is None:
        args.PREFIX = os.path.splitext(os.path.basename(args.LabFile.name))[0]
