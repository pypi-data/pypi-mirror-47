...
class StakionSender(object):
    ...
    method: str, optional
        The method to send the messages. The two currently supported values
        are POST and SOCKET.
    ...
    def __init__(self,
        ...
        # Unsupported method
        if method not in ['POST', 'SOCKET']:
            raise ValueError("Method is not supported - valid values are: [POST, SOCKET]")
        
        if host is None:
            if method == 'POST':
                host = 'https://mamooh-223220.appspot.com/log/' + project_id
            elif method == 'SOCKET':
                host ='http://mamooh-223220.appspot.com'
        else:
            if method == 'POST':
                if host[-1] == '/':
                    host = host + 'log/' + project_id
                else:
                    host = host + '/log/' + project_id
            elif method == 'SOCKET':
                host = host
        ...
        if method == 'POST':
            self.publisher = publishers.publisherPost(
                url=host,
                api_key=api_key
            )
        else:
            self.publisher = publishers.publisherSocketIo(
                url=host,
                api_key=api_key
            )
    ...
    def _validate_message(self, message):
        ...
        """
        # Event types
        supported_event_types = ['prediction_requested', 'input_features',
    ...
    def _send(self, message):
        ...
        if len(self.pendings.keys()) > 0:
            bufsize = sum(len(json.dumps(s).encode('utf-8')) for s in self.pendings.values())
