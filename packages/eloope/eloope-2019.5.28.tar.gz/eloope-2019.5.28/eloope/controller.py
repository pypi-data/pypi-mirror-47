...
class Controller(service_pb2_grpc.ControllerServicer):
    ...
    def SendStatus(self, request, context):
        ...
        for task in pickle.loads(request.tasks):
            ...
            self.task_index += 1
        logger.system(f'Status - task_count= {len(self.task_list)} task_total= {self.task_index + 1}', 'Server')
        # Return tasks
    ...
    def SendCommand(self, request, context):
        ...
        if request.command == 'start':
            ...
            dump_file = Path(self.dump_path) / self.start_time.strftime('%Y-%m-%d %H-%M-%S')
            with open(dump_file, 'wb') as f:
                pickle.dump(self.task_set, f)
            return service_pb2.SendCommandResponse(info=f'Save successfully! file path: {dump_file}.')
            ...
            return service_pb2.SendCommandResponse(info=str(self.task_index + 1))
