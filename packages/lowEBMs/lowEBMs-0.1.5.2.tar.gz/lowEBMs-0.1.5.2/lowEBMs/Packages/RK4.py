...
def controlrun(configdic,accuracy,accuracy_number):
    from lowEBMs.Packages.Varibles import datareset
    ...
    print('Starting controlrun with a temperature accuracy of %s on the GMT over %s datapoints.' %(accuracy,accuracy_number))
    controldata=rk4alg(model_equation,eq,fun)
    #controltime,controlzmt,controlgmt
    runtime=time.time()-control_time
    print('Finished controlrun over %s seconds. Runtime: %s s' %(controldata[0][-1],runtime))
