...
class Processing():
	...
	def __init__(self, config, isLibrary=False, isDebug=False):
		"""Hub for running processing and analyzing raw data.
		...
		isDebug : :obj:`bool`
			Allows extra printouts to console. Default `False`.
		"""
		# debug
	...
	def preprocess(self, df, window):
		...
		#self.console('///////////Preprocessing: Drop Fixation Cross samples', 'green')
		...
		if d_loc.shape[0] >=1:
			...
			self.log.warning('event: preprocess(self, df, window), core: %s, subject: %s, trial: %s, dotloc rows: %s'
							 %(self.thisCore, self.config['processing']['subject'], self.config['processing']['trial'], d_loc.shape[0]))
		...
		## convert '.' to null
		if self.config['preprocessing']['remove_missing']:
			self.console('Preprocessing: Remove null samples', 'green')
			## drop null
		...
		if self.config['preprocessing']['remove_bounds']:
			self.console('Preprocessing: Remove samples outside of window bounds (i.e. 0 > x > 1920, 0 > y > 1080)', 'green')
		...
		if self.config['preprocessing']['remove_spikes']:
			self.console('Preprocessing: Remove one-sample spikes', 'green')
			max_delta = self.config['classify']['spike_delta']
			#x
		...
		# else mark spikes as bad
			...
			max_delta = self.config['preprocessing']['spike_delta']
			#x
			delta = df['x'].diff().shift(-1).abs()
			delta.iloc[-1] = delta.iloc[-2]
	...
	def getData(self, path=None):
		...
		if (self.config['processing']['single_subject']):
			path = '%s/%s_%sabc.csv'%(self.config['path']['raw'], self.config['processing']['subject'], self.config['processing']['session'])
			if self.isDebug: self.console('self.getData() path:%s'%(path), 'orange')

		#read csv as dataframe
		path = Path(path)
		if self.isDebug: self.console('self.getData() pd.read_csv:%s'%(path), 'orange')
		df = pd.read_csv(path, float_precision='high')
		
		if self.isDebug: self.console('self.getData(): read csv finished', 'orange')

		##rename
		if self.config['processing']['source'] != 'eyelink':
			if self.isDebug: self.console('self.getData() rename', 'orange')
			df = df.rename(columns={"trialNumTask": "TrialNum", "sample_time": "timestamp", "isWindowSuccess": "is_calibrated", 
			"LEmotion": "left_mood","REmotion": "right_mood"})

		#sort  
		if self.isDebug: self.console('self.getData() sort', 'orange')
		...
		if self.config['processing']['source'] != 'eyelink':
			...
			if self.isDebug: self.console('self.getData() isWebcamUsed', 'orange')
			df['type'] = np.where((df['isWebcamUsed']==True), 'eyetracking', 'behavioral')

		return df, path

	def filter_data(self, df, filter_type, config):
		"""
		...
		"""
		if self.isDebug: self.console('filter_data()', 'orange')
		g_t= df['timestamp']
		g_x = df['x']
		g_y= df['y']
		...
		if filter_type=='butterworth':
			...
			wn=config['filter']['f_b']['Wn']
			order=config['filter']['f_b']['N']
		...
		#gaussian filter
			...
			sigma=self.config['filter']['f_g']['sigma']
			print('sigma: %s'%(sigma))
		...
		#median filter
			...
			size=self.config['filter']['f_m']['size']
			print('size: %s'%(size))
		...
		#savitzky golay filter
			...
			if self.isDebug: self.console('filter_data() SavitzkyGolay', 'orange')
			from scipy.signal import savgol_filter
			if self.isDebug: self.console('filter_data() SavitzkyGolay', 'orange')
			window=self.config['filter']['f_sg']['window']
			order=self.config['filter']['f_sg']['order']
			self.console('window: %s'%(window), 'orange')
			self.console('order: %s'%(order), 'orange')
			try:
				f_x=savgol_filter(g_x, window_length=window, polyorder=order)
				f_y=savgol_filter(g_y, window_length=window, polyorder=order)
				if self.isDebug: self.console('filter_data() adding to DataFrame', 'orange')
				fxy_df = pd.DataFrame({'x':f_x, 'y':f_y, 'timestamp':g_t})
			except:
				fxy_df = None
		...
		#mean filter
			...
			weights=self.config['filter']['f_a']['weights']
	...
	def roi(self, filters=None, flt=None, df=None, manual=False, monitorSize=None):
		...
		for idx, itm in enumerate(filters):
			#filter type
			flt = itm[1]
			self.console('bounds: %s'%(itm), 'orange')

			#if not running model.roi manually (posthoc)
			if (manual == False):
				#real bounding box
				monitorSize=self.config['metadata']['lab']['resolution.px']
			else:
			...
			if (manual == False):
				self.config['roi']['stim_bounds'] = stim_bounds
			...
			#if not running processing.roi() manually (posthoc)
			if (manual == False):
				self.config['roi']['roi_bounds'] = roi_bounds

			#if using eyelink data and eyelink classification
			if ((self.config['processing']['source'] == 'eyelink') and (self.config['classify']['classify_eyelink_data'] == False)):
				pts = np.array(df[['x', 'y']])
			...
			if (manual == False):
				...
				#webgazer data
					...
					#If enough samples
					if (self.config['classify']['too_few_samples'] == False):
						#-----------------------------------------------------------------all fixation events
					...
					#not enough samples
						...
						not_enough_samples = 'subject: %s, trial: %s; not enough samples'%(self.config['processing']['subject'], self.config['processing']['trial'])
						self.debug(not_enough_samples)
						self.console(not_enough_samples, 'red')
			...
			#----------------------------------------------------------------------------------------test confirmation
			test = False
			if test and (self.config['processing']['subject'] != self.current_subject):
				self.current_subject = self.config['processing']['subject']
				import matplotlib
				matplotlib.use('Agg')
				import matplotlib.pyplot as plt
				import matplotlib.patches as patches
				self.console('Preprocessing: Test to ensure ROI', 'green')
				...
				#save
				dpi = self.config['output']['fig_dpi']
				emotion = self.config['emotion']
				resolution = self.config['metadata']['lab']['resolution.px']
				img_title = 'gaze coordinates [subject:%s, session:%s, trial:%s, isCongruent:%s, left:%s, right:%s (%s, %s)]'\
												%(self.config['processing']['subject'],self.config['processing']['session'],self.config['processing']['trial'],\
			...
			#--------finished
			#timestamp
			self.console('%s finished in %s msec'%(function, ((datetime.datetime.now()-t0).total_seconds()*1000)), 'blue')
			#if not running processing.roi() manually (posthoc)         
			if (manual == False):
				return df
			else:
	...
	def process(self, window, filters, gxy_df, trial, _classify=True, ctype='simple', _param='', 
		log=False, v_th=20, dr_th=200, di_th=20, _missing=0.0, _maxdist=25, _mindur=50):
		"""
		...
		for idx, itm in enumerate(filters):
			self.console('filter: %s' % (itm[0]), 'orange')
			#-------------------------------------------------------------------------parameters
			c_xy = []  # classify data

			#if using filter
			if itm[0] != 'none':
				if self.isDebug: self.console('process() filter_data', 'orange')
			...
			if fxy_df is None:
				self.console('too few samples', 'orange')
				fxy_df = gxy_df
				self.config['classify']['too_few_samples'] = True

			fxy_df = fxy_df.reset_index(drop=True)
			
			if self.isDebug: self.console('process() fixation classification techniques', 'orange')
			...
			if ctype == 'hmm':
				...
				elif (self.config['classify']['too_few_samples']):
				  pass
				else:
			...
			if (self.config['processing']['source'] == 'eyelink'):
				self.console('eyelink fixations', 'green')
				print(eyelink_c_xy)

			#print calculated fixations if from webgazer or calculating eyelink data
			if ((self.config['processing']['source'] == 'webgazer') or
					(self.config['processing']['source'] == 'eyelink') and (self.config['classify']['classify_eyelink_data'] == True)):
				self.console('calculated fixations', 'orange')
				#print(c_xy)
			...
			if (ctype == 'hmm') and (self.config['classify']['too_few_samples'] == False)\
	...
	def run(self, path, task_type="eyetracking", single_subject=False, single_trial=False, subject=0, trial=0, isMultiprocessing=True, cores=1):
		...
		#----set config
		#set path
		self.config['path']['raw'] = path
		# single_subject
		self.config['processing']['single_subject'] = single_subject
		# is single_trial
		self.config['processing']['single_trial'] = single_trial
		# subject and trial number
		self.config['processing']['subject'] = subject
		self.config['processing']['trial'] = trial
		# classification type
		ctype = self.config['classify']['ctype']
		# classification parameters
		missing=self.config['classify']['missing']
		maxdist=self.config['classify']['maxdist']
		mindur=self.config['classify']['mindur']
		v_th=self.config['classify']['v_th']
		dr_th=self.config['classify']['dr_th']
		di_th=self.config['classify']['di_th']

		#single subject
		if (self.config['processing']['single_subject']):
			self.console('start-----------------', 'orange')
			self.console('single_subject = %s, single_trial = %s'%(single_subject, single_trial), 'orange')
			...
			#get data
			self.console('self.getData(): importing raw data', 'orange')
			...
			if isEyetracking != 'eyetracking':
				self.console('finished-----------------', 'orange')
				return

			self.console('drop practice', 'orange')
			...
			#if single trial
			if self.config['processing']['single_trial']:
				l_trials = [trial]
			else:
			...
			self.console('for each trial', 'orange')
			#------------------------------------------------for each trial
			l_fgxy = []
			#start
			for idx in l_trials:
				#reset blocker for too few trials in filtering
				self.config['classify']['too_few_samples'] = False

				#set config trial number
				self.config['processing']['trial'] = idx
				self.console('subject: %s'%(self.config['processing']['subject']), 'orange')
				self.console('session: %s'%(self.config['processing']['session']), 'orange')
				self.console('trial: %s'%(self.config['processing']['trial']), 'orange')
				...
				self.config['metadata']['lab']['resolution.px'] = monitorSize
				...
				#process data
				self.console('processing data', 'blue')
				cgxy_df, fgxy_df = self.process(monitorSize, self.filters, df, self.config['processing']['trial'],
				_classify=self.config['classify'], ctype=ctype, _missing=missing, _maxdist=maxdist, _mindur=mindur,
				v_th=v_th, dr_th=dr_th, di_th=di_th)       

				#append classify to dataframe
				if self.config['classify']:
					if self.config['classify']['ctype'] != 'hmm':
				...
				if fgxy_df.shape[0] >= 1:
					self.console('Preprocessing: trials with at least 20 samples', 'green')
			...
			#save data
			self.console('saving data', 'blue')
			subject = int(self.config['processing']['subject'])
			session = self.config['processing']['session']
		...
		#all subjects
		elif (not self.config['processing']['single_subject']):
			self.console('start-----------------', 'orange')
			self.console('single_subject = %s, single_trial = %s'%(single_subject, single_trial), 'orange')
			self.console('importing raw data', 'blue')
			#--------------------------------------------------for each subject
			def all_subjects(fdir, core):
				try:        
					#for each file
					for sbj in fdir:
						self.console('subject: %s'%(sbj), 'blue')
						#set subject name, session
						p = Path(sbj)
						_subject, _session = (p.name.replace('abc', '').replace('.csv', '')).split("_", 1)
						self.config['processing']['subject'] = _subject
						self.config['processing']['session'] = _session
						...
						if (task_type == "eyetracking"):
							...
							isEyetracking = gxy_df['type'][0]
							if isEyetracking != 'eyetracking':
								self.console('finished subject', 'orange')
								continue
							else:
								pass
						...
						for idx in l_trials:
							...
							#set config trial number
							self.config['processing']['trial'] = idx
							self.console('subject: %s'%(_subject), 'orange')
							self.console('session: %s'%(_session), 'orange')
							self.console('trial: %s'%(self.config['processing']['trial']), 'orange')
							...
							if (task_type == "eyetracking"):
								...
								self.console('importing eyelink fixations', 'blue')
								if (self.config['processing']['source'] == 'eyelink'):
	...
	def subject_metadata(self, fpath, spath):
		...
		self.console('running metadata.summary()', 'blue')
		...
		#----end
		self.console('%s finished in %s msec'%(_f,((datetime.datetime.now()-_t0).total_seconds()*1000)),'blue')
	...
	def dwell(self, df, cores=1, isMultiprocessing=False):
		...
		_t0 = datetime.datetime.now()
		_f = self.debug(message='t', source="timestamp")
		self.console('running dwell()', 'blue')

		#----run
		def run(dir_=None, core=None, queue=None):
			self.console('processing.dwell.run(%s)'%(core), 'green')
			dwell = []
			#----for each subject
		...
		def finished(output, error=None):
			self.console('processing.dwell.finished()', 'green')
			if isMultiprocessing:
				#----create output df
			...
			#----end
			#timestamp
			self.console('%s finished in %s msec'%(_f,((datetime.datetime.now()-_t0).total_seconds()*1000))+self.console['ENDC'])
			return df, error
	...
	def onset_diff(self, df0, merge=None, cores=1):
		...
		#----run
		def run(dir_=None, core=None, queue=None):
			self.console('processing.onset_diff.run(%s)'%(core), 'green')
			onset = []
			#----for each subject
			for index, row in dir_.iterrows():
				_path_ = row['path']
		...
		if ((cores == 0) or (cores == 1)):
			isMultiprocessing = False
			self.console('processing.onset_diff() not multiprocessing', 'green')
		##else if requested cores are less than/equal 7, and less than available cores plus 1
			...
			isMultiprocessing = True
			dir_p = np.array_split(dir_, cores)
			self.console('processing.onset_diff() multiprocessing with %s cores'%(cores), 'green')
		##else use less than half of total available cores
		else:
			isMultiprocessing = True
			cores = int(cores/2)
