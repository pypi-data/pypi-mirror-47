...
from rebotics_sdk.cli.common import shell
...
def validate_role(ctx, param, value):
    if not value:
        click.echo('You have not specified role to use.')
        click.echo('Run subcommand roles')
    return value
...
@click.option('-r', '--role', help="Key to specify what retailer to use", callback=validate_role)
...
@click.option('-h', '--host', help='retailer host', prompt=True)
...
def configure(ctx, user, password, host):
    ...
    click.echo('Saved configuration for {} retailer in {}'.format(ctx.key, ctx.config.filepath))
...
def version(ctx):
    ...
    click.echo(ctx.provider.version())
...
def requeue(ctx, processing_ids, requeue_type, concurrency):
    ...
    format_processing_action_output(actions_list, ctx.format)
...
def processing_actions(ctx, store, status, page, page_size):
    ...
    data = ctx.provider.get_processing_action_list(
        store,
...
def roles(ctx):
    ...
    if ctx.format == 'json':
        ...
        for key, value in config.config.items():
            data = value
            data['role'] = key
            retailer_configurations.append(data)
        click.echo(format_full_table(retailer_configurations, 100))
...
@pass_rebotics_context
def assign_planoram_through_file(ctx, planogram_assign_file, deactivate):
    try:
