...
def shell(ctx):
    """Run IPython shell with current provider"""
...
@click.command()
@click.option('-h', '--host', help='App host', prompt=True)
...
def configure(ctx, user, password, host):
    ...
    click.echo('Saved configuration for {} in {}'.format(ctx.key, ctx.config.filepath))
