...
class ReboticsScriptsConfiguration(object):
    ...
    def get_provider(self, key):
        ...
        if 'token' in config:
            provider = self.provider_class(host=config['host'], token=config['token'])
        else:
            provider = self.provider_class(host=config['host'])
        return provider
