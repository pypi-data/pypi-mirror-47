...
def regionalFrame(table, field, dt, lat, lon, depthLabel, data, frameDirectory, frame, cmap, bounds):
    fontSize = 14
...
def makeFrames(table, field, startDate, endDate, lat1, lat2, lon1, lon2, depth1, depth2, frameDirectory, cmap):
    ...
    for frame, dtIndex in enumerate(tqdm(dtRange, desc='frames')):    
        ...
        if itnum == 0:
            bounds = com.getBounds(field, data)
        regionalFrame(table, field, dt, lat, lon, depthLabel, data, frameDirectory, itnum, cmap, bounds)
    return
...
def assembleFrames(frameDirectory, field, frameRate):
    vidDir = './video/' 
    makeDir(vidDir)
    vidName = vidDir + '%s.mp4' % field
    echo = video.animateFrames(frameDirectory, frameRate, vidName)
    return echo, vidName



def regionalVideo(tables, variables, dt1, dt2, lat1, lat2, lon1, lon2, depth1, depth2, cmap='viridis'):
    frameRate = 5
    for i in tqdm(range(len(tables)), desc='overall'):
        try:
            ...
            table = tables[i]
            field = variables[i]
            makeFrames(table, field, dt1, dt2, lat1, lat2, lon1, lon2, depth1, depth2, frameDirectory, cmap)
...
def main():
    ...
    regionalVideo(tables.split(','), variables.split(','), dt1, dt2, lat1, lat2, lon1, lon2, depth1, depth2, cmap)
