...
AVAILABLE_TEMPLATES = [
    ...
    for f in importlib.resources.contents(mdc.templates)
    if f.endswith(".tex")
]
    ...
    if output_file is None:
        ...
        with NamedTemporaryFile(dir="") as temp_file:
            ...
            with TemporaryDirectory() as temp_dir:
                ...
                tf_only_name = basename(temp_file.name)
                shutil.copyfile(
                    f"{temp_dir}/{tf_only_name}.pdf", f"{output_file}"
                )
    else:
...
def main():
    ...
    try:
        ...
        run_compile(pandoc_cmd, args.output_file, args.latexmk, args.verbose)
