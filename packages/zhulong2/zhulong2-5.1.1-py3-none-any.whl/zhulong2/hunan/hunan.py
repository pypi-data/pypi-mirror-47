    ...
    for i in range(1, int(len(PAGE)) + 1):
        if sum(PAGE[:i - 1]) < num <= sum(PAGE[:i]):
            ...
            for content in contents:
                ...
                org_code = content.get('ORG_CODE')
                href = content.get('NOTICE_ID')
                href = "http://www.ccgp-hunan.gov.cn/page/notice/notice.jsp?noticeId=" + str(href)

                info = {'diqu': address, 'gg_type': gg_type + '|' + type, 'zbfs': jy_type,'org_code':org_code, 'org_name': org_name}
...
data=[
    ...
    ["zfcg_gqita_gg","http://www.ccgp-hunan.gov.cn/page/notice/more.jsp?noticeTypeID=prcmNotices",
     ["name", "ggstart_time", "href", 'info'], f1, f2],

]

def work(conp,**args):
    est_meta(conp,data=data,diqu="湖南省湖南",**args)
    est_html(conp,f=f3,**args)


if __name__=='__main__':

    conp=["postgres","since2015","192.168.3.171","lch","hunan_hunan"]
