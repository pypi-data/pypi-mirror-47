...
def f1_data(driver, num):
    locator = (By.XPATH, "//ul[@class='newsList']/li[1]/span/a")
    WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
    try:
        ...
        cnum = 1
    if num != int(cnum):
        val = driver.find_element_by_xpath("//ul[@class='newsList']/li[1]/span/a").get_attribute('href').rsplit('/',
                                                                                                                maxsplit=1)[
            1]

        driver.execute_script("JumpPage('{}')".format(num))
        locator = (By.XPATH, "//ul[@class='newsList']/li[1]/span/a[not(contains(@href, '%s'))]" % val)
        WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
    ...
    for tr in trs:
        ...
        if 'http' in href:
            ...
            link = 'http://www.gszfcg.gansu.gov.cn' + a['href'].strip()
...
data = [
    ["zfcg_zhaobiao_gongkai_gg",
     "http://www.gszfcg.gansu.gov.cn/web/doSearchmxarticle.action",
    ...
    # # #
    ["zfcg_zhaobiao_dyly_gg",
     "http://www.gszfcg.gansu.gov.cn/web/doSearchmxarticle.action",
     ["name", "ggstart_time", "href", "info"], dylyzb(add_info(f1, {'zbfs': '单一来源'})), dylyzb(f2)],
    # # #
    ["zfcg_yucai_dylygs_gg",
     "http://www.gszfcg.gansu.gov.cn/web/doSearchmxarticle.action",
     ["name", "ggstart_time", "href", "info"], dylygszb(add_info(f1, {'zbfs': '单一来源'})), dylygszb(f2)],
    ...
    ["zfcg_gqita_zhao_zhong_xygh_gg",
     "http://www.gszfcg.gansu.gov.cn/web/article/130/0/index.htm",
     ["name", "ggstart_time", "href", "info"], add_info(f1, {'zbfs': '协议供货'}), f2],
    # # #
    ["zfcg_gqita_zhao_zhong_ddcg_gg",
     "http://www.gszfcg.gansu.gov.cn/web/article/142/0/index.htm",
     ["name", "ggstart_time", "href", "info"], add_info(f1, {'zbfs': '定点采购'}), f2],
]


def work(conp, **args):
    est_meta_large(conp, data=data, diqu="甘肃省省级", **args)
...
if __name__ == '__main__':
    ...
    # url = "http://www.gszfcg.gansu.gov.cn/web/article/130/0/index.htm"
    # driver.get(url)
    # df = f2(driver)
    # print(df)
    # driver=webdriver.Chrome()
    # url = "http://www.gszfcg.gansu.gov.cn/web/article/142/0/index.htm"
    # driver.get(url)
    # for i in range(3, 9):
    #     df=f1(driver, i)
    #     print(df.values)
