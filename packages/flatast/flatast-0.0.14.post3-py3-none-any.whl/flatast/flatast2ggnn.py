...
parser.add_argument('--id_type', type=bool, default=False, help='use the <nodeid,nodetype> representation of nodes')
            ...
            if t != 'POSITION' and t != 'COMMENT' and t != '271' and t != '6':
               ...
               if opt.label:
                  ...
                  if lang == "SRCML" and t != "dummy":
                    ...
                    to = "%d,%d" % (j, flatast.fast_pb2._PYTHON3KIND.values_by_name[t].number)
                  elif lang == "SOLIDITY":
                    to = "%d,%d" % (j, flatast.fast_pb2._SOLIDITYKIND.values_by_name[t].number)
                  elif lang == "SMALI":
                    to = "%d,%d" % (j, flatast.fast_pb2._SMALIKIND.values_by_name[t].number)
                  elif lang == "COBOL85":
                    to = "%d,%d" % (j, flatast.fast_pb2._COBOL85KIND.values_by_name[t].number)
               dict[j1] = to
               if opt.maps or opt.edgemaps:
                  if not to in maps:
                     maps[to] = str(1 + len(maps))
