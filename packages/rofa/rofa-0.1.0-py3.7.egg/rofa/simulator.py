...
from .weight_model import EqualWeight, WeightModel
...
class Simulator():
    ...
    def _adjust_weights(self,
        ...
        for i in range(len(weighted)):
            ...
            weighted.iloc[i] = (
                affected_weight / affected_weight.sum()
            )
...
class QuantileSimulator(Simulator):
    ...
    def __init__(self,
                 ...
                 **kwargs) -> None:
        super().__init__(
            data, rebalance_freq, weight_model=weight_model, **kwargs)
        self.bins = bins
    ...
    def quarterly_returns(self) -> pd.DataFrame:
        if self._quarterly_returns.empty:
            self._quarterly_returns = (self.daily_returns+1).groupby(
                pd.Grouper(freq='Q')).prod() - 1
        return self._quarterly_returns

    def _discretize_based_on_quantile(self,
        ...
        discretized = rank.apply(
            lambda x: pd.qcut(x, self.bins, labels),
            axis=1, raw=True
        )
        return discretized

    def _create_generater(self):
        # Now it loops over bins,
        # but, all grouped data has same length.
        # So, it is possible to process them all efficiently.
        # TODO: remove loop over bin and process them all at once
    ...
    def calculate_cagr(self):
        cagr = ((1+self.total_returns) **
                (DAYS_PER_YEAR/len(self.daily_returns)) - 1)
        return cagr

    def _calculate_adjusted_returns(
            self, risk_free: Union[float, pd.Series] = 0.0) -> pd.DataFrame:
    ...
    def calculate_drawdown(self) -> pd.DataFrame:
        roll_max = (1+self.cumulative_returns).rolling(
            DAYS_PER_YEAR, min_periods=1).max()
    ...
    def calculate_rolling_cagr(self,
        ...
        rolling_cagr = (self.cumulative_returns+1).rolling(
            window=window, min_periods=min_window
        ).apply(
            lambda x: ((x[-1]/x[0]) ** (DAYS_PER_YEAR/window)) - 1,
            raw=True
        )
        return rolling_cagr
...
class LongShortSimulator(Simulator):
    ...
    def cumulative_returns(self) -> pd.Series:
        ...
        if not self._cumulative_returns:
            self._cumulative_returns = (
                daily_returns + 1).cumprod() - 1

        return self._cumulative_returns

    def _split_long_short(self,
                          data: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def run(self):
        ...
        short_weights_adjusted = self._adjust_weights(
            short_weighted, long=False)
        # and combined them
        weights_adjusted = long_weights_adjusted.fillna(
            short_weights_adjusted) / (2/self.gross_leverage)
