...
import napalm_aruba.device
import napalm_aruba.dialer
...
class NetworkDriver(napalm.base.NetworkDriver):
    ...
    def _netmiko_open(self, device_type, netmiko_optional_args=None):
        return napalm_aruba.dialer.dial(
            device_type,
            self.hostname,
            self.username,
            self.password,
    ...
    def get_environment(self):
        s = napalm_aruba.device.get_system_information(self.device)

        return {
            "fans": {},
            "temperature": {},
            "power": {},
            "cpu": {"0": {"%usage": s.cpu_utilization}},
            "memory": {"available_ram": s.memory_total, "used_ram": s.memory_free},
        }

    def get_facts(self):
        s = napalm_aruba.device.get_system_information(self.device)
        d = napalm_aruba.device.get_device_manufacturer_info(self.device)
        i = napalm_aruba.device.get_interfaces(self.device)

        try:
            uptime = napalm_aruba.device.get_uptime(self.device)
        except RuntimeError:
    ...
    def get_lldp_neighbors(self):
        d = napalm_aruba.device.get_lldp_neighbors(self.device)
