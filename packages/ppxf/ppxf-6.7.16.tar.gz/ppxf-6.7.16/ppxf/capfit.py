...
Copyright (C) 2017-2018, Michele Cappellari
...
A general description of the LM algorithm *without* bounds is given in:
...
class capfit(object):
    ...
    def __init__(self, func, p1, abs_step=None, bounds=(-np.inf, np.inf),
        ...
        self.cov = np.zeros((p1.size, p1.size))
...
################################################################################
    ...
    def check_conv(self, lam, f, p, h, dn, actred, prered):
        ...
        if prered < self.ftol and abs(actred) < self.ftol and actred <= 2*prered:
            status = 2   # Terminating on small function variation (More'+80)
        if linalg.norm(h[self.free]) < self.xtol*(self.xtol + linalg.norm((dn*p)[self.free])):
            if status == 2:
                status = 4   # Both ftol and xtol convergence tests are satisfied
            else:
                status = 3   # Terminating on small step

        if self.verbose == 2:
            print(f'\niter: {self.njev}  lam: {lam}  chi2: {chi2(f)}'
                  f'  ratio: {actred/prered} \np: {p} \nh: {h}')

        if status != -1 and self.verbose:
            if status == 0:
                print("\nTerminating on function evaluations count")
            elif status == 2:
            ...
            print(f'Final iter: {self.njev}  Func calls: {self.nfev}'
                  f'  chi2: {chi2(f)}  Status: {status} \nFinal p: {p}\n')
