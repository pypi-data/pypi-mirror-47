    ...
    def evaluate(self, df: pandas.DataFrame, operation, vars_subset: list = None, excluded_vars: list = None, debug=False) -> numpy.array:
        ...
        for variable in self.univariate_beta.keys():
            if variable not in excluded_vars:
                result = operation(result, self.__get_univariate_beta(df=df, variable=variable, debug=debug))
        ...
        for interaction_name in self.__bivariate_beta_flat.keys():
            ...
            if not ((first_bivariate in excluded_vars) or (second_bivariate in excluded_vars)):
                result = operation(result, self.__get_bivariate_beta(df=df, interaction_name=interaction_name, debug=debug))
