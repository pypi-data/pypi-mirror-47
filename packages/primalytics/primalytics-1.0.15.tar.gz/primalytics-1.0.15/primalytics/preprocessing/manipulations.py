...
def bin_variable(s: pandas.Series, min_value, max_value, width, how, default=None, lower=None, upper=None, errors=None):
	"""
	bin pandas Series
	...
	s_binned.loc[conditions] = list(numpy.clip(how(s_binned.loc[conditions] / width) * width, min_value, max_value))
...
def get_json_dumpable(data: dict) -> dict:
	prc_data = {}
	for k, v in data.items():
		nv = v
		if isinstance(nv, (pandas.datetime,)):
			nv = str(nv)

		if isinstance(nv, (numpy.float, numpy.float16, numpy.float32, numpy.float64)):
			nv = float(nv)

		if isinstance(nv, (numpy.int, numpy.int16, numpy.int32, numpy.int64)):
			nv = int(nv)

		prc_data.update({k: nv})
...
def fill_str(s: pandas.Series, length, fill_value: str = '0', fillna=False):
	if s.isna().any() and not fillna:
		print('Warning! {} NaNs found in input series. Mapping skipped'.format(sum(s.isna())))
