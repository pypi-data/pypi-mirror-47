...
class IterationTestCase(TestCase):
    ...
    def test_iterate_filter(self):
        ...
        for p in iterate_publications_as_json(
            ...
            returned += 1
            dois.add(p['DOI'])
            self.assertTrue('10.13039/100000925' in [f.get('DOI', '')
                                                     for f in p['funder']])
        ...
        for p in iterate_publications_as_json(
                max_results=10000,
                filter={'funder': '10.13039/100000925',
    ...
    def test_iterate_query(self):
        ...
        dois = set()
        for p in iterate_publications_as_json(
                max_results=10000, queries={'query.title': 'floyd'}):
            returned += 1
            dois.add(p['DOI'])
        self.assertEqual(returned, len(dois))
    ...
    def test_iterate(self):
        ...
        for p in iterate_publications_as_json(
                max_results=10000,
                filter={'funder': '10.13039/501100000038',
                        'type': 'journal-article'},
                queries={'query.author': 'li',
                         'query.affiliation': 'university'}):
            returned += 1
            dois.add(p['DOI'])
            self.assertTrue('10.13039/501100000038' in [f.get('DOI', '')
                                                        for f in p['funder']])
            self.assertEqual(p['type'], 'journal-article')
        self.assertEqual(returned, len(dois))
