...
def powernoise(beta, N, normalized=True):
    N2 = int(N / 2 - 1)
...
def plot_noise(serie, beta=None, save=True):
    width, height = 17, 10
    ...
    if not beta:
        ...
        color = '#0100FF'
    if beta == 0:
        title = r'White Noise $\beta$=0'
        color = '#0100FF'
    elif beta == 1:
        title = r'Pink Noise $\beta$=1'
        color = 'hotpink'
    elif beta == 2:
        title = r'Red Noise $\beta$=2'
        color = 'red' 
    ...
    if save:
        if not os.path.exists('plots'):
            os.makedirs('plots')
        path = './plots/{}.png'.format(str(dt.timestamp(dt.now())).split('.')[0])
...
def power_spectrum_density(df, start=10, end=800):
    main_psd(df, start=10, end=800)
