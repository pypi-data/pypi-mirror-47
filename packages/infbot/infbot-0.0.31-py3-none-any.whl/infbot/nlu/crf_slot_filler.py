...
from .keras_crf import get_model
...
class CRFSlotFiller(object):
    ...
    def fit(self,
        ...
        seq_pad_y = pad_sequences(seq_y, maxlen=50)

        self.crf = get_model(len(self.tokenizer_y.word_index) + 1)
        self.crf.model.fit(seq_pad, to_categorical(seq_pad_y), epochs=5)
    ...
    def predict(self, sentence_result):
        ...
        seq = self.tokenizer.texts_to_sequences(sentence_result)
        seq_pad = pad_sequences(seq, maxlen=50)
        y_pred = self.crf.predict_proba(seq_pad).argmax(-1)
