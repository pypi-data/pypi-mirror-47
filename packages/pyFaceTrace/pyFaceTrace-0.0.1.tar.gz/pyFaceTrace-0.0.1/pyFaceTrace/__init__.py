...
#載入字型
FONT = ImageFont.truetype("kaiu.ttf",20,index=0)
# 載入人臉檢測器
...
def addText2Img_cv2(img_cv2,text,font=FONT,position=(20,20),fill=(255,0,0)):
        ...
        if cap.isOpened():
            ...
            if ret: 
                try:
                    ...
                    frame=addText2Img_cv2(frame,'tag='+tag,FONT,(rect.left(), rect.top()-FONT.size*2-5))   
                #except:pass 
            ...
            try:
                ...
                for rect in rects:
                    ...
                    frame=addText2Img_cv2(frame,Tag+":"+str(round(dist,3)),
                                          FONT,(rect.left(), rect.top()-FONT.size-1))        
            except IndexError:pass    
            cv2.imshow("press esc to exit...", frame)
