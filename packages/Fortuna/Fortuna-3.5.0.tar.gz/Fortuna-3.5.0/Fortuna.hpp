...
namespace Fortuna {
    ...
    auto random_value(const std::vector<Value> & data) -> Value {
        return data[random_index(data.size())];
    }
    ...
    public:
        ...
        auto operator()() -> Value {
            ...
            const auto target = 1 + stop + back_poisson(data.size() - 1);
    ...
    auto cum_weighted_choice(const std::vector<Weight> & weights, const std::vector<Value> & values) -> Value {
