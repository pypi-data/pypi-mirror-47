...
__all__ = (
    "RandomValue", "TruffleShuffle", "QuantumMonty", "CumulativeWeightedChoice", "RelativeWeightedChoice", "FlexCat",
...
class RandomValue:
    ...
    def __call__(self, zero_cool: staticmethod = _random_index, range_to: int = 0):
        target = range_to or self.size
        return flatten(self.data[zero_cool(target)], flat=self.flat)
...
def test_warm_up(cycles=128):
    timer(random_int, min_int(), max_int(), cycles=cycles, silent=True)
