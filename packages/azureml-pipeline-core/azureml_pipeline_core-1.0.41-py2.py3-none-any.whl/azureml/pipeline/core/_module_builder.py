...
class _ModuleBuilder(object):
    ...
    @staticmethod
    def calculate_hash(module_def, hash_paths=None):
        module_hash = module_def.calculate_hash()
        if hash_paths is not None:
            content_hash = _ModuleBuilder._default_content_hash_calculator(hash_paths)
...
class _FolderModuleBuilder(_ModuleBuilder):
    ...
    def get_fingerprint(self):
        module_hash = _ModuleBuilder.calculate_hash(self._module_def, self._hash_paths)
        return module_hash
