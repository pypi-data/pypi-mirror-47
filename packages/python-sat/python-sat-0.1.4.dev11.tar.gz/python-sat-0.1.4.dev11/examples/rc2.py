...
class RC2(object):
    ...
    def __init__(self, formula, solver='g3', adapt=False, dupls=False,
    ...
    def init(self, formula, incr=False):
        ...
        # mapping from soft clauses to hard clauses
        self.cmap = {}
        ...
        for i, cl in enumerate(formula.soft):
            ...
            if len(cl) > 1:
                ...
                found_hard = None
                for cl_h in formula.hard:
                    hs = set(cl_h)
                    if -selv in hs:
                        if found_hard:  # found more than once
                            found_hard = [None]
                            break
                        else:
                            hs.remove(-selv)
                            found_hard = tuple(sorted(hs))

                # print(selv, found_hard)

                if not found_hard:  # this selector is a pure literal
                    self.oracle.add_clause([selv])
                    continue
                elif found_hard != [None]:  # if found only one hard clause
                    if found_hard in self.cmap:
                        print(selv)
                        to_forget = selv
                        selv = self.cmap[found_hard]
                        self.oracle.add_clause([-to_forget, selv])
                        self.oracle.add_clause([to_forget, -selv])
                    else:
                        print(selv)
                        self.cmap[found_hard] = selv
        ...
        if self.verbose > 1:
            print('c formula: {0} vars, {1} hard, {2} soft'.format(formula.nv,
                len(formula.hard), len(self.sels)))
...
class RC2Stratified(RC2, object):
    ...
    def __init__(self, formula, solver='g3', adapt=False, dupls=False,
        ...
        super(RC2Stratified, self).__init__(formula, solver=solver,
                adapt=adapt, dupls=dupls, exhaust=exhaust, incr=incr,
...
def parse_options():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'ac:de:hilms:t:vx',
                ['adapt', 'comp=', 'dupl', 'enum=', 'exhaust', 'help', 'incr',
                    'blo', 'minimize', 'solver=', 'trim=', 'verbose'])
    ...
    return adapt, blo, cmode, dupls, to_enum, exhaust, incr, minz, solver, \
            trim, verbose, args
...
def usage():
    ...
    print('        -d, --dupls              Try to detect duplicates of soft clauses before solving')
...
if __name__ == '__main__':
    adapt, blo, cmode, dupls, to_enum, exhaust, incr, minz, solver, trim, \
    ...
    if files:
        ...
        if blo and (max(formula.wght) > min(formula.wght) or dupls):
        ...
        # starting the solver
        with MXS(formula, solver=solver, adapt=adapt, dupls=dupls,
