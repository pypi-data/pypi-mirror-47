...
class Client(Fist):
    ...
    def get_order_info(self, order_id):
        ...
        if err_id == 0:
            ...
            print('[get_order_info] error occurred (eid){}'.format(err_id))
            return None
    ...
    def get_position(self, tg_name):
        ...
        if err_id == 0:
            ...
            print('[get_position] error occurred (eid){}'.format(err_id))
            return None
    ...
    def get_account(self, tg_name):
        ...
        if err_id == 0:
            ...
            print('[get_account] error occurred (eid){}'.format(err_id))
            return None

    # return order_id
    def insert_order(self, tg_name, exchange, ticker, price, volume, order_type, direction, offset_flag=OffsetFlag.NOT_AVAILABLE):
        ...
        if err_id == 0:
            ...
            print('[order_insert] error occurred (eid){}'.format(err_id))
            return -1

    def insert_twap_order(self, tg_name, exchange, ticker, volume, direction, start_time, end_time, sec_interval):
        req = message_trade.ReqTwapInsert()
        req.tg_name = bytify(tg_name)
        req.exchange = exchange
        req.ticker = bytify(ticker)
        req.price = 0
        ...
        req.client_name = bytify(self.fist_name)
        req.start_nano = Timer.get_nano(start_time)
        req.end_nano = Timer.get_nano(end_time)
        req.sec_interval = sec_interval
        req.parent_id = -1
        ret = self.req(self.trade_router, req, MsgType.REQ_ORDER_INSERT, 0)
        err_id = ret.get_err_id()
        if err_id == 0:
            ...
            print('[insert_twap_order] error occurred (eid){}'.format(err_id))
            return -1

    def insert_basket_order(self, tg_name, bo):
        bo.tg_name = tg_name
        bo.order_id = -1
        bo.client_name = self.fist_name
        jstring = json.dumps(bo.to_dict())
        ret = self.req(self.trade_router, jstring, MsgType.REQ_ORDER_INSERT_BASKET, 0)
        err_id = ret.get_err_id()
        if err_id == 0:
            return ret.get_req_id()
        else:
            print('[insert_basket_order] error occurred (eid){}'.format(err_id))
            return -1
    ...
    def cancel_order(self, order_id, tg_name):
        ...
        if err_id == 0:
            ...
            print('[cancel_order] error occurred (eid){}'.format(err_id))
            return -1
    ...
    def req_position(self, tg_name):
        ...
        if err_id == 0:
            ...
            print('[req_position] error occurred (eid){} (tg){}'.format(err_id, tg_name))
            return -1
    ...
    def req_account(self, tg_name):
        ...
        if err_id == 0:
            ...
            print('[req_account] error occurred (eid){} (tg){}'.format(err_id, tg_name))
            return -1
    ...
    def req_active_orders(self, tg_name):
        ...
        if err_id == 0:
            ...
            print('[req_active_orders] error occurred (eid){} (tg){}'.format(err_id, tg_name))
            return -1
    ...
    def req_cancel_active_orders(self, tg_name, req_active_orders_id=-1):
        ...
        if err_id == 0:
            ...
            print('[req_cancel_active_orders] error occurred (eid){} (tg){}'.format(err_id, tg_name))
            return -1
    ...
    def subscribe_bar(self, exchange, ticker):
        return self.subscribe(exchange, ticker, MsgType.MKT_BAR)
    ...
    def subscribe(self, exchange, ticker, msg_type):
        return self.subscribe_batch([exchange], [ticker], msg_type)

    def subscribe_batch(self, exchanges, tickers, msg_type=MsgType.MKT_SNAP):
        if not isinstance(tickers, list):
            tickers = [tickers]
        if not isinstance(exchanges, list):
            exchanges = [exchanges for i in range(0, len(tickers))]
        if len(exchanges) != len(tickers):
            print("[subscribe] exchanges and tickers, size mismatch")
            return False
        ss = json.dumps([{'exchange': ExchangeID.read(exchanges[i]), 'ticker': tickers[i], 'msg_type': msg_type} for i in range(0, len(tickers))])
        rsp = self.req(self.market_router, ss, MsgType.MKT_SUBSCRIBE, 0)
        if rsp.get_err_id() != 0:
            print("[subscribe] failed (err_id){} (msg){}".format(rsp.get_err_id(), ss))
            return False
        else:
