...
class AttributeSetter:
    ...
    Also, registering the attribute is not required if it follows one of the
    "standard" ways of setting (either the completely normal way, forced-set, or
    secret-set with `secret='set'`
    """
    def __init__(self, instance, attributes=None):
        ...
        if attributes is not None:
            self.registry.update(attributes)
    ...
    def register(self, attrname, setter, **kwargs):
        ...
        """
        self.registry[attrname] = (setter, kwargs)
    ...
    def _set_registered(self, attrname, value, **newkwargs):
        setter, kwargs = self.registry[attrname]
    ...
    def __repr__(self):
        ...
        attribute_text = ', ' +repr(self.registry) if len(self.registry) != 0 else ""
