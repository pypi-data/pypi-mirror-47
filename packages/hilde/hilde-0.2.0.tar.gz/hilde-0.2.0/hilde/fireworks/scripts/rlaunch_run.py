...
def rlaunch():
    ...
    single_parser.add_argument(
        "--offline", help="run in offline mode (FW.json required)", action="store_true"
    )
    single_parser.add_argument(
        "--pdb", help="shortcut to invoke debugger on error", action="store_true"
    )
    ...
    rapid_parser.add_argument(
        "--sleep", help="sleep time between loops (secs)", default=None, type=int
    )
    rapid_parser.add_argument(
        "--local_redirect",
    ...
    rapid_parser.add_argument(
        ...
        "--wflow",
        nargs="+",
        help="The root processes of a WorkFlow",
        type=int,
        default=[],
    )
