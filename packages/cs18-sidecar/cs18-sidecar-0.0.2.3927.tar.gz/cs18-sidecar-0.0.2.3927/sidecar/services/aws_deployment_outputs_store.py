...
class AwsDeploymentOutputsStore(DeploymentOutputsStore):
        ...
        for output in outputs:
            try:
                ...
                if entity_outputs is None:
                    entity_outputs = self._status_maintainer.get_deployment_outputs(entity_name)
    ...
    def save_service_outputs(self, service_name: str, output_json: {}):
        ...
        self._log_service_deployment_output(service_name, deployment_output)
        ...
        with self._lock:
            self._status_maintainer.update_service_outputs(service_name, deployment_output)

    def _log_service_deployment_output(self, service_name: str, deployment_output: {}):
        output_str = json.dumps(deployment_output)
        self._logger.info(f"'{service_name}' deployment output is:\n{output_str}")
    ...
    def save_application_outputs(self, app_name: str, app_instance_id: str, outputs: str):
        ...
        with self._lock:
            self._status_maintainer.update_app_instance_outputs(instance_logical_id,
                                                                app_instance_id,
                                                                app_name,
                                                                deployment_output)
