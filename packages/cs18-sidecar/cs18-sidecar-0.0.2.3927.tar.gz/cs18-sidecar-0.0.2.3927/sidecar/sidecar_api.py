...
from sidecar.services.aws_deployment_outputs_store import AwsDeploymentOutputsStore
from sidecar.services.deployment_outputs_store import DeploymentOutputsStore
...
_deployment_outputs_store = None  # type: DeploymentOutputsStore
...
def get_status_updater(config: ISidecarConfiguration,
    ...
    global _deployment_outputs_store
    ...
    if isinstance(config, AwsSidecarConfiguration):
        ...
        _deployment_outputs_store = AwsDeploymentOutputsStore(_logger, aws_session, aws_status_maintainer, _config)
...
def save_application_deployment_output(app_name: str, instance_id: str):
    ...
    _deployment_outputs_store.save_application_outputs(app_name, instance_id, output)
    return '', 200
...
def deployment_outputs_resolver():
    ...
    res = _deployment_outputs_store.get_deployment_outputs(outputs)
    # res = '$outputs.app.my_output=\'{"age": 52, "name": "John", "user": [{"user-a": "a-topic", "user-b": "c-topic", "user-c": "c-topic"}]}\'\n$outputs.app.str_output=value'
    return res, 200
...
def save_service_deployment_output(name: str):
    ...
    try:
        ...
        return err, 400
    _deployment_outputs_store.save_service_outputs(name, output)
    return '', 200
