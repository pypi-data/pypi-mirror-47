...
#
#
def monocle_trajectory(project, npc=5):
    """
...
def get_score_from_peaks(project):
    ...
    normal_df = pandas.DataFrame(R_wave.T, index=genes, columns=cells_df.index)
    normal_df.to_csv(project+'/matrix/genes_scored_by_peaks.csv', sep=',')
    return
#
#
...
#    print(project+'/matrix/genes_scored_by_peaks.csv')
#    return
#
#
...
#
#
def gene_score(project, genome="hg19", width=1000000, pvalue=0.01):
    """
    project:     Path to the project folder.
...
def differential_feature(project,
    ...
    if feature=='accesson':
        ...
        reads_file = project+'/matrix/gene_score.csv'
    else:
...
#
#
def search_supper_enhancer(project, supper_range=1000000):
    """
    project:         Path to the project folder.
    supper_range:    Genome range to search for super enhancer, default=1000000.
    """
    ...
    supper_df.to_csv(project+'/result/potential_super_enhancer.csv', sep='\t')
    print('search_supper_enhancer output files:')
