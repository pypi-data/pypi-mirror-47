...
class PrimitiveBase(typing.Generic[Inputs, Outputs, Params, Hyperparams], metaclass=PrimitiveBaseMeta):
    ...
    * A private argument prefixed with ``_`` which is used for debugging and testing.
    ...
    def can_accept(cls, *, method_name: str, arguments: typing.Dict[str, typing.Union[metadata_base.Metadata, type]],
        ...
        """
        Returns a metadata object describing the output of a call of ``method_name`` method under
        ...
        instance_methods = metadata['primitive_code'].get('instance_methods', {})

        if method_name not in instance_methods:
            raise exceptions.InvalidArgumentValueError("Unknown method '{method_name}'.".format(method_name=method_name))

        primitive_arguments = metadata['primitive_code'].get('arguments', {})
        primitive_arguments_set = set(primitive_arguments.keys())
        ...
        for instance_method_name, instance_method_metadata in instance_methods.items():
            if instance_method_name in ['fit', 'set_training_data', method_name] or instance_method_metadata['kind'] == metadata_base.PrimitiveMethodKind.PRODUCE:
                for instance_method_argument in instance_method_metadata['arguments']:
                    if instance_method_argument in primitive_required_arguments_set:
                        required_arguments_set.add(instance_method_argument)
        ...
        # All passed arguments should exist among primitive arguments.
        extra_arguments_set = arguments_keys_set - primitive_arguments_set
        if len(extra_arguments_set):
            raise exceptions.InvalidArgumentValueError(
                "Extra arguments are provided for the primitive': {extra_arguments_set}".format(
                    extra_arguments_set=extra_arguments_set,
                )
            )
        ...
        method_return_type = instance_methods[method_name]['returns']

        if issubclass(method_return_type, CallResult):
            method_return_type = utils.get_type_arguments(method_return_type)[T]  # type: ignore

        return metadata_base.DataMetadata({
            'schema': metadata_base.CONTAINER_SCHEMA_VERSION,
            'structural_type': method_return_type,
        })
...
class LossFunctionMixin(typing.Generic[Inputs, Outputs, Params, Hyperparams], metaclass=utils.GenericMetaclass):
    ...
    @abc.abstractmethod
    def get_loss_metric(self) -> typing.Optional[problem.PerformanceMetric]:  # type: ignore
        """
    ...
    @abc.abstractmethod
    def losses(self, *, inputs: Inputs, outputs: Outputs, timeout: float = None, iterations: int = None) -> CallResult[Outputs]:
        """
    ...
    def loss(self, *, inputs: Inputs, outputs: Outputs, timeout: float = None, iterations: int = None) -> CallResult[Outputs]:
        """
