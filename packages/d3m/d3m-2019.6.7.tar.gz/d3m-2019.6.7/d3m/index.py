    ...
    for primitive_path in arguments.primitives:
        ...
        try:
            ...
            if getattr(arguments, 'continue', False):
                ...
                print(f"Error loading primitive: {primitive_path}")
                has_errored = True
                continue
            else:
        ...
        try:
            if getattr(arguments, 'print', False):
                pprint.pprint(primitive.metadata.to_json_structure())

            else:
                json.dump(
                    primitive.metadata.to_json_structure(),
                    sys.stdout,
            ...
            if getattr(arguments, 'continue', False):
                traceback.print_exc(file=sys.stdout)
                print(f"Error describing primitive: {primitive_path}")
                has_errored = True
                continue
            else:
...
def main(argv: typing.Sequence) -> None:
    ...
    if not typing.TYPE_CHECKING:
        ...
        cli.index_configure_parser(parser)

        arguments = parser.parse_args(argv[1:])

        cli.index_handler(arguments, parser)
