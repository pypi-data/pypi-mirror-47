...
class Resolver:
    ...
    primitives_blacklist: Collection[str]
    ...
    def __init__(self, *, strict_resolving: bool = False, strict_digest: bool = False,
        ...
        self.primitives_blacklist = primitives_blacklist
    ...
    def _from_file(self, pipeline_description: typing.Dict) -> 'typing.Optional[Pipeline]':
        for path in self.pipeline_search_paths:
            ...
            pipeline_path = os.path.join(path, '{pipeline_id}.yml'.format(pipeline_id=pipeline_description['id']))
            try:
                with open(pipeline_path, 'r', encoding='utf8') as pipeline_file:
    ...
    def _load_primitives(self) -> None:
        ...
        index.load_all(blacklist=self.primitives_blacklist)
...
class PrimitiveStep(StepBase):
    ...
    def add_argument(self, name: str, argument_type: typing.Any, data_reference: typing.Union[str, typing.Sequence[str]]) -> None:
        ...
        argument_type : ArgumentType
            Argument type.
        data_reference : Union[str, Sequence[str]]
    ...
    def get_primitive_hyperparams(self) -> hyperparams.Hyperparams:
...
class Pipeline:
    ...
    def equals(self, pipeline: P, strict_order: bool = False) -> bool:
        """
        ...
        return PipelineHasher(self, strict_order).unique_equivalence_class_repr() == PipelineHasher(pipeline, strict_order).unique_equivalence_class_repr()

    def hash(self, strict_order: bool = False) -> int:
        """
        ...
        return hash(PipelineHasher(self, strict_order))
...
class OrderedNode(metaclass=utils.AbstractMetaclass):
    ...
    inputs_ref : typing.Optional[typing.Union[typing.Dict[InputIndex, str], typing.List[str]]]
    ...
    def remove_child(self, child: 'OrderedNode', input_index: typing.Optional[InputIndex]) -> None:
        ...
        child : OrderedNode
            The child node.
        input_index : Optional[InputIndex]
    ...
    def change_input(self, input_index: InputIndex, new_parent: 'OrderedNode', new_input_index: typing.Optional[InputIndex] = None, new_output_index: typing.Optional[OutputIndex] = None) -> None:
        ...
        new_parent : OrderedNode
            The new parent of the node.
        new_input_index : Optional[InputIndex]
            The new input index. If it is None, the original index will be kept.
        new_output_index : Optional[OutputIndex]
...
class InputsNode(OrderedNode):
    ...
    Parameters
    ----------
    pipeline_inputs : typing.List[typing.Dict]
...
class OutputsNode(OrderedNode):
    ...
    Parameters
    ----------
    pipeline_outputs : typing.List[typing.Dict]
...
class PrimitiveNode(OrderedNode):
    ...
    def __init__(self, primitive: PrimitiveStep, *, strict_order: bool) -> None:
        ...
        for name, hyperparameter_description in primitive.hyperparams.items():
            is_set = isinstance(hyperparameters[name], hyperparams.Set) if hyperparameters is not None else False
            if hyperparameter_description['type'] == metadata_base.ArgumentType.DATA:
                if utils.is_sequence(hyperparameter_description['data']):
    ...
    def _serialize_hyperparamter_value(self, name: str, data: typing.Any, is_sequence: bool) -> typing.Any:
        if self.primitive_step.primitive is not None:
            ...
            hyperparameter = hyperparams.Hyperparameter[type(data)](data)  # type: ignore
...
class SubpipelineNode(OrderedNode):
    ...
    strict_order : bool
        ...
        Otherwise we will try to sort their contents so the orders of their contents are not important.
    """
    def __init__(self, subpipeline: SubpipelineStep, *, strict_order: bool) -> None:
    ...
    def graph(self) -> typing.Optional['PipelineDAG']:
        ...
        Returns
        -------
        Optional[PipelineDAG]
            If this node has been resolved, return the graph; return None otherwise.
        """

        if self.pipeline is not None:
            return PipelineDAG(self.pipeline, strict_order=self.strict_order)
        return None
...
class PipelineDAG:
    ...
    def __init__(self, pipeline: Pipeline, *, strict_order: bool) -> None:
        self.pipeline = pipeline
        self.strict_order = strict_order
    ...
    def _convert_step_to_node(self, step: StepBase) -> OrderedNode:
        ...
        if isinstance(step, PrimitiveStep):
            node = PrimitiveNode(step, strict_order=self.strict_order)
        elif isinstance(step, PlaceholderStep):
            node = PlaceholderNode(step)
        elif isinstance(step, SubpipelineStep):
            node = SubpipelineNode(step, strict_order=self.strict_order)
        else:
...
class PipelineHasher:
    ...
    def __init__(self, pipeline: Pipeline, strict_order: bool = False) -> None:
        self.pipeline = pipeline
        self.strict_order = strict_order
        self.graph = PipelineDAG(pipeline, strict_order=strict_order)
    ...
    for pipeline_path in arguments.pipelines:
        ...
        try:
            ...
            if getattr(arguments, 'continue', False):
                ...
                print(f"Error parsing pipeline: {pipeline_path}")
                has_errored = True
                continue
            else:
        ...
        if getattr(arguments, 'check', True):
            try:
                ...
                if getattr(arguments, 'continue', False):
                    traceback.print_exc(file=sys.stdout)
                    print(f"Error checking pipeline: {pipeline_path}")
                    has_errored = True
                    continue
                else:
        ...
        try:
            ...
            if getattr(arguments, 'print', False):
                pprint.pprint(pipeline_description)
            else:
                json.dump(
                    pipeline_description,
                    sys.stdout,
            ...
            if getattr(arguments, 'continue', False):
                traceback.print_exc(file=sys.stdout)
                print(f"Error describing pipeline: {pipeline_path}")
                has_errored = True
                continue
            else:
