...
class PipelineRun:
    ...
    def to_json_structure(self) -> typing.Dict:
        ...
        if not self.is_standard_pipeline and not self.datasets:
            raise exceptions.InvalidStateError("Non-standard pipelines cannot be converted to a JSON structure.")
...
def _validate_pipeline_run_timestamps(pipeline_run: typing.Dict, parent_start: datetime.datetime = None, parent_end: datetime.datetime = None) -> None:
    ...
    start = dateparser.parse(pipeline_run['start'])
    end = dateparser.parse(pipeline_run['end'])
...
def validate_problem(problem_description: typing.Dict) -> None:
    """
    ...
    # Also validates against the schema.
    problem_description = problem.Problem(problem_description)
    ...
    if 'digest' not in problem_description:
    ...
    computed_digest = utils.compute_digest(utils.to_json_structure(canonical_problem_description))

    if problem_description['digest'] != computed_digest:
        raise exceptions.InvalidProblemError(
            "Digest field is not '{computed_digest}', but '{actual_digest}'.".format(
                computed_digest=computed_digest,
                actual_digest=problem_description['digest'],
            ),
        )
    ...
    for pipeline_run_path in arguments.pipeline_runs:
        ...
        try:
            with open(pipeline_run_path, 'r', encoding='utf8') as pipeline_run_file:
                if pipeline_run_path.endswith('.yml'):
