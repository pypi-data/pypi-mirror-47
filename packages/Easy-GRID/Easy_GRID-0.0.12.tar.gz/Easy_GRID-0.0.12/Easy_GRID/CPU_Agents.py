...
class Field():
    ...
    def set_anchors(self):
        ...
        for row in range(self.nrow): # then y
            ...
            for col in range(self.ncol): # start from x
                try:
                    name = map.iloc[row, col]
                except:
                    name = "id_%d" %(idx_name)
                    idx_name += 1
                agent = Agent(name, row, col)
                px_anchors = self.anchors[idx]
                agent.set_coordinate(x=px_anchors['x'], y=px_anchors['y'])
    ...
    def get_DF(self):
        ...
        df = df.loc[idx_keep,:]
        return df
    def get_index(self, ch_1, ch_2, ch_3=-1, isSingle=False, isRatio=False, isContrast=False, isThree=False, name_index="index"):
