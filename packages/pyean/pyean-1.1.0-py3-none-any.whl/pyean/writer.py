...
from pyean.version import version
...
class BaseWriter(object):
    ...
    def __init__(self, initialize=None, paint_module=None, paint_text=None,
        ...
        self.module_height_short = 8
        self.human_readable_barcode_ypos = 16.0 + self.text_distance
        self.label_ypos = self.text_distance
        self.label_text = "";
    ...
    def render(self, code):
        ...
        for cc, line in enumerate(code):
            ...
            c = 1
            mlist = []
            start_midle_stop = [0,1,2,44,45,46,47,48,95,94,93]
            for i in range(0, len(line) - 1):
                # if it is short line position start middle or stop module
                if i in start_midle_stop:
                    if line[i] == line[i+1]:
                        c += 1
                    else:
                        if line[i] == "1":
                            mlist.append(str(c) + "short")
                        else:
                            mlist.append(str(-c) + "short")
                        c = 1
                else:
            ...
            first_digit_xpos = bxs - 2.0
            first_part_xpos = bxs + 1.3
            for mod in mlist:
                isShort = False
                if "short" in str(mod):
                    mod = int(mod.replace("short",""))
                    isShort = True
                if mod < 1:
                ...
                self._callbacks['paint_module'](
                    xpos, ypos, self.module_width * abs(mod), color, isShort
                )
                xpos += self.module_width * abs(mod)
            bxe = xpos
            middle_of_barcode = ((bxe -bxs) / 2.0)
            second_part_xpos = middle_of_barcode  + ((bxe - middle_of_barcode) / 2.0) + 3.0
            text_xpos = [first_digit_xpos, first_part_xpos, second_part_xpos]
        ...
        if self.text and self._callbacks['paint_text'] is not None:
            ...
            if self.center_text:
                ...
                xpos = bxs
        for count, digit_and_parts in enumerate(text_xpos):
            if count == 0:
                self._callbacks['paint_text'](digit_and_parts, self.human_readable_barcode_ypos, self.text[0:1], "left")
            if count == 1:
                self._callbacks['paint_text'](digit_and_parts, self.human_readable_barcode_ypos, self.text[1:7], "right")
            if count == 2:
                self._callbacks['paint_text'](digit_and_parts, self.human_readable_barcode_ypos, self.text[7:13], "middle")
        self._callbacks['paint_text'](xpos, self.label_ypos, self.label_text, "middle")
...
class SVGWriter(BaseWriter):
    ...
    def _create_module(self, xpos, ypos, width, color, isShort):
