...
class AddRepo(object, Frame): # TODO integrate to repo class
	def __init__(self, fenetre, paths, exe, interface, **kwargs):
		...
		self.ok_button = Button(self, text='Cloner', command=self.cloneRepo)
		self.remote = Entry(self, width=60)
		self.user = Entry(self, width=30)
		...
		for i, param in enumerate(self.param):
			label = Label(self, text=labelsStr[i], justify='left')
	...
	def close(self):
		if self.interface.fen_paramGen:
			self.interface.fen_paramGen.wm_attributes("-topmost", 1)
...
class Repo(object):
	...
	def __gui(self):
		if self.__showGui:
			self.__fen_recv = Toplevel(self.__papa)
			...
			self.__fen_recv.prog = Label(self.__fen_recv, text="", justify='left')
