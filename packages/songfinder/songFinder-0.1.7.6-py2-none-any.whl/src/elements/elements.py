...
class Element(object):
	def __init__(self, nom='', etype='empty', chemin='', **kwargs):
		self.newline = globalvar.genSettings.get('Syntax', 'newline')
	...
	def diapos(self):
		...
		max_car = int(globalvar.presSettings.get('Presentation_Parameters', 'size_line')*ratio)
...
class ImageObj(Element):
	...
	@property
	def text(self):
		return globalvar.genSettings.get('Syntax', 'newslide')[0]
...
class Passage(Element):
	def __init__(self, version, livre, chap1, chap2, vers1, vers2, **kwargs):
		...
		self.chemin = classPaths.Paths().bibles + version \
						+ globalvar.genSettings.get('Extentions', 'bible')[0]
...
class Chant(Element):
	def __init__(self, chant, nom='', **kwargs):
		...
		if fonc.get_ext(chant) == '':
			chant = chant + globalvar.genSettings.get('Extentions', 'chant')[0]
