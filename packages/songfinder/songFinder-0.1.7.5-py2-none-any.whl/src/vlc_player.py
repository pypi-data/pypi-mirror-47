...
class Player(Frame):
	...
	def __init__(self, parent, fen_gestion, videopanel, title=None):
		...
		self.nb_screen, self.w1, self.h1, self.w2, self.h2 = number_screen(self)
		self.aspect_ratio = float(self.w2)/self.h2
		print self.nb_screen, self.w1, self.h1, self.w2, self.h2
		self.parent.geometry("%dx%d+%d+%d"%(self.w2, self.h2, (self.nb_screen-1)*self.w1, 0))
		self.parent.update()
