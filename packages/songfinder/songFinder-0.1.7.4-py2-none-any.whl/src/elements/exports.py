...
class ExportBase(object):
	...
	def _processNewLine(self, text):
		if globalvar.latexSettings.get('Latex_Parameters', 'saut_lignes') == 'oui':
			text = text.replace('\n', ' \\newline')
		...
		for execp in ['(bis)', '(ter)', '(x4)', '(bis) ', '(ter) ', '(x4) ']:
			text = text.replace(execp + '\\newline', execp + '\n')
	...
	def _getDiapos(self):
		...
		listText, listStype = fonc.splitPerso([text], \
								globalvar.genSettings.get('Syntax', 'newslide'), \
								listStype, 0)
		del listText[0]
		...
		for i,text in enumerate(listText):
			...
			for textRef in newListText:
				match = max(fonc.matchPara(textRef, text), match)
			if match < 4 and text.find('\\...') == -1:
		...
		if globalvar.latexSettings.get('Latex_Parameters', 'one_chorus') == 'oui':
...
class ExportLatex(ExportBase):
	def __init__(self, chemin, **kwargs):
		ExportBase.__init__(self, chemin)
		self._newlineMarker = '\\\\'
	...
	def exportText(self):
		...
		# Capo
		if globalvar.latexSettings.get('Latex_Parameters', 'capo') == 'oui' and self.capo:
		...
		# Song per page
		if globalvar.latexSettings.get('Latex_Parameters', 'one_song_per_page') == 'oui':
			text = '%s\n\\clearpage'%text
...
class ExportMarkdown(ExportBase):
	...
	def exportText(self):
		...
		# Capo
		if globalvar.latexSettings.get('Latex_Parameters', 'capo') == 'oui' \
														and self.capo:
