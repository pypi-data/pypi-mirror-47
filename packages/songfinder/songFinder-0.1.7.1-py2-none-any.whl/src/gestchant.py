...
try:
	...
	import globalvar as globalvar
	import src.creplace as creplace

	def clean_line(text):
		for i in range(5):
			text = text.replace('\n\n','\n')
			text = text.replace('\n \n','\n')
		newline = globalvar.genSettings.get('Syntax', 'newline')
		text = text.replace(newline,'')
		return text

	def numerote(text, numero, nombre, etype)	:
		if nombre > 1 or globalvar.presSettings.get(etype, 'oneslide') == 'oui':
			if globalvar.presSettings.get(etype, 'Numerote_diapo') == 'oui':
				if numero:
					text = '(%s/%s)%s'%(str(numero), str(nombre), text)
		return text

	def print_title(text, title, numero, etype):
		if globalvar.presSettings.get(etype, 'Print_title') == 'oui':
			if numero == 1:
				text = '%s\n%s'%(title.upper(), text)
		return text

	def check_bis(text, etype):
		if globalvar.presSettings.get(etype, 'Check_bis') == 'oui':
	...
	def clean_maj(text, etype):
		...
		if globalvar.presSettings.get(etype, 'Clean_majuscule') == 'oui':
			...
			for pronom in list_pronoms:
				for ponct1 in prePonct:
					for ponct2 in postPonct:
						text = text.replace(ponct1 + pronom + ponct2, ponct1 + pronom.lower() + ponct2)
			text = text.strip(' ')
		return text

	def majuscule(text, etype):
		if globalvar.presSettings.get(etype, 'Majuscule') == 'oui':
	...
	def verifie_ponctuation(text, etype):
		#verification ponctuation
		if globalvar.presSettings.get(etype, 'Ponctuation') == 'oui':
			ponctuations0 = [":"]
	...
	def verifie_ponctuation_maj(text, etype):
	...
	def genere_chant(title, nom_set_chants_dispo):
		...
		chemin = os.path.join(chemin_chants, fonc.enleve_accents(title)\
								+ globalvar.genSettings.get('Extentions', 'chant')[0])
		return chemin, title

	def saut_ligne(text, max_car, etype, force=None):
		liste_sep = ['\\newline', '.', '!', '?', ' et ', ' Et ', ';',  ':', ',']
		do_not = ['(bis)', '(Bis)']
		new_text = text
		if globalvar.presSettings.get(etype, 'Saut_ligne') == 'oui' or force == 'force':
			for i in range(5):
			...
			if globalvar.presSettings.get(etype, 'Saut_ligne_force') == 'oui' or force == 'force':
				lignes = new_text.splitlines()
				new_text = ''
				for ligne in lignes:
					trouve = -1
	...
	def nettoyage(text):
		newslide = globalvar.genSettings.get('Syntax', 'newslide')
		newline = globalvar.genSettings.get('Syntax', 'newline')
	...
	def getListStypePlus(listStype):
		...
		tmp = []
		ignore = globalvar.latexSettings.get('Latex_Parameters', 'ignore')
