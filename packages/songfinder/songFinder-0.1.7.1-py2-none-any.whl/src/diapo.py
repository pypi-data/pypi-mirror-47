...
class Diapo(object):
	def __init__(self, element, numero, stype, max_car, nombre=0, text='', **kwargs):
	...
	def text(self):
		if not self.nbLignes:
			...
			self.nbLignes = len( [ligne for ligne in text.splitlines() if ligne !=''] )
			self._text = text
		return self._text
	...
	def title(self):
		if not self._title:
			if self.element.title:
				...
				if self.nombre > 1:
					self._title =  '%s -- %s/%s'%(self._title, str(self.numero), str(self.nombre))
			else:
	...
	def beamer(self):
		text = self.text
		if globalvar.presSettings.get(self.etype, 'Numerote_diapo') == 'oui':
			num = '(%s/%s)'%(str(self.numero), str(self.nombre))
			text = text.replace(num, '{\\footnotesize %s}\n\\vspace{1em}\n'%num)
		elif globalvar.presSettings.get(self.etype, 'Print_title') == 'oui':
	...
	def backgroundName(self):
		backName = globalvar.presSettings.get(self.etype, 'Background')
		if self.etype == 'image':
