...
import src.dataAccords as dataAccords
...
class Accords(object):
	...
	def getChords(self):
		if globalvar.latexSettings.get('Latex_Parameters', 'transpose') == 'oui':
			self._transpose(self._transposeNb)
		if globalvar.latexSettings.get('Latex_Parameters', 'capo') == 'oui' and self._capo:
			self._transpose(-self._capo)
		if globalvar.latexSettings.get('Latex_Parameters', 'simple_chords') == 'oui':
			self._simplifyChord()
		if globalvar.latexSettings.get('Latex_Parameters', 'keep_first') == 'oui':
			self._keepFirst()
		if globalvar.latexSettings.get('Latex_Parameters', 'keep_last') == 'oui':
			self._keepLast()
		if globalvar.latexSettings.get('Latex_Parameters', 'sol_chords') == 'oui':
			self._translate('fr')
		self._putBrackets()
		self._noDoublon()
	...
	def _clean(self):
		try:
			self.ligneAcc = unicode(self.ligneAcc, 'utf-8')
					...
					try:
						...
						try:
							...
							index = self.data.accordsTon.index( \
										part[0] + part[1].lower().replace('e', u'é') )
							reste = part[2:]
						except (ValueError, IndexError):
	...
	def _transposeTest(self, accordsRef, accordsNon, demiTon):
		...
		for k, accord in enumerate(self._accords):
			newPart = []
