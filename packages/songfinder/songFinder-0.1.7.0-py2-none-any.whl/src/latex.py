...
import src.elements.exports as exports
import src.exception as exception
import src.accords as accords

class LatexParam(object, Frame):
	def __init__(self, fenetre, chants_selection, papa, noCompile, **kwargs):
		...
		for i, (param, item) in enumerate(self.dictParam.items()):
			var = IntVar()
			button = Checkbutton(self, text=param, variable=var, \
								# ~ command=self.save)
		...
		self.bouton_ok = Button(self, text='OK', command=self.createFiles)
		self.bouton_ok.grid(row=nb_row, column=0, columnspan=column_width//2, sticky='w' )
		self.bouton_ok = Button(self, text='Annuler', command=self.quit)
	...
	def save(self, identifyer, event=0, *args):
		...
		for param, valeur in self.dictValeurs.iteritems():
			if valeur.get():
				str_valeur = 'oui'
			else:
				str_valeur = 'non'
			globalvar.latexSettings.set('Latex_Parameters', self.dictParam[param], str_valeur)
		self.maj()

	def maj(self):
		for param in self.dictParam.values():
			if globalvar.latexSettings.get('Latex_Parameters', param) == 'oui':
				self.dictButton[param].select()
			else:
				self.dictButton[param].deselect()

		if self.pressed:
			pressedValue = globalvar.latexSettings.get('Latex_Parameters', self.pressed)
			if pressedValue == 'oui':
				if self.pressed in self.deps:
					for condition in self.deps[self.pressed]:
						globalvar.latexSettings.set('Latex_Parameters', condition[0], condition[1])
						if condition[1] == 'oui':
				...
				for condition in conditions:
					if condition[0] == self.pressed:
						if condition[1] != pressedValue:
							self.dictButton[param].deselect()
							globalvar.latexSettings.set('Latex_Parameters', param, condition[1])
	...
	def createFiles(self):
		...
		if close == 0:
			...
			self.papa.fen_latex.lift()

	def quit(self):
		if self.papa.fen_latex:
			self.papa.fen_latex.destroy()
			self.papa.fen_latex = None


class CreatePDF(object):
	def __init__(self, elements_selection, **kwargs):
		if globalvar.latexSettings.get('Latex_Parameters', 'diapo') == 'oui':
		...
		if globalvar.latexSettings.get('Latex_Parameters', 'reorder') == 'oui':
			self.__listLatex = reorderLatex(self.__listLatex)

		self.__pdflatex = commandLine.MyCommand('pdflatex')

		self.__chemin = os.path.join(globalvar.settingsPath, 'genCarnet')
	...
	def __getTableOfContent(self):
		text = ''
		if globalvar.latexSettings.get('Latex_Parameters', 'affiche_liste') == 'oui':
			# Liste sommaire
			dicoTitres = {latexElem.title:str(self.__listLatex.index(latexElem)+1) \
							for latexElem in self.__listLatex}
			# Alphabetic
			if globalvar.latexSettings.get('Latex_Parameters', 'alphabetic_list') == 'oui':
				listTitres = sorted(dicoTitres.viewkeys())
			else:
			...
			for i, title in enumerate(listTitres):
				# Tonalite
				if globalvar.latexSettings.get('Latex_Parameters', 'key') == 'oui':
			...
			if globalvar.latexSettings.get('Latex_Parameters', 'two_columns') == 'oui':
				text = '\\begin{multicols}{2}\n%s\n\end{multicols}'%text
		return text

	def __getSongList(self):
		text = '\\newcommand{\\songsPath}{%s}\n'%self.__songFolder.replace('\\', '/')
		if globalvar.latexSettings.get('Latex_Parameters', 'list') == 'non':
...
\\end{document}"""%(numPage, self.__tmpName.replace('\\', '/'))
	...
	def writeFiles(self):
		...
		with open(self.__songList, 'w') as out:
			out.write( self.__getSongList().encode('utf-8') )
		# Songs
		if globalvar.latexSettings.get('Latex_Parameters', 'list') == 'non':
			for latexElem in self.__listLatex:
				fileName = os.path.join(self.__songFolder, latexElem.nom + '.tex')
				with open(fileName, 'w') as out:
					out.write( latexElem.exportText.encode('utf-8') )
		# Table of content
		with open(self.__tableOfContent, 'w') as out:
			out.write( self.__getTableOfContent().encode('utf-8') )
	...
	def __checkFiles(self):
		...
		try:
			...
			pass
		source = os.path.join(globalvar.chemin_root, 'genCarnet')
	...
	def __getOutFile(self):
		...
		if globalvar.latexSettings.get('Latex_Parameters', 'reorder') == 'non' and \
			globalvar.latexSettings.get('Latex_Parameters', 'alphabetic_list') == 'non':
	...
	def compileLatex(self):
		...
		if code != 0:
			...
			return 1
		# Compile booklet
		if globalvar.latexSettings.get('Latex_Parameters', 'booklet') == 'oui':
			# Write bookletizer file
			with open(self.__bookletizer,'w') as out:
				out.write( self.__getBookletizer().encode('utf-8') )
			code, out, err = self.__pdflatex.run(options=['-interaction=nonstopmode', \
												self.__bookletizer])
			if code != 0:
				tkMessageBox.showerror('Attention', 'Error while '
...
def reorderLatex(listLatex):
	...
	for elem in listLatex:
		...
		if title in titles:
			...
			elem.title = title
		titles.append(title)
	sortedKeys = sorted(dictNbLigne.viewkeys(), reverse=True)
	maxLine = 40
	suptitle = 4
	newKey = []
	nb = len(sortedKeys)
	...
	for i,key in enumerate(sortedKeys):
		if key > maxLine:
			print('Song "%s" is to big to fit one page, size: %s'\
						%(listLatex[dictNbLigne[key]].title, str(key)))
		if i not in inList:
			newKey.append(key)
			inList.append(i)
