...
class AddRepo(object, tk.Frame): # TODO integrate to repo class
	...
	def cloneRepo(self, event=0):
		guiHelper.update_fen(self.fen_clone, 'Récupération des informations ...')
		repo = self.remote.get()
		name = self.user.get()
		mdp = self.password.get()
		if not (repo and name and mdp):
			guiHelper.update_fen(self.fen_clone, "    échec\n")
			tkMessageBox.showerror('Erreur', 'Erreur: les informations sont incomplètes', parent = self.fen_clone)
		else:
			self.fen_clone.prog.grid(sticky='w')
			guiHelper.update_fen(self.fen_clone, "    ok\nVerification de la connexion ...")
			...
			if repo:
				guiHelper.update_fen(self.fen_clone, "Clonage du dépôt (ceci peut prendre quelques minutes) ...")
...
"""%(repo, prefix, user, mdp, name)
				...
				guiHelper.update_fen(self.fen_clone, "    ok\n")
				tkMessageBox.showinfo('Confirmation', 'Le dépôt à été cloné.', parent = self.fen_clone)
...
class Repo(object):
	...
	def __update_fen(self, message):
		if self.__showGui:
			...
			exceList = set(self.__usbFound) | {u'ok', 'échec', 'aucune'}
	...
	def __showError(self, message):
		...
		tkMessageBox.showerror('Erreur', message, parent = self.__fen_recv)
		self.__close()
		return 1
	...
	def __showInfo(self, message):
		...
		if self.__showGui:
			tkMessageBox.showinfo('Confirmation', message, parent = self.__fen_recv)
		self.__close()
		return 0
	...
	def __checkConnection(self):
		self.__update_fen('Verification de la connexion ...')
		if commandLine.ping('google.fr') == 0:
			self.__update_fen('    ok')
	...
	def __getRemotes(self):
		...
		if self.__remotes == []:
			return self.__showError('La connection a échoué. Verifiez votre connexion à internet.\n'
								'"Aucune cle USB n\'as été trouvé"')
		return 0
	...
	def receive(self, send=0):
		...
		if out.find('no changes found') != -1 or out.find('aucun changement trouv') != -1:
			if send == 1:
				...
				return self.__showInfo('Rien à recevoir')
		elif code == 0:
			if send == 1:
				self.__update_fen("    ok")
			else:
			...
			return self.__showError('Erreur: Impossible de recupérer les modifications.\nErreur %s:\n%s'(str(code), '\n'.join([out,err])))
									...
									'Ceci effacera tout vos changements.'\
	...
	def send(self, what='all'):
		...
		if out.find('nothing changed') != -1 or out.find('aucun changement') != -1 or out.find('nothing to commit') != -1:
			...
			if returns == ['0']*len(self.__remotes):
				return self.__showInfo('Les modifications ont été envoyées sur:%s'%', '.join(self.__remotes))
			else:
