...
END'''
DSN_STRING='DATABASE={};HOSTNAME={};PORT={};PROTOCOL=TCPIP;UID={};PWD={};'
...
class DB2():
    ...
    def __init__(self, credentials):
        ...
        if 'ssldsn' in credentials and credentials['ssldsn'] is not None:
            ...
            if 'ssl' in credentials and credentials['ssl'] is not None:
                if credentials['ssl']:
                    ...
                    cert_file = None
                    if 'certificate' in credentials and credentials['certificate'] is not None:
                        cert_file = self.create_certificate_file(credentials['hostname'],
                                                                 credentials['port'],
                                                                 credentials['certificate'],
                                                                 is_base64=False)
                        credentials['certificate_base64'] = base64.b64encode(credentials['certificate'].strip().encode('UTF-8')).decode()
                    elif 'certificate_base64' in credentials and credentials['certificate_base64'] is not None:
                        cert_file = self.create_certificate_file(credentials['hostname'],
                                                                 credentials['port'],
        ...
        logger.log_debug('Connecting to DB2 ...')
    ...
    def create_certificate_file(self, hostname, port, certificate, is_base64=True):
        ...
        if is_base64:
            ...
            # cert_content = ssl.get_server_certificate((hostname, port))
