...
def get_argument_parser():
    ...
    parser.add_argument('--db2-json', help='IBM DB2 credentials for the datamart database in JSON format:  \'{"username":"<USERNAME>","password":"<PASSWORD>","hostname":"<hostname>","port":"<port>","db":"<db>","ssl":"Optional - set to true if db2 connection should use ssl. Default is false","certificate":"Optional - SSL certificate, required when using self signed certificates. Should be wrapped with \"-----BEGIN CERTIFICATE-----\" and \"-----END CERTIFICATE-----\"","certificate_base64":"Optional - Base64 encoded SSL certificate, required when using self signed certificates"}\'')
    ...
    parser.add_argument('--num-explains', default=0, help=argparse.SUPPRESS, type=int) # Number of explain requests. Default is 0
    parser.add_argument('--max-explain-candidates', default=0, help=argparse.SUPPRESS, type=int) # Maximum number of candidate scores for explain. Default is 0
    ...
    parser.add_argument('--sync-explains', action='store_true', help=argparse.SUPPRESS) # If true, make explain requests synchronous (default is asynchronous)
