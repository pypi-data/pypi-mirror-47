...
def handle(stream_id, stream_schema, API_key, ios_account_id, ios_product_id, android_account_id, android_product_id, check_delta_import, reload_data, days):
    ...
    if stream_id =="product_sales":
        ...
        date_to_check = (datetime.datetime.now()- datetime.timedelta(days=days+1)).strftime('%Y-%m-%d')
        ...
        for product_id in product_ids:
            ...
            if check_delta_import is not False:
                ...
                if result == 0:
                    ...
                    LOGGER.info("There is missing data of yesterday - platform {}".format(platform))

            LOGGER.info("--------------------")
            LOGGER.info("{}---{}".format(start_date,end_date))
            LOGGER.info("--------------------")
            time.sleep(5)
