...
from jetengine.query_builder.transform import update
...
class Aggregation(object):
    ...
    async def fetch(self, alias=None):
        ...
        results = []
        try:
            for item in await coll.aggregate(self.to_query(), cursor=False):
                self.fill_ids(item)
                results.append(edict(item))
    ...
    def avg(cls, field, alias=None):
        from jetengine.aggregation.avg import AverageAggregation

        return AverageAggregation(field, alias)

    @classmethod
    def sum(cls, field, alias=None):
        from jetengine.aggregation.sum import SumAggregation
