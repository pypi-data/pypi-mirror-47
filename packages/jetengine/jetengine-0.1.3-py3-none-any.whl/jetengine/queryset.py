...
from jetengine import ASCENDING
from jetengine.aggregation.base import Aggregation
from jetengine.connection import get_connection
from jetengine.errors import UniqueKeyViolationError, PartlyLoadedDocumentError
...
class QuerySet(object):
    ...
    async def create(self, alias=None, **kwargs):
        ...
        .. testsetup:: saving_create
            ...
            import asyncio
            from jetengine import Document, StringField, connect
    ...
    async def save(self, document, alias=None, upsert=False):
        if document.is_partly_loaded:
            ...
            raise PartlyLoadedDocumentError(msg.format(document.__class__.__name__))
        ...
        if self.validate_document(document):
            ...
            return await self._save(document, alias=alias)
        ...
        def handle(*args, **kw):
            ...
            if document._id is not None:
                self.coll(alias).update(
                    {"_id": document._id}, doc, callback=self.handle_update(document, callback), upsert=upsert
                )
            else:
    ...
    def validate_document(self, document):
        if not isinstance(document, self.__klass__):
            raise ValueError(
                ...
                % (self.__klass__.__name__, document.__class__.__name__)
            )
    ...
    def handle_update_documents(self, callback):
        def handle(*arguments, **kwargs):
            ...
            callback(edict({"count": int(arguments[0]["n"]), "updated_existing": arguments[0]["updatedExisting"]}))
    ...
    async def update(self, definition, alias=None):
        ...
        update_arguments = dict(spec=update_filters, document={"$set": definition}, multi=True)

        res = await self.coll(alias).update(**update_arguments)

        return edict({"count": int(res["n"]), "updated_existing": res["updatedExisting"]})
    ...
    def _check_valid_field_name_to_project(self, field_name, value):
        ...
        """
        if "." not in field_name and (field_name == "_id" or field_name in self.__klass__._fields):
        ...
        while tail:
            ...
            if not obj or field_value not in obj._fields:
                raise ValueError("Invalid field '%s': Field not found in '%s'." % (field_name, self.__klass__.__name__))
            else:
                ...
                if isinstance(field, EmbeddedDocumentField):
                    ...
                    if hasattr(field._base_field, "embedded_type"):
                        ...
                        return self._fill_reference_loaded_fields(head, tail, field_name, value)
                    else:
                        obj = None
    ...
    def only(self, *fields):
        ...
        :func:`~jetengine.queryset.QuerySet.all_fields` will reset any
        field filters.
    ...
    def _get_find_cursor(self, alias):
        ...
        return self.coll(alias).find(
            query_filters, projection=self._loaded_fields.to_query(self.__klass__), **find_arguments
        )
    ...
    def skip(self, skip):
        ...
        Usage::
            ...
            await User.objects.skip(20).limit(10).find_all()  # even if there are 100s of users,
    ...
    def limit(self, limit):
        ...
        Usage::
            ...
            await User.objects.limit(10).find_all()  # even if there are 100s of users,
    ...
    def order_by(self, field_name, direction=ASCENDING):
        ...
        Usage::
            ...
            from jetengine import DESCENDING  # or ASCENDING

            await User.objects.order_by('first_name', direction=DESCENDING).find_all()
        """
        ...
        def handle(*arguments, **kwargs):
            ...
            for doc in result:
                if (lazy is not None and not lazy) or not doc.is_lazy:
                    doc.load_references(
                        doc._fields, callback=self.handle_find_all_auto_load_references(callback, result)
                    )
                else:
    ...
    async def ensure_index(self, alias=None):
        ...
        for field in fields_with_index:
            res = await self.coll(alias).ensure_index(field.db_field, unique=field.unique, sparse=field.sparse)
