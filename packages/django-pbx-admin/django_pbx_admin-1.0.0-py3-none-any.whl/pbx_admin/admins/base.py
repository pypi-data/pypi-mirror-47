...
from pbx_admin.views.base import PbAdminListView, PbAdminCreateView, \
    PbAdminUpdateView, PbAdminDeleteView, PbAdminDeleteMultipleView, \
    CSVExportView, PbAdminShowView
...
class PbModelAdmin:
    ...
    list_display = ('id',)
    list_ordering = None
    export_fields = None  # fields for csv export
    ...
    # view classes
    list_view_class = PbAdminListView
    add_view_class = PbAdminCreateView
    edit_view_class = PbAdminUpdateView
    show_view_class = PbAdminShowView
    delete_view_class = PbAdminDeleteView
    delete_multiple_view_class = PbAdminDeleteMultipleView
    # Duplicate View Class have to implemented separately
    ...
    def __init__(self, model, admin_site, *args, parent=None, **kwargs):
        ...
        if parent:
            ...
            parent_fields = [f for f in self.model._meta.fields
                             if isinstance(f, ForeignKey) and
                             f.related_model == parent_model]
            if parent_fields:
    ...
    def _init_fields(self):
        ...
        elif hasattr(self.form_class, 'Meta') and \
                hasattr(self.form_class.Meta, 'fields'):
    ...
    def _init_forms(self):
        ...
        if p_field and p_field.name not in fields:
            ...
            self.form_class = modelform_factory(
                self.model, form=self.form_class,
                fields=(p_field.name,) + tuple(fields), widgets=widgets
            )
        else:
            self.form_class = modelform_factory(
                self.model, form=self.form_class, fields=fields,
            )

        if not self.search_form_class:
            model_fields = {f.name for f in self.model._meta.fields}
            self.search_form_class = modelform_factory(
                self.model, form=SearchForm,
                fields=[f for f in self.list_display if f in model_fields]
            )

        if not self.export_fields:
            self.export_fields = self.form_class.Meta.fields \
                if self.form_class else self.fields
            if not any(field in self.export_fields for field in ('id', 'pk')):
                self.export_fields = ('id',) + tuple(self.export_fields)
            ...
            if 'formset' in fieldset:
                ...
                if fieldsets:
                    ...
                    form = modelform_factory(
                        formset.model, form=formset.form, fields=fields
                    )
    ...
    def _get_fields_from_fieldsets(self, fieldsets):
        return tuple(itertools.chain(*(fieldset.get('fields', ())
    ...
    def get_urlpatterns(self):
        ...
        if self.edit_view_class:
            urlpatterns.append(
                url(rf'^{obj_pattern}/$' if obj_pattern else '',
                    self.edit_view, name='edit'))

        if self.delete_view_class:
            urlpatterns.append(
                url(rf'^{obj_pattern}/delete/$',
                    self.delete_view, name='delete'))
        if self.delete_multiple_view_class:
            urlpatterns.append(
                url(r'^delete/$',
                    self.delete_multiple_view, name='delete-multiple'))

        if self.show_view_class:
            urlpatterns.append(
                url(rf'^{obj_pattern}/show/$' if obj_pattern else '',
                    self.show_view, name='show'))

        if self.can_duplicate:
            urlpatterns.append(
                url(rf'^{obj_pattern}/duplicate/$',
                    self.duplicate_view, name='duplicate'))

        if self.csv_export_view_class:
            urlpatterns.append(
                url(r'^export-to-csv',
                    self.csv_export_view, name='csv-export'))
    ...
    def get_urls(self):
        ...
        if self.parent:
            prefix = r'{}/{}/'.format(self.parent.get_object_url_pattern(),
                                      self.namespace)
        else:
    ...
    def delete_multiple_view(self, request, *args, **kwargs):
        view_kwargs = self._get_default_view_kwargs()
        return self.delete_multiple_view_class.as_view(
            **view_kwargs,
        )(request, *args, **kwargs)
    ...
    def get_breadcrumbs(self, request, obj=None, **kwargs):
        ...
        if self.parent_field:
            ...
            qs = qs.filter(
                **self.parent_field.get_forward_related_filter(p_obj))
            breadcrumbs = self.parent.get_breadcrumbs(request,
                                                      p_obj,
                                                      **list_kwargs)
        else:
            breadcrumbs = []
        ...
        breadcrumbs += [(
            f'{self.model._meta.verbose_name_plural} ({qs.count()})',
            self.list_url(**list_kwargs)
        )]

        if obj and obj.pk:
            breadcrumb_url = self.edit_url \
                if self.has_change_permission(request, obj=obj) \
                else self.show_url
            breadcrumbs += [(getattr(obj, 'name', str(obj)),
                             breadcrumb_url(**kwargs))]
        elif obj:
            breadcrumbs += [(_('New %s') % self.opts.verbose_name,
                             self.add_url(**kwargs))]
    ...
    def get_add_modal_form(self, view, **form_kwargs):
        if self.add_modal_fields:
            return modelform_factory(
                self.model, form=ModelForm,
                fields=self.add_modal_fields
            )(**form_kwargs)
    ...
    def get_duplicate_modal_form(self, view, **form_kwargs):
        if self.duplicate_modal_fields:
            return modelform_factory(
                self.model, form=ModelForm,
                fields=self.duplicate_modal_fields
            )(**form_kwargs)
    ...
    def get_menu_items(self, request, **kwargs):
        menu_items = []
        for admin in self.children:
            if admin.has_show_permission(request):
                list_url = ''
                if self.url_kwarg in kwargs:
                    list_url = admin.list_url(**kwargs)
                menu_items.append((
                    (list_url, admin.opts.verbose_name_plural)
                ))

        if menu_items:
            if self.url_kwarg in kwargs:
                main_url = self.edit_url(**kwargs) \
    ...
    def get_list_actions(self, request, obj):
        ...
        if self.has_show_permission(request, obj):
            actions.append({
                'icon': 'eye',
                'label': _('Show'),
                'url': f'{obj_id}/show',
        ...
        if self.has_delete_permission(request, obj):
            actions.append({
                ...
                'attrs': {
                    ...
                    'data-modal-body-1':
                        _('Delete %s "%s"?') % (self.opts.verbose_name, obj)
                }
            })
        return actions
    ...
    def get_model_perms(self, request):
        return {
            ...
            'show': self.has_show_permission(request),
        }
    ...
    def get_page_title(self, request, **kwargs):
        ...
        if self.model:
            ...
            if self.url_kwarg in kwargs:
                ...
                title = '{} - {} | '.format(getattr(obj, 'name', str(obj)),
                                            model_title)
            else:
                title = '{} list | '.format(model_title)

        return _('{}Printbox Dashboard {}'.format(title,
                                                  settings.PRINTBOX_SITE_NAME))
...
class PbAdminSite:
    ...
    def register(self, model_or_iterable,
                 admin_class=None, parent_model=None, **options):
        if not admin_class:
            admin_class = PbModelAdmin
        ...
        for model in model_or_iterable:
            ...
            if options:
                admin_class = type(f"{model.__name__}Admin",
                                   (admin_class,), options)
    ...
    def get_urls(self):
        ...
        urlpatterns += [
            path('login/', auth_views.LoginView.as_view(
                ...
                extra_context={
                    'title': f'PBX2 {settings.PRINTBOX_SITE_NAME}'
                }),
                name='login'),
...
def register(*models, **kwargs):
    def _model_admin_wrapper(admin_class):
        ...
        if not isinstance(admin_site, PbAdminSite):
            raise ValueError('site must subclass PbAdminSite')

        if not issubclass(admin_class, PbModelAdmin):
            raise ValueError('Wrapped class must subclass PbModelAdmin.')
