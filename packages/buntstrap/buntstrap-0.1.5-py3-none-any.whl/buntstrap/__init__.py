...
VERSION = '0.1.5'
BOOTSTRAP_FILENAME = "buntstrap.list"
...
def get_deb_item(deb_path, field):
  ...
  return util.wrap_subprocess(
      subprocess.check_output, dpkg_cmd, env={
          'LC_ALL': 'C'
      }
  ).decode("utf-8").strip()
...
def extract_deb(deb_path, rootfs):
  ...
  with open(list_path, 'w') as listfile:
    ...
    dpkg_proc = util.wrap_subprocess(
        subprocess.Popen,
        dpkg_cmd,
        env={'LC_ALL': 'C'},
  ...
  util.wrap_subprocess(
      subprocess.check_call, ['dpkg', '-e', deb_path, control_dir],
      env={'LC_ALL': 'C'}
  )
  ...
  for deb in deb_list:
    ...
    if version_in_map is None:
      ...
      if subprocess.call(dpkg_cmd):
        package_map[package] = (deb, version)
        logging.warning(
            'Skipping obsolete %s %s, superceded by %s\n', package,
  ...
  for idx, deb_path in enumerate(deblist):
    if time.time() - last_print_time > 0.5:
      msg = '\rGeneratingReport [{:6.2f}%]:{:20s}'.format(
          100.0 * (idx + 1) / len(deblist), get_deb_item(deb_path, 'Package')
      )
      sys.stdout.write('\r')
    ...
    try:
      ...
      description = None
    report.append((
        package_name, package_size, installed_size, description,
        package_version
    ))
  ...
  for idx, deb_path in enumerate(deb_list):
    if time.time() - last_print_time > 0.5:
      msg = '\rUnpacking [{:6.2f}%]:{:20s}'.format(
          100.0 * (idx + 1) / len(deb_list), get_deb_item(deb_path, 'Package')
      )
      sys.stdout.write('\r')
      sys.stdout.write(' ' * prev_msg_len)
      sys.stdout.write('\r')
...
def get_apt_archives_plus(rootfs, external_debs=None):
  ...
  deblist = [
      os.path.join(cache_dir, filename) for filename in os.listdir(cache_dir)
...
def apply_patch_text(patch_text, apply_dir):
  ...
  patch_proc = subprocess.Popen([
      'patch', '-d', apply_dir, '-p0', '--forward', '--reject-file=/dev/null'
  ],
                                stdin=subprocess.PIPE,
                                stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE)
  stdout_text, stderr_text = patch_proc.communicate(patch_text.encode("utf-8"))
  if stdout_text:
    logging.info(stdout_text.decode("utf-8"))
  if stderr_text:
    logging.warning(stderr_text.decode("utf-8"))
  if patch_proc.returncode != 0:
    raise RuntimeError(
        'Failed to apply patch [{}]'.format(patch_proc.returncode)
    )
...
def tweak_new_filesystem(root_dir, keep_lists=False):
  """
  ...
  force_symlink(
      '../usr/lib/insserv/insserv', os.path.join(root_dir, 'sbin/insserv')
  )
  ...
  basefiles_path = os.path.join(
      root_dir, 'var/lib/dpkg/info/base-files.postinst'
  )
  ...
  if os.getuid() != 0:
    makedev_postinst = os.path.join(
        root_dir, 'var/lib/dpkg/info/makedev.postinst'
    )
  ...
  if not keep_lists:
    files_to_remove.append(
        os.path.join('etc/apt/sources.list.d', BOOTSTRAP_FILENAME)
    )
...
def install_pip_packages(chroot_app, rootfs, package_list):
  ...
  chroot_app.check_call([
      'pip', 'wheel', '--find-links=/opt/wheelhouse',
...
def initialize_rootfs(rootfs_dir, apt_sources):
  ...
  bootstrap_list_path = os.path.join(
      rootfs_dir, 'etc/apt/sources.list.d', BOOTSTRAP_FILENAME
  )
...
def rfc822_parse(infile):
  ...
  for idx, line in enumerate(infile):
    ...
    try:
      ...
      logging.warning(
          'malformed rfc822 format on %s:%d\n', getattr(infile, 'name', '?'),
          idx
      )
      raise
...
def get_default_packages(
    rootfs, include_essential=False, include_priorities=None
):
  """
...
# NOTE(josh): AllowInsecureRepositories and AllowUnauthenticated are dangerous,
# but ubuntu keyservers are down today so we don't have much choice. Should
...
def iter_mounts():
  ...
  with open('/proc/mounts', 'r') as mounts:
    for line in mounts.readlines():
      (device, mountpoint, filesystem, options, _,
       _) = line.decode('UTF-8').split()
...
def get_apt_version():
  ...
  """
  version_output = util.print_and(
      subprocess.check_output, ['apt-get', '--version']
  ).decode("utf-8").strip()

  firstline = version_output.splitlines()[0]
  # Expected output is "apt 1.6.2 (amd64)" or "apt 1.6.3ubuntu0.1 (amd64)"
  versionish = re.search(r"[\d\.]+", firstline)
  return [int(part) for part in versionish.group(0).split('.')]


def create_rootfs(config):
  config.rootfs = os.path.realpath(config.rootfs)

  if not os.path.isdir(config.rootfs):
    os.makedirs(config.rootfs)
  initialize_rootfs(config.rootfs, config.apt_sources)

  apt_cmd = get_apt_command(
      config.architecture, config.rootfs, config.apt_http_proxy
  )
  ...
  if config.is_enabled('apt-download'):
    default_packages = get_default_packages(
        config.rootfs, config.apt_include_essential,
        config.apt_include_priorities
    )
    if config.pip_packages:
      default_packages.append('python-pip')
    apt_package_list = list(
        sorted(set(config.apt_packages + default_packages))
    )
    ...
    if apt_version >= [1, 2, 24]:
      ...
      util.print_and(
          ...
          apt_cmd + ['-y', '--force-yes', 'install'] + apt_package_list
      )
  else:
  ...
  if config.is_enabled('tweak-fs'):
    ...
    tweak_new_filesystem(config.rootfs, config.apt_keep_lists)
  else:
    logging.info('skipping tweakfs')

  if config.is_enabled('dpkg-configure') and config.chroot_app is not None:
    chroot_app = config.chroot_app(
        config.rootfs, config.binds, config.qemu_binary, config.pip_wheelhouse
    )
    ...
    with chroot_app:
      ...
      configure_dpkgs(
          chroot_app, config.rootfs, config.dpkg_configure_retry_count
      )
  ...
  if config.is_enabled('apt-clean'):
    ...
    for cleanme in ['var/cache/apt/archives', 'var/lib/apt/lists']:
