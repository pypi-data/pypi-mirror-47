...
from r2c.cli.util import (  # type: ignore
    check_docker_is_running,  # type: ignore
    find_and_open_analyzer_manifest,  # type: ignore
)  # type: ignore
...
from r2c.lib.input import GitRepoCommit
...
def run_analyzer_on_local_code(
    ...
    json_output_store.delete(
        GitRepoCommit(url_placeholder, commit_placeholder), specified_analyzer
    )
    ...
    for va in versions:
        with tempfile.TemporaryDirectory(prefix=LOCAL_RUN_TMP_FOLDER) as mount_folder:
            ...
            analyzer.upload_output(
                SpecifiedAnalyzer(va),
                GitRepoCommit(url_placeholder, commit_placeholder),
                mount_folder,
            )
    ...
    results = analyzer.full_analyze_request(
        analyzer_input=GitRepoCommit(url_placeholder, commit_placeholder),
