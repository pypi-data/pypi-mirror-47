...
def signature_decomposition(signatures, mtype, directory):
    ...
    for i in range(signatures.shape[1]):
        ...
        if signatures.shape[0]==96:
            ...
            exposures, _, similarity = ss.add_signatures(sigDatabase, signatures[:,i][:,np.newaxis], presentSignatures=[0,4], solver = "nnls", metric = "l2",check_rule_negatives=[], check_rule_penalty=1.0)
            #print("Exposure after adding", exposures)
            exposures, _, similarity = ss.remove_all_single_signatures(sigDatabase, exposures, signatures[:,i], metric="cosine", solver = "nnls", cutoff=0.01, background_sigs= [], verbose=False)
        ...
        # for other contexts     
            ...
            exposures, _, similarity = ss.add_signatures(sigDatabase, signatures[:,i][:,np.newaxis], presentSignatures=[], solver = "nnls", metric = "l2",check_rule_negatives=[], check_rule_penalty=1.0)
            exposures, _, similarity = ss.remove_all_single_signatures(sigDatabase, exposures, signatures[:,i], metric="cosine", solver = "nnls", cutoff=0.01, background_sigs= [], verbose=False)
        ...
        if len(np.nonzero(exposures)[0])<5 and similarity>0.8: ########### minimum signtatures and cosine similarity needs to be fitted to become a unique signature 
...
def export_information(loopResults, mutation_context, output, index, colnames):
    ...
    fh.write('{}, {}, {}\n'.format(i, round(meanProcessStabity, 2), round(reconstruction_error, 4)))
    fh.close()
...
def make_final_solution(processAvg, allgenomes, allsigids, layer_directory, m, index, allcolnames, process_std_error = "none", signature_stabilities = " ", \
    ...
    if remove_sigs==True:
        ...
        for r in range(allgenomes.shape[1]):
            ...
            # add signatures
            exposureAvg[:, r], _, similarity = ss.add_signatures(processAvg, allgenomes[:,r][:,np.newaxis], presentSignatures=copy.deepcopy(init_add_sig_idx),cutoff=penalty, metric="l2", solver = "nnls",check_rule_negatives=[], check_rule_penalty=1.0)
            if verbose==True:
...
#---------------------------- alternative solution end -------------------------------------------------#
    ...
    index[alternative_solution-1] = index[alternative_solution-1]+"*" 
    data.index = index
    # add % signs
