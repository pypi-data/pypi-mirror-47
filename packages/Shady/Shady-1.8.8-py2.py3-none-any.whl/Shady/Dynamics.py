...
class _Sequence( object ):
	def __init__( self, container ):
		...
		self.f_prev = None
		self.timeZero = None
		self.currentKey   = self.nextKey   = 0.0
		self.currentPiece = self.nextPiece = None
		self.Pop()
		
	def Pop( self ):
		try:
			self.nextKey, self.nextPiece = next( self.items )
		except StopIteration:
			self.nextKey = self.nextPiece = None
			return False
		else:
			return True
		
	def __call__( self, t ):
		if self.timeZero is None: self.timeZero = t
		t -= self.timeZero
		if t == self.t_prev: return self.f_prev
		self.t_prev = t
		...
		stop = False
		value = None
		while nextT is None or nextT <= t:
			if nextT is None and callable( self.currentPiece ):
				try: value = self.currentPiece( t )
				except StopIteration as exc: value = None
				else: break
			self.currentKey = self.nextKey
			self.currentPiece = self.nextPiece
			if not self.Pop(): stop = True; break
			if nextT is None: break
			nextT = self.nextKey
		if value is None: value = self.currentPiece
		if callable( value ):
			try:
				value = value( t )
			except StopIteration as exc:
				if stop or len( exc.args ) != 1 or isinstance( exc.args[ -1 ], dict ):
					raise exc
				else:
					value = exc.args[ -1 ]
		elif stop:
	...
	def AddState( self, state, duration=None, next=Unspecified, onset=None, ongoing=None, offset=None ):
		...
		method is as a class decorator::
			...
			class Second( StateMachine.State ):
				...
				def onset( self ):
					print( self.name )
					# create a new private attribute to store a random duration:
