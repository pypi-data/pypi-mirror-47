...
class PulseClient(object):
	...
	def __init__(self, si_filter_regexp, si_filter_debug=False, volume=None, fade=None):
		...
		self.PulseOperationFailed, self.PulseError, self.PulseLoopStop =\
			PulseOperationFailed, PulseError, PulseLoopStop
		self.log = get_logger()
	...
	def _process_changes(self):
		...
		while self.si_queue:
			...
			except self.PulseOperationFailed: continue
		...
		if t == 'out':
			...
			for si in si_list:
				...
				if v is not None:
					...
					except self.PulseOperationFailed: pass
			self.set_music_mute()
