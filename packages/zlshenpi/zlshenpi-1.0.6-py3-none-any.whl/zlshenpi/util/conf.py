...
def update(user=None, password=None, host=None,database=None):
    if host is not None:
        sql = "update cfg_db set host='%s' " % host
        command(sql)
    if user is not None:
        sql = "update cfg_db set user='%s' " % user
        command(sql)
    if password is not None:
        sql = "update cfg_db set password='%s' " % password
        command(sql)
    if database is not None:
        sql = "update cfg_db set database='%s' " % database
        command(sql)


def add_conp(conp):
    sql = "insert into cfg_db values('%s','%s','%s','%s','%s')" % (conp[0], conp[1], conp[2], conp[3], conp[4])
    command(sql)


def get_df():
    data1 = {
        'zlshenpi':['guangdongsheng']
    }
    data = []
