...
def ext_tb(tbname):
    ...
    jytype_dict = {"gcjs": "工程建设", "zfcg": "政府采购", "yiliao": "医疗采购", "jqita": "其它类型", "qsy": "企事业单位",
                   "xm": "项目",  # 项目审批
                   }

    ggtype_dict = {"zhaobiao": "招标公告", "zhongbiao": "中标公告", "zhongbiaohx": "中标候选公告", "liubiao": "流标公告", "kaibiao": "开标公告",
                   "zgys": "资格预审公告", "zgysjg": "资格预审结果公告", 'zsjg': "资审结果", "gqita": "其他公告", "kongzhijia": "控制价公告",
                   "yucai": "预采公告", "biangeng": "变更公告", "dayi": "答疑公告", "hetong": "合同公告" , "yanshou": "验收公告", "zhongzhi": "终止公告",

                   "beian": "备案", "hezhun": "核准", "jieguo": "结果", "hzqgs": "核准前公示", "shenpi": "审批", # 项目审批
                   }
...
def create_gg(conp):
    ...
    diqu  text,
    info  text ,
    primary key(name,xm_code,href,ggstart_time)
...
def insert_tb(tbname, diqu, conp):
    ...
    sql2 = """
    insert into %s.gg
    select  distinct on (xm_code,name,href,ggstart_time ) xm_code,name,href,ggstart_time,%s::text as ggtype,%s::text as jytype,
    '%s'::text diqu, info from %s.%s as a
    where not exists(select 1 from %s.gg as b where a.name=b.name and a.href =b.href and a.ggstart_time=b.ggstart_time and a.xm_code=b.xm_code)
...
def est_html_cdc(conp, f, **args):
    ...
    # sql = "select distinct href from %s.gg as a  where  not exists (select 1 from %s.gg_html as b where a.href=b.href )" % (conp[4], conp[4])
    df = db_query(sql, dbtype="postgresql", conp=conp)
...
def get_items_large(conp, data, **krg):
    ...
    for w in data:
        ...
        if 'cdc_part' in krg.keys():
            ...
            # 获得self.total
            m.get_total(f2=w[4], url=w[1])

        ##爬全量
        else:
            page_total = m.get_total(f2=w[4], url=w[1])
