...
def multi_component_layout(
    ...
    result = cp.empty((graph.shape[0], dim), dtype=np.float32)
    ...
    if n_components > 2 * dim:
        ...
        base = cp.hstack([cp.eye(k), cp.zeros((k, dim - k))])
...
def spectral_layout_gpu(data, graph, dim, random_state, metric="euclidean", metric_kwds={}):
