...
class Instagram():
    ...
    #         È¤Àº options.add_argument("--disable-gpu")

            for i in range(76, 72, -1):
                try:
                    driver = webdriver.Chrome(f'./chromedriver/mac/chromedriver_{i}', options=options)
                    ...
                    try:
                        driver = webdriver.Chrome(f'./chromedriver/win/chromedriver_{i}', options=options)
                        driver.maximize_window()
                        self.driver = driver
                        break
                    except:
