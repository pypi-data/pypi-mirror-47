...
HuberRegressionResult = namedtuple('HuberRegressionResult',
                                  ('slope', 'intercept', 'rvalue',
                                   'pvalue', 'stderr'))

DescriptiveStats_Extend = namedtuple('DescriptiveStats',
                                    ('count', 'min', 'max', 'mean', 'median',
                                     'std', 'sum', 'range', 'noise',
                                     'percentile_5th', 'percentile_10th',
                                     'percentile_25th', 'percentile_75th',
                                     'percentile_90th', 'percentile_95th'))
                                                    
DescriptiveStats = namedtuple('DescriptiveStats', 
...
def Lineariser_Power_v3(endog, exog, initial=1, maxits=1000, prec=10**-7, weights=None, std_err=False):
    ...
    with warnings.catch_warnings():
        ...
        if std_err is not False: 
            ...
            #Determine the 95% confidence limits
            Power_Error = np.diff(np.percentile(Results_All, [2.5,97.5])).astype(float)/2
...
def stats(array, extend=True, axis=None, output=False):
    ...
    if extend:
        if array.size == 0:
            ...
            if output is False:
                return DescriptiveStats_Extend(
                    np.size(array),
                    np.nanmin(array, axis=axis),
                    ...
                    np.nanpercentile(array, 95, axis=axis))
            
            else:
                print("Descriptive Statistics")
                print("----------------------")
                print("Count = %s" % np.size(array))
...
def pearson_error(arr, n, alpha=0.05, output='lowhigh'):
    """
    Calculates the standard error of for the Pearson correlation 
    coefficient.
    
    References
    ----------
    https://en.wikipedia.org/wiki/Pearson_correlation_coefficient#Inference
    https://stats.stackexchange.com/a/73628
    """
    
    # Convert arr to an array
    arr = np.asarray(arr)
    
    # Convert n to an array if an array_like has been given
    if isarray(n):
        n = np.array(n, dtype=int)
    
    # Calculate z-score using alpha
    z_score = sp.stats.norm.ppf(1 - alpha / 2)

    # Calculate upper and lower confidence levels
    lower = np.tanh(np.arctanh(arr) - z_score / (n - 3)**0.5)
    upper = np.tanh(np.arctanh(arr) + z_score / (n - 3)**0.5)
    
    if output == 'lowhigh':
        return upper - lower
    elif output == 'errorbar':
        return np.array([arr - lower, upper - arr])
    elif output == 'confidence_intervals':
        return np.array([lower, upper])
    else:
        raise ValueError("Output option {0} is not supported. Choose either" \
                         "'lowhigh', 'errorbar' or 'confidence_intervals'".format(output))
...
def ensemble(xdata, ydata, bins, average=('mean', 'median'), method='ma', 
                    ...
                    if usestride is False:
                        ...
                        if x_err:
                            if confidence_intervals == 'standard':
                                xdata_se = 1.96*np.nanstd(data_bin_x, axis=1)/np.sqrt(elems) if xdata_average == 'mean' else 1.96*(1.253*np.nanstd(data_bin_x, axis=1)/np.sqrt(elems))
                            elif confidence_intervals == 'bootstrap':
                                xdata_se = boot.ci(data_bin_x.T, lambda x: np.mean(x, axis=0)).ptp(axis=0) if xdata_average == 'mean' else boot.ci(data_bin_x.T, lambda x: np.median(x, axis=0)).ptp(axis=0)
                            else:
    ...
    if unpack is False:
        ...
        if slim is False:
            ...
            if x_err:
                return MidPoints, Average, xdata_se, SE 
            else:
