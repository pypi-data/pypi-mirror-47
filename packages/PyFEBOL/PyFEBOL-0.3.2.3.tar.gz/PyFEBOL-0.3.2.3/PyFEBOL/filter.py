...
class ParticleFilter(Filter):
    ...
    def getBelief(self):
        ...
        assert np.all(np.isfinite(f)), 'belief matrix contains nan values. filter: {}, i: {}, j: {}, weights: {}'.format(f, i, j, self.weights)
        return f
    ...
    def _updateParticles(self, pose, obs):
        ...
        if np.any(np.isnan(self.weights)):
            print('self.weights contains nan! {}'.format(self.weights))
            print('nansum is: {}'.format(np.nansum(self.weights)))
        self.weights = np.nan_to_num(self.weights) # we get problems with nan with larger particle sizes
    ...
    def _stratifiedResample(self):
        ...
        cumsum = np.nancumsum(self.weights)
