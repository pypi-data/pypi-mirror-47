...
class ParticleFilter(Filter):
    ...
    def _updateParticles(self, pose, obs):
        ...
        self.old_sum = self.old_weights.sum()
        prob = self.sensor.prob((self.x_particles, self.y_particles), pose, obs)
        self.weights *= prob
        assert np.all(np.isfinite(self.weights)), 'weights contains nan values: weights: {}, prob: {}, old_weights: {}, old_sum: {}'.format(self.weights, prob, self.old_weights, self.old_sum)
