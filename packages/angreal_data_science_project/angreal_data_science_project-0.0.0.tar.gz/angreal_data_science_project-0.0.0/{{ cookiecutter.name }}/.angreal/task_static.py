...
# @angreal.option('--open',is_flag=True,help="Open report in browser")
def angreal_cmd(verbose, cpu, no_docker):
    ...
    if no_docker or in_container():
        ...
        subprocess.run(
            'mypy {{ cookiecutter.name }} --ignore-missing-imports --show-error-context --html-report typing_report ',
            shell=True)
