...
from flask_jwt_extended import JWTManager, verify_jwt_in_request, get_current_user,\
    create_access_token, create_refresh_token, get_jwt_claims
...
def get_access_token(user, scope=None, fresh=False, expires_delta=None):
    identity['uid'] = user.id
...
def get_refresh_token(user, scope=None, expires_delta=None):
    identity['uid'] = user.id
    identity['scope'] = scope
