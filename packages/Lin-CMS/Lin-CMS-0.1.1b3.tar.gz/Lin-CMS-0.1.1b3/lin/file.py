...
from lin.exception import FileTooLargeException, NotFound, \
...
class Uploader(object):
    ...
    def __mkdir_if_not_exists(self):
        if not os.path.isabs(self._store_dir):
            self._store_dir = os.path.abspath(self._store_dir)
        exists = os.path.exists(self._store_dir)
        if not exists:
            raise NotFound(msg='no such a directory')
        # mkdir by YYYY/MM/DD
