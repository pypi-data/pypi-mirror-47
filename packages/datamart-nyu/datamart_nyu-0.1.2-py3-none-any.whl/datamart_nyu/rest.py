...
def get_dataset_id(data: typing.Optional[container.Dataset]) -> str:
    if data:
        return data.metadata.query(())['id']
    return None


def get_resource_id(data: typing.Optional[container.Dataset]) -> str:
    if data:
...
class RESTSearchResult(datamart.DatamartSearchResult):
    def __init__(self, json_obj, connection_url: str) -> None:
    ...
    def download(self, supplied_data: typing.Optional[container.Dataset],
        ...
        dataset, path = download_dataset(res)
        if dataset:
            return dataset
    ...
    def augment(self, supplied_data: container.Dataset,
        ...
        dataset, path = download_dataset(res)
        if dataset:
            dataset = fix_metadata(dataset, path, supplied_data)
            return dataset
        raise Exception("Error while reading the HTTP response.")
...
def download_dataset(
        response: requests.Response) -> typing.Tuple[container.Dataset, str]:
...
def fix_metadata(
    ...
    new_metadata = new_metadata.update(
        ...
        metadata=dataset.metadata.query(('learningData',))
    )
    new_metadata = new_metadata.update(
        selector=(['learningData', metadata_base.ALL_ELEMENTS]),
        metadata=dataset.metadata.query(
            ('learningData', metadata_base.ALL_ELEMENTS)
        )
    )

    # keeping metadata about previosly existing columns
    ...
    for i in range(number_cols):
        new_metadata = new_metadata.update(
            selector=(['learningData', metadata_base.ALL_ELEMENTS, i]),
    ...
    new_destination = tempfile.mkdtemp()
    dataset.save('file://' + new_destination + '/datamart/datasetDoc.json')
    shutil.rmtree(dataset_path)

    dataset = container.Dataset.load(
        'file://' + new_destination + '/datamart/datasetDoc.json'
    )
