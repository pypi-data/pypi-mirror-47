...
class HiLow:
    ...
    def _read_hi(self, tag):
        try:
            ...
            return self.cfg_mgr.read_entry(section=tag, entry=self.hi_tag, default_value=self.smallest+.01)

    def _read_low(self, tag):
        try:
            return self.cfg_mgr.read_entry(section=tag, entry=self.low_tag, default_value=self.biggest)
        except ValueError:
            return self.cfg_mgr.read_entry(section=tag, entry=self.low_tag, default_value=self.biggest-.01)
