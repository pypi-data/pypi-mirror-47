...
class HiLow:
    ...
    def read_values(self, tag):
        ...
        """
        hi = self.cfg_mgr.read_entry(section=tag, entry="hi", default_value=self.smallest)
        low = self.cfg_mgr.read_entry(section=tag, entry="low", default_value=self.biggest)
    ...
    def _write_values(self, tag, values):
        ...
        :return:
        """
        self._write_hi(tag=tag, hi=values["hi"])
        self._write_low(tag=tag, low=values["low"])
    ...
    def _read_hi(self, tag):
        return self.cfg_mgr.read_entry(section=tag, entry="hi", default_value=self.smallest)

    def _read_low(self, tag):
        return self.cfg_mgr.read_entry(section=tag, entry="low", default_value=self.biggest)
...
def test_hi_low2(file_name, max_value, min_value):
    ...
    tag = "second"
    max_value += 1
    hi_low._write_values(tag=tag, values={"hi": max_value, "low": min_value})
    values = hi_low.read_values(tag=tag)
    assert_test(values == {"hi": max_value, "low": min_value}, "values should match")
    min_value -= 1
