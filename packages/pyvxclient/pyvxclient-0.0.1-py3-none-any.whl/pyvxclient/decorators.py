...
def handle_response(logger=None):
    ...
    def decorator(func):
        def call(*args, **kwargs):
            try:
                ...
                result = data.result
                return ApiResponse(data=result.get('data', None), message=result.get('message', None),
...
def paginate(func):
    def call(*args, **kwargs):
        ...
        while True:
            ...
            if items_left <= kwargs['limit']: kwargs['limit'] = items_left
            ...
            if len(data.get('data')) == 0 or len(data.get('data')) < kwargs['limit']:
                break
            # if the wanted result is retreived
