...
def partition_iterable(
    ...
    return {'train': indicies[:a], 'valid': indicies[a:b], 'test' : indicies[b:]}


from mag.py.files import filelines, streamlines
def downsample(
    files:list,
    write_to_disk:boolean = False,
    write_dir:str=None,
    prefix:str='',
    suffix:str=None,
) -> list:
    ...
    to = min([records for file, records in file_records.items()])
    # indicies of all lines per file
    ranges = [list(range(records)) for file, records in file_records.items()]
    # mix up lines
