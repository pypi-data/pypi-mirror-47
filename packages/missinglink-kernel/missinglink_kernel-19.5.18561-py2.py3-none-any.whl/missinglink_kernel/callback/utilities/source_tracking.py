...
def __fill_repo(repo, res=GitExpando()):
    ...
    res.head_sha_url = res._remote_template.format(res.head_sha) if res._remote_template and res.head_sha else res.head_sha
...
class GitRepoSyncer(object):
    ...
    def try_ex_path(cls, git, repo_path, remote_url):
        if path.isdir(repo_path):
            try:
                ...
                cls._raise_git_error_if(origin_remote.url != remote_url, 'The tracking repository origin is broken: %s != %s', origin_remote.url, remote_url)
                return repo
    ...
    def _validate_tracking_repository_or_clone(cls, repo_path, tracking_origin_url):
        git = import_git()
        repo = cls.try_ex_path(git, repo_path, tracking_origin_url) or git.Repo.clone_from(tracking_origin_url, repo_path)
    ...
    def _get_remote_path(cls, tracking_repository_remote_url):
        repo_id = 'ml_{}_{}'.format(cls.get_repo_path_hash(os.path.expanduser('~')), cls.get_repo_path_hash(tracking_repository_remote_url))
    ...
    def _warn_big_file(cls, file_mode, file_path):
        ...
        if fsize > cls.SYNC_FILE_THRESHOLD:
            eprint('Repository Tracking: The file %s is big (%s MB). You might want to add it to .gitignore' % (file_path, round(fsize / (1024 * 1024))))
    ...
    @classmethod
    def sync_working_dir_if_needed(cls, repo_obj, in_rm_context=False):
        try:
            if in_rm_context:
            ...
            commit_tag = cls.merge_src_to_tracking_repository(repo_obj.repo, shadow_repo)
            ...
            if 'error' not in src_data:
                logger.info('Tracking repository sync completed. This experiment source code is available here: {}'.format(src_data['sha_local_url']))
            else:
            ...
            logger.error("Failed to init repository tracking. This experiment may not be tracked. Got: " + ex_txt)
            return {'error': ex_txt}
    ...
    def get_shadow_repo(cls, repo_obj):
        ...
        if repo_obj.remote_url is None:
            logger.warning('The git repository in %s has not remote url. Only advanced source tracking is supported.', repo_obj.repo.working_dir)
