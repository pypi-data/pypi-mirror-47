...
class BaseCallback(BaseCallbackImages):
    ...
    def __init__(self, owner_id=None, project_token=None, stopped_callback=None, framework='none', **kwargs):
        ...
        self._special_points = {}  # Points that we don't want to drop out
        self.iteration = 0
    ...
    def batch_end(self, batch, epoch, metric_data, update_hyperparams=False, **kwargs):
        ...
        for metric_name in updated_metrics:
            if metric_name in self._special_points:
                del self._special_points[metric_name]
        ...
        if iteration <= self.get_sampling_size():
            ...
            self.set_points_candidate(data, iteration, starting_offset, updated_metrics)
        ...
        if 'points_candidate' in data:
            for metric_name in updated_metrics:
                self._special_points[metric_name] = data['points_candidate']

        if send:
            data.update(kwargs)
            self.batch_command(EventTypes.BATCH_END, data, flush=iteration == 1)

    def set_points_candidate(self, data, iteration, starting_offset, updated_metrics):
        points_candidate = self._get_point_candidate(iteration)
    ...
    def _get_point_candidate(self, iteration):
        points_candidate = random.randint(self._FIRST_ITERATION + 1, iteration)
        if points_candidate <= self.get_sampling_size() and points_candidate not in self._special_points.values():
