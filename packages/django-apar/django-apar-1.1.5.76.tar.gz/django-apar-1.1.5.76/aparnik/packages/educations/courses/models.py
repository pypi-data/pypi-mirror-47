...
class CourseSummary(models.Model):
    ...
    course = models.ForeignKey('BaseCourse', related_name='total_times', verbose_name=_('Course'))
...
class BaseCourseManager(ProductManager):
    ...
    def active(self):
        return super(BaseCourseManager, self).active()
...
class CourseManager(BaseCourseManager):
    ...
    def active(self):
        return super(CourseManager, self).active().filter(publish_date__lte=now())
...
class Course(BaseCourse):
    ...
    cover = models.ForeignKey(FileField, related_name='course_cover', verbose_name=_('Cover Image'))
    banner = models.ForeignKey(FileField, related_name='course_banner', verbose_name=_('Banner Image'))
    # TODO: set category marboot be course
    category_obj = models.ForeignKey(Category, null=True, blank=True, verbose_name=_('Category'))
    parent_obj = models.ForeignKey('self', null=True, blank=True, verbose_name=_('Parent'))
    dependency_obj = models.ForeignKey('self', related_name='dependency', null=True, blank=True, verbose_name=_('Dependency'))
...
class CourseFileManager(FileManager):
    ...
    def file_count(self, course_obj):
        return self.get_queryset().filter(course_obj=course_obj).count()


# File Model
class CourseFile(File):

    course_obj = models.ForeignKey(Course, verbose_name=_('Course'))
...
class CourseSegment(BaseSegment):
    ...
    def __unicode__(self):
        return super(CourseSegment, self).__unicode__()
