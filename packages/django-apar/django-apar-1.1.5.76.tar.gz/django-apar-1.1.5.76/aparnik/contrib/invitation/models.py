...
class Invite(models.Model):
    ...
    invite = models.ForeignKey(User, related_name='invite', verbose_name=_('Invite'))
    invited_by = models.ForeignKey(User, related_name='invite_by', verbose_name=_('Invited By'))
...
def post_save_invite_receiver(sender, instance, created, *args, **kwargs):
    ...
    if created:
        ...
        invited_gift_credits = Setting.objects.get(key='INVITED_GIFT_BON_CREDITS')
        # پس از قبول دعوت مبلغ بالا به حساب دعوت شده واریز می شود در صورتی که عدد مخالف صفر باشد.
        gift = int(invited_gift_credits.get_value())
        if gift > 0:
            # کسی که دعوت شده
        ...
        inviter_gift_credits = Setting.objects.get(key='INVITER_GIFT_BON_CREDITS')
        # پس از قبول دعوت مبلغ بالا به حساب دعوت کننده واریز می شود در صورتی که عدد مخالف صفر باشد.
        gift = int(inviter_gift_credits.get_value())
        if gift > 0:
            # کسی که دعوت کرده
