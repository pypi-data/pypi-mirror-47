...
class ProductSharingManager(BaseModelManager):
    ...
    def share_with_this_user(self, user, product_id=None):
        if not user.is_authenticated():
            return ProductSharing.objects.none()
        queryset = self.active().filter(user_product_share_with_obj=user)
        if product_id:
            queryset = queryset.filter(product_obj__id=product_id)
        return queryset
...
class ProductSharing(BaseModel):
    ...
    user_obj = models.ForeignKey(User, related_name='productsharing_user_source',verbose_name=_('User'))
    user_product_share_with_obj = models.ForeignKey(User, related_name='productsharing_user_product_share_with', verbose_name=_('Share With'))
    product_obj = models.ForeignKey('products.Product', related_name='productsharing_product', verbose_name=_('Product'))
