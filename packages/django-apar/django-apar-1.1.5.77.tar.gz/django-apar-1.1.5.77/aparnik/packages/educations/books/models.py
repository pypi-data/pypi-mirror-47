...
# Library MODEL
class Book(File):
    sample_book = models.ForeignKey(FileField, null=True, blank=True, verbose_name=_('Sample Book'))
    published_date = models.DateTimeField(verbose_name=_('Published Date'))
    category_obj = models.ForeignKey(Category, verbose_name=_('Category'))
    publisher_obj = models.ForeignKey(Publisher, verbose_name=_('Publisher'))
...
class BookSegment(BaseSegment):
    ...
    def __unicode__(self):
        return super(BookSegment, self).__unicode__()
