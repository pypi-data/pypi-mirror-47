...
from aparnik.packages.shops.coupons.models import Coupon, CouponForCoustomUser
...
class Order(BaseModel):
    ...
    coupon = models.ForeignKey(Coupon, related_name='orders', null=True, blank=True, verbose_name=_('Coupon'))
    status = models.CharField(max_length=5, default=STATUS_WAITING, choices=STATUS_CHOICES, verbose_name=_('Status'))
    address_obj = models.ForeignKey(UserAddress, null=True, blank=True, verbose_name=_('Order Address'))
    ...
    @staticmethod
    def query_success(prefix=''):
        field = 'status'
        if prefix:
            field = prefix + '__' + field
        return Q(**{field: Order.STATUS_COMPLETE}) | Q(**{field: Order.STATUS_PAID}) | Q(**{field: Order.STATUS_PAID_BY_WEBSITE})
    ...
    def query_date(prefix='', start_date=None, end_date=None):
        ...
        if not end_date:
            end_date = now()
        field = 'payments__update_at__range'
        if prefix:
            field = prefix + '__' + field
...
class OrderItem(BaseModel):
    order_obj = models.ForeignKey(Order, related_name='items', verbose_name=_('Order'))
    product_obj = models.ForeignKey(Product, related_name='products', verbose_name=_('Product'))
