...
from aparnik.utils.utils import is_app_installed, human_format
...
class BaseModelListSerializer(ModelSerializer):
    ...
    visit_count_string = serializers.SerializerMethodField()
    bookmark_count_string = serializers.SerializerMethodField()
    review_count_string = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'url', 'is_bookmark', 'review_average', 'review_count', 'review_count_string' 
                            'qa_count', 'qa_count_string', 'url_review',
                            'url_qa', 'tags', 'sort', 'visit_count', 'visit_count_string',
                            'bookmark_count', 'bookmark_count_string'
                            ]
    ...
    def get_review_count(self, obj):
        from aparnik.contrib.reviews.models import Review
        return Review.objects.model_review_count(model_obj=obj.id)

    def get_review_count_string(self, obj):
        return human_format(self.get_review_count(obj))
    ...
    def get_qa_count(self, obj):
        ...
        return QA.objects.model_question_answer_count(model_obj=obj.id)

    def get_qa_count_string(self, obj):
        return human_format(self.get_qa_count(obj))
    ...
    def get_review_average(self, obj):
        return obj.review_average if obj.review_average else 0
    ...
    def get_bookmark_count(self, obj):
        ...
        return Bookmark.objects.models(model=obj).count()

    def get_visit_count_string(self, obj):
        return human_format(obj.visit_count)

    def get_bookmark_count_string(self, obj):
        return human_format(self.get_bookmark_count(obj))
...
class ModelDetailsPolymorphicSerializer(PolymorphicSerializer):
    def __init__(self, *args, **kwargs):
        ...
        if is_app_installed('aparnik.contrib.bankaccounts'):
            from aparnik.contrib.bankaccounts.models import BankName, BankAccount
            from aparnik.contrib.bankaccounts.api.serializers import BankNameDetailSerializer, BankAccountDetailSerializer
