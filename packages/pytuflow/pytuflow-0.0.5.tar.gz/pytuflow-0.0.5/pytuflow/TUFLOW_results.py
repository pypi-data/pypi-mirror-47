...
class LP():
    def __init__(self): #initialise the LP data
        ...
        self.static = False
        self.culv_verts = []
        self.adverseH = LPAdverse()
        self.adverseE = LPAdverse()
...
class DataRL():
    def __init__(self): #initialise the Reporting Locations
        ...
        self.P_Max = RLPointMax()
        self.L_Max = RLLineMax()
        self.R_Max = RLRegionMax()
...
class PlotObjects():
    ...
    def __init__(self, fullpath): #read the file
        ...
        try:
            ...
            self.message.append('ERROR - Error reading data from: {0}'.format(fullpath))
        ...
        with open(fullpath, 'r') as csvfile:
            ...
            for (counter, row) in enumerate(reader):
                ...
                if len(chan_list) != int(row[4]):
                    if int(row[4]) != 0:
                        self.message.append('ERROR - Number of channels connected to ID doesnt match. ID: ' + str(row[1]))
                else:
...
class ResData():
    ...
    def getLPData(self, dat_type, time, dt_tol):
        ...
        if (self.times[t_ind] - time)>dt_tol:
            ...
            return  error, message
        if dat_type == 'Water Level' or dat_type.upper() == 'H':
            self.LP.Hdata = []
            ...
            for i, h_ind in enumerate(self.LP.H_nd_index):
                if i == 0:
                    ...
                    self.LP.Hdata.append(max(self.Data_1D.H.Values[t_ind,h_ind], self.LP.chan_inv[2*i-1]))
        elif dat_type == 'Energy Level' or dat_type.upper() == 'E':
            self.LP.Edata = []
    ...
    def getLongPlotXY(self, type, time):
        ...
        if 'water level' in type.lower() or type.upper() == 'H':
            if time == -99999:
                return error, message, (self.LP.dist_chan_inverts[:], self.LP.Hmax[:])
            else:
                error, message = self.getLPData('Water Level', time, 0.01)
                return error, message, (self.LP.dist_chan_inverts[:], self.LP.Hdata[:])
        
        elif 'energy level' in type.lower() or type.upper() == 'E':
            if time == -99999:
                return error, message, (self.LP.dist_chan_inverts[:], self.LP.Emax[:])
            else:
                error, message = self.getLPData('Energy Level', time, 0.01)
                return error, message, (self.LP.dist_chan_inverts[:], self.LP.Edata[:])
        ...
        elif 'pit ground levels' in type.lower():
            return False, '', (self.LP.pit_dist[:], self.LP.pit_z[:])
        
        elif 'culverts and pipes' in type.lower():
            return False, '', self.LP.culv_verts[:]  # dummy y data - ultimately not used
        
        else:
            return True, 'Result type not recognised', ([], [])