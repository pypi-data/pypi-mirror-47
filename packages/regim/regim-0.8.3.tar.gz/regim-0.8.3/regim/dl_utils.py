...
def get_missing_l(l, kernal, pad, stride):
    scans = round((l+pad-k)/stride)
    return kernal + scans * stride - l
...
def conv2d_output_shape(h_w, kernel_size=1, stride=1, pad=0, dilation=1):
    ...
    h_missed = h_w[0]+sum(pad[0])-(kernel_size[0]+(h-1)*stride[0])
    w_missed = h_w[1]+sum(pad[1])-(kernel_size[1]+(w-1)*stride[1])

    return (h, w), (h_missed, w_missed)

def convtrans2d_output_shape(h_w, kernel_size=1, stride=1, pad=0, dilation=1, out_pad=0):
...
def convtrans2d_get_padding(h_w_in, h_w_out, kernel_size=1, stride=1, dilation=1, out_pad=0):
