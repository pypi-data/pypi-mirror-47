...
class MethodLauncher():
    ...
    def _counts_validations(self, counts: pd.DataFrame, meta: pd.DataFrame) -> pd.DataFrame:
        ...
        for celltype in meta.cell_type.drop_duplicates().tolist():
            available_group = meta[meta.cell_type == celltype]['group'].drop_duplicates().tolist()
            if 'tumor' not in available_group:
