...
def make_paper_page(
    short_title: str, url: str, journal: str, papers_dir: Optional[str] = None
):
    ...
    try:
        template = (papers_dir / "template.md").read_text()
...
def extract_arxiv_data(url: str) -> Dict[str, str]:
    ...
    paper_data["title"] = next(filter_by_name(meta_tags, "citation_title"))["content"]
    paper_data["authors"] = [
