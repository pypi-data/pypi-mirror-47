...
from .dummy_content import dummy_content
...
class NotificationTemplateAdmin(TranslatableAdmin):
    ...
    def get_urls(self):
        urls = super(NotificationTemplateAdmin, self).get_urls()
        custom_urls = [
            url(
                r"preview/(?P<id>\d+)$",
    ...
    def preview(self, request, obj, **kwargs):
        ...
        body_html = env.from_string(obj.body_html).render(dummy_content.content)
