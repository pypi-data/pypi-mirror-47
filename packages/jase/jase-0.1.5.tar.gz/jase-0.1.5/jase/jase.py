...
class jase:
    def __init__(self, debug = 0, basedir = "jase", dict_fn = "data/dict_v1.csv"):
        ...
        for aline in csvreader:
            ...
            self.all_category = list(set(tmp_category))
            self.all_category.sort()
            if self.debug: print ("%s: %s" % (aline[0], key1))
            
    def match_keywords_rlist(self, thisdoc):
        return self.match_keywords(thisdoc, retval = 'list')

    def match_keywords(self, thisdoc, retval = 'dict'):
        sum1 = dict()
        alist = []
        for akey in self.all_keywords:
            ...
            if cate in sum1:
                ...
                sum1[cate] = len(m1)
        for acate in self.all_category:
            try:
                alist.append(sum1[acate])                
            except:
                alist.append(0)
        if retval == 'dict':
            return sum1
    ...
    def mark_match_keywords(self, thisdoc):
        ...
        for agroup in out3.groupby(by=['start']):
            ...
            outstr += thisdoc[idx:start] + "_" + thisdoc[start:end]
            #outstr += thisdoc[idx:end]
            outstr += "[" + cate_str + "]"
