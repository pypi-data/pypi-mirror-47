# RSA module
# Copyright (C) 2010 Sybren Stuvel, Marloes de Boer, Ivo Tamboer, Barry Mead
#
...
def bytes2int(arg):
    ...
    if not (type(arg) is list or type(arg) is bytes):
        raise TypeError("You must pass a byte string or a list")
...
def int2bytes(number):
    ...
    while number > 0:
        string = b"%s%s" % (bytes([number & 0xFF]), string)
        number //= 256
...
def to64(number):
    ...
    if 10 <= number <= 35:
        return bytes([number + 55])     #10-35 translates to 'A' - 'Z'

    if 36 <= number <= 61:
        return bytes([number + 61])     #36-61 translates to 'a' - 'z'
...
def int2str64(number):
    ...
    while number > 0:
        string = b"%s%s" % (to64(number & 0x3F), string)
        number //= 64
...
def calculate_keys(p, q, nbits):
    ...
    if not d == 1:
        raise Exception("e (%d) and phi_n (%d) are not relatively prime" % (e,
                phi_n))
    if (i < 0):
        raise Exception("New extended_gcd shouldn't return negative values")
    if not (e * i) % phi_n == 1:
        raise Exception("e (%d) and i (%d) are not mult. inv. modulo phi_n "
                "(%d)" % (e, i, phi_n))
...
def encode64chops(chops):
    ...
    for string in chips:
        encoded = encoded + string + b','    #delimit chops with comma
...
def decode64chops(string):
    ...
    chips = string.split(b',')               #split chops at commas
