...
from flask import Flask, Blueprint, render_template, request, url_for, session, redirect
...
def login_page():
    ...
    return render_template('google_signin/login_page.html', client_id=_GOOGLE_CLIENT_ID, success_redirect=request.args.get('next'))
...
def authorize():
    ...
    if _has_right(user):
        ...
        session['login'] = True
        return render_template('google_signin/login_success.html', success_redirect=request.form.get('success_redirect'))
    else:
        return 'login fail'
