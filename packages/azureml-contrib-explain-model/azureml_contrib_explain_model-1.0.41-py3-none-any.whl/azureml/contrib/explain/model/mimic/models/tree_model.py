...
from azureml.explain.model._internal.constants import ShapValuesOutput, ExplainableModelType
from azureml.explain.model.mimic.models.explainable_model import BaseExplainableModel, _get_initializer_args
from azureml.explain.model.mimic.models.tree_model_utils import _explain_local_tree_surrogate, \
    _expected_values_tree_surrogate
from sklearn.tree import DecisionTreeRegressor, DecisionTreeClassifier
from azureml.explain.model.common.explanation_utils import _get_dense_examples
...
class DecisionTreeExplainableModel(BaseExplainableModel):
    ...
    fit.__doc__ = (fit.__doc__ +
                   '\nIf multiclass=True, uses the parameters for DecisionTreeClassifier:\n' +
                   DecisionTreeClassifier.fit.__doc__ +
                   '\nOtherwise, if multiclass=False, uses the parameters for DecisionTreeRegressor:\n' +
                   DecisionTreeRegressor.fit.__doc__)
    ...
    predict.__doc__ = (predict.__doc__ +
                       '\nIf multiclass=True, uses the parameters for DecisionTreeClassifier:\n' +
                       DecisionTreeClassifier.predict.__doc__ +
                       '\nOtherwise, if multiclass=False, uses the parameters for DecisionTreeRegressor:\n' +
                       DecisionTreeRegressor.predict.__doc__)
    ...
    predict_proba.__doc__ = (predict_proba.__doc__ +
                             '\nIf multiclass=True, uses the parameters for DecisionTreeClassifier:\n' +
                             DecisionTreeClassifier.predict_proba.__doc__ +
                             '\nOtherwise predict_proba is not supported for regression or binary classification.\n')

    def explain_global(self, **kwargs):
        """Call feature_importances_ to get the global feature importances from the tree surrogate model.
