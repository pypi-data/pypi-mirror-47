...
def _build_update_graph(spec, optically_thin=None, equilibrium=None, path_length=None,
    ...
    if slit:
        if __debug__:
            printdbg(
                '... build_graph: slit given > convoluted keys can be recomputed')
...
#
                    ...
                    min_path = score_non_recomputed_per_path[min(
                        score_non_recomputed_per_path)]
                    ...
                    for k in min_path:
                        ...
                        if __debug__:
                            printdbg(
                                '... get_recompute: Added new quantity to recompute list:', k)
...
def update(spec, quantity='all', optically_thin='default', assume_equilibrium='default',
    ...
    if optically_thin not in [True, False, 'default']:
        raise ValueError(
            "optically_thin must be one of True, False, 'default'")
    ...
    if old_self_absorption != (not optically_thin):
        ...
        if verbose:
            print(('self absorption set to:',
                   spec.conditions['self_absorption']))
...
def rescale_emisscoeff(spec, rescaled, initial, old_mole_fraction, new_mole_fraction,
    ...
    if 'emisscoeff' in initial:
        ...
        _, I = spec.get('radiance_noslit', wunit=waveunit,
                        Iunit=units['radiance_noslit'])
    ...
    if emisscoeff_init is not None:
        ...
        emisscoeff = emisscoeff_init * new_mole_fraction / old_mole_fraction  # rescale x
        rescaled['emisscoeff'] = emisscoeff
    if unit is not None:
...
def rescale_mole_fraction(spec, new_mole_fraction, old_mole_fraction=None,
    ...
    if np.abs(new_mole_fraction - old_mole_fraction) > 0.3:
        warn('Large rescaling from {0} to {1}. '.format(old_mole_fraction, new_mole_fraction)+\
