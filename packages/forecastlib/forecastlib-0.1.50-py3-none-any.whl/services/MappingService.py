...
class MappingService(object):
    def __init__(self, mapping_json, history: pd.DataFrame):
    ...
    def merge_history(self, df: pd.DataFrame):
        if 'CHAN_CD' in self.history.columns:
            ...
            return df.merge(self.history[["PROD_ID", "CHAN_CD", "OFFER_PERC_M", "AVG_UPA"]], how="left", on=["PROD_ID", "CHAN_CD", "OFFER_PERC_M"])
            #return pd.merge(df.astype(str), self.history[["PROD_ID", "CHAN_CD", "OFFER_PERC_M", "UPA_ACTUAL"]].astype(str), how="left", on=["PROD_ID", "CHAN_CD", "OFFER_PERC_M"])

        return pd.merge(df, self.history["PROD_ID", "OFFER_PERC_M", "UPA_ACTUAL"], how="left", on=["PROD_ID", "OFFER_PERC_M"])
    ...
    def add_full_history(df: pd.DataFrame):
        ...
        cumsum = nolaunch.groupby(by=['PROD_ID', 'CHAN_CD', 'OFFER_PERC'])['UPA_ACTUAL'].cumsum()
        cumcount = nolaunch.groupby(by=['PROD_ID', 'CHAN_CD', 'OFFER_PERC'])['UPA_ACTUAL'].cumcount()
