...
class ForecastService(object):
    def __init__(
            ...
            history_model_name: str = "history",
            history_model_version: int = None
    ):
        loaded_models = {}
        ...
        history_path = model_loader.download(history_model_name, history_model_version)
        history = self.load_history(history_path)

        self.map_service = MappingService(map, history)
    ...
    def validate(self, forecasted_data: pd.DataFrame):
        accuracy = Metrics.model_accuracy(forecasted_data["UPA_ACTUAL"], forecasted_data["PREDICTED"])
        return accuracy
