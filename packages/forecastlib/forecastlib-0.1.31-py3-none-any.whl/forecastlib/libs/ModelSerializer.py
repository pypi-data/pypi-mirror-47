...
class ModelSerializer(object):
	...
	def save_keras_model(model, folder: str, name: str = "model"):
		model.save(folder + ModelSerializer.get_model_filename(name))
		architecture = model.to_json()
		with open(folder + ModelSerializer.get_model_architecture_filename(name), 'w') as f:
			f.write(architecture)
		model.save_weights(folder + ModelSerializer.get_model_weights_filename(name))

	@staticmethod
	def load_keras_model(folder: str, model_name="model"):
		model_arch_file = folder + "\\" + ModelSerializer.get_model_architecture_filename(model_name)
		model_weights_file = folder + "\\" + ModelSerializer.get_model_weights_filename(model_name)
	...
	def save_ml_model(model, folder: str, model_name: str = "model"):
		with open(folder + ModelSerializer.get_model_filename(model_name), 'wb') as outfile:
			pickle.dump(model, outfile)

	@staticmethod
	def load_ml_model(folder: str, model_name: str = "model"):
		with open(folder + "\\" + ModelSerializer.get_model_filename(model_name), "rb") as dump_file:
	...
	def save_scaler(scaler, folder: str, name: str = "model"):
		from sklearn.externals import joblib
		joblib.dump(scaler, folder + ModelSerializer.get_scaler_filename(name))

	@staticmethod
	def load_scaler(folder: str, model_name="model"):
		model_scaler_file = folder + "\\" + ModelSerializer.get_scaler_filename(model_name)
	...
	def load_features(folder: str, model_name="model"):
		feature_file = folder + "\\" + ModelSerializer.get_feature_filename(model_name)
