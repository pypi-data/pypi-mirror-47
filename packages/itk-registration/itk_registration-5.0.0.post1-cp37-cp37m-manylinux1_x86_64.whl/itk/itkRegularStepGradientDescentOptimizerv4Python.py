...
class itkRegularStepGradientDescentOptimizerv4D(itkGradientDescentOptimizerv4Python.itkGradientDescentOptimizerv4TemplateD):
    ...
    C++ includes: itkRegularStepGradientDescentOptimizerv4.h 
    """
    ...
    def SetMinimumStepLength(self, _arg: 'double const') -> "void":
        ...
        Minimum step
        length (learning rate) value for convergence checking. When the local
        minima is passed by taking a large step, the step length is adjusted
        (decreased) by the relaxation factor, so that smaller steps are taken
        towards the minimum point (convergence). When the step length value
        reaches a small value, it would be treated as converged.
