...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
                                 ...
                                 output_fn):
    """SQuAD Specific: Loads a data file into a list of `InputBatch`s."""
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if example_index < 20:
                ...
                tf.logging.info("token_to_orig_map: %s" % " ".join(
                    ["%d:%d" % (x, y) for (x, y) in six.iteritems(
                        token_to_orig_map)]))
                tf.logging.info("token_is_max_context: %s" % " ".join(
                    ["%d:%s" % (x, y) for (x, y) in six.iteritems(
                        token_is_max_context)]))
...
def _check_is_max_context(doc_spans, cur_span_index, position):
    ...
    for (span_index, doc_span) in enumerate(doc_spans):
        ...
        min_score = min(num_left_context, num_right_context)
        score = min_score + 0.01 * doc_span.length
...
def write_squad_predictions(all_examples, all_features, all_results,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        null_start_logit = 0  # the start logit at the slice with min null scre
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            for start_index in start_indexes:
                for end_index in end_indexes:
                    # We could hypothetically create invalid predictions, e.g.,
                    # predict
                    # that the start of the span is in the question. We throw
                    # out all invalid predictions.
        ...
        for pred in prelim_predictions:
            ...
            if pred.start_index > 0:  # this is a non-null prediction
                # Need to rework this section for sure
...
def get_final_text(pred_text, orig_text, do_lower_case,
    ...
    #
    # Therefore, we have to apply a semi-complicated alignment heruistic
    # between `pred_text` and `orig_text` to get a character-to-charcter
...
class SQuADModel(BaseModel):
    ...
    def train(self, train_samples):
        ...
        train_writer = FeatureWriter(
            filename=os.path.join(self.FLAGS.bert_output_dir, "train.tf_record"),
            is_training=True)
    ...
    def predict(self, eval_samples):
        ...
        eval_writer = FeatureWriter(
            filename=os.path.join(self.FLAGS.bert_output_dir, "eval.tf_record"),
            is_training=False)
        eval_features = []
