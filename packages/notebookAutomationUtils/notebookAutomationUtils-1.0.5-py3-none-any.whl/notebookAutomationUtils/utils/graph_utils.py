...
class GraphUtils:
    ...
    def visualize_using_boxplot(self, obj: object, numcols: int, release_criteria: int, ylimit: int):
      try:
        v = [str(self.this_release)] # `v` is a sorted (older -> newer) list of app Versions.
    ...
    def visualize_using_voilinplot(self, obj: object, numcols: int ,release_criteria: int):
      try:
        ...
        if (numcols == 1): 
          ...
          fig, ax = plt.subplots(nrows=1, ncols=numcols, figsize=(20,12), sharey=True)
          sns.violinplot(x="appVersion", y="EPT", hue="appVersion", data=obj, order=v, palette=p)
          ax.plot(np.linspace(-20,120,1000), [release_criteria]*1000, 'r')  # Draw a horizontal line at y=300 for the P95 goal 
        else:
          fig, [ax1, ax2] = plt.subplots(nrows=1, ncols=numcols, figsize=(24,12), sharey=True)

          sns.violinplot(ax=ax1, y="EPT", x="entitytype", data=obj[obj['appVersion']==self.last_release], order=self.objects_to_colors.keys(), palette=objects_to_colors.values())
          sns.violinplot(ax=ax2, y="EPT", x="entitytype", data=obj[obj['appVersion']==self.this_release], order=self.objects_to_colors.keys(), palette=objects_to_colors.values())
          plt.ylim(0,10000)
          ax1.set_title(self.last_release + ' - Wifi')
          ax2.set_title(self.this_release + ' - Wifi')
          ax1.plot(np.linspace(-20,120,1000), [release_criteria]*1000, 'r')  # Draw a horizontal line at y=300 for the P95 goal
          ax2.plot(np.linspace(-20,120,1000), [release_criteria]*1000, 'r')  # Draw a horizontal line at y=300 for the P95 goal
        sns.despine(offset=10, trim=True)
      except Exception as e:
        print("visualize_using_voilinplot():: Error ploting data: ", e)
