...
def calculate_sklearn_one_hot_encoder_output_shapes(operator):
    """
    Scikit-learn's one-hot encoder maps (some selected) categorical
    values into some binary sub-vectors. For each sub-vector, all
    coordinates but one are zeros. For example, if ['a', 'b', 1.] and
    ['c', 'c', 2.] are used to train a scikit-learn one-hot encoder and
    only the first two coordinates are encoded. Feeding ['a', 'b', 1.]
    into the trained encoder may look like
    >>> [1., 0.] + [1., 0.] + [1.]
    There are three sub-vectors. The first/second/third one is a binary
    representation of ['a']/['b']/[1.]. Note that we use Python list
    for the sake of simplicity. The actual output type is either numpy
    or scipy array. Similarly, the outcome of ['c', 'c', 2.] may be
    >>> [0., 1.] + [0., 1.] + [2.]

    Calculating non-encoded coordinates' length is simple because
    scikit-learn just append them after encoded features. For
    categorical values, we need to figure out which coordinates are
    being encoded and how many categorical values are allowed in each
    coordinate. In our example, there are two/two categorical values in
    first/second coordinate so the total length of encoded vector is:
    # two categorical values + two categorical values
    # + one non-encoded feature
    >>> 2 + 2 + 1
    ...
    # encoded_slot_sizes[i] is the number of output coordinates
    # associated with the ith categorical feature.
    encoded_slot_sizes = []
    if hasattr(op, 'categories_'):
        ...
        if op.categorical_features == 'all':
            ...
            # In this case, there are two formats to specify which
            # features are encoded.
            if all(isinstance(i, (bool, np.bool_))
                   for i in op.categorical_features):
                # op.categorical_features is a binary vector. Its ith
                # element is 0/1 if the ith coordinate is not encoded/
                # encoded.
                categorical_feature_indices = [
                    i for i, active in enumerate(op.categorical_features)
                    if active]
            else:
                # op.categorical_features is a vector containing all
        ...
        if op.n_values == 'auto':
            ...
            for i in range(len(op.feature_indices_) - 1):
                ...
                # feature indexed by index_tail is not included in
                # this category
            ...
            # Each categorical feature will be mapped to a fixed
            # length one-hot sub-vector
            for i in range(len(op.feature_indices_) - 1):
