...
class ImageAnnotationReader(ImageDatasetReader):
    ...
    def _read(self, file_path: str) -> Iterable[Instance]:
        ...
        for image_file in (file_path / self._image_dir).iterdir():
            ...
            if not annotation_file.exists():
                ...
                label_classes, label_boxes = self._parse_annotation(annotation)
            sample = img.convert('RGB')
            yield self.text_to_instance(np.array(sample), label_boxes, label_classes)

    def _parse_annotation(self, annotation) -> Tuple[List[str], List[List[float]]]:
        boxes = []
    ...
    def text_to_instance(self,
        ...
        if label_box is not None:
            ...
            fields['box_classes'] = ListField([LabelField(cls, skip_indexing=False)
