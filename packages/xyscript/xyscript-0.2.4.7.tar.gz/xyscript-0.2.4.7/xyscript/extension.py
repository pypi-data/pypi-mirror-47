...
class Extension:
    def get_current_version_des(self,fname=os.path.join(os.getcwd(),"xyscript", 'config.py')):
        """ 当前版本描述 """
    ...
    def get_location_version(self,fname=os.path.join(os.getcwd(),"xyscript", 'config.py')):
        """ 当前本地版本号 """
        with open(fname) as f:
            for line in f:
