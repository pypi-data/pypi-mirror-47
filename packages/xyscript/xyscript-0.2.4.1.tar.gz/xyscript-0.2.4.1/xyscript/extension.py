...
class Extension:
    ...
    def check_version(self):
        ...
        if remote_version != local_version :
            Warning("the latest version of xyscript is: " + remote_version + ",but you are still in version: " + local_version)
            Warning("this will cause many features to become unusable")
            Warning("you can run 'sudo pip3 install xyscript -U' to update xyscript")
        # print(Exception()._search_lastest_version("pip3 search xyscript"))


if __name__ == "__main__":
    print(Extension()._search_lastest_version("pip3 search xyscript"))