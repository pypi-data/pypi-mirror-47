...
class IOSTool:
    ...
    def add_content_to_file(self,file):
        try:
            file = IOSTool().get_file_with_name(file,os.getcwd())
            if file == None:
                faillog("cannot find a file named " + file)
            else:
                end_count = IOSTool().get_file_end_count(file,"@end")
                IOSTool().add_code(file,"@end",end_count)
                successlog("add code to " + file + " success")
        except BaseException as error:
            faillog("add code to " + file + "failed:" + format(error))
