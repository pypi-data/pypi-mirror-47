...
def package(*parameters):
    ...
    自动打包
    """
    global PROJECT_ADDRESS, PROJECT_BRANCH, TEST_PLATFORM, NET_ENV, PROJECT_VERSION, PROJECT_BUILD, NOTIFICATION_EMAILS,TAG_NAME
    #全局变量初始化
    gv._init()
    ...
    Package().auto_package(project_address=PROJECT_ADDRESS, branch_name=PROJECT_BRANCH, platform=TEST_PLATFORM,net_env=NET_ENV, project_version=PROJECT_VERSION,project_build=PROJECT_BUILD,tag_name=TAG_NAME)
...
def main(prog=None,args=None):
    ...
    args = sys.argv[2:]
    shortargs = 'a:p:b:e:n:d:m:gk:c:t:' #短选项模式
    longargs = ['address=', 'platform=', 'branch=', 'environment=', 'version=', 'build=', 'email=', 'log', 'package=','code=','tag='] #长选项模式
    try:
        try:
