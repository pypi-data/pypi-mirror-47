...
def create(engine, role=None):
    ...
    :type role: :py:class:`v1_3_0.web.vo.Role`
    :rtype: ``TEXT_TYPE``
    """
...
def get(engine, ref):
    ...
    :param ref: Reference to a
        :py:class:`delphixpy.v1_3_0.web.objects.Role.Role` object
    :type ref: ``str``
    :rtype: :py:class:`v1_3_0.web.vo.Role`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
...
def get_all(engine):
    ...
    :param engine: The Delphix Engine
    :type engine: :py:class:`delphixpy.v1_3_0.delphix_engine.DelphixEngine`
    :rtype: ``list`` of :py:class:`v1_3_0.web.vo.Role`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/role"
    response = engine.get(url)
...
def dvc_user_role(engine):
    """
    Returns the DVC user role.
    ...
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/role/dvcUserRole"
    response = engine.get(url)
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'Role'], returns_list=False, raw_result=raw_result)
