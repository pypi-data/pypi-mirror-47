...
def create(engine, source_config=None):
    ...
    :type source_config: :py:class:`v1_3_1.web.vo.SourceConfig`
    :rtype: ``TEXT_TYPE``
    """
...
def get(engine, ref):
    ...
    :param ref: Reference to a
        :py:class:`delphixpy.v1_3_1.web.objects.SourceConfig.SourceConfig`
        object
    :type ref: ``str``
    :rtype: :py:class:`v1_3_1.web.vo.SourceConfig`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
...
def get_all(engine, environment=None, repository=None):
    ...
    :rtype: ``list`` of :py:class:`v1_3_1.web.vo.SourceConfig`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/sourceconfig"
...
def validate_credentials(engine, ref, source_config_connectivity):
    ...
    :type source_config_connectivity:
        :py:class:`v1_3_1.web.vo.SourceConfigConnectivity`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
...
def default_type(engine, ref):
    ...
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/sourceconfig/%s/defaultType" % ref
    response = engine.get(url)
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=None, returns_list=None, raw_result=raw_result)
