...
class itkImageMomentsCalculatorIF2(ITKCommonBasePython.itkObject):
    ...
    def GetPrincipalAxes(self) -> "itkMatrixD22":
        ...
        corresponds to one principal moment; for example, the principal axis
        corresponding to the smallest principal moment is the vector m[0],
        where m is the value returned by this method. The matrix of principal
        axes is guaranteed to be a proper rotation; that is, to have
        determinant +1 and to preserve parity. (Unless you have foolishly made
        one or more of the spacing values negative; in that case, you get to
        figure out the consequences.) The moments are computed in physical
        coordinates. 
        """
