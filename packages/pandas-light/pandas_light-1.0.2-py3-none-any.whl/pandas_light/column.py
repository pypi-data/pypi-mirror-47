...
def type_column(df,column,type):
    ...
    Args:
        column (str or [str]): Name(s) of the column(s).
        type (type): Type to change to.
    """
    ndf = df.copy()
    column = str_listify(column)
    for col in column:
        ndf[col] = ndf[col].astype(type)
    return ndf


@pf.register_dataframe_method
def derive_column(df,name,func,*args):
    """Derive a new column from existing columns in the DataFrame.
    
    Args:
        name (str): Name of the new column.
        func (func): The func to vectorize and apply to existing columns.
    """
    return df.add_column(name,df.applyvec(func,*args))
