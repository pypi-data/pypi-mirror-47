...
from ....exceptions import TimeLimitExceededOnFileTransfer
from .. import FileUtils as FileUtilsLinuxBase

logger = logging.getLogger(__name__)

# datetime is in the form : Mmm dd hh:mm
DATETIME_PAT_RECENT = "%b %d %H:%M %Y"
STAT_DIR_FORMAT_SCAN_PATTERN_RECENT = r"\s*(?P<mode>\S+)\s+(?P<num_links>\d+)\s+(?P<owner>\d+)\s+(?P<group>\d+)\s+(?P<size>\d+)\s+(?P<month>[A-Za-z]{3})\s+(?P<day>\d+)\s+(?P<hour>\d+):(?P<minute>\d+)\s*(?P<filename>\S+)\s*$"


# datetime is in the form : Mmm dd yyyy
DATETIME_PAT_NOT_RECENT = "%b %d %Y"
STAT_DIR_FORMAT_SCAN_PATTERN_NOT_RECENT = r"\s*(?P<mode>\S+)\s+(?P<num_links>\d+)\s+(?P<owner>\d+)\s+(?P<group>\d+)\s+(?P<size>\d+)\s+(?P<month>[A-Za-z]{3})\s+(?P<day>\d+)\s+(?P<year>\d{4})\s*(?P<filename>\S+)\s*$"
...
class FileUtils(FileUtilsLinuxBase):
    ...
    def dir(self, target,
        ...
        return [urlunsplit(
            ...
            "/"+file if file[0] else file, split_url[3], split_url[4], )) \
            for file in files]
