...
def data_analyze(analysis_type, data_type, data):
    """
    数据解析主入口
    type: file/data
...
def data_statistics(analysis_data):
    """对分析出的数据进行统计"""
    ...
    for one_data in analysis_data:
        ...
        if key in statistics_data_dict.keys():
            ...
            statistics_data_dict[key].caselist.append(one_data.caseName)
        else:
            key = one_data.type + one_data.module + one_data.desc
            ...
            statistics_data.caselist.append(one_data.caseName)
...
if __name__ == "__main__":
    result1 = data_analyze('test', 'file', '/Users/yangcan/IdeaProjects/Tesla/originaldata/original_data_20190613120047')
    result2 = data_statistics(result1)
