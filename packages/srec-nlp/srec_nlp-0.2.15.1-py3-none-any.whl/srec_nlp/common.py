...
class Score:
    ...
    def __getitem__(self, slice):
        return (self.positive, self.negative, self.classification)[slice]
...
class Query:
    ...
    def __getitem__(self, slice):
        return (self.content, self.score)[slice]
...
def _parallel(func):
    ...
    if not inspect.iscoroutinefunction(func):
        ...
        @functools.wraps(func)
        async def internal(self, query):
            if isinstance(query, list):
                return await listd(self, query)
