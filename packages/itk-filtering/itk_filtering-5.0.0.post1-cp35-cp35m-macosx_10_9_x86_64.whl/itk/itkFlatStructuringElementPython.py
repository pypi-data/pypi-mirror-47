...
class itkFlatStructuringElement2(itkNeighborhoodPython.itkNeighborhoodB2):
    ...
    https://www.itk.org/Wiki/ITK/Examples/Morphology/FlatStructuringElemen
    t Erode a binary image using a flat (box) structuring element

    Wiki Examples:  All Examples

    https://www.itk.org/Wiki/ITK/Examples/Morphology/FlatStructuringElemen
    tRadiusIsParametric Generate structuring elements with accurate area
...
class itkMovingHistogramImageFilterBaseIF2IF2SE2(itkKernelImageFilterIF2IF2SE2):
    ...
    symetric, it is safe to consider that the histogram will never be
    empty.

    One histogram is created for each thread by the method NewHistogram().
    The NewHistogram() method can be overiden to pass some parameters to
    the histogram.
