...
def _constrain_and_analyze_model(model, coefficient_dict, fraction, sampling_depth, bound_model):
    ...
    with model as constrained_model:
        ...
        if sampling_depth == 'minimization':
            ...
            if bound_model == False:
                ...
                constrained_model = _apply_bounds(constrained_model, fraction)
...
def _apply_bounds(constrained_model, fraction):
...
def contextualize(model, transcription, defined = False, samples = 10000, percentiles = [50.0, 62.5, 75.0, 87.5], 
    ...
    riptide_model = _initialize_model(model)
    ...
    rm_rxns = _constrain_and_analyze_model(riptide_model, coefficient_dict, fraction, 'minimization', bound)
    ...
    if samples != 0:
        ...
        if bound == False:
            flux_object, flux_type = _constrain_and_analyze_model(riptide_model, coefficient_dict, fraction, samples, bound)
        else:
            flux_object, riptide_model, flux_type = _constrain_and_analyze_model(riptide_model, coefficient_dict, fraction, samples, bound)
