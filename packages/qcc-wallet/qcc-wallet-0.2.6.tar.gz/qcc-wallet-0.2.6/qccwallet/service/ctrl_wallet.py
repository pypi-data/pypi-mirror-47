...
async def transfer(contract_id, sender_address, data):
    """
    ...
    while True:
        ...
        temp = tx_info(cid, tx_hash)
        if temp['blockNumber'] is not None:
            logging.info('tx[%s] checked: %s' % (tx_hash, temp['blockNumber']))
            unlock(sender_address)
            return temp
        else:
            now = time.millisecond()
            if now - start > 1800000:
                raise ErrException(ERROR.E50000, extra='tx can not be mined')
            count += 1
            if count % 2 == 0:
                logging.info('tx[%s] waiting times: %s' % (tx_hash, count))


async def tx_cast(contract_id, sender_address, data):
    """
    通用交易广播
    """
    password = secret.verify_msg(data)
...
async def lock(address):
    key = _address_key(address)
    start = time.millisecond()
    while True:
        existed = rdpool.rds.get(key)
        if existed is None:
            rdpool.rds.set(key, time.millisecond())
            break
        else:
            now = time.millisecond()
            if now - start > 1800000:
                raise ErrException(ERROR.E50000, extra='wallet be locked too long')
...
def _address_key(address):
    return '{}/cdp/lock/chain/{}/address/{}'.format(config.PLATFORM, options['chain'], address)
