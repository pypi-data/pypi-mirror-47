...
from recordclass.datatype import asdict, enable_gc, collection_protocol, add_dict, add_weakref, iterable, hashable
...
#         self.assertEqual(sys.getsizeof(a), 32)
    ...
    def test_datatype4(self):
        @collection_protocol(sequence=True)
        class A(dataobject):
            x:int
            y:int
    ...
    def test_datatype5(self):
        @collection_protocol(mapping=True)
        class A(dataobject):
            x:int
            y:int
    ...
    def test_datatype6(self):
        @collection_protocol(sequence=True, mapping=True)
        class A(dataobject):
            x:int
            y:int
