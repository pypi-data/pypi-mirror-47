...
from .engineapi.typedefinitions import FileEncoding
...
def read_csv(path: FilePath,
    ...
    :param skip_mode: The mode in which rows are skipped. The options are: SkipMode.NONE, SkipMode.UNGROUPED, SkipMode.GROUPED.
        SkipMode.NONE (Dafault) Do not skip lines. Note that, if `skip_rows` is porvided this is ignored and `SkipMode.UNGROUPED` is used instead.
...
def read_postgresql(data_source: DatabaseSource, query: str) -> Dataflow:
    """
    ...
    try:
        ...
        pass
    df = Dataflow(get_engine_api())
    df = df.read_postgresql(data_source, query)
