...
Attachment::Attachment(const Mesh &mesh, const Skeleton &skeleton, const vector<Vector3> &match, 
    const VisibilityTester *tester,	double initialHeatWeight)
{
    ...
    weights.resize(nv);
    int bones = (int)skeleton.fGraph().verts.size() - 1;
    ...
    for(i = 0; i < nv; ++i) {
        ...
        //get laplacian
        double sum = 0.;
        int edgesSize = (int)edges[i].size();
        for(j = 0; j < edgesSize; ++j) {
            int nj = (j + 1) % edgesSize;
            int pj = (j + edgesSize - 1) % edgesSize;
...
{
    Mesh out = mesh;
    int i, nv = (int)mesh.vertices.size();
