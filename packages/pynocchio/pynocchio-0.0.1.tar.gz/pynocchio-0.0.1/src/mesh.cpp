...
void Mesh::fixDupFaces()
{
    ...
    for(i = 0; i < (int)edges.size(); i += 3) {
        ...
        if(faces.count(current)) {
            ...
            faces[current] = -1;
            int newOth = (int)edges.size() - 6;
            int newCur = (int)edges.size() - 3;
...
struct StlVtxHash {
	size_t operator()(const StlVtx &p) const {
...
void Mesh::readStl(istream &strm)
{
    ...
    unordered_map<StlVtx, int, StlVtxHash> vertexIdx;
