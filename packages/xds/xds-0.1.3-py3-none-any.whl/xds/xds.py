...
class lab_data:
    ...
    def get_trials_idx(self, my_type = 'R'):
        """ my_type: 'R', 'A', 'F' """
        """ 'R' for reward """
        """ 'A' for aborted """
        """ 'F' for failed """
        ...
        if len(temp) != 0:
           for n in temp:
               if np.isnan(self.trial_end_time[n]) == False:
                   ind = np.where((self.time_frame > self.trial_start_time[n]) & (self.time_frame < self.trial_end_time[n]))[0]
               trials_idx.append(ind)
        return trials_idx
    
    def get_trials_data_spike_counts(self, my_type = 'R'):
        trial_spike_counts = []
        ind = self.get_trials_idx(my_type)
        for n in ind:
            a = self.spike_counts[n, :]
            trial_spike_counts.append(a)
        return trial_spike_counts
    
    def get_trials_data_time_frame(self, my_type = 'R'):
        trial_time_frame = []
        ind = self.get_trials_idx(my_type)
        for n in ind:
            a = self.time_frame[n, :]
    ...
    def get_trials_data_EMG(self, my_type = 'R'):
        if self.has_EMG == 0:
    ...
    def get_trials_data_force(self, my_type = 'R'):
        if self.has_force == 0:
    ...
    def get_trials_data_kin(self, my_type = 'R'):
        if self.has_kin == 0:
    ...
    def get_trials_summary(self, my_type = 'R'):
        trials_summary = dict()
