...
import operator
import aiotoolz
from aiotoolz.functoolz import (curry, is_valid_args, is_partial_args, is_arity,
                             num_required_args, has_varargs, has_keywords)
from aiotoolz._signatures import builtins
import aiotoolz._signatures as _sigs
from aiotoolz.compatibility import PY3
...
def test_introspect_curry_valid_py3(check_valid=is_valid_args, incomplete=False):
    ...
    f = aiotoolz.curry(make_func('x, y, z=0'))
    assert check_valid(f)
    assert check_valid(f, 1)
    assert check_valid(f, 1, 2)
    ...
    f = aiotoolz.curry(make_func('x, y, z=0'), 1, x=1)
    assert check_valid(f) is False
    assert check_valid(f, z=3) is False

    f = aiotoolz.curry(make_func('x, y, *args, z'))
    assert check_valid(f)
...
def test_introspect_curry_py3():
    ...
    f = aiotoolz.curry(make_func('*args, **kwargs'))
    assert num_required_args(f) == 0
...
def test_introspect_builtin_modules():
    mods = [builtins, functools, itertools, operator, aiotoolz,
            aiotoolz.functoolz, aiotoolz.itertoolz, aiotoolz.dicttoolz, aiotoolz.recipes]
