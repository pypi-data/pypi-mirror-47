...
def test_instanceproperty():
    p = aiotoolz.functoolz.InstanceProperty(bool)
...
def test_flip():
    flip = pickle.loads(pickle.dumps(aiotoolz.functoolz.flip))
    assert flip is aiotoolz.functoolz.flip
    g1 = flip(f)
...
def test_curried_exceptions():
    ...
    merge = pickle.loads(pickle.dumps(aiotoolz.curried.merge))
    assert merge is aiotoolz.curried.merge
...
def test_curried_bad_qualname():
    @aiotoolz.curry
    class Bad(object):
        __qualname__ = 'aiotoolz.functoolz.not.a.valid.path'
