...
def attach_ssh_based_compute_targets(experiment, source_directory, compute_target_object):
    ...
    """
    project_context = create_project_context(experiment.workspace._auth_object,
                                             experiment.workspace.subscription_id,
                                             experiment.workspace.resource_group,
                                             experiment.workspace.name,
                                             experiment.name,
                                             experiment.workspace._workspace_id)
...
def detach_compute_target(experiment, source_directory, compute_target_name):
    ...
    with open(full_file_path, 'r') as outfile:
        data = ruamel.yaml.load(outfile)

    for key, value in data.items():
        if key == 'password':
            if value.startswith(_SECRET_PREFIX):
                remove_credentials(project_context, value[len(_SECRET_PREFIX):])
            ...
            if file_extension == COMPUTECONTEXT_EXTENSION:
                ...
                if os.path.isfile(compute_file_path):
                    with open(compute_file_path, "r") as compute_config:
                        compute_config_dict = ruamel.yaml.load(compute_config)
                        try:
...
# execution service.
def _set_password_or_key_info(name, address, username, password, ssh_key_info, project_context, compute_skeleton):
    ...
    if not ssh_key_info:
        ...
        response_json = _get_workspace_key(project_context)
...
# service lookup key.
def _get_workspace_key(project_context):
    ...
    uri += "/execution/v1.0" + project_context.get_experiment_uri_path() + "/getorcreateworkspacesshkey"

    auth_header = project_context.get_auth().get_authentication_header()
    headers = {}
