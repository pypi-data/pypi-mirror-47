...
class ArtifactsClient(WorkspaceClient):
    ...
    def download_artifact(self, origin, container, path, output_file_path):
        ...
        for i in range(RETRY_LIMIT):
            try:
                ...
                if i >= RETRY_LIMIT:
                    raise AzureMLException("Download of file {} to location {} failed with error {}"
                                           .format(uri, output_file_path, error))
                self._logger.debug("Retrying because download file failed with error {}".format(error))
    ...
    def download_artifacts_from_prefix(self, origin, container, prefix=None, output_directory=None, output_paths=None,
        ...
        for i in range(RETRY_LIMIT):
            try:
                sas_urls = self.get_files_by_artifact_prefix_id(prefix_path)
                break
            except (RequestException, HTTPError) as error:
                if i >= RETRY_LIMIT:
                    raise AzureMLException("Could not get sas urls for artifacts, failed with error {}".format(error))
                self._logger.debug('Retrying get_files_by_artifact_prefix_id because it failed with error {}'
                                   .format(error))
                time.sleep(wait_time)
                wait_time = wait_time ** 2
            ...
            try:
                with TaskQueue(worker_pool=self._pool,
                    ...
                    def perform_download_file(url, output_file_path, session):
                        download_file(url, output_file_path, session=session)
