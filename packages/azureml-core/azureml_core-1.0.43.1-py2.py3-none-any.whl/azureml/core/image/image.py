...
class Image(ABC):
    ...
    .. remarks::
        ...
        An Image object is used to deploy a user's :class:`azureml.core.model.Model` as a Webservice. The Image object
        typically contains a Model, an execution script, and any dependencies needed for Model deployment.
        The Image class has multiple subclasses such as ContainerImage for Docker Images, as well as preview
        Images like FPGA.

        See the ContainerImage class for an example of a class that inherits from the Image class:
        :class:`azureml.core.image.ContainerImage`
    ...
    def create(workspace, name, models, image_config):
        ...
        for model in models:
            if type(model) is str:
                ...
                raise WebserviceException('Models must either be of type azureml.core.model.Model or a str '
...
class ImageConfig(ABC):
    ...
    def build_create_payload(self, workspace, name, model_ids):
        ...
        .. remarks::
            Check out the :class:`azureml.core.image.container.ContainerImageConfig` class for an example of a
