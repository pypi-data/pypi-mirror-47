...
Remote resources allow you to easily scale up or scale out your machine learning experimentation by taking advantage of
accelerated CPU and GPU processing capabilities.
"""
...
class AbstractComputeTarget(object):
    ...
    def deserialize_from_dict(compute_target_name, compute_target_dict):
        """De-serialize compute_target_dict and returns the corresponding compute target object.
...
class _BatchAITarget(AbstractComputeTarget):
    ...
    def _deserialize_from_dict(compute_target_name, compute_target_dict):
        ...
        if (_BatchAITarget._SUBSCRIPTION_ID_KEY in compute_target_dict and
                _BatchAITarget._RESOURCE_GROUP_NAME_KEY in compute_target_dict and
                _BatchAITarget._CLUSTER_NAME_KEY in compute_target_dict):
...
def prepare_compute_target(experiment, source_directory, run_config):
    ...
    :return: A run object
    :rtype: azureml.core.experiment_run.ExperimentRun
    """
