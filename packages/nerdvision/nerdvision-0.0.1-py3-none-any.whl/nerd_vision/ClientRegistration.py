...
class ClientRegistration(object):
    ...
    def load_interfaces(self):
        ifaces = []
        filtered = filter(lambda x: self.network_exclude_regex.match(x) is None, netifaces.interfaces())
        _next = next(filtered, None)
    ...
    def product_extra():
        return {
            "major_version": nerd_vision.__version_major__,
            "minor_version": nerd_vision.__version_minor__,
            "micro_version": nerd_vision.__version_micro__,
            "path": nerd_vision.__file__,
            "build": nerd_vision.__props__['__Git_Commit_Id__'],
            "name": nerd_vision.agent_name,
            "version": nerd_vision.__version__,
            "properties": nerd_vision.__props__
        }
    ...
    def os_extra():
        def get_btime(file):
            ...
            line = filter(lambda _line: _line.startswith("btime"), readlines)
            btime = next(line, None)
            if btime is None:
                return btime
