...
class SqlClient:
    ...
    def __init__(self, db_connection, humour=None, logging_level=logging.DEBUG):
    ...
    def _log_query(self, query, parameters):
        logger.log(self.logging_level, "#EXECUTING QUERY @{0}".format(datetime.datetime.now()))
        logger.log(self.logging_level, "#PARAMETERS {0}".format(parameters))
        if self.humour == 'bad':
            try:
                import requests
        ...
        for line in query.split("\n"):
            if i < 5:
                logger.log(self.logging_level, "{0:03} {1}".format(i, line))
            else:
                logger.debug("{0:03} {1}".format(i, line))
            i = i + 1
        logger.log(self.logging_level, "running...")
    ...
    def _archive_query(self, logged_query, parameters, file):
        logger.log(self.logging_level, 'logging query to file: {}'.format(file))
    ...
    def _report_rowcount(self, execution_time):
        if self.cursor.rowcount >= 0:
            logger.log(self.logging_level, "Completed in {0}s. {1} rows affected\n".format(
                execution_time.seconds,
                self.cursor.rowcount))
        else:
            logger.log(self.logging_level, "Completed in {0}s\n".format(execution_time.seconds))

    def _get_queries(self, query_file, replace):
        if query_file[-4:] == ".sql":
            if os.path.isfile(query_file):
                logger.log(self.logging_level, "Loading query from {0}".format(query_file))
                f = open(query_file, "r")
                text = f.read()
                f.close()
            else:
    ...
    def execute_query(self, query, parameters=None, replace=None, first_to_run=1, archive_query=False, dry_run=False):
        ...
        for command in self._get_queries(query, replace):
            if i >= first_to_run:
                logger.log(self.logging_level, "RUNNING STATEMENT {0}...".format(i))
