...
from . import context, GET, Response
from .compat import ustr, string_types
from .routing import RouteArg
from .util.http import parse_header
...
def subrequest(view, *args, **kwargs):
    ...
    if full or isinstance(view, string_types) and '/' in view:
        app = context.app
        if full:
...
def response_to_str(response):
    ...
    try:
        ...
        # string
        if isinstance(response.content, ustr):
            content = response.content

        # ...or a list of strings?
        elif (isinstance(response.content, list) and
                all(isinstance(i, ustr) for i in response.content)):
            content = ''.join(response.content)

        # Anything else: decode the content iterator
        else:
            content = b''.join(response.content_iterator)
            if not encoding:
...
def resolve_viewspec(viewspec, *args, **kwargs):
    ...
    args, kwargs = _get_args_for_route(route, request, args, kwargs)
    return view, args, kwargs


def _get_args_for_route(route, request, args, kwargs):
    """
    Return the args/kwargs required to pass to the view callable for ``route``.

    Mutates both args and kwargs
    """
    route_kwargs = {}
    route_args = list(chain(
        ((a(request) if isinstance(a, RouteArg) else a)
            for a in route.view_args),
        (args.pop(0) for a in route.pattern.segments
    ...
    return route_args, route_kwargs
