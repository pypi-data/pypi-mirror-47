...
def deploy(project: str, function: str, url: str = None, main_file: str = None, dependencies: List = None,
    ...
    url = url or 'https://app.15rock.com/gateway/compute'
    main_file = main_file or helper.default_file_path('main.py')

    util.fail_if_no_file(main_file)
    requirements_file = requirements_file or helper.default_file_path(REQUIREMENTS_FILE) or str(
        util.make_requirements_file(tmp_folder))
    ...
    try:
        deploy_result = deploy_to_compute(archive_file, headers, new_url, raw_data)
...
def poll_if_function_created(function_id, compute_url: str, headers: Dict):
    ...
    new_headers = {**headers, **{"Content-type": "application/json"}}
    for i in tries:
        response = util.get_json(status_url, raw_data=raw_data, headers=new_headers)
        if response:
