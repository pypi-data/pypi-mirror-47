...
def get_value(obj):
    ...
    try:
        ...
        # obj.value will be None. This is undocumented; we should use
        # value(obj) instead and catch the ValueError, but then there's
        # no way to suppress the warning messages that Pyomo displays.
        # Note: for variables this could use the lowest bound instead (usually 0)
        if

        # test explicitly for uninitialized values to avoid displaying
        # warnings when using obj.value()
        if obj.value is None:
            return None
    except AttributeError:  # unlikely
        pass
