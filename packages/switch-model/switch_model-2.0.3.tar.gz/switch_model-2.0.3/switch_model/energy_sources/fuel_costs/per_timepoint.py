...
or simple module. This is identical to the simple module, except that fuel
costs are specified per timepoint instead of per period. This is useful for
production cost runs with historical time-varying fuel prices, or for
applying seasonal variation to fuel prices in future years.
...
def define_components(mod):
    ...
    ZONE_FUEL_TIMEPOINTS is a set that describes fuel availability. Each
    element of the set is (load_zone, fuel, timepoint).

    fuel_cost[(z, f, p) in ZONE_FUEL_TIMEPOINTS] describes flat fuel costs
    for each supply of fuel. Costs can vary by load zone and timepoint.
    ...
    GenFuelUseRate to 0 for load zones and timepoints where the
    projects' fuel is unavailable.

    FuelCostsPerTP[t in TIMEPOINTS] is an expression that summarizes fuel
    expenditure for the objective function.
    ...
    mod.min_data_check('ZONE_FUEL_TIMEPOINTS', 'fuel_cost')
            ...
            for (g, t2, f) in m.GEN_TP_FUELS:
                if (m.gen_load_zone[g], f, m.tp_period[t2]) in m.ZONE_FUEL_PERIODS:
                    m.FuelCostsPerTP_dict[t2] += (
                        ...
                        * m.fuel_cost[m.gen_load_zone[g], f, t2])
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    fuel_cost_per_timepoint.tab
        load_zone, fuel, timepoint, fuel_cost

    """

    switch_data.load_aug(
        filename=os.path.join(inputs_dir, 'fuel_cost_per_timepoint.tab'),
        select=('load_zone', 'fuel', 'timepoint', 'fuel_cost'),
