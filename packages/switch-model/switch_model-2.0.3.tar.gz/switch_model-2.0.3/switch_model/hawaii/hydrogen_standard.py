...
def define_arguments(argparser):
    argparser.add_argument('--no-hydrogen', action='store_true', default=False,
        ...
        # note: this is equivalent to listing all hydrogen technologies in exclude_technology in get_scenario_data
    )

def define_components(m):

    m.HYDROGEN_PROJECTS = Set()
    m.electrolyzer = Param(m.HYDROGEN_PROJECTS)  # name of electrolyzer/daily storage project
    m.liquefier = Param(m.HYDROGEN_PROJECTS)     # name of liquefier/long-term tank project
    m.fuel_cell = Param(m.HYDROGEN_PROJECTS)     # name of fuel cell project
    m.hydrogen_tank = Param(m.HYDROGEN_PROJECTS) # name of long-term storage tank project

    # # TODO:
    # electrolyzer - storage with 0 energy; output is cancelled from system balance
    # liquefier - storage with 0 energy; output is cancelled from system balance
    # fuel cell - storage with 0 energy; input is cancelled from system balance
    # tank - storage; input and output are fixed to 0
    #
    # require:
    # conservation of H2 mass each year: total electrolyzer output = total fuel cell input
    # liquefication of excess H2 each day: total (electrolyzer output - fuel cell input - liquefication) <= 0 each day
    # adequate storage tank: tank energy >= total liquefication each period
    # minimum tank size: see code for liquid_hydrogen_tank_minimum_size_kg
    # no cheap reserves: electrolyzer >= 0.1 * fuel cell and tank >= 12 * fuel cell


    if m.options.no_hydrogen:
        raise NotImplementedError("Need to constrain construction of all hydrogen technologies to 0 in all periods.")
