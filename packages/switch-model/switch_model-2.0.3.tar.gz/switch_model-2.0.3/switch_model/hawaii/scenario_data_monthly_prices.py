...
def write_tables(**args):
    ...
    if per_timepoint_fuel_costs and not simple_fuel_costs:
        raise NotImplementedError(
            "The use_per_timepoint_fuel_costs currently can only be used "
    ...
    if simple_fuel_costs:
        ...
        if per_timepoint_fuel_costs:
            write_table('fuel_cost_per_timepoint.tab',
                """
...
def write_table(output_file, query, arguments):
    # aliases, if present, is a dictionary showing an extra tag to include
    # in file names and extra arguments to use when defining those files
    # (e.g., load_scen_id); note: this will only be used to update queries
    # that are written directly to tables, so it must be used carefully.
    write_one_table(output_file, query, arguments)
    write_alias_tables(output_file, query, arguments)


def write_one_table(output_file, query, arguments):
    output_file = make_file_path(output_file, arguments)
    cur = db_cursor()
...
def write_alias_tables(output_file, query, arguments):
    arguments = arguments.copy()  # avoid modifying original
    # consider each alias configuration given in arguments[aliases]
    for tag, args in arguments.get('aliases', dict()).items():
        altered = False
        for key, arg in args.items():
            if '%({})s'.format(key) in query:
                # new value for a parameter used in this query
                altered = True
                arguments[key] = arg
        if altered:
            ...
            if len(file_parts) > 1:
                ...
                file_parts.append(tag)
            tagged_file = '.'.join(file_parts)
            write_one_table(tagged_file, query, arguments)
