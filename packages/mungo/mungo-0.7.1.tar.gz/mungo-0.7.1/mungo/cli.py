...
def _install_prompt(to_install: list, installed: dict = None, ask_for_confirmation: bool = True,
    ...
    if installed is not None:
        ...
        version_changes.update({package: (data, data) for package, data in to_install.items() if package in reinstall.keys()})
    ...
    all_widths = [_calculate_widths(_prepare_packages(packages)) for packages in [to_install, upgrades, downgrades, reinstalls]]
    widths = dict()
    for w in all_widths:
        for k, v in w.items():
