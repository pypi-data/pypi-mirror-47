...
from mungo.common import PBAR_FMT
...
def get_repository_data(channels: list,
    ...
    with tqdm(desc="Retrieving repository data", total=len(urls), position=1, bar_format=PBAR_FMT) as t:
...
def merge_repository_data(repository_data_list: List['RepositoryData']) -> 'RepositoryData':
    ...
    for r in tqdm(repository_data_list[1:], desc="Merging repository data", position=2, bar_format=PBAR_FMT):
...
class RepositoryData:
    ...
    @classmethod
    def _from_packed(cls, name: str, file: str, method=pickle,
                     method_kwargs={}) -> 'RepositoryData':
