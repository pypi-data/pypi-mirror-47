...
from mungo.conda.base.context import Context

# this is a conda context object, we need this e.g. for subdir
context = Context(None)
context.restore_free_channel = True # TODO: remove

def replace_os_vars(s: str) -> str:
    '''replace $VARIABLE in a string by environment variables'''
    ret = s
    for term in re.findall("\$[A-Z_-]+", s):
        value = os.getenv(term[1:])
        if value is not None:
            ret = ret .replace(term, value)
        else:
            return None #v ariable not found
    return ret
...
BFMT = "{desc:<40}{percentage:3.0f}%| {bar} {n_fmt:>5}/{total_fmt:>5} [{elapsed}] {postfix}"
...
tqdm.tqdm.format_interval = format_interval
...
def plot_dependency_graph(packages: list, g: Graph, to_install: list, dep_nodes: list, installed: dict = None):
