...
def solve(g: Graph, channels: List[str]) -> Tuple[List[Vertex], List[Vertex]]:
    ...
    if solver_name is not None:
        ...
        if solver is None:
            print(f"ERROR: Requested solver {solver_name} is not available, falling back to {solvers[0][0]}.")
            solver_name, solver = solvers[0]
    else:
        solver_name, solver = solvers[0]

    with tqdm(desc=f"Solving ILP (using {solver_name})", bar_format=BFMT, position=6, total=1, disable=QUIET) as t:
