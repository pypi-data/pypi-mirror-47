...
from mungo.common import BFMT, QUIET, context
...
def get_repository_data(channels: list,
    ...
    for channel in channels:
        for arch in [context.subdir, 'noarch']:
            if channel != CONTEXT.DEFAULTS_CHANNEL_NAME: # "defaults"
                urls.append((f"{CONTEXT.DEFAULT_CHANNEL_ALIAS}/{channel}/{arch}", f"{channel}/{arch}"))
            else:
                # for c in ['main', 'free', 'pro', 'r']:
                for default_channel in context.default_channels:
                    channel_name = default_channel.channel_name.rsplit("/")[-1] # TODO: i'm not sure here
                    urls.append((f"{default_channel.base_url}/{arch}", f"{channel_name}/{arch}"))

    exit
    from joblib import Parallel, delayed

    with tqdm(desc="Retrieving repository data", total=len(urls), leave="output", bar_format=BFMT, disable=QUIET) as t:
...
def merge_repository_data(repository_data_list: List['RepositoryData']) -> 'RepositoryData':
    ...
    for r in tqdm(repository_data_list[1:], desc="Merging repository data", bar_format=BFMT, disable=QUIET):
...
def parse_repository_data(repository_name: str, json_data: dict) -> DefaultDict[str, dict]:
    ...
    for filename, p in packages.items():
        ...
        if repository_name.split("/")[0] in ["main", "pro", "r"]:
            p["channel"] = f"https://repo.continuum.io/pkgs/{repository_name}"
        else:
            p["channel"] = f"{CONTEXT.DEFAULT_CHANNEL_ALIAS}/{repository_name}"
...
class RepositoryData:
    ...
    def _from_packed(cls, name: str, file: str, method=msgpack,
        ...
        if not isinstance(repository_data, defaultdict):
            repository_data = defaultdict(dict, repository_data)
        return cls(name, repository_data)
...
def _store_repodata(repodata: 'RepositoryData', force_store=False, remote_mtime=None, method=msgpack,
    ...
    if os.path.exists(local_file):
        ...
        if force_store or not remote_mtime or remote_mtime > local_mtime:
            log.debug(f"Remote {name}/repodata is newer, updating {local_file}.")
            ...
            log.debug(f"{local_file} is up to date")
            pass
    else:
        log.debug(f"Saving {name}/repodata to {local_file}")
