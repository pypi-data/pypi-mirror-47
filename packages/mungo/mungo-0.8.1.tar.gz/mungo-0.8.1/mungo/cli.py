...
import mungo
from mungo.common import vparse, plot_dependency_graph, QUIET, TqdmStream, replace_os_vars
...
def _get_condarc_channels() -> List[str]:
    for path in map(replace_os_vars, CONTEXT.SEARCH_PATH):
        if path is None:
            continue

        condarc_path = os.path.expanduser(path)

        if os.path.exists(condarc_path):
            with open(condarc_path, 'rt') as reader:
...
def _install_prompt(to_install: list, installed: dict = None, ask_for_confirmation: bool = True,
    ...
    if porcelain:
        ...
        json.dump(dict(LINK=link), sys.stdout, indent=2)
        print()
    else:
        if len(upgrades) > 0:
