...
from mungo.conda.base import context as CONTEXT
from mungo.common import context
...
def reduce_dependency_graph(g: Graph, all_package_nodes: PackageNodeDict, channels: List[str]) -> Graph:
    ...
    for priority, channel in enumerate(reversed(channels)):
        if channel == CONTEXT.DEFAULTS_CHANNEL_NAME: # "defaults"
            for default_channel in context.default_channels: # main, r (msys2 under win)
                channel_name = default_channel.channel_name.rsplit("/")[-1]
                channel_order[channel_name] = priority
        else:
            channel_order[channel] = priority
        channel_order[""] = priority + 1  # highest priority for local installed packages
    nodes_to_remove = set()
    for name, versions in tqdm(all_package_nodes.items(), desc="Pruning dependency graph",
                               bar_format=BFMT, disable=QUIET):
        if name == "root":
            continue
        seen_keys = set()

        # TODO the following code should be some kind of function import, it is used twice
...
def create_dependency_node(g: Graph,
    ...
    for p_name, p_version in valid_packages(tuple(dependency), repodata):
        p_node = create_package_node(g, p_name, p_version, repodata, all_package_nodes=all_package_nodes,
                                     all_dependency_nodes=all_dependency_nodes, edges=edges, t=t)
...
def create_package_node(g: Graph,
    ...
    for dependency_name, dependency in grouped_dependencies.items():
        # create dependency node
        d_node = create_dependency_node(g, dependency, dependency_name, repodata, all_package_nodes=all_package_nodes,
                                        all_dependency_nodes=all_dependency_nodes, edges=edges, t=t)
...
def create_dependency_graph(channels: List[str],
                            ...
                            njobs: int = 8,
                            offline: bool = False,
                            force_download: bool = False) -> Graph:
    ...
    with tqdm(total=1, desc="Building dependency graph", bar_format=BFMT, disable=QUIET) as t:
        root = create_package_node(g, "root", ("0", "0"), repodata, all_package_nodes=all_package_nodes,
                                   override_dependencies=packages, edges=edges, t=t)
...
def solve(g: Graph, channels: List[str], jobs: int = 1) -> Tuple[List[Vertex], List[Vertex]]:
    root = g.vs[0]
    ...
    for name, versions in tqdm(foo, total=len(foo), desc="Formulating constraints", bar_format=BFMT, disable=QUIET):
    ...
    solver_name = os.getenv("MUNGO_SOLVER")
    solvers = get_available_solvers()
    log.info(f"Available solvers: {solvers}")

    if solver_name is not None:
        solver = dict(solvers).get(solver_name, None)
        if solver is None:
            log.error(f"Requested solver {solver_name} is not available, falling back to {solvers[0][0]}.")
            solver_name, solver = solvers[0]
        jobs = 1  # if a specific solver has been specified, only use that one
    else:
        solver_name, solver = solvers[0]

    if jobs > 1:
        parallel_solvers = solvers if len(solvers) < jobs else solvers[:jobs]
        problems = [prob.copy() for _ in range(len(parallel_solvers))]

        def _solve(prob, solver, index, prob_index, done):
            if done.value:  # do not start solving when a solution has already been found
                return -1
            _, solver = solver
            status = prob.solve(solver(msg=False))
            if status == LpStatusOptimal:
                if not done.value:
                    done.value = True
                    prob_index.value = index
            return status

        with tqdm(desc=f"Solving ILP (using {', '.join(n for n, _ in parallel_solvers)})",
                  bar_format=BFMT, total=1, disable=QUIET) as t:
            with ThreadPoolExecutor(max_workers=jobs) as executor:
                prob_index = Value('i', -1)
                done = Value('b', False)
                tasks: List[Future] = [executor.submit(_solve, p, s, i, prob_index, done)
                                       for i, (p, s) in enumerate(zip(problems, parallel_solvers))]
                for task in tasks:
                    task.add_done_callback(lambda f: f.cancel() if done.value else 0)
                for task in as_completed(tasks):
                    try:
                        status = task.result()
                        # if we found an optimal solution, terminate all remaining and active threads non(!) gracefully.
                        if status == LpStatusOptimal:
                            done.value = True
                            for task in tasks:
                                task.cancel()
                            # non graceful termination of this executor('s threadpool)
                        ...
                        log.error(e)
                        continue
            prob = problems[prob_index.value]
            t.set_description(f"Solving ILP (using {parallel_solvers[prob_index.value][0]})")
            t.update()
    else:
        with tqdm(desc=f"Solving ILP (using {solver_name})", bar_format=BFMT, total=1, disable=QUIET) as t:
            prob.solve(solver(msg=False))
            t.update()
    tqdm.write('\n')

    if prob.status != LpStatusOptimal:
        log.error(f"Solution is not optimal (status: {LpStatus[prob.status]}).\nAborting.")
        exit(1)
    ...
    python_update = False
    for n in install_nodes:
        if n["pname"] == "python" and not n["installed"]:
            python_update = True
            break

    if python_update:
        for n in install_nodes:
            if n["data"].get("subdir", None) == "noarch":
                for dep in n.neighbors(mode="out"):
                    if dep["pname"][0] == "python":  # TODO: fixme
                        n["reinstall"] = True

    install_nodes = [n for n in install_nodes if not n["installed"] or n["reinstall"]]
...
def get_available_solvers() -> List[Tuple[str, Callable]]:
    # TODO evaluate different solvers and order them accordingly
    solvers = {
        "CBC": [PULP_CBC_CMD, COINMP_DLL, COIN_CMD],
        "GUROBI": [GUROBI_CMD, GUROBI],
        "GLPK": [GLPK_CMD, PYGLPK],
        "CPLEX": [CPLEX_DLL, CPLEX_CMD],
        "XPRESS": [XPRESS],
        "YAPOSIB": [YAPOSIB]
    }
    available_solvers = []
    for solver_group_name, solver_group in solvers.items():
        available = [s for s in solver_group if s().available()]
        if len(available) > 0:
            s = available[0]
            available_solvers.append((s.__name__, s))
...
def print_missing_package(dependency: list):
    ...
    if dependency not in missing_dependencies:
        log.warning(f"No packages found for {dependency}")
