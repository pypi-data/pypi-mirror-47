...
from mungo.common import vparse, PBAR_FMT
...
def reduce_dag(g: Graph, all_package_nodes: PackageNodeDict, channels: List[str]) -> Graph:
    # reduce DAG
...
def distances_idag(g: Graph, root: Vertex):
...
def tdone(start_time):
    print(f"Done ({time() - start_time:0.2f}s)")
...
def create_idag(channels: List[str],
                packages: Set[str],
    ...
    with tqdm(total=1, desc="Building dependency graph", position=3, bar_format=PBAR_FMT) as t:
    ...
    g.add_edges(edges)
    g = reduce_dag(g, all_package_nodes, channels)
    return g


def solve(g: Graph, channels: List[str]) -> List[Vertex]:
    root = g.vs[0]
    distances = distances_idag(g, root)
    ...
    for name, versions in tqdm(foo, total=len(foo), desc="Formulating constraints", position=5, bar_format=PBAR_FMT):
    ...
    with tqdm(desc=f"Solving ILP (using {solver.__name__})", bar_format=PBAR_FMT, position=6, total=1) as t:
