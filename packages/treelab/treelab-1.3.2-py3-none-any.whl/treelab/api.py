...
class Table(_TreelabObject):
    ...
    def _update_data(table):
        ...
        try:
            ...
            pass
            print('Missing rows/columns/views', table.id)
        finally:
    ...
    def data_frame(self, view_id: str = ''):
        ...
        """
        if view_id:
            df = self.data.data_frames_dict.get(view_id)
        else:
            df = list(self.data.data_frames_dict.values())[0]
        return df
    ...
    def data_frames(self, view_ids: List[str] = None):
        ...
        """
        if view_ids:
            data_frames = [self.data.data_frames_dict.get(view_id) for view_id in view_ids]
        else:
            data_frames = list(self.data.data_frames_dict.values())
        return data_frames
    ...
    def get_row(self, row_id: str, view_id: str = ''):
        ...
        """
        try:
            row = self.data.rows.get(row_id) if not view_id else self.data.view_dict.get(view_id).get('rows').get(
                row_id)
    ...
    def row(self, row_id: str, view_id: str = ''):
        ...
        :return:
        """
        return self.get_row(row_id=row_id, view_id=view_id)
    ...
    def get_rows(self, row_ids: List[str] = None, view_id: str = ''):
        ...
        """
        if row_ids:
            rows = [self.get_row(row_id=row_id, view_id=view_id) for row_id in row_ids]
        else:
            rows = list(self.data.rows.values()) if not view_id else list(
                self.data.view_dict.get(view_id)['rows'].values())

        row_array = RowArray(parent_object=self, objects=rows, workspace=self.workspace)
        return row_array

    def get_cell(self, row, column, view=None):
        """
        ...
        :return:
        """
        return Cell(self, row, column, view)

    def get_cells(self, rows: List = None, columns: List = None, mode: str = 'all', view=None):
        """
        ...
        if mode == 'intersection':
            cells = [Cell(self, row, column, view) for row in rows for column in columns]
        elif mode == 'pair':
            if len(rows) != len(columns):
                raise ValueError("The size of rows has to equal to the size of columns when all_cells are set as False")
            cells = [Cell(self, row, column, view) for row, column in zip(rows, columns)]
        elif mode == 'all':
            if not view:
                cells = list(self.data.cells.values())
            else:
                data = self.data.view_dict.get(view.id)
                if data and data.get('cells'):
                    cells = list(data.get('cells').values())
                else:
                    cells = []
        else:
    ...
    def get_column_by_id(self, col_id: str, view_id: str = ''):
        ...
        try:
            if view_id:
                column = self.data.view_dict.get(view_id)['columns'].get(col_id)
            else:
                column = self.data.columns.get(col_id)
        except Exception as e:
            print('get_column_by_id', e)
            column = None
        return column
    ...
    def get_columns_by_name(self, field_name: str, view_id: str = ''):
        ...
        :return:
        """
        columns = self.get_columns(mode='all', view_id=view_id).select_by_name(field_name)
        return columns
    ...
    def get_columns(self, col_ids: List[str] = None, mode: str = 'all', view_id: str = ''):
        ...
        try:
            if mode == 'all':
                columns = list(self.data.columns.values()) if not view_id else list(
                    self.data.view_dict.get(view_id)['columns'].values())
            elif mode == 'id':
                if col_ids is None:
                    raise ValueError('col_ids should not be None when mode equals id')
                columns = [self.column(col_id=col_id, view_id=view_id) for col_id in col_ids]
            else:
...
class View(_TreelabObject):
    ...
    @property
    def data(self):
        view = self.table.data.view_dict.get(self.id)
        return view

    def data_frame(self):
        return self.table.data_frame(view_id=self.id)

    def _get_event_id(self, event: EventPayload):
        return event.viewId

    def get_columns(self, col_ids: List[str] = None, mode: str = 'all'):
        return self.table.get_columns(col_ids, mode, self.id)

    def get_column_by_id(self, col_id: str, view_id: str = ''):
        return self.table.get_column_by_id(col_id=col_id, view_id=view_id)

    def get_columns_by_name(self, field_name: str):
        return self.table.get_columns_by_name(field_name=field_name, view_id=self.id)

    def column(self, col_id: str):
        return self.table.column(col_id=col_id, view_id=self.id)

    def get_row(self, row_id: str):
        self.table.get_rows()
        return self.table.get_row(row_id=row_id, view_id=self.id)

    def row(self, row_id: str, ):
        return self.table.row(row_id=row_id, view_id=self.id)

    def get_rows(self, row_ids: List[str] = None):
        return self.table.get_rows(row_ids=row_ids, view_id=self.id)

    def get_cell(self, row, column):
        return self.table.get_cell(row=row, column=column, view=self)

    def get_cells(self, rows: List = None, columns: List = None, mode: str = 'all'):
        return self.table.get_cells(rows=rows, columns=columns, mode=mode, view=self)
...
class Cell(_TreelabObject):
    ...
    def _get_value(self, row, column):
        if self.table.data.cells:
            data = self.table.data.cells.get((row.id, column.id))
...
class _TableData:
    ...
    def _parse_views(self):
        ...
        for view_id, data in self.view_dict.items():
            if 'rows' in data and 'columns' in data:
                ...
                self._data_frame_dict[view_id] = df_data
    ...
    def _parse_dict(self, table_dict: Dict):
                ...
                for row_dict in view_dict['rows']:
                    ...
                    if 'cells' in row_dict:
                        for cell_dict in row_dict['cells']:
                            ...
                            column_type = FieldType(cell_dict.get('type')) if FieldType(cell_dict.get('type')) else None
                            if column_type is FieldType.TEXT:
                                value = cell_dict.get('text')
                            elif column_type is FieldType.RECORD_REFERENCE:
                                value = cell_dict.get('foreignRow')
                            elif column_type is FieldType.NUMBER:
                                value = cell_dict.get('number')
                            elif column_type is FieldType.SELECT:
                                value = eval(cell_dict.get('selectedItem')) if cell_dict.get('selectedItem') else None
                            elif column_type is FieldType.MULTI_SELECT:
                                value = eval(cell_dict['selectedItems']) if cell_dict.get('selectedItems') else None
                            elif column_type is FieldType.MULTI_ATTACHMENT:
                                value = cell_dict.get('attachments')
                            elif column_type is FieldType.DATETIME:
                                value = cell_dict.get('dateTime')
                            else:
                                raise ValueError('Not FieldType')
        ...
        first_view_id = list(self._views.keys())[0]
        self._rows = self._views_dict.get(first_view_id).get('rows')
        self._columns = self._views_dict.get(first_view_id).get('columns')
        self._cells = self._views_dict.get(first_view_id).get('cells')
    ...
    @property
    def data_frames_dict(self) -> Dict:
        return self._data_frame_dict
