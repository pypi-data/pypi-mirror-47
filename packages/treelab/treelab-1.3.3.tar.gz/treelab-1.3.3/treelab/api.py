...
class View(_TreelabObject):
    ...
    @property
    def data(self):
        matched_views = list(filter(lambda view: self.id == view.id, list(self.table.data.views.values())))
...
class Row(_TreelabObject):
    ...
    def __init__(self, row_id: str = None, table: Table = None, order: int = 1):
        super().__init__()
        self.order = order
...
class Cell(_TreelabObject):
    ...
    def __init__(self, table: Table, row: Row, column: Column, value=None
        ...
        self.row = row
        self.column = column
        self._value = value if value is not None else self._get_value(row, column)
        self._id = '{}:{}'.format(column.id, row.id)

    def _get_value(self, row, column):
        try:
            return self.table.data.cells.get((row.id, column.id)).value
        except:
            return None
        ...
        for view_dict in views:
            ...
            if 'rows' in view_dict:
                for row_dict in view_dict['rows']:
                    row_id = row_dict['id']
                    row = Row(row_id=row_id, table=self.table)
                    ...
                    if 'cells' in row_dict:
                        for cell_dict in row_dict['cells']:
                            ...
                            if column_type is FieldType.TEXT:
                                ...
                                value = cell_dict.get('number', 0.0)
...
def datetime_to_utc(format_time: str):
    """
    convert time to utc
    :param format_time: str
                    for example : 2019-06-05 10:19:02
    :return:
    """
    date_time = datetime.strptime(format_time, "%Y-%m-%d %H:%M:%S")
    utc_time = date_time - timedelta(hours=8)
