...
class Core(_TreelabObject):
    ...
    def add_table(self, table_name: str, default_view: bool = True, default_column_row: bool = True):
...
class Table(_TreelabObject):
    ...
    def get_row(self, row_id: str, view_id: str = ''):
        ...
        if view_id:
            ...
            # view = list(self.data.views.values())[0]
            view = _TreelabCheck(self.data.views)['view']
...
class _TreelabCheck:
    ...
    def __getitem__(self, item):
        if item == 'columns':
            if len(list(self.data.values())) > 0:
            ...
            raise ValueError('The data is zero')


class _ViewDF:
    def __init__(self, rows=None, columns=None, cells=None, df=None, view=None):
        self.rows = rows
        ...
        for view_id, data in self._views_dict.items():
            if 'rows' in data and 'columns' in data:
                ...
                view_df = _ViewDF(columns=data.get('columns'), rows=data.get('rows'), cells=data.get('cells'),
                                  df=df_data, view=self.views[view_id])
