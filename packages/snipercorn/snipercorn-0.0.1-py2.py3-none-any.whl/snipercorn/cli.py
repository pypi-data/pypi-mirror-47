...
from .sni import SNIConfig as Config
...
def cli(sys_args: Optional[List[str]] = None) -> None:
    ...
    parser.add_argument(
        "--ca-certs", help="Path to the SSL CA certificate file", default=sentinel
    )
    parser.add_argument(
        "--certfile", help="Path to the SSL certificate file", default=sentinel
    )
    parser.add_argument(
        "--cert-reqs", help="See verify mode argument", type=int, default=sentinel
    )
    parser.add_argument(
        "--ciphers", help="Ciphers to use for the SSL setup", default=sentinel
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    parser.add_argument(
        "-s",
        "--sni-path",
        help="dehydrated.io-format path containing certs/[DOMAIN]/(privkey|fullchain).pem",
        dest="sni_path",
        default=sentinel,
    )
    parser.add_argument(
    ...
    if args.sni_path is not sentinel:
        config.sni_path = args.sni_path
