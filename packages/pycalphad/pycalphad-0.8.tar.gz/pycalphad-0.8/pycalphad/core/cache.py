...
def fhash(thing):
    ...
    try:
        ...
        if isinstance(thing, Mapping):
            return fhash(sorted(thing.items()))
        elif isinstance(thing, Iterable):
            try:
                return hash(tuple(fhash(t) for t in thing))
            except TypeError: # iteration over 0-d array
                return hash(type(thing))
        raise
...
class _HashedSeq(list):
    ...
    def __init__(self, tup, hash=fhash):
        self[:] = tup
