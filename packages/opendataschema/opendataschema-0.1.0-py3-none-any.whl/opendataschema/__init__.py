...
class SchemaCatalog:
    ...
    def __init__(self, path_or_url, catalog_schema_url=CATALOG_SCHEMA_URL, download_func=requests.get):
        self.download_func = download_func

        if isinstance(path_or_url, Path):
            path_or_url = str(path_or_url)

        catalog_content = load_text_from_url(path_or_url, download_func=self.download_func) \
        ...
        references = {}
        for schema in self.descriptor['schemas']:
            config = {k: v for k, v in schema.items() if k != "name"}
            references[schema['name']] = SchemaReference.from_config(config, download_func=self.download_func)
...
class SchemaReference(ABC):
    ...
    def from_config(cls, config: dict, download_func=requests.get):
        ...
        if repo_url:
            return GitSchemaReference(repo_url, **options, download_func=download_func)
        elif schema_url:
            return URLSchemaReference(schema_url, **options)
