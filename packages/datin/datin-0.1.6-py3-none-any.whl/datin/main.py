...
#  Adjustments to df_datin as per database input requirements
    ...
    df_datnames = pd.read_excel(dat_filepath)
    col_size = df_datnames.columns.size

    param_list = []
    print('hey!!')
    print(df_datin.ParameterName)
    for param in df_datin.ParameterName:
        match = (0,0)
        col = 0
        while col < col_size and match[0] < 75:
            for dat_param in df_datnames[col]:
                ratio = fuzz.ratio(str(param), str(dat_param))
                match = (ratio, dat_param)
            col += 1
        if match[0] < 75:
            print(match[1], match[0], "*** Check Match!!!")
        else:
            print(match[1], match[0])
        param_list.append(match[1])

    df_datin.ParameterName = param_list
