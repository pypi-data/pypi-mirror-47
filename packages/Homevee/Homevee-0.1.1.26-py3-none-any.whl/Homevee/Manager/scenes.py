...
def get_all_scenes(user, db: Database = None):
    if db is None:
        db = Database()
...
def get_scenes(user, location, db: Database = None):
    if db is None:
        db = Database()
    scenes = []
...
def add_edit_scene(username, id, name, location, action_data, db: Database = None):
    if db is None:
        db = Database()
...
def delete_scene(username, id, db: Database = None):
    if db is None:
        db = Database()
    with db:
        cur = db.cursor()
