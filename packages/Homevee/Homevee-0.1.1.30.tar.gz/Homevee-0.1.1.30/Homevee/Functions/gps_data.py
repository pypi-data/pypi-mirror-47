...
def update_gps(user, time, lat, lng, db: Database = None):
    if db is None:
        db = Database()

    on_location_change(user, int(time), float(lat), float(lng))

    db.insert("INSERT INTO GPS_DATA (user, TIMESTAMP, LATITUDE, LONGITUDE) VALUES (:user, :time, :lat, :lng);",
                {'user': user.username, 'time': time, 'lat': lat, 'lng': lng})

    return Status(type=STATUS_OK).get_dict()

def on_location_change(user, time, lat, lng, db: Database = None):
    if db is None:
        db = Database()
    #determine user location (at work, at school, at home, etc.)
    current_user_location = determine_user_location(user, time, lat, lng)
    set_user_location(user, current_user_location)

    return

def determine_user_location(user, time, lat, lng, db: Database = None):
    if db is None:
        db = Database()
    ...
    place = get_nearest_place(user, lat, lng)
...
def get_nearest_place(user, lat, lng, db: Database = None):
    if db is None:
        db = Database()
...
def set_user_location(user, place, db: Database = None):
    if db is None:
        db = Database()
...
def get_gps_locations(user, db: Database = None):
    if db is None:
        db = Database()
