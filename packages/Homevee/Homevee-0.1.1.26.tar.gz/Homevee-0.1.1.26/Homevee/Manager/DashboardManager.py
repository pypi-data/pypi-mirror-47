...
class DashboardManager():
    ...
    def get_user_dashboard_items(self, user, db: Database = None):
        ...
        if 'calendar' in static_data:
            if static_data['calendar']:
                calendar_entries = get_calendar_day_items(user, datetime.datetime.now().strftime("%Y-%m-%d"))
        ...
        try:
            if 'nutritionmanager' in static_data:
                if static_data['nutritionmanager']:
                    data_item = get_user_nutrition_overview(user)
    ...
    def edit_user_dashboard(self, username, dashboard_data, db: Database = None):
        if db is None:
            db = Database()
        ...
        return self.get_user_dashboard(username)
    ...
    def get_device_info(self, user, type, id, db: Database = None):
        ...
        try:
            ...
            #Z-Wave
                ...
                data = get_thermostat_info(user, item['RAUM'], type, item['THERMOSTAT_ID'])
                ...
                device_data = {'name': item['SHORTFORM'], 'id': item['ID'],
                        'icon': item['ICON'], 'room': item['RAUM'], 'type': 'value',
                               'value': get_sensor_data(user, item['RAUM'], type, id, 1)}
            ...
            #MAX Cube
                ...
                data = get_thermostat_info(user, item['RAUM'], type, id)
            ...
            #MQTT Sensor
                ...
                device_data = {'name': item['NAME'], 'id': item['ID'],
                               ...
                               'value': get_sensor_data(user, item['ROOM'], type, id, 1)}
        except:
