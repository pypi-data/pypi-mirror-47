...
def set_context(user, context_key, context_data, db: Database = None):
    if db is None:
        db = Database()
...
def get_context(user, db: Database = None):
    if db is None:
        db = Database()
...
def replace_voice_commands(text, user, db: Database = None):
    ...
    db.select_all("SELECT * FROM VOICE_COMMAND_REPLACE WHERE USERNAME = :user",
                {'user': user.username})
