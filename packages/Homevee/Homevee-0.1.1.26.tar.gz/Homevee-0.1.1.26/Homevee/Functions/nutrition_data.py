...
def get_nutrition_data(user, keyword, db: Database = None):
    if db is None:
        db = Database()
...
def submit_food(user, name, calories, portionsize, portionunit, protein, fat, saturated, unsaturated, carbs, sugar, ean, db: Database = None):
    if db is None:
        db = Database()
    remote_data = load_remote_data(user)
...
def get_user_day_nutrition_data(user, db: Database = None):
    if db is None:
        db = Database()
    return

def get_user_nutrition_overview(user, db: Database = None):
    if db is None:
        db = Database()
    user_profile = get_user_fitness_profile(user)
...
def save_user_settings(user, height, weight, birthdate, mode, activity, db: Database = None):
    if db is None:
        db = Database()
...
def load_user_settings(user, db: Database = None):
    if db is None:
        db = Database()
        data = {}
...
def add_user_weight(user, weight, db: Database = None):
    if db is None:
        db = Database()
    date = datetime.datetime.today().__format__("%Y-%m-%d")

    db.insert("INSERT INTO USER_WEIGHT_DATA (user, DATE, WEIGHT) VALUES (:user, :date, :weight)",
               {'user': user.username, 'date': date, 'weight': weight})

def delete_food_item(user, id, db: Database = None):
    if db is None:
        db = Database()
...
def get_user_fitness_profile(user, db: Database = None):
    if db is None:
        db = Database()
...
def get_user_weight_progress(user, start, end, db: Database = None):
    if db is None:
        db = Database()
...
def get_user_day_nutrition_items(user, date, db: Database = None):
    if db is None:
        db = Database()
    ...
    calories_goal = get_user_fitness_profile(user)['caloriesgoal']

    return {'items': daytime_items, 'caloriesgoal': calories_goal}

def move_item(user, id, date, daytime, deleteold, db: Database = None):
    if db is None:
        db = Database()
...
def add_edit_user_day_nutrition_item(user, id, date, daytime, name, eatenportionsize, portionsize, portionunit, calories, fat, saturated, unsaturated, carbs, sugar, protein, db: Database = None):
    if db is None:
        db = Database()
