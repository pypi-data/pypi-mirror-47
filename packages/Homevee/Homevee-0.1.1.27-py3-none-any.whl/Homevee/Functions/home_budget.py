...
def get_home_budget_data(user, startdate, db: Database = None):
    if db is None:
        db = Database()
    data = {}
...
def get_home_budget_data_graph(user, startdate, enddate, db: Database = None):
    if db is None:
        db = Database()
...
def get_home_budget_data_day_items(user, date, db: Database = None):
    if db is None:
        db = Database()
    data = []
...
def add_edit_home_budget_data(user, id, date, info, amount, db: Database = None):
    if db is None:
        db = Database()
    home_budget_item = None
    if id is not None:
        home_budget_item = Item.load_from_db(HomeBudgetItem, id)
...
def delete_home_budget_data(user, id, db: Database = None):
    if db is None:
        db = Database()
