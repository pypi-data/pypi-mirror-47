...
def conditions_true(condition_data, db: Database = None):
    if db is None:
        db = Database()
    ...
    if(isinstance(condition_data, list)):
        ...
        return check_single_condition(condition_data)

def check_single_condition(condition_data, db: Database = None):
    if db is None:
        db = Database()
    ...
    #{'action': 'sensor_value', 'type': 'MQTT Sensor', 'id': '1', 'comparator': '>', 'value': 28}
        ...
        value = get_sensor_value(condition_data['devicetype'], condition_data['deviceid'], False)
        Logger.log(value)
