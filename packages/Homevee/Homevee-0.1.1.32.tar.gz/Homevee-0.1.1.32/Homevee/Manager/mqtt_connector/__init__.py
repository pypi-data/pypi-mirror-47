...
def save_mqtt_device(user, type, location, id, data, db: Database = None):
    if db is None:
        db = Database()
...
def generate_device_data(user, location, db: Database = None):
    if db is None:
        db = Database()
    if not user.has_permission(location):
        return {'result': 'nopermission'}
...
def add_to_intermediates(id, db: Database = None):
    if db is None:
        db = Database()
        db.insert("INSERT INTO MQTT_DEVICE_INTERMEDIATES (ID) VALUES (:id)", {'id': id})

def is_in_intermediates(id, db: Database = None):
    if db is None:
        db = Database()
