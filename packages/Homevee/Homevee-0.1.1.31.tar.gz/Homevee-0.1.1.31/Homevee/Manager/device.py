...
def get_gateway_devices(user, type, db: Database = None):
    if db is None:
        db = Database()
...
def add_edit_device(user, device_id, type, data, db: Database = None):
    if db is None:
        db = Database()
    if not user.has_permission("admin"):
        return Status(type=STATUS_NO_ADMIN).get_dict()
...
def delete_device(user, type, id, db: Database = None):
    if db is None:
        db = Database()
...
def get_device_data(user, type, id, db: Database = None):
    if db is None:
        db = Database()
    if not user.has_permission("admin"):
    ...
    for room in RoomDataManager().get_rooms(user, db):
