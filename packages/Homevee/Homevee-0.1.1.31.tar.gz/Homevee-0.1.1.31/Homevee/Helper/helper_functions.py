...
def set_remote_control_enabled(user: User,enabled, db: Database = None):
    if db is None:
        db = Database()
    if(not user.has_permission("admin")):
        return {'result': 'noadmin'}
    db.set_server_data("USE_REMOTE_CONTROL", enabled, db)
    return Status(type=STATUS_OK).get_dict()

def save_remote_data(user, remote_id, linked_account, db: Database = None):
    if db is None:
        db = Database()
    if(not user.has_permission("admin")):
        return {'result': 'noadmin'}

    db.set_server_data("REMOTE_ID", remote_id, db)
    db.set_server_data("REMOTE_LINKED_ACCOUNT", linked_account, db)

    return Status(type=STATUS_OK).get_dict()

def load_remote_data(user, db: Database = None):
    if db is None:
        db = Database()
    if(not user.has_permission("admin")):
        return {'result': 'noadmin'}

    remote_id = db.get_server_data("REMOTE_ID", db)
    access_token = db.get_server_data("REMOTE_ACCESS_TOKEN", db)
    linked_account = db.get_server_data("REMOTE_LINKED_ACCOUNT", db)
...
def connect_remote_id_with_account(user, account_name, account_secret, db: Database = None):
    if db is None:
        db = Database()
    if(not user.has_permission("admin")):
        return {'result': 'noadmin'}
    ...
    if response == "ok":
        db.set_server_data("REMOTE_LINKED_ACCOUNT", account_name, db)
