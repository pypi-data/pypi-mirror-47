...
def get_graph_data(user, room, type, id, von, bis, db: Database = None):
    if db is None:
        db = Database()
...
def get_day_data(type, id, datum, db: Database = None):
    if db is None:
        db = Database()
    ...
    return {'values': values, 'einheit': get_einheit(type, id)}

'''Gibt die Höchst- & Tiefstwerte des angegebenen Datumsbereichs zurück'''
def get_day_min_max(type, id, von, bis, db: Database = None):
    if db is None:
        db = Database()
