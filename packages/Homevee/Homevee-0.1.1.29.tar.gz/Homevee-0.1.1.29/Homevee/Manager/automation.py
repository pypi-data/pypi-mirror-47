...
def get_automations(user, room, db: Database = None):
    if db is None:
        db = Database()
...
def get_full_automation_data(id, db: Database = None):
    if db is None:
        db = Database()
    data = db.select_all("SELECT * FROM AUTOMATION_DATA WHERE ID = :id", {'id': id})
...
def get_trigger_data(id, db: Database = None):
    if db is None:
        db = Database()
...
def add_edit_automation_rule(user, location, id, name, trigger_data, condition_data, action_data, is_active, db: Database = None):
    if db is None:
        db = Database()
...
def delete_automation_rule(user, id, db: Database = None):
    if db is None:
        db = Database()
...
def add_trigger_data(data, id, db: Database = None):
    if db is None:
        db = Database()
    for data in data:
