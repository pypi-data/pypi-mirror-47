...
from .errors import (
    ...
    NO_ERROR, CRITICAL,
    INVALID_TYPE, INVALID_PATTERN, INVALID_MATCH, INVALID_VALUE, INVALID_NODE,
...
class Validator(object):
    ...
    def explain_validation(self, data):
        ...
        """
        return self.explain(self.validate(data))

    def explain(self, errors, no_errors=False, level=1):
        """
        ...
        if errors:
            for name, error in errors.items():
                if isinstance(error, dict):
                    child_errors = self.explain(error, no_errors=no_errors, level=level+1)
                    if child_errors:
                    ...
                    messages.append(pad + "* {} - {}".format(name, error.debug()))
        if level == 1:
