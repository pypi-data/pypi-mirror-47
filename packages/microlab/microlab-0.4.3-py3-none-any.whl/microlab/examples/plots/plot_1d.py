...
def test_plot_1d_list():
    print('| 1D List  |', end=' ---->')
    signal_from_list = Signal_1D(values=data.List_a, verbose=True)
...
def test_plot_1d_tuple():
    print('| 1D Tuple |', end=' ---->')
    signal_from_tuple = Signal_1D(values=data.Tuple_a, verbose=True)
...
def test_plot_1d_npArray():
    print('| 1D Array |', end=' ---->')
    signal_1_from_ndarray = Signal_1D(values=data.npArray_a, verbose=True)
