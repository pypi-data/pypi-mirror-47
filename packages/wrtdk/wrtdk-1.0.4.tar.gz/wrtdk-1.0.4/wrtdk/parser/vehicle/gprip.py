...
class gprip(parser):
    ...
    def getData(self):
        ...
        0) epoch time in ms
        1) the navigation fix
        2) the fix epoch time in ms
        3) the fix latitude in dd
        4) the fix longtude in dd
        5) the estimated latitude in dd
        6) the estimated longitude in dd
        7) the depth in m
        8) the altitude above the sea floor in m
        9) the roll in radians
        10) the pitch in radians
        11) the yaw in radians
        12) the vehicle speed in mps
        '''
