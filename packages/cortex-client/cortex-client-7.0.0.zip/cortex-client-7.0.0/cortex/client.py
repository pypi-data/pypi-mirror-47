...
import jwt
from cortex_client import DatasetsClient, AuthenticationClient, SessionClient, ExperimentClient, ProfilesClient
...
class Client(object):
    ...
    def profile(self, profileId: str, version:int=3) -> Profile:
        profile_client = ProfilesClient(self._url, version, self._token.token)
        return Profile.get_profile(profileId, profile_client)

    def profile_schema(self, schemaId:str, version:int=3) -> ProfileSchema:
        profile_client = ProfilesClient(self._url, version, self._token.token)
        return ProfileSchema.get_schema(schemaId, profile_client)
