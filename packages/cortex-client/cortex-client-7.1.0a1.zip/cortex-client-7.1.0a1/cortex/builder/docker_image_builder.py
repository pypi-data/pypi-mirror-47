...
class BuilderServiceImageBuilder(_DockerImageBuilder):
    ...
    def build_and_push(self, build_dir, name, docker_repo, docker_auth=None):
        ...
        print("Building job: {}".format(response))

        def result_predicate(v):
            is_active = v.get('status', {}).get('active')
            is_unknown= v.get('status') == {} 
            return not is_active and not is_unknown

        status = self._wait_for_status(response.get('jobName'), result_predicate)
    ...
    def _wait_for_status(self, job_id, result_predicate):
        ...
        return r.wraps(get_job_print)(job_id)
    ...
    def _get_build_context_for_job(path):
        ...
        print('Build context path: ', context_path)
        return context_path
