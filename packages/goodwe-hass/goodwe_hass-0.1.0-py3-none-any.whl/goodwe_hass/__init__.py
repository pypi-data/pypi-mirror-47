...
class GoodWe(object):
    ...
    def login(self):
        ...
        r = self._post('Common/CrossLogin', headers= headers, payload=loginPayload)
        self._token = json.dumps(r.json()['data'])

    def get_current_reading(self):
        headers = { 'User-Agent': 'PVMaster/2.0.4 (iPhone; iOS 11.4.1; Scale/2.00)', 'Token': self._token }
        payload = {'powerStationId': self._system_id}
        r = self._post('PowerStation/GetMonitorDetailByPowerstationId',headers= headers, payload=payload)
