...
from apihelper.thirdparty.tornadows import soap
from apihelper.thirdparty.tornadows import xmltypes
from apihelper.thirdparty.tornadows import complextypes
from apihelper.thirdparty.tornadows import wsdl
    ...
    def get(self):
        ...
        for operations in dir(self):
            ...
            if callable(operation) and hasattr(operation, '_input') and hasattr(operation, '_output') and hasattr(
                ...
                wsdl_data = {'args': wsdl_args, 'input': ('params', wsdl_input), 'output': ('returns', wsdl_output),
        ...
        try:
            ...
            for operations in dir(self):
                ...
                if callable(operation) and hasattr(operation, '_is_operation'):
                    ...
                    if hasattr(operation, '_operation') and soapaction.endswith(
                            getattr(operation, '_operation')) and num_methods > 1:
        ...
        for e in elems:
            if e.hasChildNodes and len(e.childNodes) > 0:
                ...
                if inspect.isclass(type) and (
                        issubclass(type, xmltypes.PrimitiveType) or isinstance(type, xmltypes.Array)):
                    v = type.genType(e.childNodes[0].nodeValue)
                elif hasattr(type, '__name__') and (
                        not issubclass(type, xmltypes.PrimitiveType) or not isinstance(type, xmltypes.Array)):
