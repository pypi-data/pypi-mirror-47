...
class Geography(MUDObject, metaclass=GeographyMeta):
    ...
    def move_player(self, player, direction):
        ...
        try:
            acts = []
            acts += self.action_exit(player)
            acts += getattr(self, "action_exit_" + direction)(player)
            player.set_location(new_location)
            acts += new_location.action_enter(player)
            acts += getattr(new_location, "action_enter_" + direction)(player)
            for act in acts:
                act()
