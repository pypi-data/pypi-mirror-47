...
def format_message_body(message):
    ...
    logging.debug('Payload type: %s' % type(message))
    if type(message) == unicode or type(message) == str:
        logging.debug('Message is stringy, attempting to JSONIFY')
        message_json = json.loads(message) 
    else:
        message_json = message
    logging.debug('Post adj payload type: %s' % type(message_json))
    return message_json
...
def send_batch(message_list, queue_url):
    ...
    logging.info("Sending size %s chunks to outbound SQS queue url: %s" % (chunk_size, queue_url) )
