...
def draw_ellipse(image, rect, rot, color, width):
  ...
  def rc2box(rect, size):
    ...
    nw1 = size[0]-nw0
    nh1 = size[1]-nh0
    return cx-nw0, cy-nh0, cx+nw1, cy+nh1
...
def plot_addon(path_name, path_to_save=None, color="#ffffff", width=3):
...
def main(dir_in, dir_out, color, width):
  ...
  for i in range(n):
    ...
    plot_addon(path2plot, path2save, color, width)  ## save
