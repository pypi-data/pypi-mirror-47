...
## change shape by percentage
def chg_shape(rc, width, height):
  pw, ph = width/100., height/100.
  ## assert rc is normalized
  cx = (rc[0] + rc[2])/2.
  cy = (rc[1] + rc[3])/2.
  w_2 = ((rc[2] - rc[0])/2.)*pw
  h_2 = ((rc[3] - rc[1])/2.)*ph
  return int(cx-w_2), int(cy-h_2), int(cx+w_2), int(cy+h_2)
...
####
#def draw_glasses(image, draw, lms, color, width, frame):
def draw_glasses(image, draw, lms, color, shape):
  ...
  rect0 = chg_shape(rect0, width, height)    
  rect1 = chg_shape(rect1, width, height)    
  
  draw_ellipse(image, rect0, rot, color, thickness)  
  draw_ellipse(image, rect1, rot, color, thickness)  
  draw.line(line0, fill=color, width=thickness)
...
#def plot_addon(path_name, path_to_save=None, random_color=(0, 255, 0, 255, 0, 255), random_shape=(80,140, 80, 140, 1, 10)):
def plot_addon(path_name, path_to_save=None, random_color=None, random_shape=None):
  ...
  def gen_rand_color(random_color):  
    #r = lambda: random.randint(0,255)
    colors = []
    for i in range(3):
      colors += [random.randint(random_color[i*2], random_color[i*2+1])]
    return '#%02X%02X%02X' % (colors[0], colors[1], colors[2])

  def gen_rand_shape(random_shape):  
    shapes = []
    for i in range(3):
      shapes += [random.randint(random_shape[i*2], random_shape[i*2+1])]
  ...
  for k, d in enumerate(dets):
    ...
    ## gen glasses
    if len(random_color) == 6:
      color = gen_rand_color(random_color)
    else:
      color = "#111111"
      
    #width, height, thickness = gen_rand_shape(random_shape)
    if len(random_shape) == 6:
      shape = gen_rand_shape(random_shape)
    else:
      shape = (100, 100, 3)
          
    draw_glasses(image, draw, fl, color, shape)
...
#@click.option('--random', is_flag=True, help='generate glasses w/ different color and width randomly')
@click.option('--random_color', nargs=6, type=int, help='generate glasses w/ different color [r:0-255, g:0-255, b:0-255] randomly')
@click.option('--random_shape', nargs=6, type=int, help='generate glasses w/ different shape [frame-width, frame-height, frame-thickness] randomly')


#def main(dir_in, dir_out, color, width, random):
def main(dir_in, dir_out, random_color, random_shape):
  ...
  for i in range(n):
    ...
    plot_addon(path2plot, path2save, random_color, random_shape)
