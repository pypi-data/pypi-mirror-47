...
def simplify_result(result_bindings: Tuple[Constant]):
...
1
  ...
  #   Match short-circuits but this is still O(n-tuples)
  else:
...
def simplify_all(result_bindings):
  """Helper for forcing all DB results and simplifying them.

  May get migrated into core under some other name.
  """

  return [simplify_result(t) for t in result_bindings]
