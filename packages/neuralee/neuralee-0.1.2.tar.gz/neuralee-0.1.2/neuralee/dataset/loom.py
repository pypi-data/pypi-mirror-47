...
class LoomDataset(GeneExpressionDataset):
    ...
    def __init__(self, filename, save_path='data/', url=None):
        ...
        data, batch_indices, labels, gene_names, cell_types = \
            self.download_and_preprocess()

        X, batch_indices_, labels = \
            GeneExpressionDataset.get_attributes_from_matrix(
                data, labels=labels)
        batch_indices = batch_indices if batch_indices is not None \
            else batch_indices_
        super().__init__(X, batch_indices, labels, gene_names=gene_names,
    ...
    def preprocess(self):
        ...
        if 'BatchID' in ds.ca:
            ...
            batch_indices = \
                np.reshape(batch_indices, (batch_indices.shape[0], 1))[select]
...
class RetinaDataset(LoomDataset):
    ...
    The dataset of bipolar cells contains after their original pipeline for
    filtering 27,499 cells and 13,166 genes coming from two batches. We use
    the cluster annotation from 15 cell-types from the author. We also
    ...
    def __init__(self, save_path='data/'):
        super().__init__(filename='retina.loom',
                         save_path=save_path,
                         url='https://github.com/YosefLab/scVI-data/raw/'
                             'master/retina.loom')
        self.cell_types = ["RBC", "MG", "BC5A", "BC7", "BC6", "BC5C", "BC1A",
                           "BC3B", "BC1B", "BC2", "BC5D", "BC3A", "BC5B",
                           "BC4", "BC8_9"]
