...
def create_index(es, index_name, mapping=DEFAULT_MAPPING, settings=DEFAULT_SETTING, overwrite=True):
    """
...
def push_list_dict_to_index(es, index_name, list_dict, mode='overwrite', append_key='es_id', chunk_size = 60000, thread_count=4, request_timeout=20):
    for items in chunks(list_dict, chunk_size):
        bulk_data = items if mode == 'append' else remake_items(items, append_key) 

        print("items len: {}".format(len(items)))
        res = es.bulk(index_name, bulk_data, chunk_size=chunk_size//thread_count, thread_count=thread_count, 
