...
def get_inferred_fillna_columns(summaries):
    return [c.name for c in summaries
            if pd.api.types.is_numeric_dtype(c.pd_dtype) and
...
def get_inferred_drop_columns(summaries):
    return [c.name for c in summaries
            ...
            (not pd.api.types.is_numeric_dtype(c.inferred_dtype) and
             not pd.api.types.is_categorical_dtype(c.inferred_dtype))]
...
def get_pandas_df_summary(df):
    ...
    for col in df.columns:
        ...
        if (pd.api.types.is_integer_dtype(inferred_dtype) or
