...
def get_global_block_list_from_resolution(coarse_resolution, fine_resolution):
    ...
    for h_b in range(n_h_blocks):
        ...
        for v_b in range(n_v_blocks):
            block_list.append([h_b * block_size, v_b * block_size, block_size, block_size, h_b, v_b])
...
def make_path_global_pyramid(input_path, output_path=None, make_overviews=True, calculate_stats=True, clean_temporary_files=False, resolution=None, raise_exception=False, make_overviews_external=True, verbose=True):
    ...
    if resolution is None: # Consider removing this as I would have to implement reprojection to make this work, but atm i like it manual.
        resolution = determine_pyramid_resolution(input_path)
    ...
    if str(compression).lower() not in  ['lzw', 'deflate']:
        L.critical('rewrite_array triggered because compression was not lzw or deflate.')
        rewrite_array = True  # Default options will include lzw
    ...
    ds.SetMetadataItem('is_64_bit_pyramidal', 'yes')
    ...
    if rewrite_array:
        L.critical('make_path_global_pyramid triggered rewrite_array for ' + str(input_path))
...
def make_path_spatially_clean(input_path, make_overviews=True, calculate_stats=True, clean_temporary_files=False, raise_exception=False, make_overviews_external=True, compress=True):
    ...
    if rewrite_array:
        L.critical('make_path_spatially_clean triggered rewrite_array for ' + str(input_path))
        temp_write_path = hb.temp('.tif', filename_start='temp_write_' + str(hb.file_root(input_path)), remove_at_exit=clean_temporary_files)
        displacement_path = hb.temp('.tif', filename_start='displaced_by_make_path_global_pyramid_on_' + str(hb.file_root(input_path)), remove_at_exit=clean_temporary_files)
    ...
    if make_overviews:
        L.info('Starting to make overviews for ' + str(input_path))
        ds.BuildOverviews('nearest', [2, 4, 8, 16, 32])  # Based on commonly used data shapes

        # Difference from make_path_gobal_pyramid, for reference.
        # ds.BuildOverviews('nearest', pyramid_compatible_overview_levels[arcseconds])  # Based on commonly used data shapes
...
def assert_paths_same_pyramid(path_1, path_2, raise_exception=False):

    bool_3 = hb.is_path_same_geotransform(path_1, path_2)
...
def load_geotiff_chunk_by_cr_size(input_path, cr_size, datatype=None):
    ...
    a = ds.ReadAsArray(c, r, c_size, r_size, buf_type=datatype, callback=callback, callback_data=[input_path])

    return a

def load_geotiff_chunk_by_bb(input_path, bb, inclusion_behavior='centroid', datatype=None, output_path=None, ndv=None):
    ...
    c, r, c_size, r_size = bb_path_to_cr_widthheight(input_path, bb, inclusion_behavior=inclusion_behavior)
    L.debug('bb_path_to_cr_widthheight generated', c, r, c_size, r_size)

    a = hb.load_geotiff_chunk_by_cr_size(input_path, (c, r, c_size, r_size), datatype)

    if output_path is not None:

        data_type = hb.get_datatype_from_uri(input_path)

        src_ndv = hb.get_nodata_from_uri(input_path)
        if ndv is None:
            ndv = hb.get_nodata_from_uri(input_path)
...
def bb_path_to_cr_widthheight(input_path, bb, inclusion_behavior='centroid'):
    ...
    if not os.path.exists(input_path):
        L.warning('bb_path_to_cr_widthheight unable to open ' + str(input_path))
    ds = gdal.OpenEx(input_path)
    n_c, n_r = ds.RasterXSize, ds.RasterYSize
...
def bb_path_to_cr_widthheight_working(input_path, bb):
    # BB must be in lat-lon units (not projected units yet) in xmin, ymin, xmax, ymax order
    # Useful for getting gdal-type cr_widthheight from a subset of a raster via it's bb from path.
    # Note that gdal Open uses col, row, n_cols, n_row notation. This function converts lat lon bb to rc in this order based on the proportional size of the input_path.
    ...
    r, c = hb.latlon_path_to_rc(ll_lat, ll_lon, input_path)
    r_right, c_right = hb.latlon_path_to_rc(ll_lat + lat_size, ll_lon + lon_size, input_path)
    r_size = r_right - r
    c_size = c_right - c

    # I have no idea why this works. It seems like it should be in the latlon_path_to_rc_call
    r = n_r - r - r_size
...
def latlon_path_to_rc(lat, lon, input_path, r_shift_direction='centered', c_shift_direction='centered'):
    ...
    gt_xmin_lon = float(ulx)
    gt_ymin_lat = float(uly) + yres * float(n_r)
    gt_xmax_lon = ulx + xres * float(n_c)
    gt_ymax_lat = float(uly)
