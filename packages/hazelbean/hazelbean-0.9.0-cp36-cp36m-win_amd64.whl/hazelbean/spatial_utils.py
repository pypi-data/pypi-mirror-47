...
#     ge = None
#     print('Unable to import geoecon.\nPrior exception that caused this:\n\n' + str(error_message))
#     # raise NameError('Unable to import geoecon.\nPrior exception that caused this:\n\n' + str(error_message))
...
def rasterize_to_match(input_vector_path, match_raster_path, output_raster_path, burn_column_name, datatype=None, ndv=None, all_touched=False):
...
def clip_raster_by_bb(input_path, bb, output_path):
    output_raster = hb.load_geotiff_chunk_by_bb(input_path, bb)
    ...
    n_cols_override = 1111
    n_rows_override = 2222
    ...
    print ('not done. copy clip_raster_by_cr_heightwidth')
...
def save_array_as_geotiff(array, out_uri, geotiff_uri_to_match=None, ds_to_match=None, band_to_match=None,
                          ...
                          n_rows_override=None, compress=True, compression_method=None,
...
def reclassify_flex(input_flex, rules, output_path, target_datatype=None, target_nodata=None, compress=False, values_required=False):
    ...
    if isinstance(input_flex, hb.ArrayFrame):
        base_raster_path_band = [(input_flex.data.astype(np.float64), 1), (rules, 'raw')]
...
def add_overviews_to_path(input_path, raise_exception_on_fail=False):
    ...
    try:
        ...
        ds.BuildOverviews('nearest', [3, 6, 10, 15, 30], callback, [input_path])  # Based on commonly used data shapes for 10s data
        ds = None
    except AssertionError as error_message:
        message = 'Unable to add overviews to ' + str(input_path) + '\nPrio exception that caused this:\n\n' + str(error_message)
