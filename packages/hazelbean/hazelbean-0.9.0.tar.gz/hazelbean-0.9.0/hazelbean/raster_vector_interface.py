...
def convert_polygons_to_id_raster(input_vector_path, output_raster_path, match_raster_path,
                                  id_column_label=None, data_type=None, ndv=None, all_touched=None):
    ...
    hb.new_raster_from_base_pgp(match_raster_path, output_raster_path, data_type, band_nodata_list)
...
def zonal_statistics_flex(input_raster, zone_vector_path, zone_ids_raster_path=None, id_column_label=None, data_type=None,
                          ndv=None, zones_ndv=None, values_ndv=None,
                          all_touched=None, assert_projections_same=True, use_iterblocks=True, unique_zone_ids=None, csv_output_path=None, verbose=True):
    ...
    if not zone_ids_raster_path:
        to_return = hb.zonal_statistics(
    ...
    # NOTE that by construction, this type of zonal statistics cannot handle overlapping polygons (each polygon is just represented by its id int value in the raster).
        ...
        if not os.path.exists(zone_ids_raster_path):
            # Calculate the id raster and save it
            if verbose:
                L.info('Creating id_raster with convert_polygons_to_id_raster')
            hb.convert_polygons_to_id_raster(zone_vector_path, zone_ids_raster_path, input_raster, id_column_label=id_column_label, data_type=data_type, ndv=zones_ndv, all_touched=all_touched)
        else:
            if verbose:
...
def zonal_statistics_rasterized(zone_ids_raster_path, values_raster_path, zones_ndv=None, values_ndv=None, zone_ids_data_type=None,
                                values_data_type=None, use_iterblocks=True, unique_zone_ids=None, verbose=True):
    """
    ...
    if use_iterblocks:
        ...
        L.info('Determine data types for zones')
        if zone_ids_data_type is not None:
            zones_array = hb.as_array(zone_ids_raster_path).astype(hb.gdal_number_to_numpy_type[zone_ids_data_type])
        else:
            zone_ids_data_type = np.int
            zones_array = hb.as_array(zone_ids_raster_path).astype(zone_ids_data_type)

        L.info('Determine data types for values')
        if values_data_type is not None:
            values_array = hb.as_array(values_raster_path).astype(hb.gdal_number_to_numpy_type[values_data_type])
        else:
            values_data_type = np.float64
            values_array = hb.as_array(values_raster_path).astype(values_data_type)

        L.info('Get ndv values for zones, based on user input AND datatypes. Also cast to numpy type vs gdal number')
        if zones_ndv is not None:
            zones_ndv = hb.default_no_data_values_by_numpy_type_and_as_numpy_types[zone_ids_data_type]
        else:
            zones_ndv = hb.get_nodata_from_uri(zone_ids_raster_path)
            zones_ndv = hb.default_no_data_values_by_numpy_type[zone_ids_data_type]

        L.info('Get ndv values for values, based on user input AND datatypes. Also cast to numpy type vs gdal number')
        if values_ndv is not None:
            values_ndv = hb.default_no_data_values_by_numpy_type_and_as_numpy_types[values_data_type]
        else:
            values_ndv = hb.get_nodata_from_uri(values_raster_path)
            values_ndv = hb.default_no_data_values_by_numpy_type[values_data_type]

        hb.assert_gdal_paths_in_same_projection(zone_ids_raster_path, values_raster_path)

        a = hb.as_array(zone_ids_raster_path)
        unique_zone_ids = np.unique(a).astype(np.int64)
