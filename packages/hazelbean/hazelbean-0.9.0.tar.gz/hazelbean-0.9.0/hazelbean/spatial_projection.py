...
def assert_gdal_paths_have_same_geotransform(input_paths_list, return_result=False):
    ...
    for path in input_paths_list[1:]:
        ...
        if geotransform == first_geotransform:
            ...
            error_string = 'The path ' + str(path) + ' does not have the same geotransform as the other paths in the list given to assert_gdal_paths_in_same_projection. Geotransform of current: ' + str(geotransform)
            if return_result:
...
def resample_to_match(input_path, match_path, output_path, resample_method='bilinear',
                      output_data_type=None, src_ndv=None, ndv=None, compress=True,
    ...
    if ndv is None:
        dst_ndv = hb.get_nodata_from_uri(match_path)
    else:
...
def resample_to_match_ensuring_fit(input_path, match_path, output_path, resample_method='bilinear',
                      output_data_type=None, src_ndv=None, ndv=None, compress=True,
                      gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                      calc_raster_stats=False,
                      add_overviews=False,
    ...
    if verbose:
        L.info('resample_to_match_ensuring_fit called on', input_path)
    hb.warp_raster_hb(input_path, target_pixel_size, output_path,
                      resample_method, target_bb=target_bb, base_sr_wkt=None, target_sr_wkt=target_sr_wkt,
                      gtiff_creation_options=gtiff_creation_options,
                      n_threads=None, vector_mask_options=None,
...
def resize_and_resample_dataset_uri(
    ...
    L.critical('resize_and_resample_dataset_uri is deprecated. use pygeoprocessing warp or hb.resample (Which is a wrapper).')
    """
