...
from .. import (Share, dbg_print, QtCore, QtGui, QtWidgets, QtWebEngine, WithMenu)
...
class MyScene(QtWidgets.QGraphicsScene):
    ...
    def wheelEvent1(self, event):
        ...
        if not (modifiers & QtCore.Qt.ControlModifier):
            return QtWidgets.QGraphicsScene.wheelEvent(self, event)
...
class SvgDigest:
    ...
    def buildQuickDic(self):
        ...
        eltHead = eltAbc = None
        scale_ = 1.0  # default acc. to http://moinejf.free.fr/abcm2ps-doc/index.html
    ...
    def insertCursor(self, eltHead, colour='green'):
        ...
        dbg_print('written svg including cursor', fileName)
    ...
    def removeCursor(self):
        if self.eltCursor is not None and self.cursorsDad is not None:
            ...
            fileName = str(self.svg_file.fileName())
            self.svg_tree.write(fileName, encoding='utf-8', xml_declaration=True)
            dbg_print('written svg with cursor removed', fileName)
...
class Score(QtWidgets.QGraphicsView, WithMenu):
    ringColour = 'red'
    ...
    def __init__(self):
        ...
        self.svgView = QtWebEngine.QGraphicsWebView()
        # self.svgView = QtWidgets.QGraphicsView()
        self.svgView.setFlags(QtWidgets.QGraphicsItem.ItemClipsToShape)
        self.svgView.setCacheMode(QtWidgets.QGraphicsItem.NoCache)
        ...
        self.scene().update()
        # experimental hacking to avoid scroll bars initially!
    ...
    def printAll(self, toPDF=False):
        cursor = self.svgDigests[self.which].removeCursor()  # currently returns None!
        self.update()
        WithMenu.printAll(self, toPDF=toPDF)
        # really should do sometingn like:
        # if cursor:
        #    self.svgDigests[self.which].insertCursor(cursor)
    ...
    def wheelEvent(self, event):
        ...
        factor = 1.2 ** (event.delta() / 120.0)
        self.scale(factor, factor)
        dbg_print("MyScene.wheelEvent, scale by ", factor)
