...
!**************************************************************                      
          ...
          tau_p = delta_p/eps1 + 1.
          phi_p = n0*tau_p*A
          
          !! -- asymptotic surface propagator (inside surface)
          SsurfXX = (phi_p*cos(phi)**2+phi_s*sin(phi)**2)*cos(teta)
          SsurfXY = (phi_p-phi_s)*cos(teta)*sin(phi)*cos(phi)
          SsurfXZ = tau_p*eps0*cos(phi)*cos(teta)*sin(teta)
          SsurfYX = (phi_p-phi_s)*cos(teta)*sin(phi)*cos(phi)
          SsurfYY = (phi_p*sin(phi)**2+phi_s*cos(phi)**2)*cos(teta)
          SsurfYZ = tau_p*eps0*sin(phi)*cos(teta)*sin(teta)
          SsurfZX = -phi_p*sin(teta)*cos(phi)
          SsurfZY = -phi_p*sin(teta)*sin(phi)
          SsurfZZ = -tau_p*eps0*sin(teta)**2
