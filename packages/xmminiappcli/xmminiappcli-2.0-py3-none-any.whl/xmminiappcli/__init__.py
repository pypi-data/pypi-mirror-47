...
from .XMMiniAppWatcher import FileWatcher
...
class XMMiniAppCli(object):
    ...
    def test(self, root_path:str = os.curdir):
        if XMMiniAppCli._is_observer is False:
            XMMiniAppCli._is_observer = True
            NotificationCenter().add_observer(self, "App_Json_Change_Notification")
            FileWatcher(root_path).run()


        XMMiniAppCli._root_path = root_path
    ...
    def _notify(self, notification):
        ...
        print("*                       app.json发生改变                  *")
        print("***********************************************************")
        root_path = XMMiniAppCli._root_path
        output = os.path.join(root_path, 'testapps.json')
        test_app_json = gen_test_app_json(root_path)
        output_abs_path = os.path.abspath(output)
        with open(output_abs_path, 'w+') as ofile:
