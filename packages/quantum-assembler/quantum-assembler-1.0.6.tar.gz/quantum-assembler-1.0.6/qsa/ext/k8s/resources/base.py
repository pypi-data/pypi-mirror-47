...
class Resource(abc.ABC):
    ...
    def isunbound(self):
        ...
        """
        return self.labels.get('ops.quantumframework.org/unbound') == 'true'
    ...
    def setunbound(self, enable=True):
        ...
        tag = 'ops.quantumframework.org/env:global'
        if enable:
    ...
    def setenvironments(self, environments, annotate=True):
        ...
        if environments and annotate:
            self.annotate('environments',
                SerializedDatastructure.new([x.name for x in environments]),
                'ops.quantumframework.org')
        return self
