...
class VPoser(nn.Module):
    ...
    def forward(self, Pin, input_type='matrot', output_type='matrot'):
        ...
        q_z = self.encode(Pin)#Pin here is expected to be in matrot representation
...
class VPoserTrainer:
    ...
    def train(self):
        ...
        for it, dorig in enumerate(self.ds_train):
            ...
            drec = self.vposer_model(dorig['pose_matrot'], input_type='matrot', output_type='matrot')
    ...
    def compute_loss(self, dorig, drec):
        ...
        batch_size = dorig['pose_matrot'].size(0)
        device = dorig['pose_matrot'].device
        dtype = dorig['pose_matrot'].dtype
        ...
        loss_pose_rec = (1. - self.ps.kl_coef) * \
            torch.mean(torch.sum(torch.pow(dorig['pose_matrot'] - drec['pose_matrot'], 2), dim=[1, 2, 3]))
    ...
    def perform_training(self, num_epochs=None, message=None):
        ...
        for epoch_num in range(1, num_epochs + 1):
            ...
            early_stopping(eval_loss_dict['loss_total'])
    ...
    def vis_results(dorig, vposer_model, bm, imgpath):
        ...
        num_joints = dorig['pose_aa'].shape[2]

        mv = MeshViewer(width=imw, height=imh, use_offscreen=True)
        mv.render_wireframe = True

        dorig_matrot = dorig['pose_matrot'].view([-1,1, num_joints, 9])
        dorig_aa = dorig['pose_aa'].view([-1,1, num_joints, 3])

        prec_aa = vposer_model(dorig_matrot, input_type='matrot', output_type='aa')['pose_aa'].view(batch_size,-1)
...
def run_vposer_trainer(ps):
    ...
    ps.dump_settings(os.path.join(ps.work_dir, 'TR%02d_%s' % (ps.try_num, ps.expr_code)))
