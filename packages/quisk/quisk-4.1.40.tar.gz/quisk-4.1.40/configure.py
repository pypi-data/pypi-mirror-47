...
class Configuration:
    ...
    for k, v in list(radio_dict.items()):	# radio_dict may change size during iteration
      ...
      try:
        if fmt4 == 'text':	# Note: JSON returns Unicode strings !!!
          setattr(conf, k, v.encode('utf_8', 'replace'))
          ...
          if v == "None":
            ...
            x = eval(v)
            x = int(x)
          if k == 'hot_key_ptt2' and type(x) is not IntType:
  ...
  def OnPageChanging(self, event):
    ...
    if notebook == self.notebk:		# top level notebook with radio names
      if index >= self.radios_page_start:
        page = self.notebk.GetPage(index)
        page.MakePages()
    #elif isinstance(notebook, RadioNotebook):	# next level notebook with pages for each radio
    #  print (index, notebook)
...
'soapy_setAntenna_tx' : 'Choose the antenna to use for transmit.',
  ...
  def OnButtonChangeSoapyDevice(self, event):
    ...
    if txt[0:8] == 'Capture ':
      ...
      self.gbs.Add((self.charx, self.charx), (0, 0))
      self.row = 1
      RadioHardwareBase.MakeControls(self)
      self.MakeSoapyControls()
