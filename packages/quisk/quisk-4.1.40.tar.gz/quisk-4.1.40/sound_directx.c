...
static PyObject * py_str_utf8(LPCTSTR txt)
{       // return a UTF-8 PyString
#ifdef UNICODE
	size_t wstr_size = wcslen(txt);
	PyObject * py_unicode = PyUnicode_DecodeUTF16((const char *)txt, wstr_size * sizeof(WCHAR), "replace", NULL);
#else
	int wstr_size = MultiByteToWideChar(CP_ACP, 0, txt, -1, NULL, 0);
        PyObject * py_unicode, * py_string;
...
static BOOL CALLBACK DSEnumNames(LPGUID lpGUID, LPCTSTR lpszDesc, LPCTSTR lpszDrvName, LPVOID pyseq)
{
        ...
        PyObject * py_string = py_str_utf8(lpszDesc);
	PyList_Append((PyObject *)pyseq, py_string);
        //free(buf);
        Py_DECREF(py_string);
	return TRUE;
}
...
{	// Open the play device if the name is found in the description
	LPDIRECTSOUND8 DsDev;
        ...
        PyObject * py_string = py_str_utf8(lpszDesc);
        str = PyString_AsString(py_string);
	if (strstr (str, ((struct sound_dev *)dev)->name)) {
		errFound = DS_OK;
