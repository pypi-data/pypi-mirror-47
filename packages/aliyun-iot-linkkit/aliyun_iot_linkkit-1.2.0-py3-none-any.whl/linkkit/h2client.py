...
class FileStreamHandler(StreamHandler):
    ...
    def next(self):
        ...
        if self.__opt_crc64:
            do_crc64 = crcmod.mkCrcFun(0x142F0E1EBA9EA3693, initCrc= self.__last_crc, xorOut=0xffffffffffffffff, rev = True)
...
class H2Stream:
    ...
    def __str__(self):
        return 'H2Stream(id=%s,stream_x_id=%s,x_request_id=%s,x_data_stream_id:%s' % (self.__id,
                    self.__stream_id, self.__x_request_id, self.__x_data_stream_id)
...
class H2Client:
    ...
    def upload_file_async(self, local_filename, remote_filename = None, over_write = True,
        ...
        future_result = self.__thread_executor.submit(self.__post_file_task, file_info, upload_file_sink, upload_sink_user_data)
    ...
    def __connect(self):
        with self.__conn_lock:
            ...
            h2_endpoint = self.__product_key + ".iot-as-http2.%s.aliyuncs.com" % (self.__region)
