...
class ReloadPatterns(UniconCorePatterns):
    def __init__(self):
        super().__init__()
        self.savenv = r"^.*System config(uration)? has been modified\. Save\?.*$"
        self.confirm_reset = r'^.*Are you sure you want to reset the system \(y\/n\)\?'
        self.confirm_config = r'^(.*)Uncommitted changes found.*?'
        self.autoinstall_dialog = r'^(.*)Would you like to terminate autoinstall\? ?\[yes\]: $'
        self.module_reload = r'^.*Do you want to reload the internal AP\s?\? ((\[yes/no\]\??)|(\[y/n\]\??)):?\s?$'
        self.save_module_cfg = r'^.*Do you want to save the configuration of the AP\s?\? ((\[yes/no\]\??)|(\[y/n\]\??)):?\s?$'
        self.reboot_confirm = r'^(.*)This command will reboot the system. \(y\/n\)\?  \[n\]\s?$'
        ...
        self.reload_confirm_ios = r'^.*Proceed( with reload)?\? *\[confirm\]'
        self.reload_confirm = r'^.*Reload node \? \[no,yes\]\s?$'
...
class TraceroutePatterns(object):
    def __init__(self):
        ...
        self.target = r'^.*Target IP address: $'
        self.ingress = r'^.*Ingress traceroute \[.+\]\s?: $'
        self.source_address_interface = r'^.*Source address or interface: $'
        self.dscp = r'^.*DSCP .*\[.+\]\s?: $'
        self.numeric_display = r'^.*Numeric display \[.+\]\s?: $'
        self.timeout_seconds = r'^.*Timeout in seconds \[.+\]\s?: $'
        self.probe_count = r'^.*Probe count \[.+\]\s?: $'
        self.minimum_ttl = r'^.*Minimum Time to Live \[.+\]\s?: $'
        self.maximum_ttl = r'^.*Maximum Time to Live \[.+\]\s?: $'
        self.port_number = r'^.*Port Number \[.+\]\s?: $'
        self.style = r'^.*Loose, Strict, Record, Timestamp, Verbose\[.+\]\s?: $'
...
class CopyPatterns():
    def __init__(self):
        ...
        self.nx_hostname = r'^.*Enter hostname for the (tftp|ftp|scp) server: $'
        ...
        self.erase_before_copy = r'^.*Erase .*before (writing|copying)\? *\[confirm\] *$'
        self.net_type = r'^.*Copy.*from.*\?.*\[confirm\].*$'
        self.copy_confirm = r'^.*Copy.*from.*\?.*\[confirm\].*$'
        self.memory = r'^.* memory\? *\[confirm\] *$'
        self.copy_confirm_1 = r'^.*Are you sure\?.*\[confirm\] *$'
        ...
        self.copy_nx_vrf = r'^.*Enter vrf \(If no input,.*default.*\): $'
        self.copy_proceed = r'^.*bytes.*proceed\?.*$'
        self.tftp_addr =r'^.*Address.*$'
        self.copy_complete = r'^.*bank [0-9]+'
        self.copy_error_message = r'fail|timed out|Timed out|Error opening|Login incorrect|denied|Problem|NOT|Invalid|No memory|Failed|mismatch|Bad|bogus|lose|abort|Not |too big|exceeds|detected|[Nn]o route to host|image is not allowed'
        self.copy_retry_message = r'fail|[Tt]imed out|Error opening|Problem|NOT|Failed|Bad|bogus|lose|abort|Not |too big|exceeds|detected'
        self.copy_continue = r'Are you sure you want to continue connecting (yes/no)?'
        self.copy_other = r'^.*\[yes\/no\] *\?* *$'
...
class HaReloadPatterns(UniconCorePatterns):
    def __init__(self):
        ...
        self.reload_proceed = r'^(.*)Proceed with reload\? \[confirm\]$|^.*Escape character is .*\.\n'
        self.reload_entire_shelf = r'Reload the entire shelf \[confirm\]'
        self.reload_this_shelf = r'Reload this shelf \[confirm\]'
...
class SwitchoverPatterns:
    def __init__(self):
        self.save_config = r'^.*System configuration has been modified\. Save ?\?.*$'
...
class ResetStandbyPatterns:
    def __init__(self):
        self.reload_confirm = r'Reload peer \[confirm\]'
        self.reload_proceed = r'Preparing to reload peer'
        self.reset_abort = r'Peer reload not performed'
        self.reload_proceed1 = r'System is running in SIMPLEX mode, reload anyway? \[confirm\]'
