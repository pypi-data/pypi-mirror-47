...
class HaConfigureService(BaseService):
    ...
    def __init__(self, connection, context, **kwargs):
        ...
        self.commit_cmd = ''
        self.timeout = connection.settings.CONFIG_TIMEOUT
        self.config_lock_pattern = connection.settings.CONFIG_LOCK_PATTERN
        self.__dict__.update(kwargs)

    def pre_service(self, *args, **kwargs):
        self.prompt_recovery = kwargs.get('prompt_recovery', False)

    def verify_and_goto_config_mode(self, handle, retries, retry_sleep):
        start_state = handle.state_machine.current_state
        for i in range(retries):
            try:
                handle.state_machine.go_to('config', handle.spawn,
                    prompt_recovery=self.prompt_recovery,
                    context=self.connection.context)
            except Exception as err:
                handle.state_machine.detect_state(handle.spawn)
                if i == retries-1:
                    raise SubCommandFailure(err, handle.spawn.buffer)
                sleep(retry_sleep)
            else:
                if re.search(self.config_lock_pattern, handle.spawn.match.match_output):
                    handle.state_machine.go_to(start_state, handle.spawn)
                    if i == retries-1:
                        raise SubCommandFailure('Config locked', handle.spawn.match.match_output)
                    sleep(retry_sleep)
                else:
                    break
    ...
    def call_service(self, command=[],
        ...
        if isinstance(command, str):
            command = [command]
        timeout = timeout or self.timeout
        retries =  retries or con.settings.CONFIG_LOCK_RETRIES
        retry_sleep = retry_sleep or con.settings.CONFIG_LOCK_RETRY_SLEEP
        ...
        if len(command) == 0:
            self.command_list_is_empty = True
            self.verify_and_goto_config_mode(handle, retries, retry_sleep)
            self.result = ""
