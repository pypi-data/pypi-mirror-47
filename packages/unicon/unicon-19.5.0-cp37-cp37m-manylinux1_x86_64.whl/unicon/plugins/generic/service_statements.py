...
def login_handler(spawn, context, session):
    ...
    if context.get('tacacs_username'):
        ...
        raise SubCommandFailure("There is no information available about username/tacacas_username")
...
dest_end = Statement(pattern=pat.dest_start,
                     action=ping_handler,
...
qos = Statement(pattern=pat.dscp,
                action=ping_handler,
                args={'send_key': ''},
                loop_continue=True,
                continue_timer=True)
...
execution_statement_list = [generic_statements.confirm_prompt_stmt,
