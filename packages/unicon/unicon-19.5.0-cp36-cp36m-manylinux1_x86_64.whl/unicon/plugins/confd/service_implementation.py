...
class Configure(BaseService):
    ...
    def call_service(self, command=[],
        ...
        if not isinstance(command, collections.Sequence):
...
class Execute(BaseService):
    ...
    def call_service(self, command=[],
        ...
        saved_cli_style = sm.current_cli_style
        if style:
            con.cli_style(style)
        ...
        if (len(command) == 1) and (command[0].strip() == 'switch cli'):
            raise SubCommandFailure("'switch cli' is not supported with 'execute', "
                "use 'cli_style' instead")
        ...
        for cmd in command:
            con.log.debug("+++ execute command '%s' +++" % cmd)
            self.result = con.command(cmd,
                reply=reply,
                error_pattern=error_pattern,
                timeout=timeout)
        ...
        if sm.current_cli_style != saved_cli_style:
            con.cli_style(saved_cli_style)
