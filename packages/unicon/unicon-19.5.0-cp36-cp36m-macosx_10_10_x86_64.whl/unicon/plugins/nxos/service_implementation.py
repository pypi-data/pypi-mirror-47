...
class HANxosReloadService(GenericHAReload):
    ...
    def call_service(self, reload_command='reload',
        ...
        try:
            reload_op=dialog.process(con.active.spawn,
                           context=self.context,
                           prompt_recovery=self.prompt_recovery,
                           timeout=timeout)
        ...
        try:
            con.configure(config_commands,
                prompt_recovery=self.prompt_recovery,
                retries=con.settings.CONFIG_POST_RELOAD_MAX_RETRIES,
                retry_sleep=con.settings.CONFIG_POST_RELOAD_RETRY_DELAY_SEC)
        except:
            pass
        while counter < 31:
        ...
        orig_config_lock_retries = con.settings.CONFIG_LOCK_RETRIES
        orig_config_retry_sleep = con.settings.CONFIG_LOCK_RETRY_SLEEP
        con.settings.CONFIG_LOCK_RETRIES = con.settings.CONFIG_POST_RELOAD_MAX_RETRIES
        con.settings.CONFIG_LOCK_RETRY_SLEEP = con.settings.CONFIG_POST_RELOAD_RETRY_DELAY_SEC
        try:
            con.connect()
        finally:
            con.settings.CONFIG_LOCK_RETRIES = orig_config_lock_retries
            con.settings.CONFIG_LOCK_RETRY_SLEEP = orig_config_retry_sleep
        con.log.debug("+++ Reload Completed Successfully +++")
        self.result = True
        if return_output:
