...
class itkGDCMImageIO(ITKIOImageBaseBasePython.itkImageIOBase):
    ...
    0029|0010), you need to explicitely set the maximum length of elements
    ...
    def GetUIDPrefix(self) -> "char const *":
        ...
        Macro to access the
        DICOM UID prefix. By default this is the ITK part of an existing
        study. 
        """
    ...
    def GetPatientName(self, name: 'char *') -> "void":
        ...
        Convenience methods
        to query patient information and scanner information. These methods
        are here for compatibility with the DICOMImageIO2 class and as such
        should not be used in any new code. They rely on properly preallocated
        buffer, which is not a good practice. Instead user are encourage to
        use directly the GetValueFromTag function 
        """
    ...
    def SetMaxSizeLoadEntry(self, arg0: 'long const') -> "void":
        ...
        A DICOM file
        can contains multiple binary stream that can be very long this binary
        structure in memory since it can consume lot of memory. Therefore any
        field that is bigger than the default value 0xfff is discarded and
        just seek'd This method allow advanced user to force the reading of
        such field WARNING:  this is a GDCM 1.x only option, no effect on GDCM
        2.x 
        """
    ...
    def SetLoadSequences(self, arg0: 'bool const') -> "void":
        ...
        Parse any
        sequences in the DICOM file. Defaults to the value of
        LoadSequencesDefault. Loading DICOM files is faster when sequences are
        not needed. WARNING:  this is a GDCM 1.x only option, no effect on
        GDCM 2.x 
        """
...
class itkGDCMSeriesFileNames(ITKCommonBasePython.itkProcessObject):
    ...
    def GetOutputFileNames(self) -> "std::vector< std::string,std::allocator< std::string > > const &":
        ...
        same extact order as the input one. This could be dangerous if the
