...
class itkFiniteDifferenceFunctionICVF22(ITKCommonBasePython.itkLightObject):
    ...
    C++ includes: itkFiniteDifferenceFunction.h 
    """
    ...
    def InitializeIteration(self) -> "void":
        ...
        This method
        allows the function to set its state before each iteration of the
        finite difference solver (image filter) that uses it. This is a
        thread-safe time to manipulate the object's state.

        An example of how this can be used: the AnisotropicDiffusionFunction
        class of FiniteDifferenceFunction use this method to pre-calculate an
        average gradient magnitude across the entire image region. This value
        is set in the function object and used by the ComputeUpdate() methods
        that are called at each pixel as a constant. 
        """
    ...
    def ComputeUpdate(self, *args) -> "itkCovariantVectorF2":
        ...
        by a finite difference solver image filter at each pixel that does not
        lie on a data set boundary. The width of the data set boundary is
        defined by the width of the neighborhood being evaluated.

        The neighborhood argument is the neighborhood data. The globalData
        argument is a pointer to a data structure that holds values that need
        to be persistent across calls to this function, but cannot be stored
        in the function object itself for thread-safety reasons. Examples are
        values needed to compute the time-step for an iteration of the solver.
        See:   InitializeIteration
    ...
    def SetRadius(self, r: 'itkSize2') -> "void":
        ...
        neighborhood this FiniteDifferenceFunction needs to perform its
        calculations. 
        """
    ...
    def SetScaleCoefficients(self, vals: 'double *') -> "void":
        ...
        Set the
        ScaleCoefficients for the difference operators. The defaults a 1.0.
        These can be set to take the image spacing into account. 
        """
    ...
    def ComputeNeighborhoodScales(self) -> "itkVectorD2 const":
        ...
        Compute
        the scales that weight the neighborhood during difference operations
    ...
    def ComputeGlobalTimeStep(self, GlobalData: 'void *') -> "double":
        ...
        Computes the
        time step for an update given a global data structure. The data used
        in the computation may take different forms depending on the nature of
        the equations. This global data cannot be kept in the instance of the
        equation object itself since the equation object must remain stateless
        for thread safety. The global data is therefore managed for each
    ...
    def GetGlobalDataPointer(self) -> "void *":
        ...
        Returns a
        pointer to a global data structure that is passed to this object from
        the solver at each calculation. The idea is that the solver holds the
        state of any global values needed to calculate the time step, while
        the equation object performs the actual calculations.
    ...
    def ReleaseGlobalDataPointer(self, GlobalData: 'void *') -> "void":
        ...
        When the
        finite difference solver filter has finished using a global data
        pointer, it passes it to this method, which frees the memory.

        The solver cannot free the memory because it does not know the type to
