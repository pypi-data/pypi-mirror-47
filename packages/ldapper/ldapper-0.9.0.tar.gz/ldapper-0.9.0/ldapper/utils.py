...
def interpolate_dn(dn, **kwargs):
    """
    Interpolate a dn snippet given a dict of kwargs.

    >>> parse(dn='cn=%(cn),ou=ldap', cn='staff')
    cn=staff,ou=ldap

    >>> parse(dn='cn=%(cn),ou=ldap')
    ou=ldap
    """
    good_sections = []
    for part in ldap.dn.explode_dn(dn):
        try:
            good_sections.append(part % kwargs)
        except KeyError:
            good_sections = []
