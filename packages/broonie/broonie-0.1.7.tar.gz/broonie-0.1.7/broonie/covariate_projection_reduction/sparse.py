...
class sparseClassCovariateProjectionReductionReaddata(object):
...
# Options for computing the betas required for Z projection
    ...
    def solveBeta(self, s, Z, iflag=0):
        ...
        if (iflag !=0 and iflag!=1):
            print('Wrong iflag choice: changing to to iflag=0 linear regress solvers')
            iflag=0
        if (iflag==0):
...
# Misc methods
    ...
    def inverseZtZ(self, Z):
        """return a 2D array pseudo Inverse of the overlap matrix for the given input array
        """
    ...
    def normMatrix(self,M):
        ...
        IF the norm is ZERO then throw a warning and return M 
        unchanged
        """
        ...
        for j in range(0,k):
            ...
            while (nit < conv_params['maxiter'] and err > conv_params['tolerance'] and deltaErr > conv_params['diffTol']):
                ...
                if (np.abs(Xts).sum() < conv_params['t_val']):
                    ...
                    #    print('yyyyyyyy Skip Find theta using minimizer')
