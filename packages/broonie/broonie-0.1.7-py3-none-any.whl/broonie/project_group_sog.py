...
from broonie.covariate_projection_reduction.sparse import sparseClassCovariateProjectionReductionReaddata
...
def constructMetaDataReadMatrix(indata,inColumnsForZ,batchfilename):
    ...
    print('Input: Valid exclusion group covs are')
    print(columnsForZ)
...
def convertCovariateStringToOneHotEncoder(df_in,effect):
    """We expect Z on entry to be an nx1 (single effect) dataframe with the values
...
def convertCovariateStringToFactors(df,colname):
    ...
    Returns a new dataframe of one column (single effect)
    """
...
def getKnownGroupEffects(fulldata,batchfilename):
    ...
    if (batchfilename == False):
        print('Using hard coded batch effect names')
...
class projectGroupDataClassSOG(object):
    ...
    def __init__(self, k, infilename, inputList, outXtildemeta, batchfilename=False, scale=True):
        self.k = k
    ...
    def getReconstructedXtilde(self):
        ...
        # Start the work
        #
        # Specify as (for example) python sparseProjectGroupData_MouseTumor.py 12 "DATE_MOUSE"
    ...
    def runProjection(self):
        ...
        indexing = df_X.index.values
        headers = df_X.columns.values
        simdat = sparseClassCovariateProjectionReductionReaddata(K,metadata)
