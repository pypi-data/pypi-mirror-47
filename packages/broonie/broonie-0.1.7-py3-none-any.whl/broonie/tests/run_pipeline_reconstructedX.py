...
# Run the pipeline but use Xtilde matrices where one or more effects
# have been RETAINED (reconstructed) by zeroing out selected betas
...
initialGuess = constructInitialGuess(rawfilename,outfilename)
initialGuess.fetchInitialGuess() # Will simply returen Ben;s pregenerated data file
#initialGuess.runInitialGuess() # Will generate a cel-mean, standardized version of rawinput
...
###################################################### group effect
    ...
    x.reportParameters()
    x.runProjection()
    xtilde = x.fetchReconstructedXtilde() # Keep this for testing convergence
    xfilename = x.getReconstructedXtildeFilename()
###################################################### check convergence
...
#################################### Third run the regression
    ...
    if (iterationMode == 'raw'):
        print('Iteration will impute versus RAW R')
        inputregressionfilename = rawfilename
    else:
        print('Iteration will impute incremental R')
