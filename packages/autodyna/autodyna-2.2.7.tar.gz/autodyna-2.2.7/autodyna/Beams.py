...
def MAT_HYSTERETIC_BEAM(
    ...
    fParams : bool
        ...
        Default = True
        PRSi/PRTi = 0 if True but required input data is missing
    tbOption : str
    ...
    if batchMode:
                ...
                if XTRACT == 'on':
                    if not user_xpj:
                        ...
                        if os.path.isfile(xpj_name) == False:
                            raise AssertionError('Cannot find user-defined .xpj file.')
                        else:
                            print('   User-defined .xpj file located. Attempting to analyze in XTRACT... ')
                            # Run XTRACT
                            Autorun(
                                aData,
        ...
        if write_log:
            sys.stdout = aData.copy_stdout
            sys.stderr = aData.copy_stderr
            aData.flog.close()
        ...
        # Return keyword path if pSave specified, JSON data if not
        if pSave:
            keyPath = os.path.join(aData.pSave,aData.file_name)
            return keyPath
        else:
            return json_data
...
def MAT_ELASTIC(
    ...
    if batchMode:
        ...
        while iRun < iData.shape[1]:
            ...
            # Populate list of .key file names
            keyNames.append(aData.pSave + '\\' + util.getName(aData, '.key'))
