class info_elasticBeams():
    ...
    def __init__(self, data, iRun):
        ...
        try:
            # Name sections and curves based on Beam ID
            self.beam_num = int(self.file_name[2:])

        except (IndexError, ValueError) as error:
            # Name sections and curves based on run number
            self.beam_num = int(self.iRun + 1) * 10

        # Define class to print terminal to logfile
        class pr2fl(object):
        ...
        # Print terminal to logfile
        if self.write_log:
            import sys
            self.flog = open(self.pSave + '\\elasticBeams.log', 'w')
            self.copy_stdout = sys.stdout
            self.copy_stderr = sys.stderr
            sys.stdout = pr2fl(sys.stdout, self.flog)
            sys.stderr = pr2fl(sys.stderr, self.flog)
