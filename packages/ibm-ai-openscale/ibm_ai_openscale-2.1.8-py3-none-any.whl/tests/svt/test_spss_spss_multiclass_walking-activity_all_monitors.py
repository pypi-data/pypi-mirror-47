...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_06_subscribe_spss_asset(self):
        subscription = TestAIOpenScaleClient.ai_client.data_mart.subscriptions.add(
            SPSSMachineLearningAsset(source_uid=TestAIOpenScaleClient.source_uid,
                                     ...
                                     label_column="Class",
                                     prediction_column="$R-Class",
                                     class_probability_columns=['$RC-Class'],
    ...
    def test_10_stats_on_payload_logging_table(self):
        ...
        assert_payload_logging_python_table_content(python_table_content=python_table_content, fields=['$R-Class', '$RC-Class'])
    ...
    def test_11_payload_logging_data_distribution(self):
        ...
        self.assertTrue(data_distribution.shape[0] > 0)
        self.assertTrue(data_distribution.shape[1] > 0)
        data_columns = data_distribution.columns.values
        self.assertIn("V1", data_columns)
        self.assertIn("$R-Class", data_columns)
    ...
    def test_14_send_feedback_data(self):
        ...
        with open(file='datasets/waling-activity/User Identification From Walking Activity_feedback.csv') as file:
            feedback_data = file.read()
        self.assertIsNotNone(feedback_data, msg="Unable to load feedback data from file.")

        TestAIOpenScaleClient.feedback_records = 99
        TestAIOpenScaleClient.subscription.feedback_logging.store(feedback_data=feedback_data,
                                                                  feedback_format=FeedbackFormat.CSV, data_header=True,
                                                                  data_delimiter=',')
    ...
    def test_22_setup_explainability(self):
        TestAIOpenScaleClient.data_df = pd.read_csv("datasets/waling-activity/User Identification From Walking Activity.csv", delimiter=",")
    ...
    #     fairness_run = TestAIOpenScaleClient.subscription.fairness_monitoring.run(background_mode=False, deployment_uid="ai_drug_mlp_proba_BiasQA")
