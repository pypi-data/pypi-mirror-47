...
from ibm_ai_openscale.supporting_classes import ProblemType, InputDataType
...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_05_prepare_deployment(self):
        model_name = "AIOS Keras iris model"
        deployment_name = "AIOS Keras iris deployment"
    ...
    def test_07_select_asset_and_get_details(self):
        TestAIOpenScaleClient.subscription = TestAIOpenScaleClient.ai_client.data_mart.subscriptions.get(self.aios_model_uid)
    ...
    def test_12_score(self):
        ...
        no_scoring_requests = 4
        TestAIOpenScaleClient.scoring_records = len(scoring_payload['values']) * no_scoring_requests
    ...
    def test_16_feedback_logging(self):
        ...
        feedback_records = [
            [[1.03800476, -0.1249576, 0.70589294, 0.65911782], 1],
            [[0.67450115, 0.33784833, 0.87649005, 1.44795564], 2],
            [[0.55333328, -1.74477836, 0.36469871, 0.13322594], 1],
            [[-1.38535265, 0.33784833, -1.227541, -1.31297673], 0],
            [[0.31099753, -0.1249576, 0.64902723, 0.79059079], 2]
        ]

        no_feedback_requests = 5
        TestAIOpenScaleClient.scoring_records = len(feedback_records) * no_feedback_requests

        for i in range(0, no_feedback_requests):
            self.subscription.feedback_logging.store(feedback_data=feedback_records)
    ...
    def test_18_run_quality_monitoring(self):
        ...
        while status != 'completed' and elapsed_time < 180:
            time.sleep(10)
