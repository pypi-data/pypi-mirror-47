...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_10_stats_on_payload_logging_table(self):
        ...
        assert_payload_logging_python_table_content(python_table_content=python_table_content, fields=['$S-Drug'])
    ...
    def test_13a_send_feedback_data(self):
        from ibm_ai_openscale.supporting_classes.enums import FeedbackFormat

        with open(file='datasets/BestHeartDrug/drug_feedback_data_spaces.csv') as file:
            feedback_data = file.read()
        self.assertIsNotNone(feedback_data, msg="Unable to load feedback data from file.")

        TestAIOpenScaleClient.subscription.feedback_logging.store(feedback_data=feedback_data, feedback_format=FeedbackFormat.CSV, data_header=False, data_delimiter=' ')

    def test_13b_send_feedback_csv_object(self):
        from ibm_ai_openscale.supporting_classes.enums import FeedbackFormat

        with open(file='datasets/BestHeartDrug/drug_feedback_data_spaces.csv') as file:
            TestAIOpenScaleClient.subscription.feedback_logging.store(feedback_data=file,
                                                                  feedback_format=FeedbackFormat.CSV,
                                                                  data_header=False, data_delimiter=' ')

        TestAIOpenScaleClient.feedback_records = 60
    ...
    def test_22_setup_explainability(self):
        ...
        ## EXPLAINABILITY FOR SUCH MODEL IS NOT WORKING
        ## probability_column not provided
        ## https://github.ibm.com/aiopenscale/tracker/issues/5138

        ## issue with training data statistic class, read statistics from json
        data_df = pd.read_csv(
    ...
    def test_23_run_explainability(self):
        ...
        self.assertIsNone(explainability_run)
    ...
    #
    #     self.assertEquals(rows, 0, msg="Explainability table is empty.")
