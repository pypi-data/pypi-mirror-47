...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    model_uid = 'ulcer-recurrence'
    scoring_records = 12
    feedback_records = 18
    data_df = pd.read_csv("datasets/ulcer-recurrence/ulcer-recurrence.csv")
    ...
    def test_06_subscribe_spss_asset(self):
        subscription = TestAIOpenScaleClient.ai_client.data_mart.subscriptions.add(
            SPSSMachineLearningAsset(source_uid=TestAIOpenScaleClient.source_uid,
                                     ...
                                     label_column="result",
                                     feature_columns=['id', 'age', 'duration', 'treatment', 'time'],
                                     categorical_columns=['duration', 'treatment'],
                                     prediction_column="$C-result-1"
                                     ))
            ...
            'requestInputTable': [
                {"id": input_table_id,
                 'requestInputRow': [
                     {'input': [
                         {'name': 'id', 'value': '1.000'},
                         {'name': 'age', 'value': '48.000'},
                         {'name': 'duration', 'value': '2.000'},
                         {'name': 'treatment', 'value': '1.000'},
                         {'name': 'time', 'value': '7.000'}
                        ]
                     }
                 ]
                 }
    ...
    def test_10_stats_on_payload_logging_table(self):
        ...
        assert_payload_logging_python_table_content(python_table_content=python_table_content, fields=['$C-result-1', '$CP-result-1', '$CP-0-1', '$CP-1-1'])
    ...
    def test_13_send_feedback_data(self):
        ...
        with open(file='datasets/ulcer-recurrence/ulcer-recurrence.csv') as file:
            feedback_data = file.read()
        self.assertIsNotNone(feedback_data, msg="Unable to load feedback data from file.")

        TestAIOpenScaleClient.feedback_records = 43
