...
@cli_params
@background_option
@click.option("--distributed", type=int, metavar="N",
...
def create_run_request(ctx, command, args, machine_type, pip_packages, requirements_file, apt_packages,
    ...
    if git_repo is None:
        ...
        if len(git_repo.submodules) > 0:
            logger.warning("Spell does not currently support Git submodules. "
    ...
    if kwargs.get("conda_env"):
        logger.warning("the --conda-env option is deprecated and being ignored. You only need to provide --conda-file.")
