...
class EPPConnection(object):
    def __init__(self, host, port):
        ...
        self.ssl = ssl.wrap_socket(self.socket, None, None, False)
...
#        key_list = (msg_key, reason_key, avail_key) = 'msg', 'reason', '@avail'
#
#        resp_dict.update(
...
#            for line in raw_out:
#                response = line['epp']['response']
#                result = response['result']
#
#                msg = result.get(msg_key)
#                reason = result.get('extValue', {}).get(reason_key)
#                avail_status = response.get('resData', {}).get('domain:chkData', {}).get('domain:cd', {}).get('domain:name', {}).get(avail_key)
#
#                log.debug(result)
#                for item in zip((key_list), (msg, reason, avail_status)):
#                    k, v = item
#                    if v:
