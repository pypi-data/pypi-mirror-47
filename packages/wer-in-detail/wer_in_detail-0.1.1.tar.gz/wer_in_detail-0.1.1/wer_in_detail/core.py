...
def wer(truth: Union[str, List[str], List[List[str]]],
    ...
    :param in_detail also return substitute, delete, insert, correct, number of words
    ...
    distance, sdi = _edit_distance_detail(t, h)
    ...
    return error_rate, sdi[0], sdi[1], sdi[2], c, n
...
def _edit_distance_detail(a: List[int], b: List[int]):
    if len(a) == 0:
        raise ValueError("the reference string (called a) cannot be empty!")
    elif len(b) == 0:
        return len(a)
    ...
    # S/D/I
    sdi = np.zeros((len(b) + 1, len(a) + 1, 3)).astype(dtype=np.int32)
    sdi[0, 1:, 1] = np.arange(1, len(a) + 1)
    sdi[1:, 0, 2] = np.arange(1, len(b) + 1)
    ...
    for i in range(1, m.shape[0]):
        for j in range(1, m.shape[1]):
            if a[j - 1] == b[i - 1]:
                ...
                replace_index = np.argmin([m[i - 1, j - 1] + 1, m[i, j - 1] + 1, m[i - 1, j] + 1])

                if replace_index == 0:  # S
                    sdi[i, j] = sdi[i - 1, j - 1]
                    sdi[i, j][0] += 1
                elif replace_index == 1:  # I
                    sdi[i, j] = sdi[i, j - 1]
                    sdi[i, j][2] += 1
                elif replace_index == 2:  # D
                    sdi[i, j] = sdi[i - 1, j]
                    sdi[i, j][1] += 1
    ...
    return m[len(b), len(a)], sdi[len(b), len(a)]