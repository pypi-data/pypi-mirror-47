...
from rockset.integration import AWSExternalIdIntegration
from rockset.integration import AWSIntegration
from rockset.integration import AWSRedshiftClusterIntegration
from rockset.integration import Integration
from rockset.integration import GCPServiceAccountIntegration
from rockset.query import Query
from rockset.source import Source
from rockset.workspace import Workspace
...
class Client(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.Collection = ClientCollection(client=self)
        self.Workspace = ClientWorkspace(client=self)
        self.Source = ClientSource(client=self)
        self.Integration = ClientIntegration(client=self)
    ...
    def call_api(self, resource, method, *args, **kwargs):
        try:
            ...
            if e.status == 401:
                raise AuthError(code=e.status, message=message) from None
            elif e.status == 403 or e.status == 413 or e.status == 429:
                raise LimitReached(code=e.status, message=message, type=type) \
                    from None
            elif e.status == 400:
                raise InputError(
                    code=e.status, message=message, type=errtype
                ) from None
            elif e.status == 501:
            ...
            elif e.status >= 500 and e.status < 530:
                raise ServerError(
                    code=e.status, message=message, type=errtype
                ) from None
            elif e.status == 530:
    ...
    def list(self, workspace="commons", **kwargs):
        ...
        """
        return Collection.list(client=self, workspace=workspace)
...
class ClientCollection(object):
    ...
    def list(self, workspace="commons"):
        return Collection.list(client=self.client, workspace=workspace)
...
class ClientIntegration(object):
    def __init__(self, client):
        ...
        self.client = client
        self.AWS = ClientAWSIntegration(client)
        self.AWSExternalId = ClientAWSExternalIdIntegration(client)
        self.AWSRedshiftCluster = ClientAWSRedshiftClusterIntegration(client)
        self.GCPServiceAccount = ClientGCPServiceAccountIntegration(client)

    def list(self):
        return Integration.list(client=self.client)

    def retrieve(self, name):
        return Integration.retrieve(client=self.client, name=name)
...
class ClientAWSExternalIdIntegration(object):
    def __init__(self, client):
        self.client = client

    def create(self, name, aws_role_arn, description=None, **kwargs):
        return AWSExternalIdIntegration.create(
            name=name,
...
class ClientAWSRedshiftClusterIntegration(object):
    def __init__(self, client):
        self.client = client
    ...
    def create(
        ...
        return AWSRedshiftClusterIntegration.create(
            name=name,
            aws_key=aws_key,
            username=username,
            password=password,
...
class ClientGCPServiceAccountIntegration(object):
    def __init__(self, client):
        self.client = client

    def create(self, name, json_key, description=None, **kwargs):
        return GCPServiceAccountIntegration.create(
            name=name,
            json_key=json_key,
            description=description,
            client=self.client,
...
class ClientWorkspace(object):
    def __init__(self, client):
        self.client = client

    def create(self, name, description=None, **kwargs):
        return Workspace.create(client=self.client, name=name, description=description)

    def list(self):
        return Workspace.list(client=self.client)

    def retrieve(self, name):
        return Workspace.retrieve(client=self.client, name=name)
