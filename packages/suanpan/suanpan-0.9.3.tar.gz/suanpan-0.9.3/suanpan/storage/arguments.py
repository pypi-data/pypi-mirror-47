...
from suanpan.utils import csv, json, text
...
class Text(Data):
    ...
    def format(self, context):
        super(Text, self).format(context)
        if self.filePath:
            self.value = text.load(self.filePath)
        return self.value

    def save(self, context, result):
        text.dump(self.filePath, result.value)
        return super(Text, self).save(context, Result.froms(value=self.filePath))
