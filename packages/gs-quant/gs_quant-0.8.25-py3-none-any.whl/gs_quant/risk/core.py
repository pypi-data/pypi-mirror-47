...
def __flatten_results(item: Union[List, Tuple]):
    ...
    for elem in item:
        if isinstance(elem, (list, tuple)):
            ...
            excludedFields = ['calculationTime', 'queueingTime']
            if not issubclass(PricingContext.current, HistoricalPricingContext):
                excludedFields.append('date')
...
def scalar_formatter(result: List) -> Optional[Union[float, pd.Series]]:
    ...
    result = __flatten_results(result)
...
def structured_formatter(result: List) -> pd.DataFrame:
    return sort_risk(pd.DataFrame.from_records(__flatten_results(result)))
