...
def get_query_url(query, lang, pos, from_user):
    if from_user:
        if pos is None:
...
def query_tweets_from_user(user, limit=None):
    ...
    try:
        while True:
            new_tweets, new_pos = query_single_page(user, lang='', pos = None, from_user=True)
            logger.info("Got {} new tweets for {}".format(len(new_tweets), new_pos))
            if len(new_tweets) == 0:
