...
class Token(_Node):
    ...
    FEAT_LEMMA_SRC = 'lemmaSrc'
    # lemmatizer responsible for filling the lemma
    ...
    def __init__(self, *,
        ...
        deepLemma: Optional[str],
        lemma: Optional[str],
        pos: Optional[ud.UPos],
        feats: Mapping[str, Union[str, List[str]]],
        morphTag: Optional[str],
        fnc: Optional[ud.UDep]
    ) -> None:
    ...
    def leftChildren(self) -> List['Token']:
        ...
        """
        return [cast(Token, c) for c in self._children if c._idx < self._idx]
    ...
    def isUnknown(self):
        ...
        """
        return Token.FEAT_UNKNOWN in self.feats
...
class TectoToken(_Node):
    ...
    def __init__(self, *,
        ...
        fnc: str,
        lemma: str,
        tokenSupport: Optional[TokenSupport],       # the root or dropped phrases have no surface realization
        entityMention: Optional['Entity.Mention'] = None,
        ...
        self.entity = entity
        """ Legacy value: Entity associated with this tecto token; None if there is no such entity. 
...
class Relation:
    ...
    def isNegated(self) -> bool:
        return Relation.FEAT_NEGATED in self.feats
