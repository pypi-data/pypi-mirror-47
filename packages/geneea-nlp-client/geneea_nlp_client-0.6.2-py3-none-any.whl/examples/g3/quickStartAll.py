...
def main():
    ...
    requestBuilder = g3.Request.Builder(
        ...
        referenceDate=datetime.datetime.now().strftime('%Y-%m-%d'),
        returnMentions=True,
    ...
    with g3.Client.create(userKey=USER_KEY) as analyzer:
        for idx, text in enumerate(TEXTS):
            ...
            for e in result.entities:
                ...
                for m in e.mentions:
                    prevToken = m.tokenSupport.tokens[0].previous()
                    nextToken = m.tokenSupport.tokens[-1].next()
            ...
            for t in result.tags:
                ...
                for m in t.mentions:
                    prevToken = m.tokenSupport.tokens[0].previous()
                    nextToken = m.tokenSupport.tokens[-1].next()
                    snippet: str = f'{prevToken.text if prevToken else ""} [{m.tokenSupport.text}] {nextToken.text if nextToken else ""}'
                    print(f'\t\t{snippet}')

            print('--- Relations ---')
            for r in result.relations:
                argsStr = ', '.join(f'{a.type}: {a.name}' for a in r.args)
                modalityStr = f'{r.modality()} ' if r.modality() else ''
                negatedStr = '-not' if r.isNegated() else ''

                print(f'\t{r.type}: {modalityStr}{r.name}{negatedStr}({argsStr})')

            print('--- Document Sentiment ---')
            print(f'\t{result.docSentiment}')
