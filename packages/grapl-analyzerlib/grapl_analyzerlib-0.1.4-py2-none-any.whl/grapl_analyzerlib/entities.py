...
class ProcessQuery(object):
    def __init__(self) -> None:
        # Properties
        self._node_key = entity_queries.Has("node_key")  # type: Optional[entity_queries.Cmp]
...
{
    Binding0 as var(func: has(process_id)) @cascade
        @filter(( (NOT regexp(process_name, /.*services\.exe.*/i) AND NOT
        regexp(process_name, /.*lsass\.exe.*/i)) ))
        {
            children @filter(( (has(process_id)) )) 
                {
                    children @filter(( (has(process_id)) )) { }
                }
            }
        
        res(func: uid(Binding0) , first: True)
        {
            { 
                node_key,process_name,
                {
                    node_key,process_id,
                    children {
                    {
                        node_key,process_id
                        children { node_key,process_id, }
                    }
                } 
            } 
        } 
