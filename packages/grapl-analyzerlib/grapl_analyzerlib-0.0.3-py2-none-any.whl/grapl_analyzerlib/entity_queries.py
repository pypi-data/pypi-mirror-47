...
import grapl_analyzerlib.entity_views as views

import json
from copy import deepcopy
from typing import TypeVar, Optional, List, Union, Any, Set
...
def get_var_block(
    node: NQ, edge_name: str, binding_num: int, root: NQ, already_converted: Set[NQ]
) -> str:
    var_block = ""
...
def flatten_nodes(root: NQ) -> List[Any]:
    node_list = [root]
    already_visited = set()
    to_visit = [root]
...
def get_queries(process_query: NQ, node_key: str, count: bool = False):
...
class ProcessQuery(object):
    def __init__(self) -> None:
        # Properties
        self._node_key = None  # type: Optional[str]
    ...
    def with_node_key(self, node_key: str):
    ...
    def _get_var_block(self, binding_num: int, root: NQ, already_converted: Set[NQ]) -> str:
    ...
    def _get_var_block_root(self, binding_num: int, root: NQ, node_key: str) -> str:
    ...
    def query_first(
    ...
    ) -> Optional[views.ProcessView]:
        if contains_node_key:
            query_str = get_queries(self, node_key=contains_node_key)
        else:
        ...
        return views.ProcessView.from_dict(dgraph_client, raw_views[0])
