...
class ProcessView(NodeView):
    ...
    def get_image_name(self) -> Optional[str]:
        ...
        self_process = (
            queries.ProcessQuery().with_node_key(self.node_key).with_image_name().query()
        )
    ...
    def get_parent(self) -> Optional[P]:
        ...
        query = (
            ...
            .with_child(queries.ProcessQuery().with_node_key(eq=self.node_key))
            .with_uid()
            .to_query()
        )
    ...
    def get_uid(self):
        ...
        query = queries.ProcessQuery().with_node_key(eq=self.node_key).with_uid().to_query()
