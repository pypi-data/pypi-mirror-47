...
class SubgraphView(object):
    ...
    def from_proto(dgraph_client: DgraphClient, s: bytes) -> S:
        ...
        nodes = {k: NodeView.from_raw(dgraph_client, node) for k, node in subgraph.nodes.items()}
...
class ProcessQuery(object):
    ...
    def _get_var_block_root(self, binding_num: int, root: Any, node_key: Optional[str] = None) -> str:
    ...
    def _to_query(self, first: int=1) -> str:
        var_block = self._get_var_block_root(0, self)

        return entity_queries.build_query([var_block], ["Binding0"])
...
class FileQuery(object):
    ...
    def _get_var_block_root(self, binding_num: int, root: Any, node_key: Optional[str] = None):
        already_converted = {self}
        root_var = ""
        if self == root:
            root_var = f"Binding{binding_num} as "
...

{
    Binding0 as var(func: eq(node_key, "keyA")) {
        children @filter(( (regexp(process_name, /.*svchost\.exe.*/i)) )) { } 
    } 
    
    var(func: eq(node_key, "keyA")) @filter(( (regexp(process_name, /.*svchost\.exe.*/i)) )) {
        Binding1 as ~children { } 
    }
    
    res(func: uid(Binding0, Binding1), first: 1)
        @filter(( (regexp(process_name, /.*svchost\.exe.*/i)) ))
    {
        expand(_all_) {
            expand(_all_) {
                expand(_all_)
