...
class NodeView(object):
    ...
    @staticmethod
    def from_raw(node: Any) -> N:
        if node.process_node:
            return NodeView(ProcessView(node.process_node.node_key))
        elif node.file_node:
            return NodeView(FileView(node.file_node.node_key))
        else:
...
class SubgraphView(object):
    ...
    def from_proto(s: bytes) -> S:
        ...
        nodes = {node.node_key: NodeView.from_raw(node) for node in subgraph.nodes.values()}
...
class FileQuery(object):
    ...
    def _get_var_block_root(self, binding_num, root, node_key):
        ...
        block = f"""
            {root_var} var(func: entity_queries.Eq(node_key, "{node_key}")) {filters} {{
                {creator}
                {deleter}
                {writers}
                {readers}
