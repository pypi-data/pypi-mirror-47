...
def get_var_block(
    node: node.NQ, edge_name: str, binding_num: int, root: node.NQ, already_converted: Set[node.NQ]
) -> str:
    var_block = ""
...
def flatten_nodes(root: node.NQ) -> List[Any]:
    node_list = [root]
    already_visited = set()
    to_visit = [root]
...
def get_queries(process_query: node.NQ, node_key: str, count: bool = False):
