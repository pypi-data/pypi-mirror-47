...
def zzext(page):
    ...
    name=soup1.find('div',class_='user_info spmtop').text.strip()
...
def df2df(df):
    ...
    df1=pd.DataFrame(columns=['zzlb', 'zzbh', 'zzmc', 'bgdate', 'eddate', 'fbjg', 'name', 'tydm',
    ...
    return df1
        # if i%100==0 and i>0:
        #     db_write(df1,'qyzz',dbtype="postgresql",conp=conp,if_exists='append')
        #     df1=pd.DataFrame(columns=['zzlb', 'zzbh', 'zzmc', 'bgdate', 'eddate', 'fbjg', 'name', 'tydm',
        #             'fddbr', 'jgdz', 'gsd', 'jglx', 'qita'])
     
#sql="select * from tc.gg_html where href='http://jzsc.mohurd.gov.cn/dataservice/query/comp/compDetail/001607220057241213' "
