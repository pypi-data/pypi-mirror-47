...
def execute(**kwargs):
    ...
    EXP_DEFS  =\
    {
      ...
      'BASE_ITER_SHR_ADV_EFERA5_SMHW_ORI':{'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,  'sw_shearwe': True },
      'BASE_ITER_SHR_ADV_EFERA5_SMHW_LOW':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True },
      'BASE_ITER_SHR_ADV_EFERA5_SMHW_HIG':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True },
      'BASE_ITER_SHR_ADV_EFERA5_SMHW_G90':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True },
      'BASE_ITER_SHR_ADV_EFERA5_SMHW_L90':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True },
      'BASE_ITER_SHR_ADV_EFERA5_SMHW_Q50':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True},
      'BASE_ITER_SHR_ADV_EFERA5_SMHW_MED':  {'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False,'sw_shearwe': True},
    ...
    for iexpname,expid in enumerate(experiments):
        ...
        for istation,current_station in run_stations.iterrows():
            ...
            for imonth,month in enumerate(months):
                select_season = (records_morning_station.datetime.dt.month == month)


                dat = records_morning_station[select_season]

                wg_ge90.append(dat[dat.t2m_daymax >= dat.t2m_daymax.quantile(0.90)].wg.median())
                wg_lt90.append(dat[dat.t2m_daymax < dat.t2m_daymax.quantile(0.90)].wg.median())
                wg_q10.append(dat[dat.t2m_daymax <=  dat.t2m_daymax.quantile(0.10)].wg.median())
                wg_q50.append(dat[(dat.t2m_daymax >=  dat.t2m_daymax.quantile(0.45)) & (dat.t2m_daymax <=  dat.t2m_daymax.quantile(0.55))].wg.median())
                wg_median.append(dat.wg.median())
                wg_q75.append(dat[dat.t2m_daymax >= dat.t2m_daymax.quantile(0.75)].wg.median())
                wg_q25.append(dat[dat.t2m_daymax <=  dat.t2m_daymax.quantile(0.25)].wg.median())
                #wg_q50.append(dat.wg.median())
                #wg_q50(dat[dat.wg >= dat.wg.quantile(0.45) & dat.wg < dat.wg.quantile(0.55)].wg.median())

                # wg_q10.append(records_morning_station[select_season].wg.quantile(0.10))
                # wg_ge90.append(records_morning_station[select_season].wg.quantile(0.90))
                # wg_q50.append(records_morning_station[select_season].wg.quantile(0.50))
            ...
            if not skip_chunk:
                ...
                for (STNID,chunk,index),record_morning in records_morning_station_chunk.iterrows():
                        ...
                        wgpct = (c4gli_morning.pars.wg -c4gli_morning.pars.wwilt)/\
                                (c4gli_morning.pars.wfc - c4gli_morning.pars.wwilt)
                        wgpct_step = 0.10
                        wgpct = np.max((wgpct_step/2.,wgpct))
                        wgpct = np.min((1.-wgpct_step/2.,wgpct))

                        wgpct_hig = wgpct + wgpct_step/2.
                        wgpct_low = wgpct - wgpct_step/2.

                        wg_low = c4gli_morning.pars.wwilt + \
                                wgpct_low*(c4gli_morning.pars.wfc - \
                                       c4gli_morning.pars.wwilt)
                        wg_hig = c4gli_morning.pars.wwilt + \
                                wgpct_hig*(c4gli_morning.pars.wfc - \
                                       c4gli_morning.pars.wwilt)

                        wgpct_hig = wgpct + wgpct_step/2.

                        if expname[-4:] == "_LOW": 
                            c4gli_morning.update(source=expname,pars={'wg' : wg_low})
                            c4gli_morning.update(source=expname,pars={'w2' : wg_low})
                        if expname[-4:] == "_HIG": 
                            c4gli_morning.update(source=expname,pars={'wg' : wg_hig})
                            c4gli_morning.update(source=expname,pars={'w2' : wg_hig})

                        month = c4gli_morning.pars.datetime.month
                        imonth = months.index(month)
                        if expname[-4:] == "_Q10": 
                            c4gli_morning.update(source=expname,pars={'wg' : wg_q10[imonth]})
                            c4gli_morning.update(source=expname,pars={'w2' : wg_q10[imonth]})

                        # if expname == "BASE_ITER_ADV_EFERA5_B05_SMHW_Q50": 
                        #     c4gli_morning.update(source=expname,pars={'wg' : wg_q50[imonth]})
                        #     c4gli_morning.update(source=expname,pars={'w2' : wg_q50[imonth]})

                        if expname[-4:] == "_Q90": 

                            c4gli_morning.update(source=expname,pars={'wg' : wg_ge90[imonth]})
                            c4gli_morning.update(source=expname,pars={'w2' : wg_ge90[imonth]})

                        if expname[-4:] == "_L90": 

                            c4gli_morning.update(source=expname,pars={'wg' : wg_lt90[imonth]})
                            c4gli_morning.update(source=expname,pars={'w2' : wg_lt90[imonth]})
                        ...
                        if expname[-4:] == "_MED": 
                            c4gli_morning.update(source=expname,pars={'wg' : wg_median[imonth]})
                            c4gli_morning.update(source=expname,pars={'w2' : wg_median[imonth]})
