...
class GazeEvents:
    ...
    def __init_datatypes__(self):
        ...
        self.__events__[GazeEvents.Fixation_X] = GazeItem(GazeEvents.Fixation_X, np.dtype('f2'))
        self.__events__[GazeEvents.Fixation_Y] = GazeItem(GazeEvents.Fixation_Y, np.dtype('f2'))
        self.__events__[GazeEvents.EventIndex] = GazeItem(GazeEvents.EventIndex, np.dtype('u4'))
        self.__events__[GazeEvents.EventDuration] = GazeItem(GazeEvents.EventDuration, np.dtype('f2'))
        self.__events__[GazeEvents.AOI] = GazeItem(GazeEvents.EventDuration, np.dtype(object))
    ...
    def toDataFrame(self, ts_filter=None):
        ...
        for label, data in self.__events__.items():
            table[label] = pd.Series(data.getData(), dtype=data.getType())
