...
class Recording:
    ...
    def __exportData_CSV__(self, filepath=None, filename=None, csv_suffix='', segment_id=None, exporter=None):
        ...
        for s_id in segment_ids:
            ...
            (filepath, filename) = self.__getFileParams__(s_id, filepath, filename, csv_suffix)
            e = exporter(filepath, filename, gazedata)
            e.toCSV()

    def __getFileParams__(self, segment_id, filepath=None, filename=None, csv_suffix=''):
        if filepath is None:
            filepath = "."
        if filename is None:
            filename = "%s-S%s-%s.csv" % (self.getParticipantName(), str(segment_id), csv_suffix)
    ...
    def exportFixations(self, fixation_filter, ts_filter=None, segment_id=None, aoi_model=None):
        fixations = self.getFixations(fixation_filter, ts_filter, segment_id, aoi_model)
        fixations.exportCSV(".", self.getParticipantName() + '.csv', ts_filter=ts_filter)
