...
gflags.DEFINE_string('mysql_user', 'kbuser',
    'User for the MySQL server.')

gflags.DEFINE_string('mysql_password', 'kbpswd',
...
gflags.DEFINE_string('kegbot_pycore_package', 'jrfork-kegbot-pycore==0.0.1',
...
def run_mysql_root(subcommand, command='mysql', **kwargs):
    cmd = 'sudo {} -u {} '.format(command, 'root')
    cmd += subcommand
    return run_command(cmd, **kwargs)
...
class KegberryApp(object):
    ...
    def _update_mariadb_config(self):
        logger.info('Updating DB Server Config ...')
        mariadb_conf = write_tempfile(templates.MARIADB_CONF.substitute())
        run_command('sudo bash -c "mv {} /etc/mysql/mariadb.conf.d/50-server.cnf"'.format(mariadb_conf))
        run_command('sudo /etc/init.d/mysql restart')

    def install(self, *args):
        """Performs an first-time Kegberry install."""
        self._update_packages()

        self._update_mariadb_config()

        run_mysql_root('-e "GRANT ALL PRIVILEGES ON *.* TO \'{}\'@\'localhost\' IDENTIFIED BY \'{}\'"'.format(FLAGS.mysql_user, FLAGS.mysql_password))
        ...
        cmd = 'mysql_tzinfo_to_sql /usr/share/zoneinfo | sudo mysql -u root'
        if FLAGS.mysql_password:
            cmd += ' -p={}'.format(FLAGS.mysql_password)
        cmd += ' mysql'
        run_command(cmd)
        ...
        if FLAGS.mysql_user:
            cmd += ' --db_user="{}"'.format(FLAGS.mysql_user)
        if FLAGS.mysql_password:
