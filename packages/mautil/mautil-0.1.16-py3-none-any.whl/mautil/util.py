...
def pad_sequence(seqs, pad=0, seqs_len=None, dtype=np.int32):
    ...
    padded_seqs = np.ones([len(seqs_len), max_seq_len], dtype) * pad
    mask = np.arange(max_seq_len) < seqs_len[:, None]
    padded_seqs[mask] = np.concatenate(seqs)
...
class TrainArgParser(ArgParser):
    ...
    def add_args(parser):
        ...
        parser.add_argument("-save_epoch", "--save_epoch", type=int, default=1000000000, help="save per epochs")
        parser.add_argument("-save_step", "--save_step", type=int, default=1000000000, help="save per steps")
        parser.add_argument("-save_keep", "--save_keep", type=int, default=1000000000, help="keep number of saved copy")
        parser.add_argument("-val_step", "--val_step", type=int, default=1000000000, help="do validate per steps")
        parser.add_argument("-val_batchs", "--val_batchs", type=int, default=1000000000, help="only run  limited batchs when do validate per steps")
        parser.add_argument("-ckpt_hours", "--keep_checkpoint_every_n_hours", type=int, default=10000, help="")
