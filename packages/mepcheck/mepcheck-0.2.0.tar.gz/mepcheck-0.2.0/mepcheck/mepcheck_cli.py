...
def cli():
    ...
    parser.add_argument("-d", "--data", choices=['list', 'json', 'df'],
                            help="Return retrieved data in various formats for further usage")
    ...
    if args.country is None and args.mep is None:
        ...
        if args.data:
            if args.limit is None:
                ...
                print(votes.data_(shape=args.data, limit=50))
            else:
                votes = EUvotes(mep_id=args.mep, limit=args.limit)
                print(votes.data_(shape=args.data, limit=args.limit))
        else:
            if args.limit is None:
                votes = EUvotes(mep_id=args.mep, limit=50)
