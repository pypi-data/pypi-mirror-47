...
def get_options():
    ...
    copyright = __licence__ + " licence | " + __author__ + " | " + __author_email__ + " | " + __version__

    description = """
    snpToolkit takes vcf files as input. The vcf files could be generated using samtools mpileup, gatk HaplotypeCaller or freeBayes.
    ...
    requiredOptions1 = annotate.add_argument_group(
    ...
    requiredOptions1.add_argument('-g', required=True, type=str, dest='genbank',
                                  help='Pleae provide a genbank')

    AdditonalOptions1 = annotate.add_argument_group(
        'snpToolkit annotate additional options')

    AdditonalOptions1.add_argument('-f', required=False, dest='excludeCloseSNPs', type=int,
                                   help='exclude SNPs if the distance between them is lower then the specified window size in bp')

    AdditonalOptions1.add_argument('-q', required=False, default=20, type=int, dest='quality',
                                   help='quality score to consider as a cutoff for variant calling. default value [20]')

    AdditonalOptions1.add_argument('-d', required=False, default=3, type=int,
                                   dest='depth',  help='minimum depth caverage. default value [3]')

    AdditonalOptions1.add_argument('-r', required=False,
    ...
    AdditonalOptions1.add_argument('-e', required=False, type=str, dest='exclude',
    ...
    requiredOptions2 = combine.add_argument_group(
        'snpToolkit combine required options')

    requiredOptions2.add_argument('--location', required=True, type=str, dest='location',
                                  help='provide for example the name of the chromosome or plamid you want to create fasta alignemnt for')

    AdditonalOptions2 = combine.add_argument_group(
        'snpToolkit additional options')

    AdditonalOptions2.add_argument(
        '-r', required=False, type=float, dest='ratio', default=0.01, help='SNP ratio')

    AdditonalOptions2.add_argument('-d', required=False, type=int, dest='depth',
                                   default=0, help='depth cutoff for cheking missing data')

    AdditonalOptions2.add_argument('--bam', required=False, type=str,
                                   dest='bamFolder', help='path to the folder containing the bam files')

    AdditonalOptions2.add_argument('--snps', required=False, type=str, dest='snps', default='all', choices=['ns', 's', 'all', 'inter'],
