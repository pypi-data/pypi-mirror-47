...
class Azure(AddressPrefixCache):
    """
    Azure address prefixes
    """
    cache_filename = 'azure-prefixes.json'
    prefix_class = AzurePrefix
    ...
    def fetch(self):
        ...
        for section in data['values']:
            ...
            for prefix in section['properties']['addressPrefixes']:
                self.prefixes.append(self.prefix_class(prefix, kwargs))

        self.updated = datetime.now()
        self.prefixes.sort(key=attrgetter('version', 'region', 'service'))
