...
class Wind(object):
    ...
    def get_edb(self, stock_id, start_day, end_day: str = '', days='Alldays') -> DataFrame:
        ...
        # 用stock_id作为列头
        df: DataFrame = None
        if len(data.Data) == 1:  # 只能一行数据时特别处理
            df = DataFrame(data.Data, index=data.Times, columns=stock_id.split(','))
        else:
            df = DataFrame(data.Data, columns=data.Times, index=stock_id.split(',')).T  # .T 行列转换
