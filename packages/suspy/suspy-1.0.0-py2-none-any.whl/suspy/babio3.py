...
def run(mc, n_max, num_iw, num_iw_xi, n_tau, nloops, Ud, mu, t1, t2, t4, beta, gamma_bd, eb, tot, delta, file_name='data'):
...
def main():
    ...
    while a != 'yes':
        ...
        if not os.path.exists('./' + n):
            os.system('cp -rf {0}/{2} {1}/{2}'.format(os.path.dirname(Cabbage.__file__), os.getcwd(), n))
        ...
        print "You can change parameters in input_data.txt file"
        a = raw_input('Start computation? [yes/no] ')
        a = a.strip()
        print '\n'
        file_name = raw_input('Enter file name for output: ')
        file_name = file_name.strip().replace(' ', '_')
        
        if a.lower() == 'yes':
            run(mc, n_max, num_iw, num_iw_xi, n_tau, nloops, Ud, mu, t1, t2, t4, beta, gamma_bd, eb, tot, delta, file_name)
            break
        else:
            print "Sorry, we are close =D"
            break