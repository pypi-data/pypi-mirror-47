...
class DataUsed(APIResponse):
    def __init__(self, **kwargs):
        ...
        self.activeSessions = kwargs.get("Active_Sessions", kwargs.get("active_sessions", 0)) if kwargs else 0
        self.concurrentSessions = kwargs.get("Concurrent_Sessions", kwargs.get("concurrent_sessions", 0)) if kwargs else 0
        self.requestLimitDaily = kwargs.get("Request_Limit_Daily", kwargs.get("request_limit_daily", 0)) if kwargs else 0
        self.sessionCap = kwargs.get("Session_Cap", kwargs.get("session_cap", 0)) if kwargs else 0
        self.sessionTimeLimit = kwargs.get("Session_Time_Limit", kwargs.get("session_time_limit", 0)) if kwargs else 0
        self.totalRequestsToday = kwargs.get("Total_Requests_Today", kwargs.get("total_requests_today", 0)) if kwargs else 0
        self.totalSessionsToday = kwargs.get("Total_Sessions_Today", kwargs.get("total_sessions_today", 0)) if kwargs else 0
    def __str__(self):
