...
def run_csv(r, app): 
    ...
    if r.headers['Content-Type'] != 'application/json':
        ...
        #if the variables info has been put in a pointless list remove the list
        for key in variables_info.keys():
            
            if isinstance(variables_info[key], list):
                
                if len(variables_info[key]) == 1:
                    
                    variables_info[key] = variables_info[key][0]
