...
class SwiftWrapper(Base):
    ...
    def _connect_to_swift(self, file_store_config: dict):
        ...
        if file_store_config["auth"].get("timeout") is None:
            raise ConfigurationConflict(
                "Timeout not specified. Will default to 60"
            )
        file_store_config["auth"]["timeout"] = int(
            file_store_config["auth"]["timeout"]
        )
        auth_version = file_store_config["auth_version"]
        if auth_version == "v3":
            return self._connect_to_swift_v3(
                **file_store_config["auth"], auth_version=auth_version
            )
